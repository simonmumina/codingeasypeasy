...
*   **`valueAsDate`:** Returns a `Date` object representing the input's date and time. Returns `null` if the input doesn't contain a valid date and time. *Note:*  The returned `Date` object always represents the time in the local time zone of the user's browser.
...
*   **Consider Time Zones:**  `datetime-local` does *not* store time zone information.  If time zone is important, you'll need to handle it separately, perhaps by asking the user for their time zone and adjusting the time accordingly.  Alternatively, consider using separate date and time input fields and a time zone selection control.
