...
*   **Problem:** Standard Bootstrap buttons (`<button class="btn">`) are generally accessible. However, if you're using a link that *looks* like a button (`<a class="btn">`), you *must* add the `role="button"` attribute.
*   **Solution:**
...
.focusable { /* Class to add to elements that should be focusable via keyboard */
