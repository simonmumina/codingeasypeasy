...
date: '2024-02-29'
lastmod: '2024-10-26'
tags: ['react-native', 'expo', 'sqlite', 'database', 'mobile development', 'offline data', 'react-native sqlite', 'expo sqlite tutorial']
draft: false
...
**Important Note for EAS Build:**  If you plan to build a standalone app using EAS Build, you may need to add `expo-dev-client` to your project. While not strictly *required* for Expo SQLite functionality, it's often recommended for a smooth development and build experience, especially when dealing with debugging and more complex configurations.  Install it with:
...
const App = () => {
  ...
  useEffect(() => {
    ...
    }, null, loadTodos); // Load todos after creating the table (if needed)
  }, []);
