...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'sets', 'foreach', 'iteration', 'data structures', 'es6', 'ecmascript', 'tutorial', 'guide', 'programming']
draft: false
...
Before we dive into `forEach()`, let's quickly recap what a JavaScript Set is.  A Set is a collection of *unique* values.  This means you can't have duplicate entries in a Set. Sets provide methods for adding, deleting, checking membership, and iterating through their elements.
...
*   **`callbackFn`:**  The function to execute for each element. This function accepts three arguments:
    ...
    *   `key`:  (Surprisingly, this is also the *value* in a Set). Sets don't have keys like objects. This argument is included for consistency with Map's `forEach()` method. Think of it as the same value repeated.
...
*   **`for...in` loop (Not Recommended):**  While technically you *could* use a `for...in` loop, it's generally **not recommended** for Sets.  `for...in` is designed for iterating over object properties, and its behavior with Sets is not as predictable or efficient as `forEach()` or `for...of`.
