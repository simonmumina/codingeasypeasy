...
date: '2024-01-12'
lastmod: '2024-10-27'
tags: ['nextjs', 'api routes', 'file upload', 'serverless functions', 'formidable', 'multipart form data', 'node.js', 'image upload', 'security', 'best practices']
draft: false
    ...
    try {
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        try {
          ...
          return res.status(200).json({ message: "File uploaded successfully!", filename: newFilename, filePath: `/uploads/${newFilename}` });
        } catch (renameError) {
...
3.  **`handleUpload`:**  This is the core upload function.
    ...
    *   **`formData.append('file', selectedFile)`:**  We append the file to the `FormData` object.  **Crucially, the first argument to `append` ('file') *must* match the name of the file field that your API route expects (i.e. what you're accessing in `files.file`).**
...
*   **File Type Validation:**  Validate the file type based on the MIME type (`file.mimetype`) and file extension (`file.originalFilename`).  Only allow permitted file types.  *Do not rely solely on the file extension, as it can be easily spoofed.*
    ...
    if (!allowedExtensions.includes(`.${fileExtension.toLowerCase()}`)) {
        return res.status(400).json({ error: "Invalid file extension. Only JPG, JPEG, PNG, and GIF images are allowed." });
    }
    ```
...
form.parse(req, async (err, fields, files) => {
    ...
    if (file.size > form.maxFileSize) {
        return res.status(413).json({ error: `File size exceeds the limit of ${form.maxFileSize / (1024 * 1024)}MB` });
    }
...
form.parse(req, async (err, fields, files) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "File uploaded successfully to S3!", url: data.Location }); // Return the S3 URL
  } catch (s3Error) {
