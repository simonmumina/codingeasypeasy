...
*   **Read-only:** Child components cannot modify the props they receive.  Props are meant to be data passed *down* the component tree.
...
*   `this.setState()` is used to update the state.  This triggers a re-render of the component.  **Important:**  `setState` merges the changes.  It does *not* replace the entire state object.
...
*   The `App` component composes the `Card` and `Button` components to create a UI element.  The `<Button>` is rendered *inside* the `<Card>`.
