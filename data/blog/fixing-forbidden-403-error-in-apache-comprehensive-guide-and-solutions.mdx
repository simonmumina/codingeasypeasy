...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['apache', '403 forbidden', 'error', 'web server', 'permissions', 'htaccess', 'configuration', 'troubleshooting', 'website', 'web development', 'linux', 'ubuntu', 'centos']
draft: false
...
*   `DirectoryIndex index.php index.html index.htm`: This tells Apache to look for `index.php` first, then `index.html`, and then `index.htm` if none of the previous files exist.  If none of these exist, and `Options Indexes` is present, the server *should* display a directory listing (but this may be disabled for security reasons).
...
If you *want* users to be able to see a list of files in a directory when an `index` file is missing (this is generally **not recommended** for security reasons), you need to enable directory listing.  This is controlled by the `Options` directive in your Apache configuration.
