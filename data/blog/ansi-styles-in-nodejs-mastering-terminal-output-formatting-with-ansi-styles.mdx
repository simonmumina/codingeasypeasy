...
console.log(`${ansiStyles.red.open}${ansiStyles.bgYellow.open}Warning!  This text is red with a yellow background!${ansiStyles.bgYellow.close}${ansiStyles.red.close}`);
```
...
console.log(`${ansiStyles.bold.open}${ansiStyles.cyan.open}${ansiStyles.underline.open}Important Information!${ansiStyles.underline.close}${ansiStyles.cyan.close}${ansiStyles.bold.close}`);
```

This code will output text that is **bold**, cyan, and *underlined*.  Remember to close the styles in the *reverse* order that you open them to ensure correct rendering.
...
console.log(`${ansiStyles.green.open}Hello, ${ansiStyles.bold.open}${username}${ansiStyles.bold.close}!${ansiStyles.green.close}`);

// Example with a background color
const error = 'Invalid input.';
console.log(`${ansiStyles.bgRed.open}${ansiStyles.white.open}ERROR: ${error}${ansiStyles.white.close}${ansiStyles.bgRed.close}`);
```
...
if (supportsColor) {
  console.log(`${ansiStyles.green.open}This text will be green (if supported)!${ansiStyles.green.close}`);
} else {
...
if (supportsColor) {
  if (supportsColor.level >= 3) { // truecolor support
    colorLevel = 3;
  } else if (supportsColor.level >= 2) { // 256 colors support
    colorLevel = 2;
  } else if (supportsColor.level >= 1) {  // 16 colors support
    colorLevel = 1;
  }
}
...
function getColorForLevel(level, colorName) {
  switch (level) {
    ...
    case 2:  // 256 Colors - Need to map colorName to a 256 color code
      switch (colorName) {
        case 'red': return ansiStyles.color.ansi256[196].open; // Example: bright red
        default: return ansiStyles.color.ansi256[16].open;  // Default to gray
      }
    case 1:  // 16 Colors
      switch (colorName) {
        case 'red': return ansiStyles.red.open;
        default: return ansiStyles.gray.open;
      }
    default: return "";  // No color support
  }
}

console.log(`${getColorForLevel(colorLevel, 'red')}This text might be red, depending on terminal support!${getColorForLevel(colorLevel, 'red') ? ansiStyles.reset.close : ""}`);
```
