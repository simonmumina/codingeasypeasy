...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['react-native', 'performance', 'optimization', 'memoization', 'userMemo', 'useCallback', 'memo', 're-renders', 'javascript']
draft: false
...
*   **Over-Memoization:** Don't memoize everything! Memoization comes with a cost (memory overhead and the comparison logic).  Only memoize components or functions that are known to be expensive or cause performance bottlenecks.  Excessive memoization can actually *decrease* performance.
...
*   **Dependency Arrays:**  Be meticulous with dependency arrays in `useMemo` and `useCallback`.  Include *all* variables that the function depends on.  Omitting a dependency can lead to stale closures and incorrect results.  However, including too many dependencies can defeat the purpose of memoization.
