...
date: '2024-10-27'
lastmod: '2024-10-28'
tags: ['javascript', 'regexp', 'regular expressions', 'quantifiers', 'pattern matching', 'programming', 'web development', 'tutorial']
draft: false
...
The `+` quantifier demands "one or more occurrences" of the preceding element.  Unlike `*`, it *requires* at least one instance for a match to succeed.  It's also greedy.
...
## The `{n}` Quantifier: Exactly *n* Occurrences

The `{n}` quantifier specifies that the preceding element must appear exactly *n* times to constitute a match.
...
## The `{n,}` Quantifier: *n* or More Occurrences

The `{n,}` quantifier requires at least *n* occurrences of the preceding element. It will match *n* or more times.
...
## The `{n,m}` Quantifier: Between *n* and *m* Occurrences

The `{n,m}` quantifier specifies a range of occurrences.  The preceding element must appear at least *n* times and at most *m* times (inclusive) to be considered a match.
...
By default, quantifiers are *greedy*, meaning they attempt to match as much of the input string as possible. Sometimes, this is not the desired behavior. You can make a quantifier *lazy* (or non-greedy) by adding a `?` after it.  A lazy quantifier will match the *minimum* number of occurrences required to satisfy the pattern.
...
In the second example, the greedy regex matched the entire string because `.*` is greedy and will match as much as it can *including* the `</p>` in the middle. The lazy version stops at the *first* `</p>`.
