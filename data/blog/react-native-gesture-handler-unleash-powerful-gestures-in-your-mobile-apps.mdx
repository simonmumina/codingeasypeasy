...
3.  **iOS Setup (IMPORTANT):**
    ...
    Then, add the following code *before* the `return rootView;` line in the `didFinishLaunchingWithOptions` method:
...
const TapExample = () => {
  const handleTap = (event) => {
    if (event.nativeEvent.state === 4) { // State 4 corresponds to the ACTIVE state (tap detected)
