...
date: '2024-02-29'
lastmod: '2024-10-27'
tags: ['react-native', 'react-navigation', 'stackactions', 'navigation', 'mobile-development', 'javascript']
draft: false
...
1.  We import `StackActions` from `@react-navigation/native`.
2.  We dispatch a `StackActions.replace('Details', { itemId: 99, otherParam: 'Replaced!' })` action.  This replaces the *current* `DetailsScreen` with a *new* `DetailsScreen` instance, passing new parameters. Note that the current screen is removed from the stack and a new screen with the same name is added.
...
*   **Mixing up `navigate` and `push`:**  While `navigate` can often behave like `push`, it's important to understand the difference. `navigate` checks if a route already exists on the stack and navigates to it if it does.  `push` *always* adds a new instance of the route to the stack.
