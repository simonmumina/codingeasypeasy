...
date: '2024-01-26'
lastmod: '2024-10-27'
tags: ['react-native', 'json', 'fetch', 'data fetching', 'local storage', 'ios', 'android', 'mobile development', 'asynchronous', 'useEffect']
draft: false
...
*   **`const data = require('../../assets/products.json');`**: This line is the *core* of fetching data from the local JSON file.  `require` is used to import the JSON file, making its contents available as a JavaScript object (or array in this case).  **Important:**  The path `../../assets/products.json` is *relative* to the component file (`ProductList.js`). Make sure to adjust the path according to your project structure.
...
The `fs` (file system) module provides asynchronous methods for reading files. This is suitable for larger JSON files to avoid blocking the UI thread.  *However, using the `fs` module in React Native requires linking native modules, which adds complexity.*
...
*   For the `fs` module, you'll need to install the `react-native-fs` package: `npm install react-native-fs` or `yarn add react-native-fs`.  You'll also need to *link* the native module for it to work correctly. Instructions for linking can be found in the `react-native-fs` documentation.
