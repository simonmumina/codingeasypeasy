...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['awk', 'linux', 'text parsing', 'data extraction', 'command line tools', 'shell scripting', 'data analysis', 'pattern matching', 'data manipulation']
draft: false
...
*   `FS`: The field separator (default is whitespace).  You can set this *within* the AWK script itself if you prefer: `BEGIN { FS = "," }`
...
This example splits each line into an array of words based on the space character and then prints each word along with its index.  Note the use of `in` to iterate through the *indices* of the array.
