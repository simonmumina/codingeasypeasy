...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['next.js', 'router', 'navigation', 'programmatic navigation', 'useRouter', 'next/router', 'JavaScript', 'React']
draft: false
...
In the `shallow` routing example, only the URL is updated.  `getServerSideProps` (or `getStaticProps` if applicable) will *not* be re-executed.  This can improve performance if you're only changing a small part of the UI and don't need to re-fetch data. Note: if the page does *not* have `getServerSideProps` or `getStaticProps` defined, and you are using `shallow: true`, then the state will also not be re-populated on refresh.

### 2. `router.replace(url, as, options)`

The `router.replace()` method navigates to a specified URL, *replacing* the current URL in the browser history stack. This means that the user cannot use the browser's back button to return to the previous page.  This is useful for redirects or scenarios where you don't want the user to be able to navigate back.
