...
*   **`SECRET_KEY`:**  A strong, random `SECRET_KEY` is *crucial*.  This key is used to encrypt the CSRF tokens.  Do *not* use the example key in production!  Generate a truly random key using `secrets.token_hex(16)` or similar methods. Store this secret key safely, potentially as an environment variable.
...
*   **`{{ form.csrf_token }}` or `{{ csrf_token() }}`:**  These Jinja2 template expressions render a hidden input field containing the CSRF token. Make sure this field is *inside* the `<form>` element.
