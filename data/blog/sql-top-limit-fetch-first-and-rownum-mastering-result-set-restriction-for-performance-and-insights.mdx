...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['SQL', 'TOP', 'LIMIT', 'FETCH FIRST', 'ROWNUM', 'Database Optimization', 'Query Performance', 'Result Set Restriction', 'SQL Server', 'MySQL', 'PostgreSQL', 'Oracle']
draft: false
...
**Important Note:** `ROWNUM` is assigned *before* the `ORDER BY` clause is applied (in the inner query if using a subquery). Therefore, you need to use a subquery if you want to restrict rows *after* ordering.
...
2.  `ROWNUM` is assigned to each row *as it is being selected by the inner query* **before** the `ORDER BY` Clause.
