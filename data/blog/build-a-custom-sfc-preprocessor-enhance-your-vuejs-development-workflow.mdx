...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['vuejs', 'sfc', 'single file component', 'preprocessor', 'javascript', 'webpack', 'vite', 'custom tooling', 'development workflow']
draft: false
...
Single File Components (SFCs) are a cornerstone of Vue.js development. They provide a clean and organized way to encapsulate HTML, JavaScript, and CSS within a single `.vue` file.  But what if you want to add custom logic *before* your SFC is compiled?  That's where SFC preprocessors come in!
...
An SFC preprocessor is a tool that intercepts the `.vue` file *before* it's processed by the Vue.js compiler (e.g., `vue-loader` in webpack or `@vitejs/plugin-vue` in Vite). It allows you to modify the content of the SFC, add custom logic, or perform any other transformations you need.
...
const addTimestamp = (content) => {
  ...
  if(dataFunctionIndex !== -1) {
    ...
    const newScriptContent = scriptContent.substring(0, insertionPoint) + newData + scriptContent.substring(insertionPoint);
    return content.substring(0, scriptStart + 8) + newScriptContent + content.substring(scriptEnd);

  } else { // no data function, create one
    ...
    if(exportDefaultIndex !== -1) {
      ...
      console.warn("Could not find 'export default {' in script tag.  Preprocessor may not work correctly.");
...
const addTimestampAdvanced = (content, filename) => {
  ...
  if (descriptor.script) {
    ...
    if (dataFunctionIndex !== -1) {
      const insertionPoint = dataFunctionIndex + 'data() {'.length;
      const newData = `\n    timestamp: new Date().toISOString(),\n`;
      newScriptContent = scriptContent.substring(0, insertionPoint) + newData + scriptContent.substring(insertionPoint);
    } else {
      const exportDefaultIndex = scriptContent.indexOf('export default {');
        if(exportDefaultIndex !== -1) {
          const insertionPoint = exportDefaultIndex + 'export default {'.length;
  ...
  plugins: [
    vue({
      template: {
        ...
        preprocessOptions: {
          script: ({ content, filename }) => { // Added filename
            return {
