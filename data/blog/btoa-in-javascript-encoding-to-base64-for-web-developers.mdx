...
*   **Encryption vs. Encoding:**  `btoa()` performs encoding, *not encryption*.  Base64 is easily reversible, so it's not suitable for protecting sensitive data.  Anyone with access to the Base64 encoded string can easily decode it.
