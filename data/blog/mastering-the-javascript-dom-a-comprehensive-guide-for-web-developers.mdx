...
*   **`document.querySelector(selector)`:** Selects the *first* element that matches the specified CSS selector.  This is a powerful and flexible method that allows you to use CSS selectors to target elements. Returns a single element, or `null` if no match is found.
...
*   **`document.querySelectorAll(selector)`:** Selects *all* elements that match the specified CSS selector. Returns a `NodeList`, which is a static collection (it doesn't update automatically if the DOM changes).
...
These properties are similar to the ones above, but they only return *element* nodes, ignoring text nodes and comments.  This can make it easier to work with the DOM in some cases.
