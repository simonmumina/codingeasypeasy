...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['css', 'mask-composite', 'css masking', 'web design', 'front-end development', 'creative css', 'web development', 'user interface', 'ui design']
draft: false
...
*   **`source-out`:** The source mask is kept only where it *doesn't* overlap the destination mask. The overlapping portion is discarded.
...
*   **`destination-out`:** The destination mask is kept only where it *doesn't* overlap the source mask. The overlapping portion is discarded.
...
*   **`xor`:**  The mask becomes visible where either mask layer (source or destination) is visible, but *not* where they both overlap. This creates an "exclusive or" effect.
...
In this example, `mask2.png` is applied *over* `mask1.png`. The areas where `mask2.png` is opaque will obscure the underlying `mask1.png` and the element's background.
...
Here, only the parts of `mask1.png` that *overlap* with the opaque areas of `mask2.png` will be visible.  The rest of `mask1.png` will be discarded.
...
This example shows only the parts of `mask1.png` that do *not* overlap with the opaque areas of `mask2.png`. The overlapping section is removed.
