...
date: '2024-01-01'
lastmod: '2024-10-27'
tags: ['expo', 'react-native', 'environment variables', 'dotenv', 'development', 'production', 'configuration', 'config', 'api keys', 'secrets', 'eas build']
draft: false
...
*   **`.gitignore`:** Add your `.env` file to your `.gitignore` file to prevent accidentally committing it to your repository. This is *critical* to avoid exposing sensitive information.
...
*   **Not Suitable for Runtime Secrets:** Values are baked into the build at build time.  This is *not* secure for sensitive runtime secrets that should not be stored in the compiled app.
...
The EAS Build environment variables are automatically injected into the build process and are accessible via `process.env`.  You should access these variables in your `app.config.js` file, which will be evaluated *during the build process*.
...
3.  **Access the value:**  The `DATABASE_PASSWORD` environment variable will be available during the build process, and you can access it from your `app.config.js`. *Crucially*, the actual secret value is never stored in your `eas.json` file or your application code.
