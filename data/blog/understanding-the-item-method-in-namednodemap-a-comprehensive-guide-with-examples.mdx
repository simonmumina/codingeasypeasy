...
Before we dive into the `item()` method, let's briefly understand what `NamedNodeMap` is. It's not an array, despite behaving somewhat like one. It's a *live* collection of attribute nodes associated with an element. This means that changes to the element's attributes are immediately reflected in the `NamedNodeMap` and vice versa.
...
*   **`getAttribute(name)`:**  This method retrieves the value of an attribute by its *name*.  It's generally preferred over `item()` because it's more readable and you don't need to know the attribute's index.  It returns `null` if the attribute is not present.
