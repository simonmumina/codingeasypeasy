...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['html', 'dom', 'javascript', 'caption', 'table', 'web development', 'front-end', 'web accessibility']
draft: false
...
*   Calling `createCaption()` on a table that already has a caption will *not* create a second caption.  Instead, it will simply return a reference to the existing caption.
*   The new caption element is automatically inserted as the *first* child of the `<table>` element, directly after any `<colgroup>` or `<caption>` elements that may already exist.
