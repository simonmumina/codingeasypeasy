...
Note: `subarray()` creates a *view*, not a copy.  Changes to the subarray will also affect the original array.
...
*   `subarray()`:  Creates a new *view* of the existing Typed Array.
...
*   **Views vs. Copies:**  Understand the difference between `subarray()` which creates a *view* and `slice()` which creates a *copy*.  Modifying a subarray also modifies the original array.
