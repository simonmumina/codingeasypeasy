...
`git revert` is a command that creates a *new* commit that *undoes* the changes introduced by a *specific* commit.  Instead of rewriting history (like `git reset`), `git revert` adds a new commit that effectively cancels out the changes of the targeted commit. This leaves your history intact, which is crucial for collaborative projects.
...
*   **Easy to Track:**  The revert commit clearly shows *what* was undone and *why*.  This provides valuable context for future developers who may encounter the reverted code.
...
3.  **Review and Commit:**  Git will often open a text editor with a pre-populated commit message for the revert.  Review the changes and the commit message, and save the file to complete the revert.  If your changes are simple, Git may automatically create the revert commit without opening an editor.
    ...
    The default commit message will be something like: `Revert "Original commit message"`.  It's a good practice to add more context to the commit message explaining *why* you're reverting the commit.
...
2.  **Revert the Merge Commit:** Use the following command:
    ...
    Replace `<parent-number>` with the number of the parent branch you want to keep.  The parent numbers are determined by the order in which they appear in the commit message of the merge commit.  Generally, the main branch you merged *into* is parent 1, and the branch you merged *from* is parent 2.  It's crucial to understand which parent represents the changes you want to *keep* and specify the *other* parent's number in the `-m` option.
...
*   **Add Descriptive Commit Messages:**  Provide clear and descriptive commit messages for your revert commits.  Explain *why* you're reverting and what impact the revert is intended to have. This makes it easier for others (and your future self) to understand the history of the project.
...
*   **`git revert`:** Creates a *new* commit to undo changes. Preserves history. Safe for shared branches.
*   **`git reset`:** Moves the current branch pointer to a *previous* commit. Rewrites history.  Dangerous for shared branches as it can cause conflicts.
