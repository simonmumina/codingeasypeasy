...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'hashchangeevent', 'url', 'hash', 'web-development', 'browser-api', 'single-page-application', 'spa', 'dom']
draft: false
...
The `oldURL` property of the `HashChangeEvent` interface contains a string representing the absolute URL of the document *before* the hash was modified.  This allows you to track where the user was coming from before the hash change occurred. It gives you a snapshot of the previous URL state.
...
The `oldURL` property allows you to trigger specific actions based on where the user *came from*. For example, you might want to display a different animation or load different data depending on the previous hash.
...
*   The `oldURL` property provides the URL *before* the hash was modified, giving context about the user's previous location.
...
*   **Empty `oldURL`:**  If the hash change is the *first* event to occur on a page, `oldURL` might be empty or `null`.  Always check for this possibility before attempting to use it.
