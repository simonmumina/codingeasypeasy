...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'infinity', 'positive_infinity', 'number', 'data-types', 'numerical-computation', 'js-fundamentals']
draft: false
...
`POSITIVE_INFINITY` can be used as an initial value when searching for the *smallest* value in a dataset.  Because `POSITIVE_INFINITY` is larger than any other finite number, the first value you compare against it will always be smaller, correctly initializing your minimum value search.
...
As you can see, `NEGATIVE_INFINITY` is the negative counterpart to `POSITIVE_INFINITY`.  It's commonly used as an initial value when searching for the *largest* value in a dataset.
