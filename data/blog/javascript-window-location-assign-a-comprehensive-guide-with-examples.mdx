...
*   **`assign()`:**  Navigates to a new URL *and adds the new URL to the browser's history stack*. The user can use the "Back" button to return to the previous page.
*   **`replace()`:** Navigates to a new URL *but replaces the current entry in the browser's history*. The user cannot use the "Back" button to return to the previous page.  This is useful for scenarios like redirecting after login or logout, where you don't want the user to be able to go back to the login page.
