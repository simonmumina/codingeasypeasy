...
- **Don't use cursor changes as the _only_ indicator of functionality.** Ensure that interactive elements are clearly identifiable through other visual cues like color changes, underlines, or borders.
...
- **Using `cursor: pointer;` excessively.** Only apply it to elements that are _actually_ interactive (links, buttons, form controls). Using it on non-interactive elements can confuse users.
