...
date: "2024-01-01"
lastmod: "2024-10-27"
tags: ["vue", "electron", "desktop apps", "javascript", "tutorial", "framework", "cross-platform", "vuejs", "electronjs"]
draft: false
...
*   `webPreferences`:  Configures the window's behavior.
    ...
    *   `preload`:  Specifies a script that will be executed *before* the web page loads. This is often used for more secure communication (see later section).
...
export default {
  ...
  methods: {
    async openDialog() {
      const result = await window.electronAPI.showMessageBox({  // Use the exposed API
        title: 'My App',
