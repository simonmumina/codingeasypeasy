...
date: '2024-02-29'
lastmod: '2024-03-08'
tags: ['nestjs', 'saga pattern', 'distributed transactions', 'microservices', 'event sourcing', 'compensating transactions', 'architecture']
draft: false
...
export class PaymentService {
  ...
  async processPayment(command: ProcessPaymentCommand): Promise<boolean> {
    ...
    if (isPaymentSuccessful) {
      ...
      this.amqpConnection.publish<string>('order.exchange', 'payment.processed', { orderId: command.orderId });
      return true;
    } else {
      console.error(`Payment failed for order ID: ${command.orderId}`);
       this.amqpConnection.publish<string>('order.exchange', 'payment.failed', { orderId: command.orderId, reason: "Payment declined" });
      return false;
    }
  }
  ...
  async cancelPayment(command: CancelPaymentCommand): Promise<void> {
    ...
    this.amqpConnection.publish<string>('order.exchange', 'payment.cancelled', { orderId: command.orderId });
  }
...
export class InventoryService {
  ...
  async reserveInventory(command: ReserveInventoryCommand): Promise<boolean> {
    console.log(`Reserving inventory for order ID: ${command.orderId}, Product ID: ${command.productId}, Quantity: ${command.quantity}`);
    ...
    if (isInventoryAvailable) {
      ...
      this.amqpConnection.publish<string>('order.exchange', 'inventory.reserved', { orderId: command.orderId });
      return true;
    } else {
      console.error(`Inventory unavailable for order ID: ${command.orderId}`);
      this.amqpConnection.publish<string>('order.exchange', 'inventory.reservation.failed', { orderId: command.orderId, reason: "Insufficient Stock" });
      return false;
    }
  }

  async releaseInventory(command: ReleaseInventoryCommand): Promise<void> {
    console.log(`Releasing inventory for order ID: ${command.orderId}, Product ID: ${command.productId}, Quantity: ${command.quantity}`);
    // Simulate inventory release (e.g., update the inventory database)
    // In a real application, you would interact with an inventory management system here
    this.amqpConnection.publish<string>('order.exchange', 'inventory.released', { orderId: command.orderId });
  }
...
export class ShippingService {
  ...
  async scheduleShipping(command: ScheduleShippingCommand): Promise<boolean> {
    ...
    if (isShippingPossible) {
      ...
      this.amqpConnection.publish<string>('order.exchange', 'shipping.scheduled', { orderId: command.orderId });
      return true;
    } else {
      console.error(`Shipping failed for order ID: ${command.orderId}`);
      this.amqpConnection.publish<string>('order.exchange', 'shipping.scheduling.failed', { orderId: command.orderId, reason: "Shipping not available to this location" });
      return false;
    }
  }
  ...
  async cancelShipping(command: CancelShippingCommand): Promise<void> {
    ...
    // In a real application, you would interact with a shipping provider API here
    this.amqpConnection.publish<string>('order.exchange', 'shipping.cancelled', { orderId: command.orderId });
  }
...
export class SagaOrchestratorService {
  ...
  async handleOrderCreatedEvent(orderCreatedEvent: OrderCreatedEvent): Promise<void> {
    ...
    await this.amqpConnection.publish<string>('order.exchange', 'payment.process', processPaymentCommand);
  ...
  async handlePaymentProcessedEvent(msg: { orderId: string }): Promise<void> {
    ...
    await this.amqpConnection.publish<string>('order.exchange', 'inventory.reserve', reserveInventoryCommand);
  ...
  async handleInventoryReservedEvent(msg: { orderId: string }): Promise<void> {
    ...
    await this.amqpConnection.publish<string>('order.exchange', 'shipping.schedule', scheduleShippingCommand);
  ...
  async handlePaymentFailedEvent(msg: { orderId: string; reason: string }): Promise<void> {
    this.logger.error(`Received payment.failed event for order ID: ${msg.orderId}, Reason: ${msg.reason}`);
    // Cancel the order
  ...
  async handleInventoryReservationFailedEvent(msg: { orderId: string; reason: string }): Promise<void> {
    this.logger.error(`Received inventory.reservation.failed event for order ID: ${msg.orderId}, Reason: ${msg.reason}`);
    ...
    await this.amqpConnection.publish<string>('order.exchange', 'payment.cancel', cancelPaymentCommand);
     await this.cancelOrder(msg.orderId, msg.reason);
  }
  ...
  async handleShippingSchedulingFailedEvent(msg: { orderId: string; reason: string }): Promise<void> {
    this.logger.error(`Received shipping.scheduling.failed event for order ID: ${msg.orderId}, Reason: ${msg.reason}`);
     ...
     const releaseInventoryCommand: ReleaseInventoryCommand = {
      ...
      await this.amqpConnection.publish<string>('order.exchange', 'inventory.release', releaseInventoryCommand);
       await this.cancelOrder(msg.orderId, msg.reason);
  ...
  private async cancelOrder(orderId: string, reason: string): Promise<void> {
    ...
    await this.amqpConnection.publish<string>('order.exchange', 'order.cancelled', orderCancelledEvent);
  ...
  async handleOrderCancelledEvent(msg: { orderId: string, reason: string }): Promise<void> {
        this.logger.log(`Received order.cancelled event for order ID: ${msg.orderId}, Reason: ${msg.reason}`);
