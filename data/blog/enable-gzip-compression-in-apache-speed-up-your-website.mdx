...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['apache', 'gzip', 'compression', 'web performance', 'website optimization', 'http', 'mod_deflate', 'server configuration']
draft: false
...
Gzip is a popular data compression algorithm used to reduce the size of files transmitted over the internet.  When a browser requests a file from your server, and Gzip compression is enabled, the server compresses the file using Gzip *before* sending it. The browser then decompresses the file upon arrival.  This significantly reduces the amount of data transferred, leading to:
...
*   **`SetOutputFilter DEFLATE` vs. `AddOutputFilterByType DEFLATE`:**  `SetOutputFilter DEFLATE` compresses *all* content, regardless of MIME type.  It's generally not recommended as it can cause issues with binary files (images, videos, etc.). `AddOutputFilterByType` is the preferred method as it allows you to selectively compress specific file types.
