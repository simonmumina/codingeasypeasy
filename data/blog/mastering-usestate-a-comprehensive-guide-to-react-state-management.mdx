...
The argument you pass to `useState` is used only during the *initial render*. If you try to re-run `useState`, React will ignore the passed value. This is an important point to understand.
...
*   The `setCount` function now receives a callback function.  This callback receives the *previous* state value (`prevCount`) as its argument.
*   The callback then returns the *new* state value (`prevCount + 1`).
...
*   The `updateFirstName` function uses the spread operator (`...`) to create a *new* object that copies all the properties from the previous `user` object.
*   We then update the `firstName` property in the new object.
*   It's crucial to create a *new* object (or array) when updating state that is an object or array. React relies on *immutability* to detect changes and trigger re-renders. Directly modifying the existing object will not trigger a re-render.
...
*   The `addTodo` function uses the spread operator (`...`) to create a *new* array that contains all the elements from the previous `todos` array, plus the new `newTodo` element.
*   The `removeTodo` function utilizes the `filter` method to create a *new* array that excludes the todo at the specified `indexToRemove`.  The filter method returns a new array, ensuring immutability.
*   Again, creating *new* arrays is essential for triggering re-renders.
