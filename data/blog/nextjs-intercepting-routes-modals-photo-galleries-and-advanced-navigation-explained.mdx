...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['nextjs', 'intercepting routes', 'nextjs 13', 'app router', 'modals', 'photo gallery', 'navigation', 'typescript']
draft: false
...
2.  **Intercepting Route Convention `(.)`:**  Prefixing a segment in your route structure with `(.)` tells Next.js to intercept the route if it's navigated to from the *current* route segment (or a parent segment).
...
*   `app/(.)blog/[slug]/page.tsx` is the *intercepting route*.  When you navigate to `blog/[slug]` from within `app/page.tsx`, this route is loaded instead of `app/blog/[slug]/page.tsx`.
...
*   When you click on a blog post link in the blog list (`app/page.tsx`), Next.js will match the URL (`/blog/[slug]`) to the intercepting route (`app/(.)blog/[slug]/page.tsx`) because the navigation originates from the *same* route segment or a parent segment.
