...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['java', 'exception handling', 'UnsupportedOperationException', 'immutable collections', 'java collections', 'debugging', 'java programming', 'error resolution']
draft: false
...
`Arrays.asList()` returns a fixed-size list backed by the original array.  It's *not* a `java.util.ArrayList`.  While it implements the `List` interface, it doesn't support the `add()` operation.  Hence, the `UnsupportedOperationException` is thrown.
...
`subList()` returns a *view* of the original list.  Modifying the sublist directly affects the original list *unless* the original list is structurally modified *before* the sublist is modified.  If you modify the original list's structure (e.g., adding or removing elements) before modifying the sublist, the sublist becomes invalid, and any operation on it can throw `ConcurrentModificationException` (more likely) or `UnsupportedOperationException` in some implementations.  However, clearing the sublist works because it uses the original list's modification methods under the hood.
...
Override the method in the subclass if you need to provide a specific implementation.  If the method truly *shouldn't* be supported in the subclass, leave it as is and document why the exception is thrown.
...
Avoid calling modification methods on the unmodifiable wrapper.  If you need to modify the list, do it on the original list *before* creating the unmodifiable wrapper, or create a *copy* of the original and modify the copy, then create the unmodifiable wrapper around that copy.
