...
*   **`beforeUnmount` (Vue 3) / `beforeDestroy` (Vue 2):** This hook is called *before* the component is unmounted (Vue 3) or destroyed (Vue 2).  It's the ideal place to remove any event listeners that were added during the component's lifetime.  This ensures that listeners are removed *before* the component is no longer needed, preventing memory leaks.

*   **`unmounted` (Vue 3) / `destroyed` (Vue 2):**  This hook is called *after* the component has been unmounted/destroyed. While you *can* potentially remove listeners here, it's generally better to use `beforeUnmount`/`beforeDestroy` because the component is still fully functional at that point.
...
*   **Function Reference:**  When removing an event listener, you *must* use the **same function reference** that was used when adding it. If you create a new anonymous function when removing the listener, it won't work. This is a very common source of errors!
