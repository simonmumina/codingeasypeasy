...
Web browsers don't always have access to every font. To account for this, the `font-family` property allows you to specify a *font stack*.  A font stack is a list of font families, separated by commas. The browser will attempt to use the first font in the list. If it's unavailable, it will move on to the second, and so on.  This ensures that even if a user doesn't have your preferred font installed, they will still see a readable typeface.
...
Generic font families provide broad classifications of fonts, ensuring that *some* font is always displayed, even if the user doesn't have the specific fonts you intended. Here are the most common generic font families:
...
*   **`font-display`:** This is a *critical* property for optimizing font loading.  It controls how the browser handles the display of text while the font is loading. Common values include:
