...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'html dom', 'queryselectorall', 'element selection', 'web development', 'front-end', 'dom manipulation']
draft: false
...
*   **`NodeList` (returned by `querySelectorAll()`):**
    *   A *static* collection of nodes.  Changes to the DOM after the `NodeList` is created are *not* reflected in the `NodeList`.
...
*   **`HTMLCollection` (returned by `getElementsByClassName()`, `getElementsByTagName()`):**
    *   A *live* collection.  Changes to the DOM are immediately reflected in the `HTMLCollection`.
    *   Contains only element nodes.
    *   Does *not* directly support `forEach()`. You can iterate over it using a `for` loop or convert it to an array (e.g., `Array.from(htmlCollection)`).
