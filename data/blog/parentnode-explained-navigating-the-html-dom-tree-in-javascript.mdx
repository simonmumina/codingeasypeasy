...
*   **Direct Parent:** It returns only the *direct* parent. To traverse higher up the DOM tree, you may need to chain `parentNode` multiple times.
...
While you can't *set* the `parentNode` of an element, you can use it to modify attributes or styles of the parent element.
...
While `parentNode` returns the parent node, including text and comment nodes, `parentElement` *only* returns the parent element node (or `null` if there isn't one).  This difference is crucial in certain situations.

**When to use `parentElement`:**

*   When you are specifically interested in getting the parent *element* and don't want to deal with text or comment nodes.
...
The last `console.log` will output `null` because a comment node does not have a parent *element*, even though it has a parent *node*.
