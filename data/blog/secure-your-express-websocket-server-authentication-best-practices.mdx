...
*   **`server.on('upgrade')`:**  The `upgrade` handler now runs the `sessionMiddleware` *before* handling the WebSocket upgrade.  This ensures that `req.session` is populated with the user's session data. We check if `req.session.user` exists. If not, the connection is rejected.
...
*   **`secure: true`:** In production, *always* set the `secure` option to `true` for your session cookie when using HTTPS. This prevents the cookie from being transmitted over unencrypted HTTP connections.
