...
date: '2024-02-29'
lastmod: '2024-10-27'
tags: ['next-js', 'route-groups', 'file-system-routing', 'react', 'web-development', 'nextjs-tutorial', 'nextjs-best-practices']
draft: false
...
The `(shop)` directory *only* serves as an organizational tool.
...
When the user clicks on the thumbnail, the `Modal` component will render *within* the `GalleryPage` layout, creating the effect of a modal window.  Clicking "Close" will navigate back to the gallery page.
...
*   **Confusing with Regular Folders:** Route groups are *not* the same as regular folders. They only affect the file structure, not the URL structure.
