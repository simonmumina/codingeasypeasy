...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['compressed sensing', 'sparse signal processing', 'signal recovery', 'mathematics', 'linear algebra', 'optimization', 'data compression', 'image processing', 'machine learning']
draft: false
...
Compressed sensing (CS), also known as compressive sensing or sparse recovery, is a revolutionary signal processing technique that allows us to accurately reconstruct a signal from significantly fewer samples than required by the Nyquist-Shannon sampling theorem.  This counter-intuitive result is possible when the signal is *sparse* in some domain, meaning it contains only a few significant components.
...
A signal `x` of length `N` is considered *K-sparse* if it has at most `K` non-zero coefficients in some basis `Ψ`.  This can be written as:
...
*   **Sparsifying Basis:**  The recovery is modified.  The sparsifying basis matrix `Psi` is created to map *back* from the wavelet coefficients to the signal domain. The constraint now becomes `A @ Psi @ s = y` where s is a vector of the wavelet coefficients that we need to find via L1 minimization.
