...
2.  **`useEffect` Hook:**  This hook ensures that the container element is created and appended to the `document.body` on the first render of the component.  It also handles cleanup by removing the container when the component unmounts. *This is important to avoid memory leaks!*  The container is created with the id `portal-container`.
