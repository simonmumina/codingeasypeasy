...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['node.js', 'express.js', 'connect-flash', 'flash messages', 'middleware', 'session management', 'web development', 'javascript']
draft: false
...
Flash messages are temporary notifications stored in the user's session. They are designed to be displayed only *once* to the user, typically after a redirect.  Think of them as a "flash" of information - they appear briefly and then disappear. Common use cases include:
...
Next, you need to configure your Express.js application to use sessions.  This is *essential* for `connect-flash` to work correctly.  Open your main application file (e.g., `app.js` or `server.js`) and add the following:
...
*   **`session()` Middleware:**  Configures the session middleware.
    *   `secret`:  A *critical* option.  Replace `'your-secret-key'` with a long, random, and securely stored secret key.  This key is used to sign the session ID cookie, protecting against tampering.  **Do not use the example key in production!**
...
*   **Secret Key:** As emphasized above, *never* hardcode your session secret in production.  Store it in an environment variable and retrieve it from there.  A strong, random secret is vital for security.
*   **HTTPS:** In production environments, *always* use HTTPS and set `cookie: { secure: true }`. This prevents session cookies from being intercepted over insecure HTTP connections.
...
*   **`res.redirect('/profile')`:**  Crucially, we redirect the user *after* setting the flash messages. This is because the messages are stored in the session and will be retrieved on the next request (the request for `/profile` after the redirect).
