...
*   **Key Types:**  `Map` allows you to use *any* data type (including objects, functions, and primitives) as keys.  Plain objects, on the other hand, only allow strings or symbols as keys.
...
You *could* iterate through the `Map` and manually count the entries. However, this is significantly less efficient than using the `size` property.
...
Another *highly* inefficient method is converting the `Map` to an array and then getting the length of the array.  Avoid this approach.
