...
*   **`lastChild`**:  Returns the *last child node* of an element, regardless of its node type.  This includes elements, text nodes, and comment nodes.
*   **`lastElementChild`**: Returns the *last child element* of an element. It ignores text nodes, comment nodes, and other non-element nodes.  If there are no element children, it returns `null`.
...
In this example, `lastChild` returns the comment node because it is the actual last child.  `lastElementChild`, on the other hand, returns the `<p>` element because it's the last *element* child.
