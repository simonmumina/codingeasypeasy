...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['expo', 'react-native', 'deep linking', 'universal links', 'app linking', 'mobile development', 'routing', 'navigation']
draft: false
...
3.  **Configure `android:autoVerify="true"` in your `AndroidManifest.xml`:**  This tells Android to automatically verify your app links. Expo manages this for you after prebuilding.
    ...
    *If you are using EAS build and configured your `android` block correctly, you do NOT need to manually alter your `AndroidManifest.xml`.*  Expo handles the auto-verification when you do `expo prebuild`.
...
*   **HTTPS:**  Both the `apple-app-site-association` and `assetlinks.json` files *must* be served over HTTPS.
...
*   Open a link to your website (e.g., `https://yourdomain.com/profile/123`) in Safari on a real device (not the simulator). The device *must* be physically connected or using TestFlight because iOS doesn't support associated domains on emulators.  **This is a common mistake.** The link should open directly in your app.  If it opens in Safari, double-check your `apple-app-site-association` file and Xcode configuration.
