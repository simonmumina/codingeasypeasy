...
The `ISNULL()` function evaluates `check_expression`. If it's `NULL`, the function returns `replacement_value`. If `check_expression` is *not* `NULL`, the function returns the value of `check_expression` itself.
...
While you *can* use `ISNULL()` in a `WHERE` clause, it's usually not the most efficient approach and can hinder index usage.  It's generally better to use `IS NULL` or `IS NOT NULL` for filtering based on `NULL` values.
...
In this example, `COALESCE()` will first check if `Email` is `NULL`. If not, it returns `Email`. If `Email` *is* `NULL`, it then checks if `Phone` is `NULL`. If not, it returns `Phone`. Finally, if both `Email` and `Phone` are `NULL`, it returns 'No Contact Info'.
