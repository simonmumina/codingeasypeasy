...
date: "2024-10-27"
lastmod: "2024-10-27"
tags: ["PageRank", "Google", "Algorithm", "Mathematics", "Linear Algebra", "Eigenvectors", "Markov Chains", "SEO", "Search Engine Optimization"]
draft: false
...
The PageRank of a page *i*, denoted as PR(*i*), is calculated as follows:
...
*   **T1, T2, ..., Tn:** The pages that link to page *i*. These are the "in-links" of page *i*.
*   **PR(Tj):** The PageRank of page *Tj*, a page that links to page *i*.
*   **C(Tj):** The number of outgoing links on page *Tj*. This represents the "vote" strength of page *Tj*.  A page with many outgoing links dilutes its "vote" for each page it links to.
...
If page *j* has *n* outgoing links, then M(i, j) = 1/n  if page *j* links to page *i*, and 0 otherwise.
...
However, this equation only works if the matrix *M* is modified to handle "dangling nodes" (pages with no outgoing links) and to incorporate the damping factor.
...
To address this, we modify the transition matrix to ensure that all columns sum to 1.  For dangling nodes, we assume that the user jumps to a random page on the web. This means that for any dangling node *j*,  M(i, j) = 1/N  for all pages *i* where N is the total number of pages.
