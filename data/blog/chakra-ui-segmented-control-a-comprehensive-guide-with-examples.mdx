...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['chakra ui', 'segmented control', 'react', 'ui components', 'component library', 'accessibility']
draft: false
...
1.  **Import necessary components:** We import `ButtonGroup`, `Button`, `useRadioGroup`, and `Stack` from `@chakra-ui/react`.  While `useRadioGroup` isn't strictly *required* for a simple segmented control, it's highly recommended for enhanced accessibility and managing focus states, especially with keyboard navigation. However, it can increase the complexity, so we'll show a simplified version first.  We also import `useState` to manage the selected option.
