...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['react', 'react hooks', 'eslint', 'code quality', 'javascript', 'best practices', 'useeffect', 'uselayoutEffect', 'useref', 'usestate']
draft: false
...
In this example, the `useEffect` hook only runs once when the component mounts because the dependency array is empty.  If `userId` changes, the effect will *not* re-run, and the component will display stale data.
...
**Important:** Only ignore dependencies when you are absolutely sure you understand the consequences.  Ignoring dependencies can lead to subtle and difficult-to-debug bugs.  Document *why* you are ignoring the dependency with a clear comment.
