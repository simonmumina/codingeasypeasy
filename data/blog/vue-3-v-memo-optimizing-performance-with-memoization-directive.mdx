...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['vue', 'vue3', 'v-memo', 'memoization', 'performance', 'optimization', 'directive', 'javascript']
draft: false
...
*   `v-memo="[item.name, filter]"` tells Vue to only re-render an individual list item if its `name` property *or* the `filter` value has changed.
...
*   **`v-memo` vs. `v-once`:**  `v-once` renders an element (and its children) only once.  `v-memo` is more flexible as it allows you to specify dependencies.  If you need an element to *never* re-render, `v-once` is the more appropriate choice. If you need to re-render based on certain conditions, `v-memo` is the way to go.
