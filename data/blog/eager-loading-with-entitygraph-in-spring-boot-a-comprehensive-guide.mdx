...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['spring-boot', 'jpa', 'entitygraph', 'eager-loading', 'performance', 'hibernate', 'data-access']
draft: false
...
Lazy loading, the default behavior in many cases, postpones fetching related entities until they are explicitly accessed. While this avoids unnecessary data retrieval in some situations, it can lead to the N+1 problem when you eventually *do* need the related entities.

Eager loading, on the other hand, fetches the related entities along with the parent entity in a single query. This reduces the number of database round trips and can significantly improve performance. However, eagerly loading *all* relationships can also be inefficient if you don't always need them.  `@EntityGraph` allows you to selectively eager load specific relationships.
...
*   **`FETCH`**: This option overrides the default lazy loading strategy.  If the entity mapping already defines a relationship as `FetchType.EAGER`, using `FETCH` will not change anything for that specific relationship.  However, it *will* eagerly load any relationships that are specified in the entity graph, even if they are normally lazy-loaded.  Be cautious when using `FETCH`, as it can potentially lead to over-fetching and performance issues if not used carefully.
