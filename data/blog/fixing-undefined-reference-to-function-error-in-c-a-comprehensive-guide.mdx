...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['C++', 'undefined reference', 'linker error', 'debugging', 'programming', 'cpp', 'function definition', 'makefile', 'cmake', 'object files']
draft: false
...
The "Undefined reference" error occurs when the linker cannot find the compiled code for a function that's being called in your program. It essentially means you've *declared* a function (told the compiler it exists) but haven't *defined* it (provided the actual code for the function).
...
In this case, the `add` function is declared in `header.h` and used in `main.cpp`, but it's never *defined*. This will result in an "Undefined reference to `add(int, int)`" error during linking.
...
The function signature in the declaration and definition must match *exactly*.  A slight difference in the return type, number of arguments, or argument types can lead to this error.
