...
date: '2024-01-01'
lastmod: '2024-08-16'
tags: ['javascript', 'cookies', 'web development', 'browser', 'storage', 'security', 'http', 'api', 'example code', 'frontend']
draft: false
...
*   **`expires=expirationDate`**: This specifies when the cookie will expire.  If no expiration date is set, the cookie will be a *session cookie*, meaning it will be deleted when the browser is closed.  The `expirationDate` should be in the form of a date string in UTC format.
...
**Important Note:**  While JavaScript can *read* `HttpOnly` cookies when running in privileged environments (e.g., browser extensions or Node.js contexts), it is best practice to leave the setting of these to the server to ensure consistent protection across different browsers and security contexts.
