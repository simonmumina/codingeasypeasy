...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['nextjs', 'fonts', 'optimization', 'google fonts', 'local fonts', 'performance', 'web development', 'react']
draft: false
...
export default function Home() {
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      <h1 className="text-4xl font-bold font-custom">
        Welcome to my Next.js App!
      </h1>
      <p className="font-custom">
        This is a paragraph using my custom local font.
      </p>
    </main>
  );
}
...
*   **Preload Critical Fonts:**  For fonts that are used in the initial viewport, consider preloading them to ensure they are loaded as early as possible.  However, with Next.js 13's built-in optimization, this is often handled automatically and may not be necessary. If you *do* need to preload, you can add a `<link>` tag in your `<head>`:
