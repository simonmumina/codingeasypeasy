...
In simple terms, a CSS selector is a pattern used to select the HTML elements you want to style. The selector acts as a bridge between your CSS rules and the HTML structure. It tells the browser *which* elements the style rules should be applied to.
...
*   **Child Selector (`>`):** Selects all elements that are direct children of a specified element.
    ...
    This will only make the *direct* child `<p>` element within the `#container` blue. The paragraph nested inside the inner `div` will *not* be affected.
...
*   **General Sibling Selector (`~`):** Selects all elements that are siblings of a specified element. The elements must share the same parent.
    ...
    This will make *all* `<p>` elements that are siblings of the `<h1>` green, regardless of whether they are immediately adjacent or separated by other elements. The list item is not a paragraph so it wont be affected.
...
*   **`:nth-of-type(n)`:**  Similar to `:nth-child(n)`, but only considers elements of the same type.
    ...
    This will only select the *second* `<p>` element within the `div`.  The span is ignored because it's not a paragraph.
...
*   **`::first-line`:** Selects the first line of a block-level element.
    ...
    ```html
    <p>This is a long paragraph.  It spans multiple lines. This is some more text to ensure it wraps.</p>
    ```
