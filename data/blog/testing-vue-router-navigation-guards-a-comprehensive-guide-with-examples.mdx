...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['vue', 'vue-router', 'testing', 'javascript', 'navigation guards', 'unit testing', 'vue test utils', 'vitest', 'jest']
draft: false
...
3.  **Install the Router:** This step is *crucial*.  You need to install the router instance into your Vue application before mounting components that use the router.  For Vue 3, use `app.use(router)` within the test setup. If you are using Vue 2, you should use Vue.use(VueRouter) and then inject the router in the created method of your component.
...
`beforeResolve` guards are executed *after* all in-component guards have been resolved, but *before* the navigation is confirmed.  This is useful for fetching data that's required before a component is fully rendered.
...
describe('beforeResolve Navigation Guard', () => {
  ...
  beforeEach(() => {
    ...
    const routes = [
      { path: '/data', name: 'data', component: { template: '<div>Data: {{ $route.params.data.message }}</div>' }, meta: { requiresData: true } },
    ];
...
`afterEach` guards are executed *after* the navigation has been confirmed.  They're typically used for side effects like analytics tracking or updating the document title.
...
describe('afterEach Navigation Guard', () => {
  ...
  beforeEach(() => {
    ...
    const routes = [
      { path: '/', name: 'home', component: { template: '<div>Home</div>' }, meta: { title: 'Home Page' } },
      { path: '/about', name: 'about', component: { template: '<div>About</div>' }, meta: { title: 'About Us' } },
    ];
