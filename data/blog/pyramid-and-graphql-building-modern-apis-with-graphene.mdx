...
Now, you can query the users from the database using GraphQL.  You'll also need to configure Pyramid to use the SQLAlchemy database session and initialize it.  Add these lines to your `__init__.py` file *after* the `config = Configurator(settings=settings)` line and before `config.include('pyramid_chameleon')`:
