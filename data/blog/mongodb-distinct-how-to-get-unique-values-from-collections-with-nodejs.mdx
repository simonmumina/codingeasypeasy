...
```json
[
  { "_id": { "$oid": "65b3c212f0705344a1346535" }, "name": "Laptop", "category": "Electronics", "price": 1200 },
  { "_id": { "$oid": "65b3c212f0705344a1346536" }, "name": "Mouse", "category": "Electronics", "price": 25 },
  { "_id": { "$oid": "65b3c212f0705344a1346537" }, "name": "T-Shirt", "category": "Clothing", "price": 20 },
  { "_id": { "$oid": "65b3c212f0705344a1346538" }, "name": "Jeans", "category": "Clothing", "price": 60 },
  { "_id": { "$oid": "65b3c212f0705344a1346539" }, "name": "Keyboard", "category": "Electronics", "price": 75 }
]
```
...
async function getDistinctCategoriesByPrice() {
  try {
    const db = await connectToDatabase();
    const distinctCategories = await db.collection('products').distinct('category', { price: { $gt: 50 } });
...
async function getDistinctCategoriesWithTimeout() {
  try {
    const db = await connectToDatabase();
    const distinctCategories = await db.collection('products').distinct('category', {}, { maxTimeMS: 5000 });
...
*   **Using `aggregate()` for Complex Scenarios:**  For more complex scenarios involving multiple fields or more sophisticated filtering, consider using the MongoDB aggregation pipeline with the `$group` operator. The `$group` operator combined with the `$addToSet` accumulator can achieve the same result as `distinct()` but offers greater flexibility.
    ...
    async function getDistinctCategoriesUsingAggregate() {
      try {
        ...
        const distinctCategories = aggregationResult.length > 0 ? aggregationResult[0].distinctCategories : [];
