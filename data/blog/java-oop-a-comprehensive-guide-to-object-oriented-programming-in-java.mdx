...
An interface is a completely abstract class. It contains only abstract methods (and constant variables, which are implicitly `public static final`). Classes _implement_ interfaces using the `implements` keyword. A class can implement multiple interfaces.
