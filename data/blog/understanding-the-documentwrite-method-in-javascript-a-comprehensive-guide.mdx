...
When `document.write()` is executed, the browser parses the string(s) you provide and inserts the resulting HTML (or executes the JavaScript code) into the document at the point where the script tag containing `document.write()` is located. This happens *while* the page is being parsed.
...
3.  **Very Simple Content Insertion (Caution Advised):** For extremely simple scenarios where you need to insert a small amount of HTML content *during* the initial page load.  Even then, it's often better to use DOM manipulation.
...
1.  **Overwriting the Document (After Page Load):**  The most critical danger is that calling `document.write()` *after* the initial page load (e.g., in a function triggered by a button click) will *completely wipe out* the existing HTML of your page.  This is because it implicitly calls `document.open()`, which clears the existing content.
