...
*   **Requirement for `position`:** The `z-index` property *only* works on elements that have a `position` value other than `static`. The `position` property must be set to `relative`, `absolute`, `fixed`, or `sticky`.  This is the most common gotcha when working with `z-index`.

## The Stacking Context

Before diving into code examples, it's essential to understand the concept of a *stacking context*. A stacking context is a three-dimensional conceptualization of HTML elements along an imaginary "z-axis", which indicates their order in front of the user who is facing the screen.
...
**Why is this important?**  `z-index` values are only relative to their *parent stacking context*. This means that an element with a `z-index` of 999 inside a stacking context can be hidden behind an element with a `z-index` of 1 in a different stacking context. The stacking context acts as a "container" for `z-index` values.
...
Here, the red box has a negative `z-index`. This means it will be placed *behind* the blue box (if the blue box also has a position set).  If any content exists behind the blue box (like the `body`'s background), the red box will be placed behind that as well.
...
In this example, you might expect the green box (with `z-index: 2`) to be on top of the red box (with `z-index: 0`).  However, this is *not* the case. The gray container creates a stacking context because it has `position: relative` and `z-index: 1`.  The green box's `z-index` is only relative to the gray container. Since the red box's `z-index` is defined within the *root* stacking context and is higher than the gray container (even though it's explicitly set to `0`, it's higher than the `auto` value of the root context), it will be displayed on top of the entire gray container and its contents.
...
Even though the green box has a higher z-index value (2) relative to the blue box, the red box, which has `opacity: 0.5` creates a new stacking context.  Because the red box's z-index in the *root* stacking context is higher than the blue box, the red box is stacked on top. If the red box didn't have an opacity value less than 1, the green box would be displayed on top.
