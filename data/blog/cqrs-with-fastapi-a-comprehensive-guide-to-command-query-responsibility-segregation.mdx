...
date: "2024-01-26"
lastmod: "2024-01-26"
tags: ["fastapi", "cqrs", "python", "api", "architecture", "command query responsibility segregation", "design patterns"]
draft: false
...
*   **Commands:** Handle write operations. They *change* the state of the system.  Examples include creating a user, updating a product, or deleting a record. Commands are typically named with verbs in the imperative form (e.g., `CreateUserCommand`).

*   **Queries:** Handle read operations. They *read* the state of the system without modifying it.  Examples include fetching a user's profile, listing products, or retrieving order details. Queries are typically named with nouns (e.g., `GetUserByIdQuery`).
