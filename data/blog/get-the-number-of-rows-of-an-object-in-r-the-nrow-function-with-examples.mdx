...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['R programming', 'data science', 'nrow', 'dataframe', 'matrix', 'rows', 'dimensions', 'length']
draft: false
...
In this case, `nrow(my_array)` returns `3`, representing the size of the first dimension of the array, which is often interpreted as the number of rows. Note that this interpretation applies because the 'row' concept is being applied across the *first* dimension of the array.
...
*   **`length(x)`:** Returns the total number of elements in a vector, list, or other object. For a data frame or matrix, it returns the number of columns. It does *not* return the number of rows.
