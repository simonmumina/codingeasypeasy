...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['nestjs', 'guards', 'roles', 'access control', 'authorization', 'authentication', 'security', 'api security', 'typescript']
draft: false
...
*   `requiredRoles.some((role) => user.roles?.includes(role))`:  This checks if *any* of the required roles are present in the user's roles. If at least one required role is found, the user is authorized.
...
*   `@UseGuards(RolesGuard)`: Applies the `RolesGuard` to *all* route handlers within the `UsersController`. This means every route in this controller will be protected by the `RolesGuard` unless overridden.  You can also apply `@UseGuards` directly to individual route handlers if you want more granular control.
...
*   **Order Matters:** When using multiple guards with `@UseGuards`, the order they are listed in is important.  Typically, you'll want to run authentication guards *before* authorization guards.
