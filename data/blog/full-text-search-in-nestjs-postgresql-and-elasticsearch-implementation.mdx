...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['nestjs', 'full-text search', 'postgresql', 'elasticsearch', 'nestjs search', 'database search', 'text search']
draft: false
...
export class AddBlogPostSearchIndex implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(`CREATE INDEX "IDX_blog_post_search_vector" ON "blog_post" USING GIN ("search_vector")`);
    await queryRunner.query(`
...
export class BlogPostService {
  ...
  async search(query: string): Promise<BlogPost[]> {
    ...
    const results = await this.blogPostRepository
      ...
      .where(
        `blog_post.search_vector @@ plainto_tsquery('english', :query)`,
        { query },
      )
      .orderBy(
        `ts_rank(blog_post.search_vector, plainto_tsquery('english', :query))`,
        'DESC',
      )
      .getMany();
...
@Module({
  imports: [
    ...
    TypeOrmModule.forRootAsync({  // Use forRootAsync
