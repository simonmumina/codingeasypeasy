...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['html', 'dom', 'javascript', 'events', 'event-handling', 'web-development', 'online-events', 'event-listeners', 'event-bubbling', 'event-capturing']
draft: false
...
*   **Keyboard Events:**
    ...
    *   `keypress`: Occurs when a key is pressed and released (character keys only).  *Deprecated in modern browsers.*
...
*   **Window/Document Events:**
    ...
    *   `unload`:  Occurs when the user navigates away from the page.  *Generally discouraged due to performance issues.*
...
3.  **`addEventListener()` and `removeEventListener()` (The Recommended Approach):**  This is the most flexible and powerful method for attaching and detaching event handlers.  It allows you to add multiple handlers to the same event type for a single element.
    ...
    *   `event`: A string representing the event type (e.g., 'click', 'mouseover', 'keydown').  *Do not include "on" prefix (e.g., use 'click', not 'onclick').*
    *   `function`: The function to be executed when the event occurs.  This is also known as the *event listener* or *event handler*.
...
When an event occurs, an *event object* is created. This object contains information about the event, such as the target element, the type of event, and any relevant data (e.g., mouse coordinates for a click event, the key pressed for a keydown event).
...
*   `keyCode`: The key code of the key that was pressed (for keyboard events). *Deprecated; use `key` instead.*
...
When an event occurs on an element, the browser follows a specific process called the *event flow* to determine which event listeners should be invoked. This process consists of two phases:

1.  **Capturing Phase:** The event travels down the DOM tree from the window to the target element.  Event listeners attached in the *capture phase* are triggered first.
...
3.  **Bubbling Phase:** The event "bubbles up" the DOM tree from the target element back to the window. Event listeners attached in the *bubbling phase* are triggered last.
