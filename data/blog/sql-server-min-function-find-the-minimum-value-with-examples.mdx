...
date: '2024-02-29'
lastmod: '2024-03-05'
tags: ['SQL Server', 'MIN Function', 'Minimum Value', 'Aggregate Functions', 'T-SQL', 'Database', 'Query Optimization']
draft: false
...
The result will still be `10`.  To find the minimum *unique* quantity, use the `DISTINCT` keyword:
...
In this case, the result will also be `10` since removing the duplicate `10` doesn't affect the minimum value.  However, if the *only* occurrence of the minimum was duplicated, then `DISTINCT` would change the outcome.
...
If *all* the values in the `expression` are `NULL`, then `MIN()` will return `NULL`.
