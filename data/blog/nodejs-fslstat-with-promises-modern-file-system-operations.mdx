...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['node.js', 'fs.lstat', 'promises', 'asynchronous programming', 'file system', 'nodejs tutorial', 'javascript']
draft: false
...
This code retrieves the stats of 'my-file.txt' and then, inside the callback, checks if it's a file, directory, or symbolic link. Crucially, the `console.log` at the end executes *before* the callback function runs, illustrating the asynchronous nature of the callback approach.
...
*   `fs.stat('link.txt')` returns the stats of the *target* file (`original.txt`). It follows the symbolic link.
*   `fs.lstat('link.txt')` returns the stats of the *symbolic link itself*. It tells you that `link.txt` is a symbolic link, and its size is likely to be smaller than the target file. `lstatLink.isSymbolicLink()` will return `true`.
