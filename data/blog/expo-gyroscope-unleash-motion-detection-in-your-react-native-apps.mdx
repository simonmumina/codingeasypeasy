...
In this example, we've set the update interval to 200 milliseconds, which means the gyroscope data will be updated 5 times per second. Experiment with different values to find the optimal balance for your application.  Remember to call `Gyroscope.setUpdateInterval()` *before* you subscribe to the data stream.
