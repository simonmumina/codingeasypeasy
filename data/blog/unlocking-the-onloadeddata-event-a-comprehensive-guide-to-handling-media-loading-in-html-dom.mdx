...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['HTML DOM', 'onloadeddata', 'JavaScript', 'Media Events', 'Web Development', 'Audio Events', 'Video Events', 'Event Listeners']
draft: false
...
Think of it like this:  `onload` signals the readiness of the entire web page, while `onloadeddata` signals the readiness of the media element to start playing *right now*.
...
Specifically, `onloadeddata` is triggered *after* the `loadedmetadata` event (which provides information about the media's metadata like duration, dimensions, etc.) but *before* the `canplay` event (which signifies that the media is ready to play all the way through without buffering).
...
*   Remember that `onloadeddata` fires *after* `loadedmetadata` but *before* `canplay`.
