...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'scope', 'variable scope', 'lexical scope', 'global scope', 'function scope', 'block scope', 'closures', 'javascript tutorial', 'es6', 'var', 'let', 'const']
draft: false
...
**Important Note:**  Variables declared with `var` inside a block *do not* have block scope.  They are scoped to the enclosing function (or global if declared outside a function). This is a crucial difference and a common source of bugs.
...
JavaScript uses lexical scoping, also known as static scoping.  This means that the scope of a variable is determined by its *physical location* in the source code, not by how the code is executed.  A function can access variables declared in its own scope, as well as variables declared in the scopes of its parent functions (the functions in which it's nested).
...
Hoisting is a JavaScript mechanism where variable and function declarations are moved to the top of their scope before code execution. However, it's important to understand that only the *declarations* are hoisted, not the initializations.
...
*   **`let` and `const` variables are hoisted but *not* initialized.** This means that accessing a `let` or `const` variable before its declaration will result in a `ReferenceError` (the Temporal Dead Zone).
