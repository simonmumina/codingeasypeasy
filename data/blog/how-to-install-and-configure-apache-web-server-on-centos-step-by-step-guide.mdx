...
date: '2024-01-01'
lastmod: '2024-10-27'
tags: ['apache', 'centos', 'web server', 'linux', 'installation', 'configuration', 'httpd', 'firewall', 'ssl', 'security']
draft: false
...
If you don't have a domain name registered or want to test locally, add the following line to your `/etc/hosts` file on *your client machine* (not the server):
...
*   **Disable Directory Listing:** Prevent Apache from displaying a list of files in a directory if no index file is present.  In your virtual host configuration, ensure the `Options` directive does *not* include `Indexes`.  For example:
