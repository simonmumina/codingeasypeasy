...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['database', 'index', 'clustered index', 'non-clustered index', 'sql', 'performance tuning', 'database optimization', 'data retrieval']
draft: false
...
*   **Leaf Nodes are Data Pages:** The leaf nodes of the clustered index contain the actual data rows.  This is a crucial point:  the index *is* the data.
...
A **non-clustered index**, on the other hand, does *not* determine the physical order of data. It's like the index at the back of a book, listing keywords and page numbers. The index stores a copy of the indexed column(s) along with a pointer (Row ID or RID) to the actual data row in the table.  Therefore, multiple non-clustered indexes can exist on a single table.
...
*   **Slower for Range Queries (Potentially):** Range queries may be slower than with a clustered index because the data is not physically sorted in the index order, potentially requiring more physical I/O operations.  However, if the query only requires data from the indexed columns, the database *may* be able to fulfill the query directly from the index without going to the base table (covered index).
