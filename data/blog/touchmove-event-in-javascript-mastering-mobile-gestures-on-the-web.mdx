...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'html', 'dom', 'events', 'touchmove', 'mobile', 'gestures', 'touch events', 'web development', 'front-end']
draft: false
...
*   **`touches`:**  A `TouchList` object representing all current touch points on the surface, regardless of the target element.  Think of it as *all* fingers currently touching the screen.
*   **`targetTouches`:** A `TouchList` object representing touch points that started on the *same element* as the event target.  This is often the most useful list for handling events on specific elements.
*   **`changedTouches`:** A `TouchList` object representing touch points that have changed state since the last touch event (e.g., started, moved, or ended). In the `touchmove` event, it will contain touch points that have *moved* since the last `touchmove` event.
*   **`target`:**  The element on which the touch event originated.
*   **`clientX` and `clientY`:** The horizontal and vertical coordinates of the touch point *relative to the viewport* (the visible area of the browser window).
*   **`pageX` and `pageY`:** The horizontal and vertical coordinates of the touch point *relative to the entire document*.
*   **`screenX` and `screenY`:** The horizontal and vertical coordinates of the touch point *relative to the screen*.
...
2.  **Passive Event Listeners:** If your event handler doesn't call `event.preventDefault()`, you can improve scrolling performance by using a passive event listener.  Passive event listeners allow the browser to continue scrolling even while the event handler is running.  To make an event listener passive, pass the `passive: true` option in the `addEventListener` options object.
    ...
    **Important:**  If you *do* call `event.preventDefault()` within a passive event listener, the browser will ignore the call.  Passive event listeners are only beneficial when you don't need to prevent default behavior.
