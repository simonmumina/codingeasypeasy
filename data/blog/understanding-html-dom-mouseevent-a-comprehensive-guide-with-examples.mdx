...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['html', 'dom', 'javascript', 'mouseevent', 'event handling', 'web development', 'front-end development', 'click events', 'mouseover events', 'mouseout events']
draft: false
...
*   **`clientX` and `clientY`:** These properties represent the horizontal (`clientX`) and vertical (`clientY`) coordinates of the mouse pointer relative to the *viewport*.  They tell you where the mouse was on the screen when the event occurred.
...
*   **`offsetX` and `offsetY`:**  These properties represent the horizontal (`offsetX`) and vertical (`offsetY`) coordinates of the mouse pointer relative to the *target element*.  They tell you where the mouse was within the element when the event occurred.
...
*   **`pageX` and `pageY`:** These properties represent the horizontal (`pageX`) and vertical (`pageY`) coordinates of the mouse pointer relative to the *entire document*.  This is important if the document is scrollable.
...
*   **`screenX` and `screenY`:** These properties represent the horizontal (`screenX`) and vertical (`screenY`) coordinates of the mouse pointer relative to the *entire screen*.
...
*   **`buttons`:**  A more robust alternative to `button`. It provides a bitmask representing all the buttons that are currently pressed.  You can check for specific buttons using bitwise operators.
    ...
    ```javascript
    document.getElementById("myDiv").addEventListener("mousedown", function(event) {
        if (event.buttons & 1) { // Check if left button is pressed
            console.log("Left mouse button is pressed");
        }
        if (event.buttons & 2) { // Check if right button is pressed
            console.log("Right mouse button is pressed");
        }
        if (event.buttons & 4) { // Check if middle button is pressed
