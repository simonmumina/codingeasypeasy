...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['linear algebra', 'matrix decomposition', 'jordan normal form', 'eigenvalues', 'eigenvectors', 'mathematics', 'canonical form']
draft: false
...
Before diving into the JNF, let's quickly review the concept of diagonalization. A square matrix *A* is diagonalizable if it can be written as:

*A = PDP<sup>-1</sup>*

where *D* is a diagonal matrix and *P* is an invertible matrix whose columns are the eigenvectors of *A*. Diagonalization is a powerful tool because it simplifies many matrix operations, such as calculating powers of the matrix.
...
The Jordan Normal Form of a square matrix *A* is a block diagonal matrix *J* such that:

*A = PJP<sup>-1</sup>*

where *J* is the Jordan Normal Form and *P* is an invertible matrix called the **generalized eigenvector matrix**.  The matrix *J* consists of **Jordan blocks** along its diagonal.

A **Jordan block** *J<sub>i</sub>(λ)* is a square matrix of the form:
...
where λ is an eigenvalue of *A* and the block has size *i x i*.  Notice that the eigenvalue λ appears along the main diagonal, and 1's appear along the superdiagonal.

**Key Properties:**

*   **Uniqueness:** The Jordan Normal Form of a matrix is unique up to the order of the Jordan blocks.
*   **Eigenvalues:** The eigenvalues of *A* appear along the diagonal of *J*. The algebraic multiplicity of an eigenvalue is equal to the sum of the sizes of the Jordan blocks associated with that eigenvalue.
...
1.  **Find the Eigenvalues:** Calculate the characteristic polynomial *det(A - λI)*, where *I* is the identity matrix. Solve for λ to find the eigenvalues of *A*.
...
4.  **Find the Generalized Eigenvectors:**  For each Jordan block *J<sub>i</sub>(λ)*, find a chain of generalized eigenvectors *v<sub>1</sub>, v<sub>2</sub>, ..., v<sub>i</sub>* such that:

    *(A - λI)v<sub>1</sub> = 0*  (v<sub>1</sub> is a regular eigenvector)
    *(A - λI)v<sub>2</sub> = v<sub>1</sub>*
    *(A - λI)v<sub>3</sub> = v<sub>2</sub>*
    ...
    *(A - λI)v<sub>i</sub> = v<sub>i-1</sub>*

    The vector *v<sub>i</sub>* is the generalized eigenvector of height *i*.
5.  **Construct the Matrix P:** The columns of *P* are the generalized eigenvectors you found in the previous step, arranged in the same order as the corresponding Jordan blocks in *J*.
6.  **Construct the Jordan Normal Form J:** Create the block diagonal matrix *J* with the appropriate Jordan blocks along the diagonal.
...
1.  **Find the Eigenvalues:** The characteristic polynomial is:
    ...
    *det(A - λI) = det([5-λ -4 0], [2 -1-λ 0], [0 0 3-λ]) = (3-λ)((5-λ)(-1-λ) + 8) = (3-λ)(λ<sup>2</sup> - 4λ + 3) = (3-λ)(λ-3)(λ-1) = -(λ-3)<sup>2</sup>(λ-1)*
...
This Python code uses the `jordan_canonical_form` function from `scipy.linalg` to compute the Jordan Normal Form. The matrix *J* contains the Jordan blocks, and the matrix *P* contains the generalized eigenvectors.  As you can see, when multiplied back together (PJP<sup>-1</sup>), we recover the original matrix *A*.  The above code also provides an illustration of how to compute a Jordan form. For numerical stability, specialized algorithms beyond simple linear algebra libraries might be needed for large matrices.
...
*   **Analyzing Matrix Functions:**  The JNF is useful for calculating functions of matrices, such as *e<sup>At</sup>*, where *A* is a matrix and *t* is a scalar.
