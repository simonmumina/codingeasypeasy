...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['fastapi', 'https', 'hsts', 'security', 'python', 'web development', 'ssl', 'tls', 'deployment']
draft: false
...
*   **Self-Signed Certificates (For development and testing only):** Self-signed certificates are easy to create but are *not* trusted by browsers by default. They should only be used for local development and testing, *never* in production.
...
HSTS instructs browsers to *always* access your site over HTTPS, even if the user types `http://` in the address bar. This prevents man-in-the-middle attacks that attempt to downgrade connections to HTTP.
...
*   `includeSubDomains`:  Indicates that HSTS should also apply to all subdomains of your domain (e.g., `api.yourdomain.com`, `www.yourdomain.com`). Be careful enabling this; ensure *all* your subdomains are served over HTTPS before enabling this.
