...
In this example, the `useEffect` hook updates the document title after each render.  Note that this runs *after* the component has rendered, so the user won't see a flicker of the old title.
...
**Problem:** This effect will run *after every render*, even if the `count` state hasn't changed. This can lead to unnecessary computations and performance issues, especially with more complex side effects.

## Using the Dependency Array: Optimizing Performance

To optimize the performance of your `useEffect` hook, you can provide a *dependency array* as the second argument. The dependency array is an array of values that `useEffect` depends on. The effect will only re-run when one of these values changes between renders.
...
*   The effect will now *only* run if the value of `count` changes between renders. If `count` remains the same, the effect will be skipped, improving performance.
...
To achieve this, you provide an **empty dependency array `[]`**.  This tells React that the effect *doesn't* depend on any values from the component. Therefore, it will only run once when the component is initially rendered.
...
*   The effect will only run once after the *initial* render of the component.
...
`useEffect` allows you to do this by returning a *cleanup function* from the effect function. This cleanup function will be executed when the component unmounts, or before the effect re-runs (if the dependencies have changed).
...
1.  **Infinite Loops:**  A common mistake is creating infinite loops by including a state variable that's updated *within* the `useEffect` hook in its dependency array. This causes the effect to re-run every time the state updates, leading to an infinite loop.
...
*   **`useRef` for Mutable Values:**  If you need to access the *latest* value of a prop or state variable within an effect that only runs once, but you don't want to trigger re-renders, you can use `useRef`.
