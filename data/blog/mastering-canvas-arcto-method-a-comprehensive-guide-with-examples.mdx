...
4.  **`ctx.arcTo()`**: Draws the rounded corners using the `arcTo()` method.  Each `arcTo()` call connects the preceding `lineTo()` to the next `lineTo()` with a rounded corner of the specified `radius`.  The first two arguments define the corner point, the next two define where the next line will go *if it were to continue straight*, and the final argument defines the radius of the rounded corner.
