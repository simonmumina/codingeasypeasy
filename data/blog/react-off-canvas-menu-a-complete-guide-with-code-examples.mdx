...
const OffCanvasMenu = () => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (menuRef.current) {
        const firstFocusable = menuRef.current.querySelector('a, button, input, select, textarea, [tabindex]:not([tabindex="-1"])');
        if (firstFocusable) {
  ...
  return (
    <div>
      ...
      >
        <button className="close-button" onClick={toggleMenu} aria-label="Close">&times;</button>
      ...
      return (
        <div>
          ...
          <div
            ...
            aria-modal="true"
          >
            <button className="close-button" onClick={toggleMenu} aria-label="Close"><FontAwesomeIcon icon={faTimes} /></button>
    ...
    const OffCanvasMenu = () => {
      ...
      return (
        <div>
          ...
          <div className={`overlay ${isOpen ? 'open' : ''}`} onClick={toggleMenu}></div> {/* The overlay div */}
