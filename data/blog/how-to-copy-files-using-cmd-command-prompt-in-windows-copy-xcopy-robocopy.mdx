...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['cmd', 'command prompt', 'windows', 'copy', 'xcopy', 'robocopy', 'file management', 'batch scripting', 'powershell']
draft: false
...
*   `/MAXAGE:n`:  Specifies the maximum file age (don't copy files older than *n* days or date).
*   `/MINAGE:n`:  Specifies the minimum file age (don't copy files newer than *n* days or date).
*   `/MAXSIZE:n`:  Specifies the maximum file size (exclude files larger than *n* bytes).
*   `/MINSIZE:n`:  Specifies the minimum file size (exclude files smaller than *n* bytes).
...
*   `/MON:n`: Monitors source; runs again when more than *n* changes are detected.
*   `/MOT:m`: Monitors source; runs every *m* minutes.
