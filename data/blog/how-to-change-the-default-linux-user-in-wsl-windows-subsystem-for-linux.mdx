...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['wsl', 'linux', 'windows', 'ubuntu', 'default user', 'windows subsystem for linux', 'command line', 'tutorial', 'configuration', 'sysadmin']
draft: false
...
2. **Modify `.bashrc` or `.zshrc` (or similar shell startup files):**
   ...
   Edit the startup file for your shell (e.g., `.bashrc` for Bash, `.zshrc` for Zsh) in the home directory of the user you *want* to become the default.  For example, if you want to change the default to a user named `mydevuser`, you would edit `/home/mydevuser/.bashrc`.
   ...
   Replace `"1000"` with the actual UID of the *current* default user. You can find this out by logging in as the *current* default user and running `id -u`.  Replace `mydevuser` with the username you want to become the default.  This script checks if the current user is the original default user (identified by its UID). If it is, it switches to the desired user using `sudo -u` and then exits the original shell instance.

   Save the file and exit the editor.

3. **Make the User Passwordless Sudo (Optional, but often required):**

   For the `sudo -u` command to work without prompting for a password, you need to configure passwordless sudo for the *original* default user. This is generally *NOT RECOMMENDED* for security reasons but is often a necessary evil for this method to function properly.  If you are doing this on a production or sensitive system, consider alternative authentication methods.
   ...
   This will open the `sudoers` file in an editor.  Add the following line to the end of the file, replacing `originaluser` with the username of the *original* default user:
...
*   **UID Accuracy:**  Ensure you replace `1000` (or whatever the original default UID is) with the *correct* UID.
