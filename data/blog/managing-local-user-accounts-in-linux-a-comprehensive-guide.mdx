...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['linux', 'user accounts', 'user management', 'command line', 'security', 'adduser', 'userdel', 'usermod', 'groupadd', 'groupdel', 'chage']
draft: false
...
This command deletes the user account `john` *and* the home directory `/home/john`.
...
*   `-G <group1,group2,...>` or `--groups <group1,group2,...>`: Changes the user's supplementary groups. This **overwrites** the existing list of supplementary groups. To *add* groups, use the `-a` option in conjunction with `-G`.
...
*   Each user has a *primary group*.  This is specified when the user is created with `adduser -g` or modified with `usermod -g`.  The primary group is associated with newly created files.
*   Users can also belong to *supplementary groups*.  These are added using `usermod -aG`.  A user has the permissions of all groups they belong to.
