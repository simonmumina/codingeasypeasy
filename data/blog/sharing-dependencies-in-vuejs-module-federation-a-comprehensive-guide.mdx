...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['vuejs', 'module federation', 'microfrontends', 'dependency sharing', 'webpack', 'javascript']
draft: false
...
*   `shared`: This is the key section.
    ...
    *   `eager: true`:  Indicates that the host application should load these dependencies immediately. This makes them available to the `remote-module` as soon as it's loaded.  The first application to load the dependency *must* load it eagerly.
...
*   `shared`:  Similar to the host application, we define the shared dependencies. Notice that `eager` is *not* set to `true` here, because the host application is already loading these dependencies eagerly. The `requiredVersion` is still important for version compatibility checks.
