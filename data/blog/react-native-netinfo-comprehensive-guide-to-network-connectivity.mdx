...
date: '2023-10-27'
lastmod: '2024-08-16'
tags: ['react-native', 'netinfo', 'network connectivity', 'internet connection', 'offline', 'mobile development', 'cross-platform', 'internet status']
draft: false
...
const App = () => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.text}>
        Connection Type: {connectionInfo.type || 'Unknown'}
      </Text>
      <Text style={styles.text}>
        Is Connected: {connectionInfo.isConnected ? 'Yes' : 'No'}
      </Text>
      {connectionInfo.details && connectionInfo.type === 'wifi' && (
        <Text style={styles.text}>
          WiFi SSID: {connectionInfo.details.ssid || 'Not Available'}
        </Text>
      )}
    </View>
  );
  ...
  useEffect(() => {
    const unsubscribe = NetInfo.addEventListener(
      ...
      },
      {
        ...
        reachabilityTest: async (response) => {  // Customize the test
