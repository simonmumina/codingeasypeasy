...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'closures', 'scope', 'lexical environment', 'interview questions', 'functional programming']
draft: false
...
At its core, a closure is the combination of a function bundled together (enclosed) with references to its surrounding state (the **lexical environment**). In simpler terms, a closure gives you access to an outer function’s scope from an inner function, even *after* the outer function has finished executing.
...
The key takeaway is that the inner function *closes over* the variables in its lexical environment, creating a closure.
...
Each time we call `myCounter()`, the `increment` function executes, increments the `count` variable that it closed over, and returns the updated value.  Crucially, the `count` variable is *private* to the `myCounter` instance. Other code cannot directly access or modify it.
...
*   **Explain the output of the following code snippet (often involving loops and closures):**
    ...
    (This classic example highlights the "var" keyword's scope and how it interacts with closures. All the timeouts will print '5' because the loop completes before the timeouts execute, and all of them reference the *same* `i` variable. The solution is to use `let` instead of `var`, or to wrap the `setTimeout` call in an immediately invoked function expression (IIFE) to create a new scope for each iteration.)
