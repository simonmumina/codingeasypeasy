...
2.  **Basic SSG with `getStaticProps`:**
    ...
    *   The `return` statement in `getStaticProps` *must* include a `props` key, which is an object containing the data you want to pass to your component.
...
export async function getStaticPaths() {
  ...
  const paths = posts.slice(0, 10).map((post) => ({  // Limit the number of statically generated pages for demonstration
...
While SSG is great for static content, it can be challenging to update content frequently.  Incremental Static Regeneration (ISR) allows you to update static pages *after* they've been built.
