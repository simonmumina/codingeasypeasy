...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['webflux', 'spring boot', 'reactive programming', 'backpressure', 'slow clients', 'reactor', 'flow control']
draft: false
...
**Important Note:**  Even though we don't explicitly use `onBackpressureXXX` operators, the default behaviour respects the `REQUEST` (demand) from the consumer. The consumer subscribes to the stream and *requests* a certain number of items. The producer emits only that many items and waits for another request from the consumer.
