...
*   Ensure the `OrderService` is running *before* running this test or else the test will fail because the client cannot connect.
*   The `httpx.AsyncClient` uses `async with` to ensure the client is properly closed after use.
*   The `mock_product_service.verify()` function is *essential* to verify the contract.  Without it, the test might pass even if the contract is not met.
...
*   Ensure the `publish_pact` call happens *after* the mock service is stopped.
...
3.  **`product_service` fixture:**
    *   This fixture uses `multiprocessing` to run the FastAPI app in a separate process.  This is *crucial* because the `Verifier` needs to make HTTP requests to the running Provider.
