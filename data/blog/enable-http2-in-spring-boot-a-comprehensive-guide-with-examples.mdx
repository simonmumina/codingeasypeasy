...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['spring boot', 'http/2', 'performance', 'web development', 'java', 'ssl', 'jetty', 'tomcat', 'undertow']
draft: false
...
*   **SSL/TLS Certificate:** HTTP/2 *requires* a secure connection (HTTPS). You'll need an SSL/TLS certificate.  You can use a self-signed certificate for development, but for production, you should obtain a certificate from a trusted Certificate Authority (CA).
...
3.  **Configure Jetty with a `WebServerFactoryCustomizer`:**
    ...
    *   **ALPNServerConnectionFactory:**  This is *crucial* for enabling ALPN with Jetty.  It negotiates the protocol with the client during the TLS handshake.
...
**Important Security Note:** *Self-signed certificates are NOT suitable for production environments.* Browsers will display warnings to users because they are not signed by a trusted CA. Obtain a certificate from a trusted Certificate Authority (CA) for production.
