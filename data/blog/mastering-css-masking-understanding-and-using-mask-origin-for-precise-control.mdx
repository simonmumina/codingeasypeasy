...
date: '2024-01-26'
lastmod: '2024-10-27'
tags: ['css', 'masking', 'mask-origin', 'web development', 'front-end', 'web design', 'image manipulation']
draft: false
...
In this example, the mask image (a simple circle) is positioned with its top-left corner aligned with the top-left corner of the *border box*.  The border affects the placement of the mask.
...
Here, the mask is shifted inwards because the origin is now at the top-left corner of the *padding box*. The border is not considered in the mask's positioning.
...
The mask is now shifted even further inwards because the origin is the top-left corner of the *content box*, effectively ignoring both the border and padding.
...
*   **`mask-position`:**  Specifies the initial position of the mask image.  While `mask-origin` defines the *origin* point, `mask-position` defines the *initial offset* of the mask image from that origin.  These work together to provide precise control.
