...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['CSS', 'Content Box', 'Box Model', 'Layout', 'Web Development', 'Front-End Development', 'CSS Box Model']
draft: false
...
*   **Content:** The actual content of the element (text, images, etc.). Its size is determined by the `width` and `height` properties.  This is the *content box*.
...
*   **Size Definition:** The `width` and `height` properties define the size of the content area. If `box-sizing` is set to `content-box` (the default), `width` and `height` only apply to the content area, and any padding, border, or margin are added *on top* of that size.
...
*   The specified `width` and `height` include the padding and border, but *not* the margin.
...
*   The *total* width of the element, including padding and border, is 200px. The content area will be smaller (200px - 20px - 20px - 5px - 5px = 150px).
*   The *total* height of the element, including padding and border, is 100px. The content area will be smaller (100px - 20px - 20px - 5px - 5px = 50px).
...
The total width *with margin* will be 200px + 10px (margin-left) + 10px (margin-right) = 220px.

The total height *with margin* will be 100px + 10px (margin-top) + 10px (margin-bottom) = 120px.
