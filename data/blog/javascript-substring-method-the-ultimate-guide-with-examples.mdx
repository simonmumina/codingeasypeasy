...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'substring', 'string methods', 'string manipulation', 'web development', 'javascript tutorial']
draft: false
...
*   **`indexEnd` (optional)**: An integer between 0 and the length of the string, specifying the index *before* which to end the substring.  If omitted, `substring()` extracts characters to the end of the string.
...
*   If `indexStart` is greater than `indexEnd`, `slice()` returns an empty string.  It *does not* swap the arguments like `substring()`.
...
*   `substr()` takes the starting index and the *length* of the substring to extract, not the ending index.
*   `substr()`'s behavior with negative `indexStart` is somewhat inconsistent across browsers. It's generally best to avoid negative `indexStart` with `substr()`.  It is also considered a *legacy* function.
