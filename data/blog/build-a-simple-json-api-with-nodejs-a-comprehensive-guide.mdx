...
*   `app.use(express.json())`: This is *crucial*.  It's middleware that parses incoming requests with JSON payloads. Without this, `req.body` will be undefined.  It needs to be placed *before* any route handlers that rely on `req.body`.
...
*   `users.findIndex(u => u.id === userId)`: Finds the *index* of the user in the `users` array. This is important because we need to replace the existing user object. `findIndex` returns -1 if the user isn't found.
