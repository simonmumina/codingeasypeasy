...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['nextjs', 'hubspot', 'integration', 'marketing automation', 'react', 'next-hubspot', 'lead generation', 'forms', 'analytics']
draft: false
...
3.  **Create a `lib/hubspot.js` file (optional, but recommended for organization):**
    ...
    export async function submitHubspotForm(portalId, formId, fields) {
        try {
            if (!hubspotClient) {
                console.warn('HubSpot client not initialized.  Check your API key and environment variables.');
    ...
    export async function trackHubspotEvent(eventName, properties = {}) {
        try {
            ...
            if (!properties.email) {
                console.warn("Email address is required for identifying contacts in HubSpot event tracking.");
