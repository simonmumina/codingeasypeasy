...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'encodeURIComponent', 'url encoding', 'url', 'web development', 'escape characters']
draft: false
...
*   `A-Z a-z 0-9 - _ . ! ~ * ' ( )`:  These characters are considered *unreserved* and are not encoded.
...
*   `encodeURI()` does *not* encode characters that have special meaning in a URI as a whole, such as `/`, `?`, `:`, `@`, `&`, `=`, `+`, `$`, and `,`.
...
*   **Encode before building the URL:** Construct the URL string *after* encoding the individual components.
