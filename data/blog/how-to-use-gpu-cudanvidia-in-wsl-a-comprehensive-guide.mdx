...
1.  **Download the CUDA Toolkit installer:**
    ...
    Visit the NVIDIA CUDA Toolkit download page ([https://developer.nvidia.com/cuda-downloads](https://developer.nvidia.com/cuda-downloads)). Select "Linux" -> "x86_64" -> _your Ubuntu version_ -> "deb (network)" and follow the instructions provided to download and install the CUDA Toolkit. This usually involves running a series of commands similar to the ones used to add the NVIDIA repository in the previous steps:
