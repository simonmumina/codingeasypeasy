...
date: '2024-01-26'
lastmod: '2024-10-27'
tags: ['computer architecture', 'cpu', 'memory', 'registers', 'instruction set architecture', 'ISA', 'assembly language', 'pipeline', 'cache memory', 'von neumann architecture', 'harvard architecture']
draft: false
...
When the CPU needs to access data, it first checks the cache.  If the data is in the cache (a *cache hit*), it can be accessed quickly.  If the data is not in the cache (a *cache miss*), the CPU must access main memory, which is much slower.
...
*   **Von Neumann Architecture:** Uses a single address space for both instructions and data. This is the most common architecture used in modern computers.  A key characteristic is the *von Neumann bottleneck*, where the single pathway for data and instructions can limit performance.
