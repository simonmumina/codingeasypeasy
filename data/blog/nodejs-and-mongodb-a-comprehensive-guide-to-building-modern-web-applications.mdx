...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['nodejs', 'mongodb', 'javascript', 'web development', 'database', 'backend', 'mern stack', 'nosql', 'mongoose']
draft: false
...
*   **`mongoose.model()`:** Creates a `Product` model based on the schema.  The first argument, 'Product', is the *singular* name of the collection that Mongoose will manage (it will automatically pluralize it to 'products' in MongoDB).
*   **`pre('save')` middleware:**  This middleware function is executed *before* a document is saved to the database. It updates the `updatedAt` field to the current date and time, ensuring that the timestamp is always current.  This is a very useful pattern for auditing and data management.
