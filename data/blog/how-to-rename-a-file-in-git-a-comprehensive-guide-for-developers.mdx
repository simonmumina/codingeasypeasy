...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['git', 'rename file', 'version control', 'git rename', 'git mv', 'git tutorial', 'developer tools']
draft: false
...
Simply renaming a file in your file system and then staging the changes with `git add` and committing *will work*, but Git won't recognize it as a rename. Instead, it will see it as *deleting* the old file and *creating* a new one.  This approach loses the file's history, which is a major drawback of using version control!  Using the correct method ensures Git understands the action as a rename, preserving the file's full history across commits.
...
Similarly, you can move a file *and* rename it simultaneously.  For example, to move `docs/old_file.txt` to a new directory called `archive` and rename it to `archived_file.txt`:
...
While less common, `git mv` *can* rename multiple files at once, but it works a bit differently than you might expect. You specify all the source files, and then the *destination* must be a directory. All source files are then moved/renamed into the destination directory.
...
While `git mv` is the recommended way to rename files in Git, you *could* achieve the same result manually using a combination of other commands:
