...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['SQL Server', 'MAX Function', 'Aggregate Functions', 'Database Querying', 'T-SQL', 'Maximum Value', 'SQL Tutorial']
draft: false
...
While possible, using `DISTINCT` with `MAX()` is usually redundant because the maximum value will already be unique in the context of finding the *single* maximum.  However, it's syntactically valid:
...
*   **Forgetting the `GROUP BY` clause when needed:** If you are using `MAX()` to find the maximum value *within* groups, forgetting the `GROUP BY` clause will result in an incorrect result.
