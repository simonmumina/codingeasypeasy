...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['SQL', 'NULL', 'Database', 'Programming', 'SQL NULL', 'IS NULL', '= NULL', 'Comparison Operators', 'SQL Best Practices']
draft: false
...
The `=` operator is designed for comparing known values. `NULL` represents an *unknown* value. Comparing an unknown value to another unknown value (which `NULL` essentially represents on both sides of the `=` operator) results in an *unknown* outcome. SQL interprets this as `FALSE`, causing the `WHERE` clause to filter out any rows with `NULL` in the `department` column.
...
To find all employees who do *not* have a department assigned (i.e., `department` is `NULL`), we use `IS NULL`:
...
To find all employees who *do* have a department assigned (i.e., `department` is *not* `NULL`), we use `IS NOT NULL`:
