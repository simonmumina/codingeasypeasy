...
Getting the width using `element.style.width` only retrieves the *inline* style value.  If the width is defined in an external CSS file or a `<style>` tag, this method will return an empty string or `null`. To get the actual computed width of an element, you should use the `getComputedStyle()` method.
