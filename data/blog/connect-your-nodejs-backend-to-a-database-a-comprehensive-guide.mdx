...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['Node.js', 'Database', 'Backend Development', 'MySQL', 'PostgreSQL', 'MongoDB', 'Sequelize', 'Mongoose', 'ORM', 'Data Persistence']
draft: false
...
pool.query(sql, [1], (error, results, fields) => { // Assuming you want to select user with id 1
  if (error) throw error;
...
sequelize.sync()  //  { force: true } to drop and recreate the table
  .then(() => {
