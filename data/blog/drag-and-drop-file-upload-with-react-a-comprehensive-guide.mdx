    ...
    >
      {files.length > 0 ? (
        ...
        <>
          ...
          <input type="file" multiple onChange={handleInputChange} style={{ display: 'none' }} id="file-upload" />
          <label htmlFor="file-upload" style={{ cursor: 'pointer', color: 'blue', textDecoration: 'underline' }}>Choose Files</label>
        </>
      )}
    </div>
  );
