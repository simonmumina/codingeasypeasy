...
date: '2024-01-01'
lastmod: '2024-10-27'
tags: ['vuejs', 'vue3', 'ie11', 'browser-compatibility', 'polyfills', 'transpilation', 'javascript', 'modernizr']
draft: false
...
*   **`babel-loader`:**  Configures Babel to transpile JavaScript files.
    ...
    *   **`useBuiltIns: 'usage'`:** This tells Babel to only include polyfills that are *actually used* in your code. This significantly reduces the final bundle size compared to including all polyfills. The alternative, `'entry'`, is only really helpful with a single entry point, which is now discouraged.
...
Modernizr helps detect browser features. You can use it to conditionally load polyfills *only* when they're needed. This can improve performance by avoiding unnecessary polyfills in modern browsers.
...
4.  **Conditionally Load Polyfills:**  In your `main.js` or a similar entry point, use Modernizr to check for feature support and load polyfills accordingly:
    ...
    if (!Modernizr.cssvariables) {
      ...
      require('css-vars-ponyfill')({ // Or use a CDN
...
IE11 does not natively support the `fetch` API.  You'll need a `fetch` polyfill.  However, be aware that some `fetch` polyfills rely on `Promise` support. Ensure you include a `Promise` polyfill *before* the `fetch` polyfill.
