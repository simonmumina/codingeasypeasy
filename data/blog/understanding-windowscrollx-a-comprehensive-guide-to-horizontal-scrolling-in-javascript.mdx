...
```html
<div id="contentContainer" style="display: flex; flex-direction: row; white-space: nowrap; overflow-x: auto;">
  <div style="width: 300px; height: 200px; background-color: lightblue; margin: 10px;">Content Block 1</div>
  <div style="width: 300px; height: 200px; background-color: lightgreen; margin: 10px;">Content Block 2</div>
</div>
```
...
*   **Scrolling Elements:**  `window.scrollX` reflects the horizontal scroll position of the *document* itself. If you're working with a specific element that has its own scrollbar (e.g., a `div` with `overflow: auto`), you should use the element's `scrollLeft` property instead.
