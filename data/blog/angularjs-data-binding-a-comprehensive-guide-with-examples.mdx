...
date: '2023-10-27'
lastmod: '2024-10-27'
tags: ['angularjs', 'data binding', 'two-way binding', 'one-way binding', 'angular', 'frontend development', 'javascript', 'mvvm']
draft: false
...
1.  **One-Way Data Binding:** Data flows from the model to the view.  Changes in the view *do not* affect the model.
...
3.  **One-Time Binding (Introduced in AngularJS 1.3):** The expression is evaluated once, and its value is used in the view. Subsequent changes to the model *do not* update the view. This is useful for static data.
...
In this example, the HTML will display "Hello, John Doe! My age is: 30". If you change `name` or `age` in the `MyController` in your `app.js`, the view will automatically update. However, if you try to modify the text within the `<h1>` tag, it will *not* update the `$scope.name` in the `MyController`.
