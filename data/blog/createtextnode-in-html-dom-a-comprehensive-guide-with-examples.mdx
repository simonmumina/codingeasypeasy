...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'html', 'dom', 'createTextNode', 'web development', 'frontend', 'document object model']
draft: false
...
*   **`innerText`:** The `innerText` property gets or sets the text content of an element, including spacing and visible text (as rendered in the browser).  However, it's less standardized than `textContent` and has some performance implications.  It should be used with caution and is generally less preferred than `textContent`. Crucially, it *does not* escape HTML characters and therefore, should never be used with potentially unsafe or user-provided content.
