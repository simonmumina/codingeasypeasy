...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['HTTP', 'Request Methods', 'GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'REST API', 'API Development', 'Web Development']
draft: false
...
The `GET` method is used to retrieve data from a server. It's the most common HTTP method and is considered a *safe* and *idempotent* method.
...
*   If the resource doesn't exist at the specified URL, a `PUT` request *can* create it, but the HTTP specification states `POST` should generally be used for that.
...
*   **Idempotent:**  Calling the same `DELETE` request multiple times will eventually result in the resource being deleted.  Subsequent calls after the resource is deleted will typically return a 404 Not Found error, but the *state* of the server remains the same (the resource is gone).
...
*   **Idempotency:**  The idempotency of `PATCH` depends on the specific `Content-Type` and how the server handles the patch request. For example, a `PATCH` that increments a counter is *not* idempotent. A `PATCH` that replaces the value of a field *is* idempotent.
...
The `HEAD` method is similar to `GET`, but the server *must not* return a message-body in the response. The `HEAD` method is often used to check the following:
