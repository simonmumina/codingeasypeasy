...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['chakra-ui', 'shadow-dom', 'web-components', 'frontend-development', 'react', 'ui-library', 'custom-elements']
draft: false
    ...
    this.reactRoot.render(
      <ChakraProvider theme={theme}>
        <div>
          ...
          <Button colorScheme="blue" onClick={this.handleButtonClick}>Click Me</Button>
        </div>
      </ChakraProvider>
    );
  }
...
*   **`this.addEventListener('my-custom-event', ...)`:**  Adds an event listener to the custom element *outside* the Shadow DOM.
