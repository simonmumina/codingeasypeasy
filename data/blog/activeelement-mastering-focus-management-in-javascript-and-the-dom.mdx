...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['javascript', 'dom', 'activeElement', 'focus', 'accessibility', 'web development', 'frontend']
draft: false
...
The `document.activeElement` property returns the element within the current document that currently has focus.  This means the element is receiving keyboard events, or is the target of screen readers. It's a read-only property, meaning you can't directly *set* the active element using `document.activeElement = someElement;`.  Instead, you must use the `focus()` method on the specific element you want to bring into focus.
...
This code stores the `document.activeElement` *before* the modal is opened.  When the modal is closed, it restores focus to that element, providing a seamless user experience.
...
function checkFocusStyles(element) {
  ...
  if (contrastRatio < 4.5) { // WCAG 2.0 AA requirement for normal text
    console.warn(`Warning: Insufficient contrast for focused element: ${element}. Contrast ratio: ${contrastRatio}`);
  }
}

document.addEventListener('focus', (event) => {
  checkFocusStyles(event.target); // Check the focused element
}, true); // Use capture phase to ensure this runs before other focus handlers
```
...
*   **Shadow DOM:**  Working with Shadow DOM requires extra care. `document.activeElement` will return the shadow host if the focus is within the Shadow DOM. To get the focused element *inside* the shadow DOM, you need to access the shadow root and then its `activeElement` property:
...
*   **Event Bubbling:** The `focus` and `blur` events *do not* bubble.  To listen for focus changes across the entire document, you need to use event capturing (as shown in the accessibility example) by passing `true` as the third argument to `addEventListener`.
...
*   **Incorrectly Assuming Focus:** Don't assume that a specific element *always* has focus. Use `document.activeElement` to *verify* the currently focused element before performing actions.
