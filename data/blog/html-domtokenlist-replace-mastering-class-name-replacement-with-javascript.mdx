...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['HTML', 'DOM', 'JavaScript', 'DOMTokenList', 'replace', 'className', 'web development', 'front-end']
draft: false
...
*   The `replace()` method only replaces the *first* occurrence of `oldToken`. If the element has the same class name multiple times (which is generally bad practice), only the first instance will be replaced.
*   If `oldToken` does not exist in the `DOMTokenList`, the `replace()` method does *nothing* and does *not* throw an error.
