...
The `absolute` position removes an element from the normal document flow.  The element is positioned relative to its *nearest positioned ancestor*. A positioned ancestor is an element that has its `position` property set to `relative`, `absolute`, `fixed`, or `sticky`. If an absolutely positioned element has no positioned ancestor, it is positioned relative to the `<html>` element (the document root).
...
**Important Note:** If the outer `div` *did not* have a `position` value other than `static`, the inner, absolutely positioned `div` would be positioned relative to the `<html>` element (effectively the browser window).
