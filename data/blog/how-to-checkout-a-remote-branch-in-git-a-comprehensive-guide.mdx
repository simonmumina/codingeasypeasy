...
This command connects to the remote repository named `origin` and retrieves the latest information about all the branches and tags on that repository. It doesn't merge any changes into your local branches; it simply updates your local *remote-tracking branches*.
...
2.  Sets up a *tracking connection* between the local branch `feature/new-feature-local` and the remote branch `origin/feature/new-feature`.  This means that `git pull` and `git push` will automatically work with the remote branch.
