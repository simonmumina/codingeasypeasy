...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['node.js', 'vscode', 'javascript', 'development', 'debugging', 'npm', 'project setup', 'environment setup', 'tutorial']
draft: false
...
*   **`--save` (deprecated in npm 5, default behavior in npm 5 and later):** Saves the package as a *production dependency*.  Production dependencies are required for your application to run in production.

    ```bash
    npm install express --save  //  (Not needed in npm 5+, express will be added automatically)
    ```

*   **`--save-dev`:** Saves the package as a *development dependency*. Development dependencies are only needed during development (e.g., testing libraries, linters).
