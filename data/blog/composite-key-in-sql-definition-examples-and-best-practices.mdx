...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['SQL', 'Database Design', 'Composite Key', 'Primary Key', 'Data Modeling', 'Database Constraints']
draft: false
...
The fundamental difference is that a **primary key** can be a single column *or* a composite key. A primary key uniquely identifies each row in a table.  A composite key is simply a *type* of primary key that uses multiple columns.
...
| Existence       | Each table *must* have one     | A table *may* have a composite key, or a single-column primary key |
| Null Values     | Cannot contain NULL values      | Cannot contain NULL values in *any* of the composite key columns |
