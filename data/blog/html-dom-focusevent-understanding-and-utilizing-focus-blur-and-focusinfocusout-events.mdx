...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['html', 'dom', 'javascript', 'focusevent', 'focus', 'blur', 'focusin', 'focusout', 'accessibility', 'web development']
draft: false
...
*   **`focus` and `blur` are non-bubbling events.** This means that if you attach a `focus` or `blur` event listener to a parent element, it *won't* be triggered when a child element receives or loses focus. These events are targeted directly at the element gaining or losing focus.

*   **`focusin` and `focusout` are bubbling events.**  This means that if you attach a `focusin` or `focusout` event listener to a parent element, it *will* be triggered when a child element receives or loses focus. This bubbling behavior makes them ideal for handling focus events at a higher level in the DOM tree.
...
*   Critically, the event listeners are attached *directly* to the input element.
...
*   The event listeners are attached to the *container* element, allowing us to handle focus events for all child elements with a single listener.  This is delegation!
...
*   Note the inclusion of `:focus` in the CSS.  This is important so that *mouse* users also see the focus indicator when they click into the input field.  It's important not to *only* rely on Javascript for styling focus.  CSS gives the best experience.  Keyboard and mouse users should both have a clear visual indicator of focus.
...
const FocusManagementExample = () => {
  ...
  return (
    <div>
      ...
      <input type="text" id="firstName" ref={firstInputRef} /><br />
      <label htmlFor="lastName">Last Name:</label>
      <input type="text" id="lastName" ref={secondInputRef} /><br />
