...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['javascript', 'array', 'slice', 'method', 'tutorial', 'guide', 'programming', 'web development']
draft: false
...
*   **`endIndex` (Optional):** The index to end extraction *before*. This is exclusive.  The element at this index will *not* be included in the new array.
...
*   If the original array contains primitive values (numbers, strings, booleans), the new array will contain copies of those values.  Changes to these values in the new array will *not* affect the original array.
*   However, if the original array contains objects (including other arrays), the new array will contain *references* to the same objects.  Therefore, if you modify an object referenced in both the original and the new array, the changes will be reflected in both arrays.
...
In the example above, modifying `newArray[0].age` also changes the `age` property in `originalArray[0]` because both arrays contain references to the same object. If you need to create a *deep* copy (where objects are completely independent), you'll need to use other techniques, such as the `JSON.parse(JSON.stringify(array))` method (with limitations on data types) or a dedicated deep copy library.

## `slice()` vs `splice()`

It's easy to confuse `slice()` and `splice()`, but they have distinct purposes.

*   **`slice()`:** Extracts a section of an array and returns it as a *new* array.  The original array is *not* modified.
*   **`splice()`:**  Changes the contents of an array by removing or replacing existing elements and/or adding new elements in place.  The original array *is* modified.
