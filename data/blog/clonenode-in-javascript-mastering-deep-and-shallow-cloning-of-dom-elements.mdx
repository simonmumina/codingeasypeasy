...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'dom', 'cloneNode', 'deep clone', 'shallow clone', 'web development', 'html', 'performance']
draft: false
...
The `cloneNode()` method, available on all DOM Node objects in JavaScript, creates a duplicate of the node on which it is called.  Crucially, you get to control *how* much of the original node is copied. This control is achieved using a boolean parameter that determines whether the clone should be a deep clone or a shallow clone.
...
*   `deep`: A boolean value.
    *   `true`:  Performs a *deep clone*. This copies the node *and all of its descendants* (child nodes, attributes, and text content).
    *   `false`: Performs a *shallow clone*. This copies only the node itself, without any of its descendants.  The cloned node will have no child nodes.

### Return Value

The `cloneNode()` method returns the cloned node.  However, the cloned node is *not* automatically added to the DOM.  You'll need to use methods like `appendChild()` or `insertBefore()` to insert the clone into the document tree.
...
*   Changes made to the original node after cloning will *not* affect the cloned node, and vice versa.  They are independent entities.
...
A shallow clone creates a copy of the node itself, *without* any of its descendants.  The cloned node will have the same attributes as the original, but it will not contain any child nodes.
...
*   **Event Listeners:** `cloneNode()` *does not* copy event listeners that are attached using `addEventListener()`.  You'll need to manually re-attach any event listeners to the cloned node. This is often desired, as you may not want the cloned elements to have the same behavior.  However, inline event handlers (e.g., `<button onclick="myFunction()">`) *will* be copied.  It's generally best practice to avoid inline event handlers.
