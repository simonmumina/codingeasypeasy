...
React Hooks are powerful tools that allow you to use state and other React features in functional components. However, they come with specific rules that you *must* follow to avoid the "Invalid Hook Call" error. Understanding these rules is key to preventing and resolving this issue.

The core principle is: **Hooks must be called directly inside a React function component or a custom Hook.** This means:

*   **Inside a function component:** The call to `useState`, `useEffect`, `useContext`, or any other Hook *must* be within the body of a React function component.
...
The best approach here is usually to create a separate component that renders each item and *that* component calls the hook.
...
While you can technically call a Hook *inside* a function component that's an event handler, avoid passing a function containing Hooks *directly* to a DOM element's event handler.
...
function MyComponent() {
  ...
  return (
    <button onClick={handleClick}> // Correct: Passing the function to the onClick handler
      Click me ({count})
    </button>
...
The incorrect example technically works, but only because `handleClick` is defined *inside* the function component. The issue arises when you try to do something like this (which you should never do):
...
```
**Solution:** Always define the entire function component *inside* the React component or a custom hook.
