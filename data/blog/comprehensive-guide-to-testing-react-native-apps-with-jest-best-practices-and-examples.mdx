...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react-native', 'testing', 'jest', 'unit-testing', 'integration-testing', 'e2e-testing', 'mocking', 'react-native-testing-library']
draft: false
...
describe('DataComponent', () => {
  ...
  it('displays an error message if fetching data fails', async () => {
    const fetchDataMock = jest.spyOn(dataService, 'fetchData').mockRejectedValue(new Error('Failed to fetch'));
...
*   `mockRestore()`:  This is *very* important.  It restores the original implementation of the `fetchData` function after the test is complete.  Failing to do this can cause unexpected behavior in other tests.  While `afterEach` is sometimes used, explicitly calling `mockRestore` at the end of the test is generally safer because it ensures that the mock is cleared *even if the test fails*.
...
*  **Snapshot Testing (Use sparingly):** While Jest snapshot testing can be useful for quickly identifying unexpected UI changes, it can also lead to brittle tests that require frequent updates.  It's generally better to focus on more targeted tests that verify specific behaviors and properties. If using snapshots, review changes *very* carefully.
