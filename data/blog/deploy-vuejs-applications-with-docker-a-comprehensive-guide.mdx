...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['vuejs', 'docker', 'deployment', 'containerization', 'nginx', 'ci/cd', 'frontend', 'web development']
draft: false
...
*   **`COPY package*.json ./`**: This copies the `package.json` and `package-lock.json` (or `yarn.lock`) files to the working directory.  Crucially, we copy these *before* copying the entire source code. This allows Docker to cache the installation of dependencies, as it only needs to reinstall them if the `package.json` or `package-lock.json` files change.
