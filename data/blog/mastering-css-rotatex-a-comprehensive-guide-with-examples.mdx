...
The `rotateX()` CSS function is a part of the `transform` property family. It rotates an element around its X-axis (a horizontal line passing through the element). Imagine sticking a skewer horizontally through your element – that's the X-axis! The value you provide to `rotateX()` determines the angle of rotation.
...
**Important:** The order in which you specify the transformations _matters_. In the example above, the element is first rotated around the X-axis and then around the Y-axis. Changing the order would produce a different result.
