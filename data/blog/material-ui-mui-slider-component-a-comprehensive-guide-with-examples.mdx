...
date: '2024-01-03'
lastmod: '2024-01-03'
tags: ['material-ui', 'mui', 'react', 'slider', 'component', 'javascript', 'web development', 'ui components']
draft: false
...
function DisabledSlider() {
  return (
    <Slider defaultValue={30} disabled aria-label="Disabled" valueLabelDisplay="auto" />
  );
}
...
*   The `value` prop is bound to the state, making this a *controlled* component.  This is generally preferred over using `defaultValue` when you need to react to value changes.

### 2. `onChangeCommitted` Event

The `onChangeCommitted` event fires *after* the user releases the mouse or touch. This is useful when you only want to perform an action after the user has finished adjusting the slider.
