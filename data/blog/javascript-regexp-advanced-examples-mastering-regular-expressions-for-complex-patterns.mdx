...
date: '2024-01-01'
lastmod: '2024-12-26'
tags: ['javascript', 'regexp', 'regular expressions', 'regex', 'advanced javascript', 'pattern matching', 'string manipulation']
draft: false
...
This example uses a positive lookbehind `(?<=\$)` to match only the digits that are preceded by a dollar sign.  The dollar sign itself is *not* included in the match.
...
This cleverly uses multiple positive lookaheads to ensure all the password criteria are met *before* matching any characters.  The `.{8,}` part then requires at least 8 characters of any kind to be present. This avoids having to specify the order in which these characters must appear.
...
Here, we capture the first word in group 1, the middle part in group 2, and the last word in group 3.  The `replace` method uses `$1`, `$2`, and `$3` to refer to these groups and reorders them.  This is different from `\1` which is used within the *regex pattern* itself. `$1`, `$2`, etc. are used within the *replacement string* in `replace()`.
...
Without the `m` flag, `^` and `$` would only match the very beginning and very end of the *entire* string, and the regex wouldn't find a match.
