...
Sometimes, after splitting the commit, you might realize that the last commit is incomplete or needs a minor adjustment. You can use `git commit --amend` to modify the last commit *before* pushing it to a remote repository.
