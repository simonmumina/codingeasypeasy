...
Remember that `toFixed()` returns a *string*. If you need to perform further mathematical operations, you'll need to convert the string back to a number using `parseFloat()` or `Number()`.
