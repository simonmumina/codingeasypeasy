...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'window', 'screenTop', 'screenLeft', 'browser-api', 'web-development', 'positioning']
draft: false
...
The `window.screenTop` property is a read-only property of the JavaScript `window` object. It returns the *vertical* distance (in pixels) between the top edge of the current browser window and the top edge of the user's screen. In simpler terms, it tells you how far down from the top of the screen the top of your browser window is located.
...
function adjustUI() {
  ...
  if (topOffset < 50) { // If the window is within 50 pixels of the top
...
Alongside `window.screenTop`, the `window.screenLeft` property provides the *horizontal* distance (in pixels) between the left edge of the browser window and the left edge of the screen.  These two properties together give you the complete coordinates of the window's top-left corner relative to the screen.
...
While `window.screenTop` provides valuable information about the window's position relative to the *screen*, you might also be interested in its position relative to the *viewport* (the visible portion of the browser window). For that, you can use methods like `getBoundingClientRect()` on a specific element within the page or the `window.scrollY` and `window.scrollX` properties to determine the scroll position.  However, there isn't a direct equivalent that provides the window position relative to the entire screen without relying on `window.screenTop` and `window.screenLeft`.
