...
date: '2024-02-29'
lastmod: '2024-03-01'
tags: ['CSS', 'transform-origin', 'HTML DOM', 'Style Object', 'Animation', 'CSS Transforms', 'Web Development', 'Front-End']
draft: false
...
In simple terms, `transform-origin` defines the origin point for transformations. Think of it as the pivot point. When you apply a `transform` like `rotate`, `scale`, or `skew`, the element will transform *around* the point defined by `transform-origin`.
...
const RotateExample = () => {
  ...
  return (
    <div>
      ...
      <button style={buttonStyle} onClick={handleRotate}>Rotate</button>
    </div>
  );
};
...
const ScaleExample = () => {
  ...
  return (
    <div>
      ...
      <button style={buttonStyle} onClick={handleScale}>Scale Up</button>
    </div>
  );
};
...
function changeTransformOrigin() {
  ...
  let currentOriginIndex = element.dataset.originIndex ? parseInt(element.dataset.originIndex) : 0;
