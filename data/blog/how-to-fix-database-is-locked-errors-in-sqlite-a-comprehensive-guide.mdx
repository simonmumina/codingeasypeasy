...
*   **Disable Shared Cache (Consider Carefully):** In some scenarios, especially in multi-process environments where each process has its own SQLite connection, shared cache mode can cause problems. Disabling it *might* resolve locking issues, but consider the implications carefully, as it can impact performance.  Only disable shared cache if you've thoroughly investigated and understand the potential consequences.  It's *generally not recommended*.
...
async function writeToDB(data) {
  return new Promise((resolve, reject) => {
    ...
    db.serialize(() => { // Serialize execution to prevent conflicts
