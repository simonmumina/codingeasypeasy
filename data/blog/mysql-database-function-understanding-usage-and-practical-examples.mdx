...
date: '2024-01-26'
lastmod: '2024-01-27'
tags: ['MySQL', 'Database', 'SQL', 'Function', 'DATABASE()', 'Schema', 'Current Database', 'Information Retrieval', 'Practical Examples', 'SQL Tutorial']
draft: false
...
While less common, you *can* technically combine the result of `DATABASE()` with a table name (though it's generally not recommended for most use cases, and often indicates a potential design flaw). Consider a scenario where you want to query a table that follows a naming convention based on the database name.

**Warning:**  This approach can lead to SQL injection vulnerabilities if not handled carefully.  It's *highly* recommended to avoid dynamically constructing table names from user input.
