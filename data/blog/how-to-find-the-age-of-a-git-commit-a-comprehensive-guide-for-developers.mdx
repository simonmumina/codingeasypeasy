...
While not directly providing the age of *a specific commit*, the `--since` and `--until` flags of `git log` can be useful to understand the age of *commits within a range*.  This is valuable for identifying commits older than a certain date.
