...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['Ito Calculus', 'Stochastic Calculus', 'Financial Modeling', 'Brownian Motion', 'Wiener Process', 'Derivatives Pricing', 'Quantitative Finance', 'Mathematical Finance', 'Risk Management']
draft: false
...
Unlike standard calculus, which deals with smooth, predictable functions, Ito calculus is designed to handle the "roughness" and unpredictability of stochastic processes.  At its heart lies the *Ito integral*, which defines the integral of a non-anticipating function with respect to a Brownian motion.
...
*   $\sigma(X_t, t)$ is the *diffusion* term (volatility).
...
This last rule is perhaps the most surprising and crucial aspect of Ito calculus.  It reflects the fact that while the change in Brownian motion over a small time interval is normally distributed with variance proportional to the time interval, the *square* of this change contributes a non-negligible amount to the overall change in the function.
...
*   $V(S, t)$ is the price of the option as a function of the stock price *S* and time *t*.
