...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['javascript', 'object', 'getOwnPropertyDescriptor', 'property descriptor', 'configurable', 'enumerable', 'writable', 'javascript tutorial', 'javascript example', 'property attributes']
draft: false
...
The `Object.getOwnPropertyDescriptor()` method returns a *property descriptor* for an own property (that is, one directly present on an object and not inherited through the prototype chain) of a given object.  A property descriptor is an object containing information about the property's attributes:
...
`getOwnPropertyDescriptor` only retrieves information about *own* properties. It does not traverse the prototype chain.
...
*   **Confusing with `Object.keys()` and `for...in`:**  `Object.keys()` and `for...in` only return *enumerable* properties.  `getOwnPropertyDescriptor()` gives you information about *all* own properties, regardless of their enumerability. Use `Object.getOwnPropertyNames()` to retrieve the names of all own properties, whether enumerable or not.
*   **Incorrect Scope:**  Ensure you are calling `getOwnPropertyDescriptor()` on the correct object, especially when dealing with inheritance. Remember that it only retrieves information about *own* properties.
