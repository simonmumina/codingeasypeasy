...
*   **`setSeconds()`:**  Sets the seconds portion of a `Date` object according to *local time*.  This means the result will be affected by the user's time zone and any daylight saving time adjustments.
*   **`setUTCSeconds()`:**  Sets the seconds portion of a `Date` object according to *Coordinated Universal Time (UTC)*. This provides a consistent and timezone-independent way to manipulate the seconds.
