...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['css', 'interview', 'frontend', 'web development', 'css properties', 'box model', 'flexbox', 'grid', 'responsive design', 'css preprocessors', 'sass', 'less', 'interview questions']
draft: false
...
*   **`box-sizing` Property:**  This property controls how the `width` and `height` of an element are calculated.
    ...
    *   `content-box` (default): The `width` and `height` apply only to the content area. Padding and border are added *on top* of the specified width and height.
    *   `border-box`: The `width` and `height` properties include the padding and border, but *not* the margin. This makes layout calculations much easier.  It's often recommended to use `box-sizing: border-box;` globally:
...
*   `relative`:  The element is positioned relative to its normal position.  `top`, `right`, `bottom`, and `left` properties can be used to adjust its position.  Crucially, it *doesn't* affect the position of other elements around it; it leaves a gap where it would have been.
