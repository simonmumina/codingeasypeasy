...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['fullscreen API', 'fullscreenElement', 'web development', 'javascript', 'browser API', 'immersive experiences', 'web standards']
draft: false
...
The `fullscreenElement` property of the `document` object provides valuable information about the current fullscreen state of your web page.  Specifically, it tells you *which* element, if any, is currently being displayed in fullscreen.
...
If you need to know *which* specific element is in fullscreen, you can directly access the `fullscreenElement` property.
...
4.  **Fullscreen Trigger:** The example shows how to trigger fullscreen mode on a video element (`myVideo`) when the user double-clicks it.  It uses the appropriate prefixed `requestFullscreen` method based on the browser.  This part is crucial for *entering* fullscreen.
