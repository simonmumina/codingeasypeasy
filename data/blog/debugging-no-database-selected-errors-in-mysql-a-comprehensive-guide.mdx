...
The error message "No database selected" is MySQL's way of telling you, "Hey, I don't know which database you want me to use!  You need to tell me!"  Essentially, you're trying to run queries against a blank canvas. MySQL needs to know *which* database within your server to target.
...
Replace `your_database_name` with the actual name of your database.  Remember to run the `USE` statement *every time* you reconnect to the MySQL server.
