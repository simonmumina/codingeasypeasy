...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'dom', 'getBoundingClientRect', 'element positioning', 'web development', 'browser API']
draft: false
...
`getBoundingClientRect()` takes CSS transformations (e.g., `transform: rotate()`, `transform: scale()`, `transform: translate()`) into account.  This is a powerful feature, but it can also be a source of confusion if you're not aware of it.  The returned `width` and `height` will be the *rendered* width and height, reflecting any scaling or rotation applied through CSS.  The `x` and `y` values are also affected by translations and rotations.
