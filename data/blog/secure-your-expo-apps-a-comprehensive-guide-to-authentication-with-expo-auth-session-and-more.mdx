...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['expo', 'authentication', 'auth session', 'oauth', 'react native', 'mobile security', 'social login', 'firebase', 'supabase']
draft: false
...
*   **`useAuthRequest` hook:**  This hook initiates the authentication request.
    *   `clientId`:  The OAuth 2.0 client ID you obtained from the Google Cloud Console. Ensure you're using the *web* client ID for broader compatibility, especially when facing deep linking issues.
  ...
  const exchangeCodeForAccessToken = async (code) => {
      ...
      try {
          const backendResponse = await fetch('YOUR_BACKEND_URL/auth/facebook', {
              ...
              body: JSON.stringify({ code, redirectUri: AuthSession.makeRedirectUri({ scheme: 'your-app-scheme' }) }),
          });
...
app.post('/auth/facebook', async (req, res) => {
    ...
    try {
        ...
        const userResponse = await fetch(`https://graph.facebook.com/me?access_token=${accessTokenData.access_token}&fields=id,name,email`);
