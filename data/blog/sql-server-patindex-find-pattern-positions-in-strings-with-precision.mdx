...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['SQL Server', 'PATINDEX', 'String Functions', 'Pattern Matching', 'T-SQL', 'Database', 'SQL Tutorial', 'SQL Examples', 'Wildcard', 'Character Index']
draft: false
...
`PATINDEX()` returns the starting position of the *first* occurrence of a pattern in a specified string.  If the pattern is not found, it returns 0.  The function uses a limited set of wildcards similar to the `LIKE` operator, making it ideal for searching for patterns rather than exact matches.
...
*   **`'%pattern%'`**: This is the pattern you're searching for.  It *must* be enclosed in single quotes and can contain wildcards (see below).  The surrounding `%` symbols are typically used to search for the pattern anywhere within the string.  You can remove these if you want to search at the beginning or end of the string.
...
*   **`[^]` (Square Brackets with Caret)**: Specifies a range of characters to *not* match. For example, `[^0-9]` matches any character that is not a digit.
