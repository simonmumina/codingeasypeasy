...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react-native', 'debugging', 'network requests', 'api', 'http', 'devtools', 'flipper', 'charles proxy', 'react native debugging']
draft: false
...
*   **React Native Code (Often Not Required):**  While you don't *typically* need to modify your React Native code, some libraries (like `axios`) might require explicit proxy configuration in certain circumstances.
...
if (__DEV__) {
  Reactotron
    .configure({ name: 'MyApp' }) // Optionally add a script to load react-native
