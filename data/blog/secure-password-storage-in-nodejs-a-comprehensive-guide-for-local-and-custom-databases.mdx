...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['nodejs', 'security', 'passwords', 'bcrypt', 'hashing', 'database security', 'local database', 'custom database', 'authentication', 'salting']
draft: false
...
*   **Adaptive Hashing:** The computational cost of `bcrypt` can be increased to make it more resistant to brute-force attacks. This is controlled by the *cost factor* or *rounds*.
...
*   **Database Storage:**  The example code shows where you would store the username and the `hashedPassword` into your database.  Remember *never* store the original password.
...
*   **Important:** You *must* retrieve the `hashedPassword` from your database first, based on the user's username or other identifying information.
...
.then(() => {
  ...
  // Example calls
  createUser('testuser', 'MySecretPassword123')
    .then(() => authenticateUser('testuser', 'MySecretPassword123'));
})
