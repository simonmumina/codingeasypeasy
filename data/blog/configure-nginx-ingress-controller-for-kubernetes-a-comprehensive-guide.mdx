...
date: '2024-02-29'
lastmod: '2024-03-07'
tags: ['kubernetes', 'nginx', 'ingress', 'ingress controller', 'load balancing', 'deployment', 'k8s', 'microservices', 'devops']
draft: false
...
*   `spec.rules`: Defines the routing rules.
    *   `http.paths`:  A list of paths to match.
        ...
        *   `pathType: Prefix`: Matches any URL that *starts* with the specified path.  Alternatives include `Exact` (must be an exact match) and `ImplementationSpecific` (behavior depends on the Ingress Controller).
