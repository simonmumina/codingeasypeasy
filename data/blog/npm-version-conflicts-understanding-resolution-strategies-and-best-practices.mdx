...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['npm', 'package management', 'dependency resolution', 'version conflicts', 'semantic versioning', 'nodejs']
draft: false
...
2.  **Satisfying Dependencies:** NPM attempts to satisfy each dependency's version requirements. It tries to find the *highest* possible version of each package that meets all the constraints imposed by its dependencies.

3.  **Deduplication (Hoisting):** NPM tries to "deduplicate" dependencies.  If multiple packages in the dependency tree require the *same* version (or compatible versions) of a particular package, NPM will try to install that package only once, at the highest possible level in the `node_modules` directory. This is called "hoisting".  Hoisting reduces the overall size of your `node_modules` and avoids unnecessary duplication.
...
In this example, `lodash` appears multiple times.  While NPM has deduplicated them (meaning it's using a single compatible version where possible), it's important to understand *which* version is being used and whether it's truly compatible with all dependencies.
