...
`VARCHAR` (Variable Character) is a variable-length character data type. When you define a column as `VARCHAR(n)`, you are specifying the *maximum* length of the string that can be stored in that column. Unlike `CHAR`, `VARCHAR` only uses the space necessary to store the actual string, plus a small amount of overhead to store the length of the string.
...
*   `VARCHAR` does *not* pad strings. The string is stored exactly as it is entered, up to the specified maximum length.
...
*   **Use `CHAR` only when the length is *always* fixed and known:**  This can be beneficial for storing short, fixed-length codes or abbreviations.
