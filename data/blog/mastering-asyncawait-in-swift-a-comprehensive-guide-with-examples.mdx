...
*   **Concurrency vs. Parallelism:** While often used interchangeably, they are different. Concurrency means managing multiple tasks *at the same time*, potentially interleaving their execution on a single core. Parallelism means executing multiple tasks *simultaneously* on multiple cores. Swift's concurrency model primarily focuses on concurrency, allowing efficient use of a single core, while also enabling parallelism when appropriate.
...
4.  **`await` required:** You *must* use `await` when calling methods on an actor from outside the actor's context.
