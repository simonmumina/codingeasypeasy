...
1.  **`getServerSideProps(context)`:** This asynchronous function runs on the server *before* the component is rendered.  It receives a `context` object that contains information about the request, such as the query parameters, headers, and cookies.
...
3.  **Returning Props:**  The function *must* return an object with a `props` property. The value of this `props` property will be passed as props to the `HomePage` component.
...
*   **Server-Side Only:** `getServerSideProps` runs *only* on the server.  Code within this function will not be included in the client-side bundle, making it safe to include server-side secrets or perform operations that are not suitable for the client.
