...
date: '2024-01-01'
lastmod: '2024-01-05'
tags: ['HTML', 'DOM', 'JavaScript', 'Events', 'Web Development', 'Audio', 'Video', 'ondurationchange']
draft: false
...
*   The `toggleQualityButton` changes the `src` attribute of the video element and calls `video.load()` to load the new source. This is *crucial* to force the video element to fetch the metadata of the new source.
...
When the `ondurationchange` event is triggered, an event object is passed to the event listener function.  While the event object itself doesn't contain specific properties related to *why* the duration changed, it does provide access to standard event object properties, and most importantly, the video or audio element itself (`event.target`). You can then use properties of the audio or video element, such as `duration` and `currentSrc`, to understand the change.
