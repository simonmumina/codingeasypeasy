...
date: '2024-02-29'
lastmod: '2024-10-27'
tags: ['component composition', 'react', 'ui development', 'reusable components', 'design patterns', 'javascript', 'front-end', 'composition over inheritance']
draft: false
...
**Component composition offers a superior alternative.** Instead of creating a `SubmitButton` *inheriting* from `Button`, you *compose* a `Button` with `submit` functionality.
...
function App() {
  ...
  return (
    <Layout header={myHeader} content={myContent} footer={myFooter} />
  );
}
