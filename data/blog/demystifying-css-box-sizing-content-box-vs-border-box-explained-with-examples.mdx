...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['css', 'box-sizing', 'content-box', 'border-box', 'css-layout', 'web-development', 'frontend-development']
draft: false
...
*   **`content-box`**:  This is the default value.  When `box-sizing` is set to `content-box`, the `width` and `height` properties only apply to the *content area* of the element.  Padding and border are added *on top* of that width and height, increasing the overall size of the element.
*   **`border-box`**:  With `border-box`, the `width` and `height` properties apply to the *entire* element, including padding and border.  The content area shrinks to accommodate the padding and border within the specified dimensions.
...
<div class="content-box-example">
  This is an example using content-box.
</div>
```
...
<div class="border-box-example">
  This is an example using border-box.
</div>
```
...
With `border-box` enabled, the `width` and `height` properties now represent the *total* size of the element, including padding and border. The content area automatically adjusts to accommodate them.

*   **Width:** The *total* width is 200px. The content area will be 200px - 20px (left padding) - 20px (right padding) - 5px (left border) - 5px (right border) = **150px**
*   **Height:** The *total* height is 100px. The content area will be 100px - 20px (top padding) - 20px (bottom padding) - 5px (top border) - 5px (bottom border) = **50px**
...
*   **Specific Components with Intentional Padding/Border Expansion:** In very specific cases, you might want an element's padding and border to *intentionally* expand its overall size beyond the specified width and height. This is uncommon, but could be used for particular visual effects.
