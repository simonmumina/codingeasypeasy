...
date: '2024-01-26'
lastmod: '2024-01-27'
tags: ['react', 'jwt', 'authentication', 'authorization', 'security', 'javascript', 'frontend', 'tutorial']
draft: false
...
2.  **Install necessary dependencies:**
    ...
    *   `jsonwebtoken`: (Optional - Primarily useful for *decoding* the token on the frontend.  Signature verification should always be done on the *backend*.)  If you plan to just store and send the token and only have the backend verify it, then you don't *need* this on the frontend.
    ...
    const loadUserFromToken = async () => {
        if (token) {
            try {
                ...
                const response = await axios.get('/api/validateToken', { // Backend needs to validate the token and return user info
...
```jsx filename="src/components/Login.jsx"
...
```jsx filename="src/components/Register.jsx"
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
...
```jsx filename="src/components/ProtectedRoute.jsx"
import React from 'react';
...
```jsx filename="src/components/Home.jsx"
import React from 'react';
...
```jsx filename="src/components/Profile.jsx"
import React from 'react';
import { useAuth } from '../context/AuthContext';
...
*   **Backend Verification is Crucial:** The frontend should *never* be considered the source of truth for user authentication.  The *backend* must always verify the JWT.  The frontend is only storing and presenting data based on the verified JWT.
