...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['CSS', 'background-clip', 'HTML DOM', 'Style Object', 'Web Development', 'Frontend Development', 'CSS Properties', 'Text Effects']
draft: false
...
```html
<div id="paddingBoxExample" style="
    width: 200px;
    height: 100px;
    border: 10px solid red;
    padding: 10px;
...
Here, the light blue background extends to the edge of the 10px padding, but it *stops* at the beginning of the red border.  The border itself is not covered by the background.
...
```html
<div id="contentBoxExample" style="
    width: 200px;
    height: 100px;
    border: 10px solid red;
    padding: 10px;
...
2.  **`background-image: url(...)`**:  Sets the background image you want to use.  Replace the URL with your desired image.
...
*   **`text-fill-color`:**  You *must* set the `text-fill-color` to `transparent` (or equivalent) to see the effect.  If you leave the text color as the default, it will cover the background.
