...
In Vue 3, lifecycle hooks like `onMounted` are functions that are called at specific stages of a component's lifecycle.  `onMounted` is triggered *after* the component's initial render and the corresponding DOM node has been inserted into the document.  It's the go-to hook for tasks that depend on the component existing in the DOM.
...
*   This example demonstrates fetching data from an API (using `fetch`) *after* the component has mounted.
...
*   **Why use `onMounted` here?** Because the API request and subsequent UI update only make sense *after* the component is ready to be displayed.
...
*   **Use `nextTick` if necessary:** If you need to access the DOM after a reactive update, use `nextTick` to ensure that the DOM has been updated before you attempt to access it.  `nextTick` allows you to execute a callback *after* the DOM has been updated by Vue.
