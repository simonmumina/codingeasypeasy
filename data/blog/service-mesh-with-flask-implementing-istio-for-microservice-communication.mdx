...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['flask', 'service mesh', 'istio', 'microservices', 'python', 'devops', 'kubernetes', 'api gateway', 'sidecar proxy']
draft: false
...
2.  **Create a Service and Deployment for the New Version:**
   ...
   Apply this configuration.  Note that the Service has *the same name* as the original, but the Deployment specifies `version: v2`.  The important part is the labels.
...
3.  **Update the VirtualService to Route a Percentage of Traffic:**
   ...
   *   **DestinationRule:** The `DestinationRule` defines the subsets (`v1` and `v2`) based on the `version` label.  This tells Istio *how* to find the different versions of your service.  Crucially, the host specified here **must** match the `host` specified in the `VirtualService`.
