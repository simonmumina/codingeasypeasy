...
Before diving into the technical details, it's worth considering *why* you need to support IE11. While its market share is negligible in many regions, certain industries and organizations might still rely on it. Understanding your target audience will help you decide if the effort of supporting IE11 is justified.
...
**Important:** The `@babel/preset-env` configuration (explained in the next section) handles *which* polyfills from `core-js` are actually included in your build, based on your browser targets.
