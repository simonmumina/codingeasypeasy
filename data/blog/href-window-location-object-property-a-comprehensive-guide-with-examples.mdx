...
The power of the `href` property lies in its ability to *change* the current URL. By assigning a new URL to `window.location.href`, you can redirect the user to a different page.
...
*   **No History Entry:**  Assigning to `href` adds a new entry to the browser's history. This means the user can use the back button to return to the previous page. If you want to *replace* the current history entry (preventing the user from returning), use the `window.location.replace()` method (discussed later).
...
As mentioned earlier, `window.location.replace()` is used to *replace* the current history entry.  This means that after the redirection, the user won't be able to use the back button to return to the previous page.
