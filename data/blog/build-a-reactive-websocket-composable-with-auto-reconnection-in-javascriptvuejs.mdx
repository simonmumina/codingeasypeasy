...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['websocket', 'reactive programming', 'javascript', 'vue.js', 'composable', 'auto-reconnect', 'real-time', 'frontend', 'reactive websocket', 'websocket tutorial']
draft: false
...
export function useWebSocket(url) {
  ...
  const reconnect = () => {
    ...
    console.log(`Attempting to reconnect in ${timeout/1000} seconds... (Attempt ${reconnectAttempts})`);
...
<template>
  <div>
    ...
    <p>Connection Status: <span :class="{ connected: isConnected, disconnected: !isConnected }">{{ isConnected ? 'Connected' : connecting ? 'Connecting...' : 'Disconnected' }}</span></p>
...
export default {
  setup() {
    const message = ref('');
    const { data, isConnected, connecting, error, send, close } = useWebSocket('ws://localhost:8080'); // Replace with your WebSocket server URL
