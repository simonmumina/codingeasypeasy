...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['fastapi', 'firebase', 'authentication', 'python', 'api', 'security', 'google cloud', 'oauth', 'jwt']
draft: false
...
*   **firebase\_admin:**  The Firebase Admin SDK, allowing us to interact with Firebase from our server.
...
*   **`credentials.Certificate("path/to/your/serviceAccountKey.json")`:** This is the most common method.  Download the service account key JSON file from your Firebase project settings (Service accounts tab).  **Important:** Store this file securely and *never* commit it to your public repository! Replace `"path/to/your/serviceAccountKey.json"` with the actual path to your file.
...
*   **`get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security))`:** This is a *dependency* function.  FastAPI will call this function before any route that depends on it.
...
*   **`@app.post("/items/", dependencies=[Depends(get_current_user)])`:** This is the key part.  We've added `dependencies=[Depends(get_current_user)]` to the route decorator.  This tells FastAPI that the `create_item` route *depends* on the `get_current_user` function.  FastAPI will call `get_current_user` *before* calling `create_item`.  If `get_current_user` raises an exception (because the token is invalid), the `create_item` route will *not* be called, and the client will receive a 401 Unauthorized error.
