...
*   **Two-Dimensional Control:** Unlike Flexbox, which primarily focuses on one-dimensional layouts (either rows *or* columns), CSS Grid empowers you to control *both* rows and columns simultaneously. This opens up a world of layout possibilities previously difficult or impossible to achieve with older methods like floats or tables.
...
*   **Grid Tracks:** The spaces between grid lines. A *row track* is the space between horizontal grid lines, and a *column track* is the space between vertical grid lines.
...
The key to using CSS Grid effectively is defining the grid structure using properties on the *grid container*.  Let's look at the most important ones:
...
*   **`auto-fill`:** Similar to `auto-fit`, but it adds *empty* tracks if necessary to fill the available space. This means the grid items will *not* stretch to fill the remaining space.
...
*   **`justify-items`:** Aligns grid items along the *inline* (horizontal) axis within their grid cells.  Applied to the **grid container**.
*   **`align-items`:** Aligns grid items along the *block* (vertical) axis within their grid cells. Applied to the **grid container**.
*   **`justify-content`:** Distributes grid tracks along the *inline* (horizontal) axis within the grid container. Applied to the **grid container**.
*   **`align-content`:** Distributes grid tracks along the *block* (vertical) axis within the grid container. Applied to the **grid container**.
*   **`justify-self`:** Aligns a *single* grid item along the *inline* (horizontal) axis within its grid cell. Applied to the **grid item**.
*   **`align-self`:** Aligns a *single* grid item along the *block* (vertical) axis within its grid cell. Applied to the **grid item**.
...
If you place a grid item outside the explicitly defined grid tracks (using `grid-template-columns` and `grid-template-rows`), the browser will automatically create *implicit grid tracks* to accommodate the item.
