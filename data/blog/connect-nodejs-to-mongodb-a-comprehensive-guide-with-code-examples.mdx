...
2.  **Define a Schema:** Create a model file (e.g., `user.model.js`) to define the structure of your data.
    ...
    *   We create a model using `mongoose.model()`. The first argument ('User') is the *singular* name of your model; Mongoose will automatically look for a collection with the *pluralized* and lowercased version of your model name (in this case, 'users').
