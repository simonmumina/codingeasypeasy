...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'negative infinity', 'number', 'number.negative_infinity', 'coding', 'web development', 'js']
draft: false
...
This demonstrates comparing a very small number with `NEGATIVE_INFINITY`.  Because there's nothing *smaller* than negative infinity, any other representable negative number will be larger.
...
While initializing `minValue` with `Number.NEGATIVE_INFINITY` in this specific example would lead to an incorrect result (because the values in the array are all *greater* than `NEGATIVE_INFINITY`), this demonstrates the concept of using it to initialize values when dealing with minimum or maximum boundaries.  When *finding the largest* number you can initialize with `Number.NEGATIVE_INFINITY`.
