...
date: '2024-01-01'
lastmod: '2024-10-27'
tags: ['react-native', 'deep linking', 'universal links', 'mobile development', 'url schemes', 'app links', 'ios', 'android', 'linking']
draft: false
...
1.  **Associate Your App with Your Website:**
    ...
    *   **Content of `assetlinks.json`:**
        ...
        *   **Important:** This file *must* be served with the `Content-Type` header set to `application/json`.  Verify with `curl -I https://yourdomain.com/.well-known/assetlinks.json`.
...
2.  **Configure `AndroidManifest.xml` for App Links:**
    ...
    *   Open `android/app/src/main/AndroidManifest.xml`.
    *   Add an `<intent-filter>` to the `<activity>` tag that handles the main activity (usually the one with the `MAIN` and `LAUNCHER` intent filters).  This is *similar* to deep linking, but with specific `android:host` and `android:scheme` attributes.
