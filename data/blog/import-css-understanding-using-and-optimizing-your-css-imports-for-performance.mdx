...
One legitimate use case for `@import` *might* be to conditionally load stylesheets based on media queries. However, even this is often better handled with `<link>` tags using the `media` attribute.
