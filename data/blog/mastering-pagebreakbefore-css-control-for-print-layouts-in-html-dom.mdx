...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['css', 'html', 'dom', 'page-break', 'print-styling', 'print-media', 'pageBreakBefore', 'styling-for-print']
draft: false
...
*   **`always`:**  Forces a page break *before* the element.  The element will always start on a new page.

*   **`avoid`:** Instructs the browser to *avoid* a page break before the element if possible.  The browser will prioritize keeping the element on the same page as the preceding content. This is helpful for preventing headings from being separated from their associated content.
...
<body>
  ...
  <h2>Heading</h2>
  <p class="no-break">This is a long paragraph of text that should not be broken across multiple pages.  We are using the <code>page-break-inside: avoid;</code> property to ensure that the entire paragraph stays on the same page.</p>
...
Here, the CSS rule `.no-break { page-break-inside: avoid; }` is applied to the `<p>` element with the class "no-break." This prevents the browser from inserting a page break *within* that specific paragraph. We also added `p { page-break-before: avoid; }`  to avoid a page break right after the heading. Note the use of `page-break-inside` which is a different property focused on preventing breaks *within* an element.
