...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['fastapi', 'testing', 'mocking', 'database', 'pytest', 'python', 'asyncio', 'dependency injection']
draft: false
...
*   **`test_db` fixture:** This fixture creates an in-memory SQLite database. It sets up the database schema using `Base.metadata.create_all(bind=engine)` and provides a session (`test_db`) that can be used for testing.  Crucially, it *cleans up* after itself thanks to `yield`
*   **`override_db` fixture:** This fixture is the key to dependency injection. It overrides the `get_db` dependency in your FastAPI app *only during testing*. It replaces the "real" database session generator with one that provides the in-memory `test_db`. After the test is complete, it resets the dependency overrides to avoid affecting other tests.
