...
Peer dependencies are a way for a package to declare that it requires a specific version (or range of versions) of another package to be installed in the *parent project* that uses it.  They express compatibility expectations, not hard dependencies that the package will install itself.

Think of it like this: imagine you're building a React component library that uses a specific version of React. You don't want to bundle React *inside* your library because the parent project already *has* a React version installed. Instead, you want to ensure that your library is compatible with the existing React version in the parent project.  That's where peer dependencies come in.
...
*   **Solution:**
    ...
    *   **Upgrade the existing dependency:** If possible, upgrade the existing dependency to a version that satisfies the requirements of both your project and the package with the peer dependency.
        ...
        ```
    *   **Downgrade the package:** If upgrading isn't feasible, you might need to downgrade the package with the peer dependency to a version that's compatible with your existing dependencies. This is often less desirable, but it might be necessary in some cases.  *Make sure to carefully test this*.
...
const MyComponent = ({ name }) => {
  return (
    <div>
      Hello, {name}! This is MyComponent.
    </div>
  );
};
