...
date: "2024-01-01"
lastmod: "2024-10-27"
tags: ["expo", "ios", "production build", "react native", "mobile development", "app store", "apple", "fastlane", "eas"]
draft: false
...
*   **`ios.buildNumber`:**  A monotonically increasing build number for each submission to the App Store.  You *must* increment this for *every* build you upload. Starts with 1.
*   **`ios.supportsTablet`:**  Boolean indicating whether your app supports tablets.
*   **`ios.infoPlist.NSPhotoLibraryUsageDescription`:**  If your app accesses the photo library, you *must* provide a description of why you need access.
*   **`ios.infoPlist.NSCameraUsageDescription`:** If your app accesses the camera, you *must* provide a description of why you need access.
*   **`ios.infoPlist.NSLocationWhenInUseUsageDescription`:** If your app accesses the user's location, you *must* provide a description of why you need access.
...
*   **Uniqueness:**  The `bundleIdentifier` *must* be globally unique.  Choose a value that is unlikely to be used by other developers.
