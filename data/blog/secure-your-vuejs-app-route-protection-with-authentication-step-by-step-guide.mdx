...
3.  **`router.beforeEach`:** This is the core of the route guard.
    ...
    *   If authentication is required and the user is *not* authenticated (`!isAuthenticated()`), it redirects to the home page (`/`) using `next({ path: '/' })`.  The `query: { redirect: to.fullPath }` allows you to store the originally requested route in the query string, so you can redirect the user back to that route after they log in.
    *   If authentication is required and the user *is* authenticated, it calls `next()` to proceed to the requested route.
    *   If the route does *not* require authentication, it also calls `next()` to proceed. **It's crucial to always call `next()` in your `beforeEach` guard to avoid blocking navigation.**
...
6.  **Login Component (Updated):**  Update your login component to make an API call to your backend to authenticate the user and store the JWT.
    ...
    export default {
      ...
      methods: {
        async login() {
          try {
            const response = await axios.post('/login', { // Replace '/login' with your backend login endpoint
    ...
    router.beforeEach((to, from, next) => {
      if (to.matched.some(record => record.meta.requiresAuth)) {
        ...
        if (to.matched.some(record => record.meta.roles)) {
          ...
          if (!requiredRoles.every(role => hasRole(role))) { // Check if the user has ALL required roles
        ...
        login ({ commit }, credentials) {
          return axios.post('/login', credentials)
            .then(response => {
              ...
              return axios.get('/user')  // Replace with your API endpoint to fetch user info
                  .then(userResponse => {
