...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['SQL', 'Database', 'Top Records', 'Query Optimization', 'LIMIT', 'ROW_NUMBER()', 'RANK()', 'DENSE_RANK()']
draft: false
...
*   Doesn't handle ties in a way that includes *all* tied rows.  If the 5th and 6th products have the same sales, only the one that happens to be processed first will be included.
...
*   The outer query filters the results to include rows where `product_rank` is less than or equal to 5.  This will include *all* products that share a rank within the top 5.
