...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react-native', 'collapsible header', 'scroll effects', 'animation', 'react-native-reanimated', 'react-native-gesture-handler', 'ui design', 'mobile development']
draft: false
...
const Content = () => (
    <View style={styles.content}>
        {Array.from({ length: 50 }).map((_, index) => (
            <Text key={index} style={styles.contentText}>Item {index + 1}</Text>
        ))}
    </View>
);
...
const CollapsibleHeader = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Content />
        <View style={{height: SCREEN_HEIGHT}} />  {/* Important! Prevents content from being hidden behind header */}
      </Animated.ScrollView>
    </View>
  );
};
...
To further enhance the effect, you can animate the content *within* the header.  Let's add an opacity animation to the header text, making it fade out as the header collapses:
