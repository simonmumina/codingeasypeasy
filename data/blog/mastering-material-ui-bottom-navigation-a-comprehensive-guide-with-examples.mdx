...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['material-ui', 'mui', 'react', 'bottom-navigation', 'component-library', 'UI', 'UX', 'frontend-development', 'accessibility']
draft: false
...
export default function AccessibleBottomNavigation() {
  ...
  return (
    ...
    >
      <BottomNavigationAction label="Recents" icon={<RestoreIcon />} aria-label="View recent items" /> {/* Descriptive aria-labels for each action */}
      <BottomNavigationAction label="Favorites" icon={<FavoriteIcon />} aria-label="View your favorite items" />
      <BottomNavigationAction label="Nearby" icon={<LocationOnIcon />} aria-label="Find nearby locations" />
    </BottomNavigation>
  );
}
```
