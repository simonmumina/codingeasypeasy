...
`Object.seal()` is a JavaScript method that *seals* an object.  This means that it prevents new properties from being added to the object and marks all existing properties as non-configurable.  Crucially, unlike `Object.freeze()`, existing properties can still be changed (if they are writable) after the object is sealed.  In essence, `Object.seal()` ensures the *structure* of the object remains fixed, while allowing for modifications of its *values* (under certain conditions).
...
3. **Existing Writable Properties Can Still Be Modified:** This is a key difference from `Object.freeze()`. If a property is writable *before* sealing the object, it remains writable after sealing. You can still change the value of that property.

4. **Existing Non-Writable Properties Cannot Be Modified:**  If a property is already non-writable *before* sealing, it remains non-writable.  You cannot change its value.
...
`Object.seal()` only applies shallowly. If an object contains other objects as properties, those nested objects are *not* automatically sealed.  You need to explicitly seal them as well, if desired.
...
*   **`Object.seal()`:** Prevents adding *and* deleting properties, and marks all existing properties as non-configurable.  Existing writable properties *can* still be modified.
*   **`Object.freeze()`:** Prevents adding, deleting, *and* modifying properties. It makes the object effectively immutable.
...
*   **Careful with Writable Properties:**  Be mindful that writable properties can still be changed after sealing. If you want to prevent any modifications, consider using `Object.freeze()` or defining the properties as non-writable *before* sealing.
