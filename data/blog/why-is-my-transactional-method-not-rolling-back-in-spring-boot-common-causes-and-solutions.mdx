...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['spring-boot', 'transactional', 'rollback', 'java', 'data-persistence', 'database', 'exception-handling']
draft: false
...
In this example, if `data` is "error," an `IllegalArgumentException` is thrown, but it's caught and the transaction *will not* be rolled back.  The `myRepository.save(new MyEntity(data))` will be committed.
...
In this case, the `IOException` will *not* cause a rollback by default.
