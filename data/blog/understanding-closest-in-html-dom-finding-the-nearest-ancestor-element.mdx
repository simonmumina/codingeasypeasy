...
*   **Searches Upwards:**  `closest()` only traverses *upwards* in the DOM tree, from the element it's called on towards the root `<html>` element.  It never searches downwards or sideways.
*   **Returns the Element:**  If a matching ancestor is found, `closest()` returns the *element itself*, not just a boolean indicating its existence.
...
*   **Starts with the Element Itself (Potentially):** The search *includes* the element on which `closest()` is called *if* the element itself matches the selector.
