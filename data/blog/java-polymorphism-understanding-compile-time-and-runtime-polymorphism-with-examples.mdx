...
date: '2024-08-21'
lastmod: '2024-08-21'
tags: ['java', 'polymorphism', 'object-oriented programming', 'compile-time polymorphism', 'runtime polymorphism', 'method overloading', 'method overriding']
draft: false
...
In this example, we have a base class `Animal` with a `makeSound()` method. The `Dog` and `Cat` classes inherit from `Animal` and *override* the `makeSound()` method to provide their own specific implementations.

The crucial part is when we create an array of `Animal` objects and assign `Dog` and `Cat` objects to it.  When we iterate through the array and call `a.makeSound()`, the JVM determines at *runtime* which `makeSound()` method to call based on the actual object type.  This is why we get "Woof!" and "Meow!" even though the variable `a` is declared as an `Animal`.  This is runtime polymorphism in action.
...
|--------------------|------------------------------------------------------|-------------------------------------------------------|
| **Definition**     | Multiple methods with the same name in the *same* class | A method in a subclass redefining a method in its superclass |
