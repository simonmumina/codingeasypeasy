...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'dom', 'getElementById', 'html', 'document object model', 'web development', 'web programming', 'front-end', 'element selection']
draft: false
...
*   **Uniqueness of IDs:** The `id` attribute *must* be unique within the entire HTML document.  If multiple elements share the same ID, `getElementById()` will only return the *first* matching element, leading to unexpected behavior.  This is a critical point!
...
*   **Context:** `getElementById` always searches the *entire* document. If you want to limit your search to a specific element's descendants, use `querySelector` or other DOM traversal methods *after* first selecting the parent element.
