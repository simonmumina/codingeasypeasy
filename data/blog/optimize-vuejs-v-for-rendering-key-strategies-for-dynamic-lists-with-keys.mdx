...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['vuejs', 'v-for', 'performance', 'optimization', 'frontend', 'dynamic lists', 'key attribute']
draft: false
...
The `key` attribute is *the most important* aspect of optimizing `v-for` rendering.  It provides Vue.js with a unique identifier for each item in the list.  This allows Vue to efficiently track and update elements when the list changes.
...
*   **Avoid Using Indexes:**  *Do not use the index of the array as the `key`* unless your list is truly static and never changes. Using the index as the `key` can lead to severe performance problems and incorrect rendering when items are added, removed, or reordered in the list.
