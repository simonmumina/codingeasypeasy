...
`@redux-saga/delay-p` is a helper function that returns a *Promise* which resolves after a specified delay. It's designed to be used within Redux Saga generators (sagas) to pause execution for a given duration.  It provides a clean and declarative way to introduce delays, making your sagas easier to understand, reason about, and test.
...
*   **`takeLatest` Effect:**  The `takeLatest` effect ensures that only the *latest* `SEARCH_INPUT_CHANGED` action is processed.  If a new `SEARCH_INPUT_CHANGED` action is dispatched while the previous one is still waiting for the delay, the previous one will be cancelled.
