...
7.  `const rowSpan = Math.ceil((itemHeight + gridGap) / (gridAutoRows + gridGap));`: Calculates the number of rows the item should span. This formula ensures the item occupies enough rows to accommodate its content, taking into account the grid gap. The `Math.ceil` function rounds the result up to the nearest integer.
8.  `item.style.gridRowEnd = `span ${rowSpan}`;`: Sets the `grid-row-end` property of the item to the calculated span value.
