...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['redux', 'javascript', 'state management', 'frontend development', 'actions', 'reducers', 'store']
draft: false
...
Actions are plain JavaScript objects that represent an intention to change the application's state. Think of them as messengers, carrying information about *what* happened in the application. They are the *only* way to trigger a state change in Redux.
...
4.  **`case ADD_TODO:` and `case TOGGLE_TODO:`:**  Each `case` handles a specific action type.  Inside each case, the reducer creates and returns a *new* state object.
...
*   **Immutability is Crucial:**  Reducers *must not* mutate the existing state. Always create a new state object.
