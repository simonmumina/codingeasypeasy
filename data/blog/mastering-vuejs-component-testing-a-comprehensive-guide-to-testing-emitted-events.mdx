...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['vuejs', 'vue', 'testing', 'unit testing', 'component testing', 'event testing', 'vue test utils', 'jest', 'vitest']
draft: false
...
*   **Test-Driven Development (TDD):**  Consider writing your tests *before* you write the component code. This can help you design your components in a testable and maintainable way.
...
*   **Testing Events Triggered by Child Components (Event Bubbling):** When a child component emits an event, and the parent component listens for that event on the child element, you can test this by triggering the event on the child and verifying that the parent's event handler is called.
    ...
    it('parent receives event from child', async () => {
      const wrapper = mount(ParentComponent);
      await wrapper.findComponent({ name: 'ChildComponent' }).find('button').trigger('click');
      expect(wrapper.vm.childEventData).toBe('Data from child');
    });
    ```
