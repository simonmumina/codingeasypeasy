...
Think of it like this: instead of a component directly creating a `HttpClient` instance to fetch data, it *asks* Angular to provide an instance.  Angular then either creates a new instance of `HttpClient` (if one doesn't exist) or provides an existing instance.
...
*   The module provides both `Logger` and an alias:  `{ provide: ILogger, useExisting: Logger }`.  This tells Angular that whenever the `ILogger` interface is requested, it should provide the *existing* `Logger` instance.
