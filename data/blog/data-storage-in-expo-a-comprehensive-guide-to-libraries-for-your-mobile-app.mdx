...
date: '2024-01-26'
lastmod: '2024-10-27'
tags: ['expo', 'react-native', 'data storage', 'asyncstorage', 'sqlite', 'realm', 'redux-persist', 'zustand-persist', 'mobile development']
draft: false
...
const MyComponent = () => {
  ...
  return (
    <View>
      {tasks.map(task => (
        <Text key={task._id}>{task.name} - {task.status || 'Not started'}</Text>
      ))}
    </View>
  );
};
