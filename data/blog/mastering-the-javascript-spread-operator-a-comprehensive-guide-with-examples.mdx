...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'spread operator', 'es6', 'tutorial', 'programming', 'arrays', 'objects', 'immutable', 'concat', 'clone']
draft: false
...
The spread operator creates a *shallow* copy. This means that if your object contains nested objects, the inner objects are still referenced by the original and the copy.  Modifying a nested object in the copy *will* affect the original.
...
To create a *deep* copy (where nested objects are also copied), you'll need to use techniques like `JSON.parse(JSON.stringify(object))` (which has limitations) or a dedicated library like Lodash's `_.cloneDeep`.  Be aware that `JSON.parse(JSON.stringify(object))` will not copy functions or properties with a value of `undefined`.
...
While not *exactly* the spread operator, rest parameters use a similar syntax (`...`) and are often used in conjunction with it. Rest parameters allow a function to accept an indefinite number of arguments as an array:
...
In this example, `a` and `b` receive the first two arguments, and the remaining arguments are collected into the `args` array.  Rest parameters *must* be the last parameter in the function definition.
...
*   **Forgetting Immutability:**  Remember that the spread operator creates a *shallow* copy.  Be mindful of nested objects.
