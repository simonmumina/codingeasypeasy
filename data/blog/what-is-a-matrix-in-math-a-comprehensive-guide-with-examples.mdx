...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['matrix', 'matrices', 'linear algebra', 'mathematics', 'math', 'vectors', 'scalars', 'linear transformations']
draft: false
...
Matrices are typically denoted by uppercase letters (e.g., A, B, C).  Individual entries within the matrix are denoted by lowercase letters with subscripts indicating their row and column position.  For example, `a<sub>ij</sub>` represents the element in the *i*-th row and *j*-th column of matrix A.

## Types of Matrices

There are several special types of matrices, each with unique properties and applications:

*   **Square Matrix:** A matrix with an equal number of rows and columns (m = n). The *order* of the square matrix is n.
...
*   **Diagonal Matrix:** A square matrix where all entries *off* the main diagonal are zero.
...
*   **Matrix Multiplication:** The product of two matrices A (m x n) and B (p x q) is defined only if the number of columns in A is equal to the number of rows in B (n = p).  The resulting matrix C will have dimensions m x q.
    ...
    The entry `c<sub>ij</sub>` in the resulting matrix C is calculated as the dot product of the *i*-th row of A and the *j*-th column of B.
    ...
    **Important Note:** Matrix multiplication is **not** commutative, meaning A * B is generally not equal to B * A.

*   **Transpose:** The transpose of a matrix A, denoted as A<sup>T</sup>, is obtained by interchanging its rows and columns. If A is an *m x n* matrix, then A<sup>T</sup> is an *n x m* matrix.
...
*   **Inverse:** The inverse of a square matrix A, denoted as A<sup>-1</sup>, is a matrix such that A * A<sup>-1</sup> = A<sup>-1</sup> * A = I (the identity matrix).  A matrix is invertible (or non-singular) if and only if its determinant is non-zero.
