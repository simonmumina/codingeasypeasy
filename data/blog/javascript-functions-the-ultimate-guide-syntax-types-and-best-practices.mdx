...
Arrow functions do *not* have their own `this` binding. They inherit the `this` value from the surrounding lexical scope. This can be very useful, especially when working with callbacks.
...
Function declarations are hoisted, which means that you can call a function before it is declared in your code.  However, function expressions are *not* hoisted.
