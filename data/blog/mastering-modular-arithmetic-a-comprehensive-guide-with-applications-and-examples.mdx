...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['modular arithmetic', 'number theory', 'mathematics', 'cryptography', 'programming', 'mathematical concepts', 'math']
draft: false
...
Formally, we say that two integers, `a` and `b`, are *congruent modulo n* if their difference, `a - b`, is divisible by `n`. This is written as:
...
Division in modular arithmetic is a bit trickier. Instead of dividing directly, we multiply by the *modular inverse*. The modular inverse of `a` modulo `n` (denoted as `a⁻¹ mod n`) is a number `x` such that:

`(a * x) ≡ 1 (mod n)`

The modular inverse exists *only if* `a` and `n` are coprime (their greatest common divisor is 1).  We can find the modular inverse using the Extended Euclidean Algorithm.
...
Modular exponentiation is the process of calculating `bᵉ mod n` where `b`, `e`, and `n` are integers.  A naive approach of calculating `bᵉ` first and then taking the modulus is inefficient for large values of `e`.  A much faster method is *exponentiation by squaring* (also known as binary exponentiation).
