...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['tomcat', 'https', 'ssl', 'tls', 'wireshark', 'debugging', 'java', 'keystore', 'pcap', 'apache', 'server']
draft: false
...
This is the recommended method. Set the `SSLKEYLOGFILE` environment variable *before* starting Tomcat. This tells the Java Virtual Machine (JVM) to log the session keys.
...
*   **Linux/macOS:**
    ...
    Modify your Tomcat startup script (e.g., `catalina.sh`) to include this line *before* the `java` command that starts Tomcat.  For example:
...
*   **Tomcat Restart:**  Make sure you restarted Tomcat *after* setting the `SSLKEYLOGFILE` environment variable.
...
To enable it, add the following option to the `JAVA_OPTS` environment variable *before* starting Tomcat:
