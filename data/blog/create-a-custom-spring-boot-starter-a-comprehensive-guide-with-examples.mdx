...
date: "2024-01-26"
lastmod: "2024-01-26"
tags: ["spring-boot", "spring-starter", "custom-starter", "java", "dependency-injection", "auto-configuration", "library", "microservices"]
draft: false
...
*   `@ConditionalOnMissingClass`:  Configures a bean only if a specific class is *not* present on the classpath.
*   `@ConditionalOnBean`: Configures a bean only if another bean of a specific type is present in the application context.
*   `@ConditionalOnMissingBean`: Configures a bean only if another bean of a specific type is *not* present in the application context.
