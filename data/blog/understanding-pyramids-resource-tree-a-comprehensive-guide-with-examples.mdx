...
5.  **Routing:** We are using `config.add_route` for defining routes, which maps URLs to names, but importantly it doesn't tell Pyramid how to find the *resource*. That's the job of the resource tree.  While you *can* explicitly bind a view to a route *and* a resource, Pyramid's traversal allows you to access resources even without predefined routes.  When a request comes in, Pyramid traverses the resource tree based on the URL path to find the appropriate resource. For example, if you request `/users/john`, Pyramid will:
