...
export default function App() {
  const handleCustomVibration = () => {
    if (Platform.OS === 'android') {
      ...
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
      console.log("Custom vibration patterns are only supported on Android.  Using fallback on iOS.")
    }
  };
...
*   **Accessibility:** Always consider users with disabilities. Provide alternative ways to access information if haptic feedback is crucial.  Don't rely *solely* on haptics as a primary method of communication.
