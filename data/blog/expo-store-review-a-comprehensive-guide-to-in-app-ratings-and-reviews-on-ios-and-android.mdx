...
date: '2024-08-25'
lastmod: '2024-10-27'
tags: ['expo', 'react-native', 'store review', 'in-app rating', 'ios', 'android', 'mobile development', 'app ratings']
draft: false
...
export default function App() {
  ...
  useEffect(() => {
    async function incrementAppOpenCount() {
      if (!reviewRequested) { // Don't increment if we already requested
  ...
  const requestReview = async () => {
    try {
      ...
      Alert.alert(
        "Thank You!",
        "Thanks for using our app! Your feedback is appreciated.",
        [
          { text: "OK", onPress: () => console.log("OK Pressed") }
        ]
      );
...
*   **`setReviewRequested(true)` after successful request:**  Crucially, the `setReviewRequested(true)` is called after the *attempt* to request a review is made (regardless of whether the OS shows the prompt or not), preventing repeated prompts.
