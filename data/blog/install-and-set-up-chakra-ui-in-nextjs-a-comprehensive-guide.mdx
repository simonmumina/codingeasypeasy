...
class MyDocument extends Document {
      ...
      ctx.renderPage = () =>
        originalRenderPage({
          enhanceApp: (App) => (props) =>
            sheet.collectStyles(<App {...props} />), // Collect styles for styled-components
        });
  ...
  render() {
    return (
      <Html>
        <Head>
           {/* Add any required CSS links here if needed */}
        </Head>
        <body>
          <Main />
          <NextScript />
...
This example includes support for `styled-components` as well, as it's a common pairing with Next.js.  If you're *only* using Chakra UI/Emotion, the  `ServerStyleSheet` and associated code can be removed. Consult the Emotion documentation for the most up-to-date recommendations on SSR if you encounter issues.
