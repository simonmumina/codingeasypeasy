...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'window location', 'host', 'hostname', 'port', 'url manipulation', 'web development', 'browser api']
draft: false
...
The `host` property returns the hostname and port number of a URL. It includes the port number *only* if it's explicitly specified in the URL.
...
The `hostname` property returns only the hostname portion of the URL, *without* the port number.
...
*   **Security:** Be cautious when using `window.location` properties for security purposes.  While these properties can be used to check the current domain, they should not be the *only* line of defense against XSS or other attacks. Always sanitize user input and use proper security headers on your server.
...
*   **URL Manipulation:**  While you can read from `window.location`, you can also *change* the URL using properties like `window.location.href` or `window.location.assign()`.  Be mindful of how you modify the URL, as it can affect the user's experience and SEO.
