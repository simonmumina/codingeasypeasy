...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['fastapi', 'azure', 'deployment', 'app service', 'azure functions', 'containers', 'docker', 'ci/cd', 'python', 'cloud']
draft: false
...
3.  **Create an App Service Plan:**  An App Service Plan defines the resources allocated to your app.
    ...
    ```
    *Note:*  `B1` is a Basic pricing tier, suitable for testing.  Consider a higher tier for production. `--is-linux` is important because we're deploying a Python application.
...
4.  **Create the App Service:**
    ...
    ```
    *Replace `myFastAPIApp` with a unique name for your app.*  Ensure the Python version matches your local environment.
...
2. **Configure Startup Command (Important!)**:  We need to tell App Service how to *start* the application. Go to the Azure portal -> Your App Service -> Configuration -> General Settings
...
2.  **Create a Function App:**
    ...
    ```
    *Replace `myFastAPIFunctionApp` with a unique name.* Ensure the Python version matches your local environment.
...
2.  **Login to Azure Container Registry (ACR):**  You'll need an ACR to store your image. If you don't have one, create one:
    ...
    ```
    *Replace `myACRName` with a unique name for your ACR.*
