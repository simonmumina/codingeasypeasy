...
date: '2024-02-29'
lastmod: '2024-10-27'
tags: ['HTML', 'DOM', 'JavaScript', 'Document Object Model', 'Web Development', 'Browser API', 'DOM Manipulation', 'document object']
draft: false
...
*   **`document.getElementById(id)`**: Returns the element with the specified `id` attribute.  This is the *fastest* and most specific way to find a single element.
...
*   **`document.querySelector(selector)`**: Returns the *first* element that matches a specified CSS selector.  Offers the flexibility of CSS selectors, but can be slower than `getElementById`.
...
*   **`document.querySelectorAll(selector)`**: Returns a `NodeList` of *all* elements that match a specified CSS selector. Similar to `getElementsByClassName` but uses CSS selectors.
...
*   **`HTMLCollection`**:  Is *live*. This means that if the DOM changes after you retrieve the collection, the `HTMLCollection` will automatically update to reflect those changes.

*   **`NodeList`**:  Can be *static* (like the one returned by `querySelectorAll()`) or *live*.  A static `NodeList` will *not* update if the DOM changes.  A live `NodeList` *will* update. It's generally safer to assume `querySelectorAll()` returns a static `NodeList` unless specified otherwise by the browser implementation.
