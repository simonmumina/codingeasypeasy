...
```html
<div id="myElement" class="animated-element">
  This element will scale up and then fade out.
</div>
```
...
myElement.addEventListener('animationend', function(event) {
  if (event.animationName === 'scaleUp') { // Important: Check the animation name
...
```html
<div id="myElement2" class="animated-element2">
...
*   **Consider `animationiteration` Event:**  If you need to perform actions after each iteration of an animation (e.g., for repeating animations), consider using the `animationiteration` event instead of `animationend`.  This fires for each iteration *before* the final iteration and `animationend` event.
