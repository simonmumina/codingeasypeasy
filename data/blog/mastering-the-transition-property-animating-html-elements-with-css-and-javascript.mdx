...
date: '2024-02-29'
lastmod: '2024-03-08'
tags: ['css', 'transition', 'animation', 'html', 'dom', 'javascript', 'style object', 'frontend development']
draft: false
...
<head>
  ...
  <style>
    .box {
      ...
      transition: width 0.5s ease-in-out, height 0.5s ease-in-out; /* Important! Define the transition in CSS */
    }
  </style>
</head>
<body>
...
1.  **CSS Transition:**  The `transition` property is defined in the CSS for the `.box` class.  This is crucial; without it, the style changes will be instantaneous, not animated. This tells the browser *which* properties to animate and *how*.
...
*   **`all` Keyword:**  You can use `transition: all 0.3s ease;` to transition *all* animatable properties. However, this is generally not recommended as it can lead to unexpected behavior if you later add new properties. It's better to be explicit about which properties you want to animate.
