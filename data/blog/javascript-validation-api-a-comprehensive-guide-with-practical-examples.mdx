...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'validation', 'html5', 'form validation', 'constraint validation API', 'web development']
draft: false
...
The JavaScript Validation API is a set of methods and properties exposed by HTML form elements (such as `<input>`, `<textarea>`, and `<select>`) that allows you to validate user input directly in the browser, *without* relying on external libraries. It leverages HTML attributes and JavaScript code to define validation rules and provide feedback to the user.
...
*   **`valid`:** All validation constraints are met (the input is valid).  This is `true` if *none* of the other properties are `true`.
...
*   **`setCustomValidity(message)`:** Sets a custom error message for the input element.  If `message` is an empty string, the input is considered valid (unless another validation constraint is violated). This is *essential* for providing meaningful error messages.
...
```html
<form id="myForm">
  <label for="password">Password (must contain at least one uppercase letter and one number):</label>
...
<script>
  ...
  form.addEventListener('submit', function(event) {
    if (!validatePassword(passwordInput.value)) {
      ...
      passwordInput.setCustomValidity("Password must contain at least one uppercase letter and one number.");
...
6.  **Clearing Custom Error:** If the password is valid, `setCustomValidity("")` is called to clear any previously set custom error messages.  This is crucial; if you *don't* clear the custom validity, the error will persist even if the user corrects the input.
