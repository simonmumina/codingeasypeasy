...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['bootstrap', 'bootstrap 5', 'performance optimization', 'web development', 'bundle size', 'css', 'javascript', 'webpack', 'purgecss', 'tree shaking']
draft: false
...
*   **Side Effects:**  Tell your bundler if your code has side effects (modifications outside the current scope). In your `package.json`, you can add ` "sideEffects": false` if your code doesn't have side effects, allowing the bundler to aggressively remove unused code.  If certain files *do* have side effects, specify them:
...
2.  **Configure PurgeCSS:** You can integrate PurgeCSS into your build process using tools like Webpack, PostCSS, or Gulp.
    ...
    *   `safelist`: A crucial option.  It allows you to specify CSS selectors that should *never* be removed by PurgeCSS. This is important for dynamic classes (added by JavaScript) and components that might not be directly visible in your initial HTML.  The example shows common classes like `show`, `collapsing`, and `fade` (used for transitions), as well as prefixes for tooltips, popovers, and the `bs-` prefix Bootstrap uses for many utility classes.  **Carefully consider what you need to safelist.** If elements don't render correctly after PurgeCSS runs, add their classes to the `safelist`.
...
The most important step is to audit your website's performance *before* and *after* implementing these optimization techniques. Use tools like Google PageSpeed Insights, WebPageTest, or Lighthouse (built into Chrome DevTools) to measure your website's performance metrics, such as:
