...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['next.js', 'context API', 'state management', 'react', 'javascript', 'frontend development', 'component communication']
draft: false
...
```javascript filename="contexts/ThemeContext.js"
...
2.  **`ThemeProvider` Component:**
    ...
    *   The `value` prop of the `ThemeContext.Provider` is where you pass the data you want to share through the context.  In this case, it's an object containing the current `theme` and the `toggleTheme` function.  Passing the function allows child components to *mutate* the context value.
...
3.  **`useTheme` Hook:**
    ...
    *   This makes it easy to use the theme data in any component that needs it.  This is the *preferred* way to consume context values in modern React.
...
```javascript filename="components/ThemeToggle.js"
...
```javascript filename="components/MyComponent.js"
import { useTheme } from '../contexts/ThemeContext';
...
```javascript filename="contexts/AuthContext.js"
