...
In this example, the catch-all middleware (the one that sends a 404) is defined *before* the `/api/users` route. Therefore, any request that doesn't match a previous route will be caught by this middleware, resulting in a 404 even if the `/api/users` route exists.
...
*   **Authentication Middleware:** If a route requires authentication, ensure the authentication middleware is correctly implemented and placed *before* the route handler.
