...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['JavaScript', 'InputEvent', 'DOM', 'Web Development', 'Event Handling', 'data Property', 'HTML', 'Accessibility']
draft: false
...
*   **Deletion:** If the user deletes text using backspace, delete, or cut operations, `data` will contain the string that was deleted. *Crucially, if multiple characters are deleted with a single action (e.g., selecting multiple words and pressing delete), `data` will contain all the deleted characters.*
...
2.  Inside the `handleInput` function, we retrieve the current input value using `event.target.value`. It's crucial to use `event.target.value` rather than `event.data` here.  `event.data` only represents the *change*, not the entire value.
3.  We check if the length of the *entire* input value (`event.target.value`) is within the `maxLength`.
...
3.  We use a regular expression (`^[a-zA-Z\s]*$`) to check if the *entire* `event.target.value` contains only letters and spaces.
...
*   **`event.target.value` vs. `event.data`:**  As demonstrated in the examples, it is crucial to understand the difference between `event.target.value` and `event.data`. `event.target.value` gives you the *current state* of the input field, while `event.data` provides information about the *specific change* that triggered the event. Use the appropriate property based on your needs.
