...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['node.js', 'pagination', 'database', 'performance', 'api', 'sorting', 'ids', 'javascript', 'backend']
draft: false
...
**2.  Create an Express server:**
   ...
   async function seedProducts() {
       if(await Product.countDocuments() === 0){
           const products = [];
           for (let i = 1; i <= 55; i++) {
               products.push({name: `Product ${i}`, description: `Description for Product ${i}`, price: Math.floor(Math.random() * 100)})
           }
   ...
   app.get('/products', async (req, res) => {
     ...
     try {
       ...
       const nextPageProducts = await Product.find({_id: {$gt: lastProductId}}).limit(1).exec();
...
*   **`GET /products` Endpoint:**
    ...
    *   **Query Construction:**  Constructs a Mongoose query to find products where the `_id` is greater than the `starting_after` value.  This ensures that we only retrieve records that come *after* the cursor.
    *   **Sorting:** The `.sort({ _id: 1 })` is *critical*.  We always sort by the id field in ascending order to ensure consistent results across pages.  Without sorting, the database might return records in an unpredictable order, leading to issues.
...
*   **Response:** The API returns a JSON response containing:
    ...
    *   `next_starting_after`: The id of the *last* item of the current page. This is the new cursor value you need to pass to `starting_after` for the next request.  If `has_next` is `false`, this will be `null`.
...
*   **Alternative Cursor Representations**: Instead of just using the ID, you could concatenate multiple fields that are used in the sorting.  For instance, if you are sorting by `createdAt` *and then* by `ID`, you would return both the `createdAt` and `ID` values in the cursor.
