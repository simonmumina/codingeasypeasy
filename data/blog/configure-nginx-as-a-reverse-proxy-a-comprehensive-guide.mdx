...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['nginx', 'reverse proxy', 'configuration', 'web server', 'proxy server', 'http', 'https', 'security', 'performance']
draft: false
...
3.  **Define the Backend Server:**
    ...
    You can define the backend server directly in the `proxy_pass` directive, but it's often better to use an upstream block.  This allows you to easily configure multiple backend servers for load balancing.  Add the following upstream block to the top of your configuration file, *before* the `server` block:
