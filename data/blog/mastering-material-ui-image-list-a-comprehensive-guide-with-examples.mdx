...
*   **`variant`:**  Changes the visual style of the image list.  Possible values are `"standard"` (default), `"masonry"`, `"woven"`, and `"quilted"`.
    ...
    *   **Quilted:** Creates a grid with some images spanning multiple rows or columns.  You'll define the `rows` and `cols` props *directly* on the `ImageListItem`.
