...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'html', 'dom', 'events', 'onmouseleave', 'mouse leave', 'web development', 'front-end development']
draft: false
...
*   **`onmouseleave`:** This event is triggered only when the mouse pointer *actually* leaves the element.  It doesn't fire when the mouse moves from the parent element to a child element within the parent.

*   **`onmouseout`:** This event is triggered when the mouse pointer leaves the element *or* enters one of its child elements. This is called **event bubbling**.
...
In this scenario, when the mouse moves from the `parent` div to the `child` div, `onmouseout` will be triggered on the parent, but `onmouseleave` will *not*. `onmouseleave` will only be triggered when the mouse completely leaves the `parent` div, moving outside its borders.
