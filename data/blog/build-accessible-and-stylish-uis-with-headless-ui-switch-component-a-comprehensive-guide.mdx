...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['headlessui', 'react', 'accessibility', 'tailwind', 'switch component', 'ui components', 'headless components', 'aria attributes']
draft: false
...
Headless UI is a library of accessible and unstyled UI components for React and Vue.  The "headless" aspect means these components provide the underlying functionality and accessibility best practices *without* prescribing any specific visual style. This gives you unparalleled control over the look and feel of your UI.  This approach allows you to bring your own styling solution – whether it's Tailwind CSS, styled-components, CSS Modules, or vanilla CSS – ensuring your components seamlessly integrate with your existing design system.
...
*   **`checked` and `onChange` props:** The `checked` prop binds the state (`enabled`) to the switch's state.  The `onChange` prop calls the `setEnabled` function to update the state when the user toggles the switch.  This is what makes it a *controlled* component.
  ...
  return (
    <div className="flex items-center">
      <Switch
        ...
        } relative inline-flex h-6 w-11 items-center rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2`}
      >
        <span className="sr-only">Enable notifications</span>
        <span
          className={`${
...
While less typical for the Headless UI Switch, you *could* technically simulate an uncontrolled behavior by using a ref. However, it goes against the core principle of Headless UI, which is providing accessible and predictable behavior through controlled components.  It is not recommended.
