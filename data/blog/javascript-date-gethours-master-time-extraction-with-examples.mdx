...
It's important to remember that the `getHours()` method returns the hour based on the *local* time zone of the system where the code is running. If you need to work with UTC (Coordinated Universal Time), you should use the `getUTCHours()` method instead.
