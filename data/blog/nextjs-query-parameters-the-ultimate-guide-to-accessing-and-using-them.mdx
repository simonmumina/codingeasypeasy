...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['nextjs', 'query parameters', 'url parameters', 'dynamic routes', 'getserversideprops', 'getstaticprops', 'useRouter', 'client-side rendering', 'server-side rendering', 'static site generation', 'next js']
draft: false
...
`getStaticProps` allows you to fetch data and pre-render pages at build time. This is ideal for content that doesn't change frequently.  Accessing query parameters in `getStaticProps` is slightly different because you're dealing with a static generation context.  However, `getStaticProps` is typically used when you *know* the query parameter values at build time via dynamic routes.
...
async function fetchPost(slug) {
  ...
  const mockPosts = [
    { slug: 'hello-world', title: 'Hello World!', content: 'This is the content of the hello-world post.' },
...
*   **`getStaticPaths()`:**  This function is *required* for dynamic routes when using `getStaticProps`. It defines which paths should be pre-rendered at build time.  It returns an array of objects, where each object defines a `params` object containing the `slug` for that specific post.
