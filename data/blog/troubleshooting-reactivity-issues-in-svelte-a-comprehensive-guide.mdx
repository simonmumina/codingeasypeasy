...
One of the most common sources of reactivity frustration is directly mutating objects or arrays. Svelte's reactivity system is triggered by *assignments*, not mutations.
...
In this scenario, clicking the button changes the `name` property of the first item in the `items` array.  However, the UI doesn't update because Svelte isn't aware that `items` has changed.  We directly mutated the object *within* the array, not the array itself.
...
*   **Spread Operator:** Create a new array using the spread operator, making changes in the new array and reassigning it to `items`. This is often the cleanest approach.
    ...
    By using `.map` and the spread operator (`{ ...item, name: newName }`), we create a *new* array with the updated item, triggering reactivity when `items` is reassigned.
...
*   **Using `.slice()`:**  While less common, creating a shallow copy using `.slice()` can also work if you're only concerned with top-level array changes (and not mutations within the objects themselves).
    ...
    **Important Note:** If you are mutating nested properties within the objects *inside* the array, you still need to apply the spread operator technique to the objects themselves. `.slice()` will only trigger reactivity if the array *reference* changes, not if the contents *within* the array change.
...
In this example, `sum` is only calculated *once* during the initial component render.  The `$: {}` block is not automatically re-run when the component mounts. It only runs after `x` or `y` are *changed* through the button clicks.  You might expect the initial value of `sum` (3) to be displayed, but it will be `undefined` or whatever the default value of `sum` is (if you had initialised it).
...
In this case, clicking the "Change Name in Child" button will *not* update the `name` in the parent component. The `bind:name={name}` only works one way (from parent to child initially). The child is directly mutating its own `name` prop, which doesn't trigger any reactivity in the parent.
...
While this *usually* works, there are situations, especially with more complex components or certain browser optimizations, where the UI might not update immediately after `data` is assigned within the `fetchData` function.  This is because the update might happen outside of Svelte's component lifecycle.
...
If you don't provide a key or provide a key that isn't unique or stable, Svelte might re-render the *entire* list on every update, leading to performance issues or unexpected behavior.
