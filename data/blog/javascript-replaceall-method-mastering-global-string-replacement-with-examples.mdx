...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'string manipulation', 'replaceAll', 'string methods', 'web development', 'programming']
draft: false
...
In this example, all instances of the word "test" in the original string are replaced with "example". Note that `replaceAll()` returns a *new* string and doesn't modify the original string.  String immutability is a key concept in JavaScript.
...
If you attempt to use a regular expression *without* the global flag `/g`, you will get an error:
