...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'closures', 'scope', 'functions', 'interview questions', 'programming', 'web development']
draft: false
...
4.  `myClosure` now holds a reference to `innerFunction`. Critically, it also holds a *closure* over the `outerFunction`'s environment, including the `outerVar` variable.
...
Why does this happen?  Because `var` has function scope (or global scope outside of functions).  By the time the `setTimeout` callbacks are executed, the loop has already finished, and `i` has reached a value of 5.  All the callbacks are referencing the *same* `i` variable.
...
Now, each callback closes over a *different* `i` variable, resulting in the expected output.  This is a classic example of how understanding closures helps avoid common JavaScript pitfalls.
