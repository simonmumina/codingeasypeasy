...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['nestjs', 'cli', 'command-line', 'typescript', 'automation', 'developer tools', '@nestjs/cli', 'extensions', 'custom commands']
draft: false
...
*   `@nestjs/schematics`:  Provides utilities for generating code and modifying files within your project.  This is *crucial* for actually automating file changes.
...
2.  **Update `my-custom.command.ts`:** Modify `src/cli/commands/my-custom.command.ts` to use the schematic:
    ...
    export class MyCustomCommand extends CommandRunner {
      ...
      async run(passedParams: string[], options?: BasicCommandOptions): Promise<void> {
        ...
        if (!serviceName) {
            const answers = await this.inquirer.ask<{ serviceName: string }>('service-name-question', undefined);
