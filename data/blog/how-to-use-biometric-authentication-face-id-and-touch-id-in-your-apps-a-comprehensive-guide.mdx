...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['biometric authentication', 'face id', 'touch id', 'ios', 'android', 'react native', 'security', 'mobile development', 'app security', 'authentication']
draft: false
...
*   **Permissions:**  No special permissions are *required* at runtime for the `BiometricPrompt` API.  The user grants the permission implicitly when they enable biometric authentication on their device.
*   **`BiometricManager`:**  Use `BiometricManager` to check the availability and enrollment status of biometric authentication *before* presenting the authentication dialog.
