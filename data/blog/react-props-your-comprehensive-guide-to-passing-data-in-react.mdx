...
function DisplayMessage(props) {
  return (
    props.isVisible ? <p>This message is visible!</p> : <p>This message is hidden.</p>
  );
}
...
It's crucial to remember that props are **read-only** from the perspective of the child component.  A child component should *never* attempt to modify the props it receives. Doing so violates the unidirectional data flow principle of React and can lead to unpredictable behavior and bugs.
...
In short, use props to pass data *down* the component tree and state to manage data that changes *within* a component.
