...
- `form.process(onvalidation=lambda form: rename_file(form.vars.file_data)).accepted`: The `onvalidation` argument allows you to specify a function to be called _before_ the form is accepted but _after_ the form has been submitted. In this case we pass the original filename to rename_file, get a new filename, and Web2py stores the _new_ filename.
- `db(db.renamed_files.id == form.vars.id).update(file_name = request.vars.file_data)`: This line uses the ID of the record that has just been created to update the file_name field with original filename. This is very important for displaying and referencing the file properly later. The `request.vars.file_data` contains the _original_ file name provided by the user.
