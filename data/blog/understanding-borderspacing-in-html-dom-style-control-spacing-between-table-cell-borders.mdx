...
*   **Empty String:** If the `borderSpacing` is *not* explicitly set in the CSS, the `myTable.style.borderSpacing` will return an empty string (`""`). This is because the `style` object only reflects inline styles (styles applied directly to the element using the `style` attribute). If the style is defined in an external stylesheet or a `<style>` tag, you'll need to use `window.getComputedStyle` to retrieve the actual computed value.
...
*   **String Values Required:** You *must* set the `borderSpacing` value as a string, including the unit of measurement (e.g., "px", "em", "rem").  Omitting the unit will usually result in the style being ignored by the browser.
...
*   **`border-collapse: collapse;`:** Remember that `borderSpacing` has *no effect* if `border-collapse` is set to `collapse` (or is not explicitly set, as `collapse` is the default).  Always ensure that `border-collapse: separate;` is applied to the table element.
