...
date: '2024-01-01'
lastmod: '2024-10-27'
tags: ['expo', 'react-native', 'expo-router', 'file-based routing', 'navigation', 'mobile development', 'javascript']
draft: false
...
export default function AuthLayout() {
  return (
    <AuthProvider>
      <Stack screenOptions={{ headerShown: false }} /> {/* Example: Hide headers for auth screens */}
    </AuthProvider>
  );
}
```
...
*   The routes inside the `(auth)` directory will be `/login` and `/register`, *not* `/(auth)/login` and `/(auth)/register`.  The parentheses are ignored for routing purposes.
