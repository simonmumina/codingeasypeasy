...
The `<Routes>` component is a critical part of React Router v6. It uses a *best-match* algorithm to determine which `<Route>` component to render.  Only *one* `<Route>` will be rendered at a time, even if multiple paths technically match the current URL. The `<Route>` with the most specific matching `path` will be chosen.
...
*   The `Route` components for `Profile` and `Settings` are nested within the `Route` component for `Dashboard`. This indicates that they are children of the `Dashboard` route.  Notice that the paths are *relative* to the parent route (i.e. `/dashboard`).  So the full path to profile is `/dashboard/profile`.
