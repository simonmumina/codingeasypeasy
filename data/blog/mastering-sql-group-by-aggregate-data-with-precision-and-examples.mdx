...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['SQL', 'GROUP BY', 'Database', 'Aggregation', 'Data Analysis', 'SQL Tutorial', 'Data Aggregation']
draft: false
...
*   **`WHERE condition`**: (Optional) Filters the rows *before* the grouping is applied.
...
The `HAVING` clause is used to filter the results of a `GROUP BY` query *after* the grouping has been performed.  It's similar to the `WHERE` clause, but `WHERE` filters rows *before* grouping, while `HAVING` filters groups *after* grouping.  You can only use `HAVING` with `GROUP BY`.
...
Important note: If the intention was to only show customers that had *more* than one laptop order, and there wasn't data for that, the `HAVING` clause would need to reflect that: `HAVING COUNT(*) > 1`.
