...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['git', 'git alias', 'version control', 'developer tools', 'productivity', 'command line', 'terminal']
draft: false
...
*   **`git squash`:** Squash the last *n* commits into one.

    ```bash
    git config --global alias.squash '!f() { git reset --soft HEAD~$1 && git commit -m "$(git log -n $1 --pretty=format:%B | head -n 1)"; }; f'
    ```

    *Note: this is a more complex example using a shell function. Usage: `git squash 3` will squash the last 3 commits.*
