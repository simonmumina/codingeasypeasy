...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['nodejs', 'zip', 'file compression', 'download', 'expressjs', 'archiver', 'streaming', 'file handling', 'server-side']
draft: false
...
7.  **Add files and directories to the archive:**
    *   `archive.file(file1Path, { name: 'example1.txt' });` adds a single file to the archive.  The `name` option specifies the name the file will have *within* the ZIP archive.
    *   `archive.directory(directoryPath, 'images');` adds a directory (and its contents recursively) to the archive. The second argument sets the base directory *within* the ZIP.  If you want the contents to be at the root of the ZIP, use `.` as the second argument (but be careful with name collisions).
8.  **Error Handling:** We listen for `'error'` events on the `archive` object to catch any errors during the archiving process and send an error response to the client.  Proper error handling is *critical* in real-world applications.
9.  **Finalize the archive:** `archive.finalize()` signals to the archiver that no more data will be added and that it should finish writing the ZIP file.  This is *essential*.
