...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['expo', 'typescript', 'react-native', 'mobile development', 'javascript', 'tutorial', 'guide', 'coding']
draft: false
...
const MyComponent: React.FC<Props> = ({ greeting }) => {
  return (
    <Text style={localStyles.text}>{greeting}</Text>
  );
};
...
const localStyles = StyleSheet.create({ // Assuming a local stylesheet
    text: {
        fontSize: 20,
        fontWeight: 'bold',
    }
