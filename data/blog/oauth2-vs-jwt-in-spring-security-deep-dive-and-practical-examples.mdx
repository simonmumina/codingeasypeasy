...
OAuth2 (Open Authorization) is an authorization framework that enables a third-party application to obtain limited access to an HTTP service on behalf of a resource owner.  It allows users to grant access to their resources (data, profiles, etc.) hosted on one service to another application *without* sharing their credentials (username and password).
