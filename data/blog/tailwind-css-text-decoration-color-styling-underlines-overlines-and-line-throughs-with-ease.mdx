...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['tailwind css', 'text decoration', 'css styling', 'frontend development', 'underline color', 'overline color', 'line-through color', 'text styling']
draft: false
...
**Important:** The `decoration-{color}` utility class *only* applies the color. You *must* also use one of the `underline`, `overline`, or `line-through` classes to actually display the decoration.  If you only specify `decoration-red-500`, nothing will happen because there's no `text-decoration-line` defined.
...
```jsx
<p className="underline decoration-brand-primary decoration-4 hover:decoration-brand-secondary">
  Styled link.
</p>
```
