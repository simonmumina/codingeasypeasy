...
*   You *must* push the changes from the submodule's repository *before* updating the main repository's pointer.  Otherwise, collaborators will be unable to access the commit hash referenced by the submodule.
...
*   **Submodule changes not reflected:** Ensure you've committed and pushed changes from the submodule's repository *before* updating the main repository.
...
Edit the `.git/config` file in the *main* repository. Look for a section like this and remove it:
