...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['SQLAlchemy', 'Pyramid', 'Database', 'Hanging', 'Session', 'Request Lifecycle', 'Transaction Management', 'ZopeTransactionExtension', 'Debugging', 'Python', 'Web Development']
draft: false
...
*   **`long_running_task.delay(model_id)`:**  This queues the task to be executed asynchronously by Celery.  The main request processing can continue without waiting for the task to complete.  Note that a *separate* session is created within the Celery task as the request's session is not available in the worker.
