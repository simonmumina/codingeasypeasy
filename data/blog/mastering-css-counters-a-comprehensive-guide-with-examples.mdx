...
```css
counter-reset: [ <counter-name> <integer>? ]+ | none | inherit | initial | revert | revert-layer | unset;
```
...
*   `counter-reset` applies to the *element* itself, not its children. If you want a counter to be reset for *each* child, you need to apply the `counter-reset` to the *parent* element.  See nested counters example below.
...
```css
counter-increment: [ <counter-name> <integer>? ]+ | none | inherit | initial | revert | revert-layer | unset;
```
...
*   `counter-increment` applies to the *element* itself.  It doesn't automatically increment counters for its children.
...
In this case, a screen reader wouldn't necessarily understand the "step" elements as being related in a hierarchical way. If you *really* couldn't use headings for some reason (and you should try to!), you *might* consider:
