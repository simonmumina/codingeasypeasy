...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['SQL', 'Subquery', 'Database', 'Correlated Subquery', 'Nested Subquery', 'Performance', 'Query Optimization']
draft: false
...
*   **Evaluated Once:** The subquery is typically evaluated *once* and its result is used by the outer query.
...
1.  **Inner Query:** The subquery `SELECT DISTINCT C.City FROM Customers C JOIN Orders O ON C.CustomerID = O.CustomerID WHERE O.Amount > 150` finds all *distinct* cities where orders exceed $150.  In this case, it would return `'London'` and `'Paris'`.  This subquery can be executed independently.
...
A **correlated subquery**, on the other hand, is a subquery that depends on the outer query for its execution.  It references columns from the outer query.  This means the subquery is executed *repeatedly*, once for each row processed by the outer query. Think of it as a calculation that's done *for each row* of the main query.
...
*   **Evaluated Repeatedly:**  The subquery is evaluated for *each row* in the outer query.
...
Let's find all customers who have an order with an amount greater than the average order amount for *that specific customer*.
