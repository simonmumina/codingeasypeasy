...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['node.js', 'mongodb', 'authentication', 'login', 'security', 'express', 'bcrypt', 'javascript', 'web development', 'mern stack']
draft: false
...
*   **`userSchema.pre('save', ...)`:** This is middleware that runs *before* the user is saved.  We hash the password using `bcrypt` before saving it to the database.  This is essential for security – never store passwords in plain text! We only hash the password if it's been modified (new user or password change).
...
router.post('/register', async (req, res) => {
  try {
    ...
    req.session.save((err) => {
      ...
      res.status(201).json({ message: 'User registered successfully', user: {username: newUser.username, email: newUser.email} });  //Consider returning less data
    });
...
*   **`/register`:**
    ...
    *   Saves the new user to the database. The `pre('save', ...)` middleware in the User model handles password hashing *before* saving.
    *   After successful registration, it *optionally* logs the user in by setting `req.session.userId` and `req.session.username`.  This avoids the user having to log in immediately after registering. **Error handling is crucial after attempting to save the session.**
...
app.use(session({
  ...
  store: MongoStore.create({  // Use connect-mongo for session storage in MongoDB
    mongoUrl: dbURI,
