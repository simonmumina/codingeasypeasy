...
date: '2024-01-27'
lastmod: '2024-01-27'
tags: ['spring boot', 'auto-configuration', 'java', 'configuration', 'exclude', 'enableautoconfiguration', 'spring initializer']
draft: false
...
3.  **Using `@ConditionalOnMissingBean`**
    ...
    While not directly excluding an auto-configuration, `@ConditionalOnMissingBean` provides a powerful mechanism to conditionally disable certain auto-configuration behavior.  If a bean of a specific type is *already* defined, the auto-configuration *won't* kick in.  This allows you to "preempt" the auto-configuration with your own bean definition.
    ...
    *   `@ConditionalOnMissingBean` checks the bean *type*, not the bean *name*.
