...
date: '2024-01-01'
lastmod: '2024-10-27'
tags: ['firebase', 'react-native', 'expo', 'javascript', 'mobile development', 'authentication', 'firestore', 'realtime database', 'cloud functions', 'push notifications']
draft: false
...
1.  **Firebase JS SDK (Web SDK):** This is the official Firebase SDK designed for web applications.  It *can* be used in React Native Expo projects.  However, it might face challenges regarding device-specific features and limitations regarding some native modules.
...
*   **Expo Go Limitations:** React Native Firebase relies on native modules.  Expo Go (the standard Expo development app) *does not* support custom native modules.  You *cannot* use React Native Firebase directly in Expo Go.
...
**Important:** *Carefully follow the React Native Firebase installation instructions for Expo.*  This involves adding plugins to your `app.json` file to configure the native modules correctly. These plugins handle linking native dependencies during the build process.  Refer to the official React Native Firebase documentation for the latest installation instructions. *Failure to configure the plugins correctly will lead to errors.*
...
const FirestoreScreen = () => {
  ...
  useEffect(() => {
    const unsubscribe = firestore().collection('items').onSnapshot(querySnapshot => {
      const itemsArray = [];
...
exports.addNumbers = functions.https.onCall((data, context) => {
  ...
  if (!Number.isInteger(num1) || !Number.isInteger(num2)) {
    throw new functions.https.HttpsError('invalid-argument', 'The function must be called with two numbers "num1" and "num2".');
  }
