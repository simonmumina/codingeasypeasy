...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['Hadoop', 'Cluster Management', 'Data Engineering', 'Big Data', 'Node Addition', 'Hadoop Administration']
draft: false
...
5.  **SSH Configuration:**  Enable SSH and configure passwordless SSH access from the Hadoop master node to the new nodes.  This allows for remote management and configuration. Generate an SSH key on the master node and copy it to the authorized\_keys file on each new node.
...
1.  **Format the New DataNodes (Only if it's a brand new DataNode):**  *DO NOT FORMAT the NameNode.* If you are adding brand new disks to new servers, format the DataNodes. This ensures the directories specified in `hdfs-site.xml` as `dfs.datanode.data.dir` are properly initialized.
    ```bash
