...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['database', 'indexing', 'performance', 'sql', 'mysql', 'postgresql', 'optimization', 'database design', 'query optimization']
draft: false
...
*   **Composite Indexes:** A composite index covers multiple columns. The order of columns in a composite index is crucial.  The most selective (most unique values) column should generally come first.
    *   **Example (MySQL/PostgreSQL):** `CREATE INDEX idx_last_name_first_name ON users (last_name, first_name);`  This index is useful for queries that filter on both `last_name` and `first_name`, or just on `last_name`.  It is *not* useful for queries that only filter on `first_name`.
...
*   **Covering Indexes:** A covering index includes all the columns required to satisfy a query.  The database can retrieve all the data from the index without having to access the table itself, resulting in significant performance gains.  This often requires including more columns in the index than are strictly used in the `WHERE` clause.
    *   **Example (MySQL/PostgreSQL):**
        ...
        ```
        In this example, the index `idx_username_email_created_at` *covers* the query because it includes `username`, `email`, and `created_at`, which are the columns selected and used in the `WHERE` clause.
