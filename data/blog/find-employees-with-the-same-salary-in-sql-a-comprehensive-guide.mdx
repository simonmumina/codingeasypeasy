...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['SQL', 'Database', 'Employee Salary', 'SQL Query', 'SQL Tutorial', 'Data Analysis', 'SQL Server', 'MySQL', 'PostgreSQL']
draft: false
...
*   `WHERE e1.employee_id <> e2.employee_id`: This ensures that we don't compare an employee to themselves. We only want pairs of *different* employees with the same salary.
...
While `DENSE_RANK()` is generally preferred here because it avoids gaps in the ranking, you *could* use `RANK()`.  However, in this specific use case, the difference between the two is negligible since we are only interested in knowing if there *are* duplicates, not the exact rank.
