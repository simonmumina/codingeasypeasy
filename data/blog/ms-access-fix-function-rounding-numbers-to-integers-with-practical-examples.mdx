...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['MS Access', 'Access VBA', 'Fix Function', 'Int Function', 'Rounding Numbers', 'Database Development', 'VBA Tutorial']
draft: false
...
The `Fix()` function removes the fractional part of a number and returns the resulting integer value.  It essentially "truncates" the number, discarding any digits after the decimal point.  Unlike some rounding functions that might round up based on the decimal value, `Fix()` *always* rounds towards zero. This behavior is key to understanding when to use `Fix()` effectively.
