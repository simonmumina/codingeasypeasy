...
In this case, TypeScript *can* infer the return type, but it's better to explicitly define it:
...
*   `age?: Ref<number>`:  The `age` property is marked as optional. This means that a consumer of the composable doesn't have to access `age`.  However, if `age` *is* accessed, it will be of type `Ref<number>`. In the implementation age is initialized to `undefined`.
