...
- **`window.innerWidth` and `window.innerHeight`:** These properties provide the dimensions of the _browser window_ itself, not the entire screen. They are often more relevant for responsive design as they reflect the actual space available to the website. Crucially, these values adjust according to browser zoom levels.
