...
date: '2024-08-23'
lastmod: '2024-08-23'
tags: ['next.js', 'cookies', 'javascript', 'web development', 'authentication', 'session management', 'server-side cookies', 'client-side cookies', 'httpOnly', 'secure cookies']
draft: false
...
export default async function handler(req, res) {
  const logoutCookie = serialize('username', '', { // Set value to empty string
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
