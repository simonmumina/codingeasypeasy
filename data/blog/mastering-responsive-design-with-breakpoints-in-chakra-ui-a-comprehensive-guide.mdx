...
*   **Ems (`em`):**  Ems are relative to the font size of the *parent* element. This can lead to unexpected behavior if font sizes are not consistently defined in the parent hierarchy.
*   **Rems (`rem`):**  Rems are relative to the font size of the *root* element (`<html>`). This provides a more consistent and predictable scaling behavior, making rems the recommended unit for defining breakpoints in most cases.
