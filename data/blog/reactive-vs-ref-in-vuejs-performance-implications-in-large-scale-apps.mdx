...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['vuejs', 'performance', 'reactive', 'ref', 'javascript', 'optimization', 'vue3', 'large scale application']
draft: false
...
*   For objects, `ref()` doesn't automatically make the object deeply reactive like `reactive()`.  If the value inside a ref is an object, only the `.value` property is reactive.  Changes *inside* that object will not trigger updates unless the object is also made reactive using `reactive()`.
...
In this example, changes to `shallowState.address.city` will *not* trigger a reactivity update because `address` is not made reactive by `shallowReactive`. This can be a performance optimization if you know that nested properties will not change or don't need to be reactive.  Updating `deepState.address.city` *will* trigger updates, and `nameRef` is reactive, as expected.
