...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['SQL Server', 'SPACE Function', 'String Functions', 'T-SQL', 'Database Development', 'SQL Programming', 'Data Manipulation', 'SQL Tips']
draft: false
...
This will return a string containing ten blank spaces.  You might not *see* the result in a typical query tool output without some surrounding context, but the spaces are indeed there.
...
The `REPLICATE()` function takes two arguments: the character to repeat and the number of times to repeat it. While semantically similar, `REPLICATE()` is generally more versatile because it can repeat *any* character, not just a space.
