...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['vuejs', 'ssr', 'server-side-rendering', 'async-data', 'data-fetching', 'nuxt', 'vue-server-renderer', 'performance', 'seo']
draft: false
...
If you're using Nuxt.js (a framework built on top of Vue.js), the `asyncData` hook is the recommended approach.  It's specifically designed for asynchronous data fetching on the server *before* the component is rendered.
    ...
    export default {
      ...
      head() {
        return {
          ...
          meta: [
            { hid: 'description', name: 'description', content: this.post ? this.post.content.substring(0, 150) : '' }
          ]
        }
      }
    }
