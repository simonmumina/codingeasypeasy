...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['html', 'dom', 'style', 'textDecorationLine', 'css', 'javascript', 'web development', 'text styling']
draft: false
...
*   CSS is used to remove the default underline from the link and set a blue color.  The `:hover` pseudo-class *also* directly adds an underline on hover, providing a CSS-only solution as well as the JS example.
...
This example shows how to combine different text decorations based on user interaction (though it's not necessarily a *good* UX!)
