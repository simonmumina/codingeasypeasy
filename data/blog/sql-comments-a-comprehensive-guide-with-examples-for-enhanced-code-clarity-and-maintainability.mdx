...
- **Be Concise and Clear:** Comments should be brief and to the point, explaining the _why_ rather than simply restating the _what_.
...
- **Avoid Obvious Comments:** Avoid comments that simply repeat what the code already clearly states. For example, `SELECT * FROM users; -- Select all users` is redundant. Instead, explain _why_ you are selecting all users if it's not immediately apparent.
