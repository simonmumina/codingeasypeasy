...
date: '2024-10-27'
lastmod: '2024-11-20'
tags: ['debugging', 'bus error', 'core dump', 'segmentation fault', 'memory management', 'C++', 'C', 'Linux', 'gdb', 'valgrind', 'code optimization']
draft: false
...
Essentially, the "bus" is the communication channel between the CPU and memory. A bus error signifies that the CPU tried to access memory through this bus in a way the hardware couldn't handle. This is different from a segmentation fault, which indicates an invalid memory access (e.g., accessing memory outside the allocated bounds of an array). While both involve memory issues, a bus error is *specifically* related to misalignment.
...
*   It then *intentionally* misaligns the pointer by adding 1 to its address.  Since integers typically require alignment on 4-byte boundaries, adding 1 makes the address misaligned.
