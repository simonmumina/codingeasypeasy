...
*   **Timers (setInterval, setTimeout):** Failing to clear timers using `clearInterval` or `clearTimeout` can prevent the garbage collector from reclaiming memory.  Timers keep the callback functions and any variables they reference alive.
    ...
    **Important:**  Even with `clearInterval`, if the callback function inside the `setInterval` retains a reference to a large object outside its scope, that object *could* still remain in memory.  The `clearInterval` only stops the timer from firing again, not necessarily cleaning up everything the callback referenced.
