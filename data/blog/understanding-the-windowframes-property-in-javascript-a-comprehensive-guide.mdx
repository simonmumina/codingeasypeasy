...
*   **Same-Origin:** If the parent page and the frame content come from the *same* origin, you can freely access and manipulate the frame's content.

*   **Cross-Origin:** If the parent page and the frame content come from *different* origins, your access will be heavily restricted. You generally *cannot* access the frame's DOM or execute JavaScript code within the frame.
...
window.addEventListener("message", (event) => {
  if (event.origin === "https://yourdomain.com") { // Replace with parent's origin
