...
*   **`importHelpers: true`**: This is the most important setting.  It tells the TypeScript compiler to *not* emit helper functions directly into the compiled output.  Instead, it will generate import statements that reference `tslib`.
...
**Compiled JavaScript (without `tslib` - *bad*):**
...
**Compiled JavaScript (with `tslib` - *good*):**
