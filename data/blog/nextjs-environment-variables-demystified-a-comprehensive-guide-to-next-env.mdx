...
*   **Server-Side:**  Code executed on the server (e.g., API routes, `getServerSideProps`, `getStaticProps`) can access *all* environment variables, including those that *don't* have the `NEXT_PUBLIC_` prefix.

*   **Client-Side:**  Code executed in the browser (e.g., React components) can *only* access environment variables that are prefixed with `NEXT_PUBLIC_`.  This is a security measure to prevent exposing sensitive server-side configuration to the client.
