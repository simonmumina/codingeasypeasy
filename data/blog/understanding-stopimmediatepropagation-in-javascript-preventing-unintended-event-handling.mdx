...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'event bubbling', 'event capturing', 'stopImmediatePropagation', 'event handling', 'dom', 'web development']
draft: false
...
1.  **Event Capturing:** The event travels down the DOM tree, starting from the `window` object and propagating to the target element where the event originated.  Event listeners attached in the *capturing* phase are executed first.

2.  **Event Bubbling:** After reaching the target element, the event bubbles up the DOM tree, from the target element to its parent, then to its grandparent, and so on, until it reaches the `window` object again. Event listeners attached in the *bubbling* phase are executed next.

By default, most event listeners are attached in the *bubbling* phase.
...
1.  **Prevents further execution of event listeners on the *same* element:**  No other event listeners attached to the *same* element will be executed after the listener that called `stopImmediatePropagation()`.  This is the crucial difference from `stopPropagation()`.
...
The main distinction is that `stopPropagation()` only stops the event from bubbling up the DOM tree but *doesn't* prevent other event listeners attached to the *same* element from executing. `stopImmediatePropagation()` does *both*.
