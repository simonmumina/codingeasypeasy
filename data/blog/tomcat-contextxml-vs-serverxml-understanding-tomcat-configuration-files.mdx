...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['tomcat', 'java', 'configuration', 'server.xml', 'context.xml', 'web applications', 'deployment', 'application server']
draft: false
...
*   **`server.xml`:** This is the *primary* configuration file for the entire Tomcat server. It defines the overall structure of the server, including connectors (ports for listening to requests), hosts (virtual hosts), and global resources.  Modifications here affect the entire Tomcat instance.

*   **`context.xml`:**  This file configures a *specific* web application. It defines the resources (data sources, mail sessions, etc.) and behaviors (session management, security realms, etc.) specific to that application.  Changes here are isolated to that particular web app.
...
*   **Configuring Global Resources:** While less common, you *can* define resources (like data sources) in `server.xml` making them available to all web applications deployed on the server. However, using `context.xml` for application-specific resources is generally preferred for isolation and maintainability.
...
1.  **`${CATALINA_BASE}/conf/\[enginename]/\[hostname]/[webappname].xml`:** (e.g., `conf/Catalina/localhost/myapp.xml`)  This is *specific to a particular context and host*. It overrides any settings defined in the global `context.xml`.  The filename (excluding the `.xml` extension) *must* match the context path of the application.  For example, if your application is accessible at `http://localhost:8080/myapp`, the file should be named `myapp.xml`.  If the application is deployed as a WAR file (`myapp.war`), the context path will default to `myapp` unless otherwise specified.

2.  **`${CATALINA_HOME}/webapps/[webappname]/META-INF/context.xml`:** This is *packaged within the WAR file* of the web application.  This makes the configuration portable with the application itself.  Tomcat automatically deploys this file when the WAR is deployed.
...
*   **`<Context>`:**  The root element, representing the context (web application).
    ...
    *   `reloadable`:  Whether Tomcat should automatically reload the web application when changes are detected. Useful for development.  **Warning:**  Do *not* use this in production.
...
*   **Enabling Automatic Reloading (Development Only):**  The `reloadable="true"` attribute is useful during development as it automatically redeploys the application whenever changes are made to the web application's files.  *Never* use this in production!
