...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['css', 'namespace', 'scoped css', 'styling', 'web development', 'front-end', 'css architecture', 'prefixing', 'css modules']
draft: false
...
The `|` (pipe) character is crucial. It acts as the *namespace separator* and tells the browser that the selector to the right belongs to the namespace identified by the prefix on the left.
...
Let's say you have a custom element called `<my-component>`. You can use `@namespace` to create styles that apply *only* within that element.  This example is more of a conceptual illustration, as directly styling custom elements with `@namespace` in a framework like React requires more complex integration involving injecting the CSS with the namespace.
...
<button class="button">Another button</button> <!-- This button should not be styled -->
```
...
**Generally, use CSS Modules or Scoped CSS features offered by your framework *first* before resorting to `@namespace`.** `@namespace` is more valuable when dealing with situations where you need to interact with existing, possibly conflicting, styles or when working with XML-based formats like SVG.
