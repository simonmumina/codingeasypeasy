...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react', 'javascript', 'window history', 'navigation', 'spa', 'frontend development', 'state management', 'router', 'history api']
draft: false
...
In this example, clicking the button adds a new entry to the history with the URL `/about` and the state object `{ page: 'about', data: { message: 'Welcome to the About page!' } }`.  While this code *adds* to the history, it doesn't actually *display* the About page content.  You need to combine this with a mechanism (described later) to listen for history changes and update your React component accordingly.

### 2. `history.replaceState(state, title, url)`

The `replaceState()` method modifies the current history entry.  It's similar to `pushState()`, but it *replaces* the current entry instead of adding a new one. This is useful when you want to update the URL or state without adding to the navigation history (e.g., when updating query parameters).
...
*   `delta`: An integer representing the relative position in the history.
    ...
    *   `history.go(n)`: Goes forward or backward *n* entries.
