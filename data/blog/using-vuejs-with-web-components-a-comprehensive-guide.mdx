...
*   **Manual Attribute Update:**  **This is crucial.** Vue's reactivity system *doesn't automatically update attributes on Web Components*.  We need to manually set the `user-data` attribute on the Web Component using `element.setAttribute()` inside the `watch` block. We select the web component using `document.querySelector('my-greeting')`. We have to do this as Vue will not automatically update web component properties.
*   **`deep: true` in `watch`:**  Ensures that the `watch` handler is triggered even when properties *within* the `userData` object change.
