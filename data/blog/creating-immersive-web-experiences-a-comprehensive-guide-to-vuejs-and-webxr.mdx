...
date: '2024-03-08'
lastmod: '2024-03-08'
tags: ['vuejs', 'webxr', 'xr', 'virtual-reality', 'augmented-reality', 'web-development', 'immersive-experiences', 'javascript', '3d-graphics']
draft: false
...
While not strictly *required*, Three.js is a popular JavaScript 3D library that greatly simplifies working with WebGL, which is the underlying technology used by WebXR.  It provides a higher-level abstraction, making it easier to create and manipulate 3D scenes.
...
2.  **`script`:**
    ...
    *   **`startXR()`:**  This function initiates the XR session.  It uses `navigator.xr.requestSession()` to request an `immersive-vr` session.  The `requiredFeatures` array specifies any features the session *must* have.  `local-floor` is common for VR, allowing the user to move around based on a detected floor.
    ...
    *   **`onSessionEnded()`:** This function cleans up the WebXR resources when the session ends.  It sets `xrSession` to null and removes the renderer's DOM element from the `xrContainer`. It also disposes of the renderer, scene, and camera to free up resources. *This is important to prevent memory leaks*.
  ...
  if (pose) {
    for (const inputSource of this.xrSession.inputSources) {
      if (inputSource.handedness === 'left') {
        ...
        const gripPose = frame.getPose(inputSource.gripSpace, this.xrSession.requestReferenceSpace('local'));
        if (gripPose) {
