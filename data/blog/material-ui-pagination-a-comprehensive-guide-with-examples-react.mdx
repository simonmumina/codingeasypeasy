...
The example above demonstrates *controlled pagination*.  This means that the `page` state is managed by the parent component and passed down to the `Pagination` component as a prop.  This gives you complete control over the pagination state.

An *uncontrolled* pagination component would manage its own state internally.  While simpler for basic use cases, it's generally recommended to use controlled pagination, especially when dealing with server-side data.
...
* **`totalItems` State:** This state stores the *total* number of items available on the server.  This is crucial for calculating the correct number of pages.
...
*   **Incorrect number of pages:** Verify that you're calculating the `count` prop correctly based on the total number of items and the number of items per page.  The `Math.ceil` function is important. Ensure that your server returns the *total* number of items and that you're correctly setting `totalItems` state.
