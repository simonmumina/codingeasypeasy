...
*   **`document.querySelector()` and `document.querySelectorAll()`**: These are the most versatile methods. They allow you to use CSS selectors to select elements.  `querySelector()` returns the *first* matching element, while `querySelectorAll()` returns a `NodeList` (similar to `HTMLCollection`, but doesn't update live).
...
*   **`text`**: Gets or sets the text content of the anchor element (the visible link text).  *Note: This is deprecated in favor of `textContent`.*
