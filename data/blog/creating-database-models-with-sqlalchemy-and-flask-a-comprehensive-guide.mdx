...
*   **`User` Model:**
    ...
    *   `password`:  A string (max length 60 characters), and cannot be null.  In a real application, *never* store passwords in plain text!  Use a library like `bcrypt` to hash them.
