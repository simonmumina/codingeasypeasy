...
**Important:** We've now defined the GRPC client *twice*.  Once in the GreeterModule, and once in the AppModule. The `url` is only needed in the AppModule. It's acceptable for a microservice to be both a client *and* a server, hence the seemingly duplicated configuration. If the AppModule *only* acts as a client, you can remove the `GreeterController` from the AppModule controllers array, and remove the GreeterModule's gRPC setup.
