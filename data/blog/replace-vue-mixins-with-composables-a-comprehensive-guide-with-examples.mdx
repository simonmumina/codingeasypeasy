...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['vue', 'vue3', 'composables', 'mixins', 'javascript', 'refactoring', 'best practices', 'vue composables', 'vue mixins']
draft: false
...
export default {
  setup() {
    ...
    const { currentPage, pageSize, totalPages, setPage, totalItems: totalItemsComposable } = usePagination(totalItems.value, 10);
...
*   **Data Sharing:**  We pass `totalItems.value` to the composable *initially* and then separately update a ref in the composable whenever `totalItems` changes.
