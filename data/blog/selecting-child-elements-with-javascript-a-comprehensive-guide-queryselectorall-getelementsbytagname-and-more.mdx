...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['javascript', 'dom manipulation', 'web development', 'child elements', 'queryselectorall', 'getelementsbytagname', 'children', 'dom traversal']
draft: false
...
*   **Live HTMLCollection:** Like `getElementsByTagName()`, returns a *live* `HTMLCollection`.
...
The original example had a logical error.  The `querySelectorAll('.active')` was being used on the *child* elements of each `div` with the class `item`.  However, the class `active` was applied to the *div* itself, not to its children.

The corrected example using `querySelectorAll('div.item.active p')` directly and efficiently targets:

1.  `div.item.active`: Div elements that have *both* the `item` and `active` classes applied directly to them.
