...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['node.js', 'password hashing', 'cryptography', 'security', 'crypto module', 'bcrypt', 'scrypt', 'pbkdf2', 'salting', 'nodejs security', 'authentication']
draft: false
...
**Here's why you *must* hash passwords:**
...
*   **Salt:** A randomly generated string added to the password *before* hashing. This prevents attackers from using pre-computed "rainbow tables" to crack passwords.  Salts should be unique for each user and stored alongside the hashed password.
...
*   **bcrypt:** A widely used and well-regarded password hashing function. It incorporates salting and a configurable work factor, making it resistant to brute-force and rainbow table attacks. *While bcrypt is not directly available within the `crypto` module, a library is needed to use it like `bcrypt` or `bcryptjs`.*
...
*   **PBKDF2 (Password-Based Key Derivation Function 2):** While generally considered less secure than `bcrypt` or `scrypt` for *new* applications, it's still often used in legacy systems. It allows you to specify the number of iterations (similar to the cost factor). It's available within the `crypto` module.

**Algorithms to Avoid:**

*   **MD5 & SHA-1:**  These algorithms are *cryptographically broken* and should **never** be used for password hashing.  They are too fast and vulnerable to collision attacks.  While available in the `crypto` module, their use is highly discouraged.
...
async function hashPasswordWithScrypt(password, salt = crypto.randomBytes(16).toString('hex')) {
  ...
  return new Promise((resolve, reject) => {
    crypto.scrypt(password, salt, keylen, { N: cost, r: blocksize, p: parallelization }, (err, derivedKey) => {
      if (err) {
        reject(err);
      } else {
        resolve({
...
async function verifyPasswordWithScrypt(password, hashedPassword, salt, cost, blocksize, parallelization) {
    return new Promise((resolve, reject) => {
...
async function main() {
  ...
  const { salt, hashedPassword, algorithm, cost, blocksize, parallelization } = await hashPasswordWithScrypt(password);
  ...
  const isMatch = await verifyPasswordWithScrypt(password, hashedPassword, salt, cost, blocksize, parallelization);
  console.log('Password Match:', isMatch); // Output: true

  const wrongPasswordMatch = await verifyPasswordWithScrypt("wrongPassword", hashedPassword, salt, cost, blocksize, parallelization);
...
async function verifyPasswordWithPBKDF2(password, hashedPassword, salt, iterations, keylen, digest) {
    return new Promise((resolve, reject) => {
      crypto.pbkdf2(password, salt, iterations, keylen, digest, (err, derivedKey) => {
        if (err) {
          reject(err);
...
async function mainPBKDF2() {
  const password = 'MySecurePassword123!';
  const { salt, hashedPassword, algorithm, iterations, keylen, digest } = await hashPasswordWithPBKDF2(password);
  ...
  const isMatch = await verifyPasswordWithPBKDF2(password, hashedPassword, salt, iterations, keylen, digest);
  console.log('Password Match:', isMatch); // Output: true

  const wrongPasswordMatch = await verifyPasswordWithPBKDF2("wrongPassword", hashedPassword, salt, iterations, keylen, digest);
  console.log('Password Match (wrong password):', wrongPasswordMatch);
}
...
*   **Algorithm Parameter Storage:** You *must* store the `salt`, `iterations`, `keylen`, and `digest` algorithm along with the hashed password.
...
*   `algorithm`: Stores the hashing algorithm used. *Not needed for bcrypt.*
*   `salt`: Stores the unique salt used for hashing. *Not needed for bcrypt.*
