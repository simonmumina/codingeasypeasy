...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'dom manipulation', 'class toggle', 'html', 'front-end development', 'web development']
draft: false
...
element.className = element.className.replace(new RegExp('(^|\\s)' + oldClass + '(\\s|$)', 'g'), ' ');
```
...
This approach is highly discouraged because it will erase all existing classes on the element. It's only useful when you *specifically* want to replace all existing classes with a new set.
