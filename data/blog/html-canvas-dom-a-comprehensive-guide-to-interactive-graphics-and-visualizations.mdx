...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['HTML Canvas', 'JavaScript', 'DOM', 'Graphics', 'Visualization', 'Interactive Web', 'Canvas API', 'Game Development', 'Data Visualization']
draft: false
...
*   **`width="500"` and `height="300"`:**  Sets the dimensions of the canvas in pixels.  **Important:** Always set these as attributes on the `<canvas>` tag itself, *not* via CSS.  Setting the width and height via CSS will scale the canvas, leading to distorted or blurry graphics.

Next, you'll need to access the canvas element using JavaScript and obtain a *drawing context*:
...
*   **`canvas.getContext('2d')`:**  Returns a *rendering context* object.  The `'2d'` context provides a rich set of APIs for drawing 2D shapes, text, images, and more.  Another context, `'webgl'`, is used for 3D graphics (outside the scope of this guide, but worth noting for advanced use cases).
...
*   **`img.onload`:**  A function that is executed when the image has finished loading.  You *must* wait for the image to load before drawing it on the canvas.
