...
*   **Improve query readability and organization:** While complex queries *can* sometimes be written without subqueries, using them often makes the logic clearer and easier to understand.
...
*   **Correlated Subqueries:**  These subqueries rely on the outer query for their execution.  They are executed *once for each row* processed by the outer query. This can impact performance, so use them judiciously.
...
1.  **Execution Order:** Generally, the subquery is executed *before* the main query.
...
While less frequent in simple scenarios, a row subquery *could* be used like this:
...
*Note:  A simple `WHERE City = 'London'` would be more efficient in this specific example. Row subqueries become more valuable when the subquery is more complex.*
...
**Problem:** Find the customers who have placed an order with an amount greater than the average order amount for *their* CustomerID.
...
*   The subquery `SELECT 1 FROM Orders o WHERE o.CustomerID = c.CustomerID AND o.Amount > (SELECT AVG(Amount) FROM Orders WHERE CustomerID = c.CustomerID)` is a correlated subquery because it references the `c.CustomerID` from the outer query.  It's executed *for each row* in the `Customers` table.
*   For each customer, the subquery calculates the average order amount *for that specific customer* and checks if there are any orders for that customer with an amount greater than their average.
