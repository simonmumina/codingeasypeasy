...
The `deltaMode` property indicates the unit of measurement for the `deltaX`, `deltaY`, and `deltaZ` properties of the `WheelEvent`. It helps you understand *how* the user scrolled. The possible values for `deltaMode` are represented by the following constants in the `WheelEvent` interface:
