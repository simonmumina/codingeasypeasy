...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['tomcat', 'postgresql', 'oracle', 'database connectivity', 'jdbc', 'apache tomcat', 'java', 'datasource', 'jndi', 'connection pooling']
draft: false
...
**Important:** Placing the JDBC driver in the web application's `WEB-INF/lib` directory instead of Tomcat's `lib` makes it available *only* to that specific web application. While this can be useful for isolating dependencies, it often leads to duplicated driver files and potential conflicts if multiple web applications need access to the database. Using Tomcat's `lib` directory is generally the preferred approach for shared resources.
...
1.  **Edit `context.xml`:**  Navigate to the `conf` directory in your Tomcat installation directory. Edit the `context.xml` file.  This file defines the context configuration for all web applications in Tomcat.
    ...
    **Important:**  Never directly modify the `conf/context.xml` file unless you understand the implications. Modifications here will affect *all* web applications deployed on Tomcat. For application-specific data sources, configure them within the application's `META-INF/context.xml` file (more on that below).
    ...
    *   `name`:  The JNDI name used to look up the data source in your web application (e.g., `jdbc/MyDatabase`).  *Important:* The `java:comp/env/` prefix is added by the container when accessed by the web application, so you don't include it here.
...
6.  **Resource Cleanup:**  The `finally` block ensures that resources (ResultSet, Statement, Connection) are closed properly, even if exceptions occur. *Always close resources in reverse order of their creation.* This is *critical* for preventing resource leaks and maintaining database stability.
...
*   **SQL Injection:**  Protect against SQL injection attacks by using parameterized queries (using `PreparedStatement` instead of `Statement`) whenever possible.  *Never* directly concatenate user input into SQL queries.
