...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['next-js', 'webpack', 'next-config', 'plugins', 'compose-plugins', 'configuration', 'optimization']
draft: false
...
Here, we're adding a custom Webpack rule to handle SVG files using `@svgr/webpack`. The `webpack` function receives the existing Webpack configuration (`config`) and an object containing information about the build environment. Remember to *return* the modified `config` object.
