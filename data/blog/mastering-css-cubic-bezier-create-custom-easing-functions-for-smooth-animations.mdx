...
`cubic-bezier()` is a CSS timing function that defines a *cubic Bézier curve*.  Think of it as a recipe for describing how an animation progresses over time. It's used with the `transition-timing-function` and `animation-timing-function` properties to control the speed of an animation or transition at different points in its duration.
...
Imagine a graph where the x-axis represents the *time* of the animation (from 0 to 1) and the y-axis represents the *progress* of the animation (also from 0 to 1).
