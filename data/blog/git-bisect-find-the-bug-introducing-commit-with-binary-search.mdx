...
5.  **Mark as Good or Bad:**  After testing, let's say the user profiles *are* loading correctly in this commit.

    ```bash
    git bisect good
    ```

6.  **Repeat:** Git checks out another commit. This time, the profiles *are not* loading correctly.
