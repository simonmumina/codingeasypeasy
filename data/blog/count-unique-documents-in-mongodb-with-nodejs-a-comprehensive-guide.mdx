...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['mongodb', 'nodejs', 'aggregation', 'distinct', 'unique documents', 'count', 'javascript', 'database']
draft: false
...
// index.js (Inside the main function, after the unique city count)
   // Count unique combinations of name and city using distinct within an aggregation pipeline
    ...
    const uniqueCombinations = await collection.aggregate([
      {
        $group: {
    ...
    const uniqueCombinationCount = uniqueCombinations.length > 0 ? uniqueCombinations[0].totalUniqueCombinations : 0;
...
// index.js (Inside the main function, after the unique combination count)
    ...
    const uniqueCount = await collection.aggregate([
      {
        $group: {
          _id: { name: "$name", city: "$city" },
...
async function main() {
  try {
    ...
    console.log(`Number of unique combinations of name and city using distinct: ${uniqueCombinationCount}`);
    ...
    console.log(`Number of total unique documents based on name and city using aggregation pipeline: ${totalUniqueDocuments}`);
