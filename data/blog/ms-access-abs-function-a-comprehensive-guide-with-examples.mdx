...
**Important Note:** While the `IIf()` approach _works_, it's generally less efficient than performing the `Abs()` calculation in VBA beforehand, especially for large datasets. The VBA approach allows you to pre-calculate the absolute values and store them (if needed) or directly use them in your query.
