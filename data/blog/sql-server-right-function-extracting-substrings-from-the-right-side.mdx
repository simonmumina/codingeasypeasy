...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['SQL Server', 'RIGHT Function', 'String Manipulation', 'Substring', 'T-SQL', 'Database', 'SQL Tutorial']
draft: false
...
*   **`integer_expression`**: This is an integer value that specifies the number of characters you want to extract from the right side of the string. This value *must* be a positive integer. If the `integer_expression` is larger than the length of the `character_expression`, the entire string will be returned.  If it's 0, an empty string will be returned.  If it's negative, you'll encounter an error.
...
**Important Note:** The above SQL is *not* the correct way to extract file extensions with `RIGHT()`.  The `CHARINDEX` approach attempts to find the position of the period, but this will fail if there are multiple periods in the filename (e.g., `my.document.docx`).  A more robust approach uses `REVERSE()` and `CHARINDEX()` together:
...
2.  **`CHARINDEX('.', REVERSE(FileName)) - 1`**:  Finds the position of the first period in the reversed string and subtracts 1.  This is the number of characters *before* the last period (which, when reversed back, gives us the extension).
