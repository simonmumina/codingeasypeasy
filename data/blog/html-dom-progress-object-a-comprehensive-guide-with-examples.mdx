...
Sometimes you don't know the exact progress of a task, but you still want to provide feedback to the user that something is happening.  In this case, you can use an *indeterminate* progress bar.  To create one, simply omit the `value` attribute. The browser will display an animated progress bar.
