...
Before diving into the commands, it's important to differentiate between *reverting* and *resetting*.

*   **Reverting:** Creates a *new* commit that undoes the changes introduced by the specified commit (in our case, the last commit). This preserves your Git history, making it a safer option for shared repositories.
...
*   `<commit-hash>`:  The hash of the *first* commit you want to revert from (exclusive).  The `..` operator signifies a range.
...
*   `--soft`: Moves the `HEAD` pointer to the specified commit, but *preserves* the changes in the staging area and working directory.  This means your changes are still there, ready to be modified or committed again.
*   `HEAD~1`:  Refers to the commit *before* the last commit (equivalent to `HEAD^`).
...
3.  You can then modify these changes, add them to the staging area, and create a *new* commit.
