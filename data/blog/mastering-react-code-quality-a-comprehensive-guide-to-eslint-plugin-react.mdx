...
3.  **Install `eslint-plugin-react-hooks` (Highly Recommended):**
    ...
    For React projects using Hooks, it's *highly* recommended to also install and configure `eslint-plugin-react-hooks`:
...
If you *do* need to import `React` (e.g., for class components), these rules will ensure that the import is actually used.
