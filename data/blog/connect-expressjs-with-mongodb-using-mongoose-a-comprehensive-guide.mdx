...
*   **`.then(...)`:** This executes if the database connection is successful. It logs a message to the console and starts the Express server using `app.listen()`. The server only starts *after* a successful database connection.
...
*   **`const Product = mongoose.model('Product', productSchema)`:** Creates a Mongoose model named `Product` based on the `productSchema`.  The first argument, `'Product'`, is the *singular* name of the model.  Mongoose will automatically pluralize this and look for a collection named `products` in your MongoDB database.
...
app.patch('/products/:id', async (req, res) => {  // PATCH for partial updates
  try {
    const product = await Product.findById(req.params.id);
    if (product == null) {
...
*   **`app.patch('/products/:id', ...)`:** Defines a route to *partially* update a product by its ID. We use `PATCH` here to indicate that we might only be updating a few fields.
    *   It retrieves the product using `Product.findById(req.params.id)`.
    *   It checks if the product exists (404 if not found).
    *   It *conditionally* updates the product's fields based on what's present in the `req.body`.  This is important because you don't want to accidentally overwrite fields with `null` or undefined values.
