...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react-native', 'expo', 'magnetometer', 'sensors', 'mobile-development', 'magnetic-fields', 'magnetism']
draft: false
...
These values represent the components of the magnetic field vector in three-dimensional space. The units are typically in microteslas (µT). The values are *relative* to the device's orientation. This means that changing the device's orientation will affect the x, y, and z values.
...
Here's a simplified example of how to use the magnetometer data to create a basic compass.  This example does *not* include calibration or declination correction, which would be necessary for a real-world compass application.
