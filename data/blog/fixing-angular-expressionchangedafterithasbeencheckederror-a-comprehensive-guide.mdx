...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['angular', 'error', 'debugging', 'lifecycle', 'performance', 'ExpressionChangedAfterItHasBeenCheckedError']
draft: false
...
These lifecycle hooks are executed *after* Angular has rendered the component's view or projected content. Modifying a property that is bound to the template within these hooks is a frequent culprit.
...
-   In `ngAfterViewInit`, we use `setTimeout` to asynchronously update the `message` property. This change occurs *after* Angular has already checked the view in the initial change detection cycle, leading to the error.

**2.  Modifying Data in a Child Component After Parent Has Been Checked**

This occurs when a child component's modification affects the parent component's bindings, especially if the child component's change detection runs *after* the parent's.
...
@Component({
    ...
    template: `<div #observedElement>Observe Me!</div><p>Message: {{message}}</p>`
})
...
Using `setTimeout` with a delay of 0 milliseconds (`setTimeout(() => { ... }, 0)`) can defer the property update to the next change detection cycle. This can be a quick fix, but it's important to understand *why* the update is happening late and consider other options first.  Overuse of `setTimeout` can lead to performance issues.
...
@Component({
  ...
  template: `
    <p>Current value: {{ interval$ | async }}</p>
  `
})
