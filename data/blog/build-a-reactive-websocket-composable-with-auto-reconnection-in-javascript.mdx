...
date: '2024-01-04'
lastmod: '2024-01-04'
tags: ['javascript', 'websocket', 'reactive programming', 'composable', 'auto-reconnect', 'frontend development', 'web development']
draft: false
...
function MyComponent() {
  ...
  const { connectionStatus$, message$, error$, close$, send, stop, start } = createWebSocketComposable('ws://localhost:8080', {
  ...
  return (
    <div>
      ...
      <div>
        ...
        <button onClick={handleStart} disabled={status !== 'closed'}>Start</button>
      </div>
    </div>
  );
}
