...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'array', 'includes', 'indexOf', 'find', 'contains', 'check array', 'javascript array']
draft: false
...
The `find()` method returns the *value* of the first element in the array that satisfies the provided testing function.  The `findIndex()` method returns the *index* of the first element in the array that satisfies the provided testing function. Both return `undefined` if no element satisfies the testing function. These are useful when you need more complex matching criteria than simple equality.
...
While not recommended for most use cases due to performance and readability concerns, you *can* use a custom loop to check for array containment.
...
*   **Use `indexOf()` if you need the index of the element:**  If you need to know *where* the element is located, `indexOf()` is the appropriate choice.
