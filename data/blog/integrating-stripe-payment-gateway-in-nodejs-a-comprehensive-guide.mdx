...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['node.js', 'stripe', 'payment gateway', 'payment integration', 'ecommerce', 'backend', 'javascript', 'api']
draft: false
...
While using Payment Intents is generally the recommended approach, you *can* also create charges directly.  **This approach is generally less flexible and doesn't support features like 3D Secure authentication as well as Payment Intents.** Here's how you would implement it:
...
4.  **Secure Your Webhook Endpoint:**  Stripe sends a signature in the `Stripe-Signature` header of each webhook request. You *must* verify this signature to ensure that the request is actually coming from Stripe and hasn't been tampered with.
...
1.  **`express.raw({ type: 'application/json' })`:** This middleware is crucial. It tells Express *not* to parse the request body as JSON because `stripe.webhooks.constructEvent` needs the raw request body for signature verification.
...
5.  **200 OK Response:**  It's *essential* to return a 200 OK response to Stripe after successfully processing the event.  If Stripe doesn't receive a 200 OK response within a reasonable time, it will retry the webhook request.
...
app.post('/create-subscription', async (req, res) => {
  try {
    ...
    res.send({ subscriptionId: subscription.id, clientSecret: subscription.latest_invoice.payment_intent.client_secret });
...
3.  **Handle Subscription Events in Webhooks:**  You'll need to handle subscription-related events in your webhook endpoint, such as:
