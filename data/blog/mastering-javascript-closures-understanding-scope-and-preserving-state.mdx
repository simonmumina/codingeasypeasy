...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'closures', 'scope', 'lexical environment', 'function scope', 'javascript tutorial']
draft: false
...
In this example, `innerFunction` has access to `outerVar` because it was defined *inside* `outerFunction`. This access remains even after `outerFunction` has completed execution.  This is the essence of a closure.  `innerFunction` *closes over* the scope of `outerFunction`, preserving access to `outerVar`.
...
In this example, `createCounter` returns an object with three methods: `increment`, `decrement`, and `getValue`.  These methods all have access to the `count` variable, which is declared within the `createCounter` function.  Crucially, `count` is *not* accessible from outside the `createCounter` function.  This effectively creates a private variable that can only be modified through the `increment` and `decrement` methods.
