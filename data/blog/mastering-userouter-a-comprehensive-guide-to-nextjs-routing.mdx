...
date: '2024-01-02'
lastmod: '2024-10-27'
tags: ['nextjs', 'useRouter', 'routing', 'navigation', 'client-side', 'dynamic routes', 'query parameters', 'react']
draft: false
...
function ProductButton({ productId }) {
  ...
  return (
    <button onClick={handleClick}>View Product {productId}</button>
  );
}
...
function ArticleButton({ articleId }) {
  ...
  return (
    <button onClick={handleClick}>View Article {articleId}</button>
  );
}
