...
date: '2024-01-01'
lastmod: '2024-01-02'
tags: ['npm', 'yarn', 'package.json', 'dependencies', 'updates', 'javascript', 'nodejs', 'frontend', 'backend', 'security']
draft: false
...
This will update `axios` to the latest version that satisfies the version range specified in your `package.json`. It *will not* update to a version that breaks the semantic versioning rule (e.g., updating from `^1.2.0` to `2.0.0`).
...
To update a dependency to the *absolute latest* version, even if it violates the semver range in your `package.json`, you can use the `npm install` command with the `@latest` tag:
...
The `npm update` command *should* update the version in your `package.json` to reflect the new allowed range based on the installed version.  However, sometimes it's necessary to explicitly save the new version.  You can do this using `--save`:
