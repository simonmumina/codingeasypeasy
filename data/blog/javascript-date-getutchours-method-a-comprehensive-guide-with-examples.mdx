...
It's crucial to understand the difference between `getHours()` and `getUTCHours()`.  `getHours()` returns the hour based on the *local* time zone of the user's computer or browser. `getUTCHours()`, as we've discussed, returns the hour based on UTC.
