...
date: '2024-01-01'
lastmod: '2024-01-05'
tags: ['node.js', 'sockets.io', 'realtime', 'javascript', 'web sockets', 'chat application', 'event driven programming']
draft: false
...
*   `socket.on("chat message", (msg) => { ... })` listens for "chat message" events from the client. When a message is received, it's logged to the console and then broadcast to all connected clients using `io.emit("chat message", msg);`.  `io.emit` sends the event to *all* connected clients. `socket.emit` sends an event only to the originating client.
...
Broadcasting allows you to send data to all connected clients or to a specific subset of clients (e.g., clients in a specific room).  `io.emit()` sends an event to *all* connected clients in all namespaces. `io.to(room).emit()` sends an event to all clients in a specific room. `socket.broadcast.emit()` sends an event to all *other* clients connected to the same socket, excluding the sender.  `socket.broadcast.to(room).emit()` sends an event to all *other* clients in a specific room, excluding the sender.
