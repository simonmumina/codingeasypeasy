...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react', 'jwt', 'authentication', 'security', 'refresh token', 'token expiration', 'axios interceptor', 'http-only cookie']
draft: false
...
2.  **JWT Storage:** The JWT is stored securely in the client-side (e.g., in memory, `localStorage`, or `sessionStorage`, *with caveats* - see security considerations below).
...
6.  **New Tokens:** The server validates the refresh token (and potentially revokes the old JWT) and issues a new JWT and a new refresh token.  The new JWT replaces the old one in the client. If a new refresh token is issued, it *replaces* the old one in the HTTP-only cookie.
...
This interceptor can handle 401 (Unauthorized) errors, which might occur if the JWT is invalid (e.g., revoked or expired) *before* the request interceptor has a chance to refresh it.
...
*   **Refresh Token Rotation:**  Each time a refresh token is used, issue a *new* refresh token and invalidate the old one. This limits the impact if a refresh token is compromised.  The example above demonstrates refresh token rotation.
...
*   **HTTP-Only Cookies for Refresh Tokens:** This is *the* most crucial security measure.  It prevents client-side JavaScript from accessing the refresh token, mitigating the risk of XSS attacks.
...
*   **Validate JWTs on the Server:** Always validate the JWT on the server for every request to ensure it is authentic and has not been tampered with. Do *not* rely solely on the client-side JWT decoding.
