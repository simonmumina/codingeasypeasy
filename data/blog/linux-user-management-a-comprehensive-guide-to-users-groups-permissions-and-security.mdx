...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['linux', 'user management', 'user accounts', 'groups', 'permissions', 'security', 'command line', 'adduser', 'userdel', 'usermod', 'groupadd', 'groupdel', 'chown', 'chmod', 'sudo']
draft: false
...
*   **`sudo` (Superuser Do)**:  Allows you to execute a single command with root privileges. This is generally preferred over directly logging in as root.
    ...
    You'll be prompted for *your* password (the user invoking `sudo`), not the root password (unless configured otherwise).  Users who are allowed to use `sudo` are typically members of the `sudo` or `wheel` group (depending on the distribution). The sudoers file (`/etc/sudoers`) controls which users can execute which commands with `sudo`.
