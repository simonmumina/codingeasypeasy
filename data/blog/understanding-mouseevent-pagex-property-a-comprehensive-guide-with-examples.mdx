...
The `MouseEvent.pageX` property returns the horizontal coordinate (offset) of the mouse pointer **relative to the left edge of the entire document**. This means it includes the distance scrolled horizontally.  In simpler terms, it tells you how far from the left of the *entire* webpage (including content that's scrolled out of view) the mouse is located when the event occurs.
...
*   **`clientX`:** This is the horizontal coordinate relative to the **viewport** (the visible part of the browser window).  It *doesn't* account for scrolling. If you scroll the page to the right, `clientX` will still measure from the left edge of the visible window, whereas `pageX` will continue measuring from the left edge of the entire document.
...
2.  **Scrolling to the right:**  `pageX` will increase significantly as you move the mouse further right, reflecting the distance from the *true* left edge of the document. `clientX` will only change within the bounds of the viewport. `screenX` will be unaffected if the mouse pointer remains within the same position relative to the screen.
...
4.  We use `getBoundingClientRect()` to get the position and dimensions of the `scrollableDiv` relative to the *viewport*.  Crucially, this is viewport-relative.
5.  We subtract `rect.left` and `rect.top` from `event.clientX` and `event.clientY` respectively.  Since `rect` is viewport relative and `clientX` and `clientY` are too, this gives us the *relative* position of the click within the `div`.
6.  We display the calculated coordinates in an alert box.  This demonstrates how to handle scrolling without directly using `pageX`.  If we *needed* to calculate relative to the entire document, then using `pageX` would be appropriate, but in this case, we're dealing with an element's viewport-relative position.
