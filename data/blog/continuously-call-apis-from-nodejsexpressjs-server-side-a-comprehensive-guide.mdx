...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['nodejs', 'expressjs', 'api', 'server-side', 'cron jobs', 'setInterval', 'axios', 'request', 'background tasks', 'polling']
draft: false
...
setInterval(async () => {
  try {
      const job = await apiQueue.add('api-call', {}, { // Add an empty object for data
...
(async function() { // Wrap in an async function for top-level await
  await agenda.start();
