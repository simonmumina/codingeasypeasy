...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['javascript', 'browser-api', 'window-object', 'innerheight', 'web-development', 'responsive-design', 'viewport', 'dom']
draft: false
...
The `window.innerHeight` property returns the **interior height of the browser window in pixels**.  This includes the height of the document's content *excluding* browser UI elements such as toolbars, scrollbars (if present and non-overlay), and browser chrome (the frame around the window).

In essence, it tells you the available space for your web page content *within* the browser's viewing area.
...
*   **`window.innerHeight`:** The height of the viewport, *excluding* the browser's UI elements (toolbars, scrollbars, etc.).

*   **`window.outerHeight`:** The *entire* height of the browser window, including the browser's UI elements.

Think of it like this: `window.outerHeight` is the total height of the window frame, while `window.innerHeight` is the height of the space *inside* that frame that your content can occupy.
...
Another property often confused with `window.innerHeight` is `document.documentElement.clientHeight`.  While they *can* return the same value in some scenarios, their primary function differs:
