...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['nestjs', 'webrtc', 'websockets', 'voice chat', 'realtime', 'api development', 'javascript', 'typescript']
draft: false
...
2.  **Update `src/chat/chat.gateway.ts`:**
    ...
    ```typescript
    import { WebSocketGateway, SubscribeMessage, MessageBody, WebSocketServer, OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect } from '@nestjs/platform-ws';
    ...
    export class ChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
      ...
      handleOffer(
        ...
        this.server.to(data.roomId).emit('offer', { sdp: data.sdp, senderId: client.id, receiverId: data.receiverId });
      ...
      handleAnswer(
        ...
        this.server.to(data.roomId).emit('answer', { sdp: data.sdp, senderId: client.id, receiverId: data.receiverId });
      ...
      handleIceCandidate(
        ...
        this.server.to(data.roomId).emit('iceCandidate', { candidate: data.candidate, senderId: client.id, receiverId: data.receiverId });
