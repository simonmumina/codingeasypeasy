...
date: '2024-10-27'
lastmod: '2024-10-28'
tags: ['javascript', 'fullstack', 'react', 'nodejs', 'mongodb', 'issue tracker', 'mern stack', 'project management']
draft: false
        ...
        <tbody>
          {issues.map(issue => (
            <tr key={issue._id}>
              ...
              <td>
                <Link to={"/edit/" + issue._id}>Edit</Link> | <a href="#" onClick={() => { deleteIssue(issue._id) }}>Delete</a>
              </td>
            </tr>
          ))}
        </tbody>
...
const EditIssue = () => {
  ...
  const onSubmit = (e) => {
    ...
    axios.post('http://localhost:5000/issues/update/' + id, issue)
      .then(res => console.log(res.data));
...
*   **Replace `<Route component={...}` with `<Route element={<... />}`**: The `component` prop is replaced with the `element` prop in react-router-dom v6. You pass the component you want to render *as* the element.
