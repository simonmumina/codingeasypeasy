...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'addEventListener', 'event handling', 'dom', 'html', 'web development', 'front-end']
draft: false
...
*   **`function`:** The function to be executed when the event occurs. This is the *event handler*. It can be an anonymous function (defined directly within `addEventListener()`) or a named function defined elsewhere.  This function receives an `event` object as its argument, containing information about the event that occurred.
*   **`useCapture` (optional):** A boolean value that specifies whether the event listener should be invoked during the *capture* phase or the *bubbling* phase of event propagation. Defaults to `false` (bubbling phase).  Understanding capture and bubbling is crucial for complex event handling scenarios.

    *   **Capture Phase:**  Events travel *down* the DOM tree, from the window to the target element.  Event listeners registered in the capture phase are triggered first.
    *   **Bubbling Phase:** Events travel *up* the DOM tree, from the target element to the window. Event listeners registered in the bubbling phase are triggered after the capture phase (if any) and after the target element's own event handlers.
...
This example demonstrates the use of the `DOMContentLoaded` event. The script will only execute *after* the HTML document has been fully parsed, ensuring that the paragraph element exists when the script tries to access it.  This prevents errors that can occur if the script runs before the DOM is fully loaded.
...
The arguments to `removeEventListener()` must be the same as those used when adding the listener.  **Important:** This means you *must* use a named function if you intend to remove the listener later.  Anonymous functions cannot be reliably removed.
