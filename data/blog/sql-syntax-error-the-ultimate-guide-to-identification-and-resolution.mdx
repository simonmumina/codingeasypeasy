...
While SQL is generally case-insensitive for keywords, object names (table and column names) *can* be case-sensitive depending on the specific DBMS and its configuration.

For instance, PostgreSQL, by default, treats unquoted identifiers as lowercase. If you create a table with a mixed-case name like `"MyTable"`, you *must* refer to it with the double quotes and the exact same casing every time.
