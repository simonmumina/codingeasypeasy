...
date: '2024-01-01'
lastmod: '2024-01-01'
tags: ['vuejs', 'v-model', 'v-bind', 'data binding', 'two-way binding', 'one-way binding', 'vue js tutorial']
draft: false
...
`v-bind` establishes a **one-way** binding between a component's data property and an HTML attribute or component prop. This means that changes in the data property will update the attribute or prop, but changes in the attribute or prop will *not* affect the data property.  It's a one-way street for data flow.
...
*   Importantly, if you were to somehow try to directly edit the `src` attribute in the browser's developer tools, it would *not* update the `imageUrl` data property. The flow is strictly one-way.
