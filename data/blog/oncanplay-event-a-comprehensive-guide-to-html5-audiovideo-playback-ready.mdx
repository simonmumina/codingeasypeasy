...
The `oncanplay` event fires when the browser estimates that it has enough data to start playing the audio/video without having to stall for further buffering.  Crucially, this doesn't mean the *entire* file is downloaded, but rather that enough is available to begin playback smoothly.
