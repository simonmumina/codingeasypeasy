...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'let', 'var', 'const', 'scope', 'variable declaration', 'block scope', 'es6', 'modern javascript']
draft: false
...
*   **Hoisting:** `var` declarations are "hoisted," meaning the variable is moved to the top of its scope during compilation.  However, only the *declaration* is hoisted, not the initialization. This can lead to variables being `undefined` if accessed before their declaration in the code.
...
You might expect the output to be 0, 1, 2, 3, 4. However, because `var` is function-scoped and hoisted, by the time the `setTimeout` callbacks execute, the loop has already completed, and `i` is equal to 5.  All callbacks refer to the *same* `i` variable.
...
With `let`, each iteration of the loop creates a *new* binding for `i`. Each `setTimeout` callback captures its own unique `i` value, resulting in the desired output.  This makes `let` essential for creating closures within loops.

## `let` vs. `const`: Choosing the Right Tool

While `let` introduces block scope, `const` provides even stronger immutability. `const` declares a *constant* variable, meaning its value cannot be reassigned after it's initialized.
...
**Important Note about `const`:**  `const` doesn't make the *value* of the variable immutable, just the *binding* of the variable.  If the `const` variable holds an object or array, the properties of the object or elements of the array can still be modified.
