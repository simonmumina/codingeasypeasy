...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['ms access', 'access vba', 'split function', 'string manipulation', 'database programming', 'vba tutorial']
draft: false
...
4.  We use `UBound(myArray)` to determine the index of the last element in the array.  Remember that arrays in VBA are zero-based, meaning the first element has an index of 0.  `UBound()` returns the *upper bound* of the array.
...
We check if the `UBound()` of the resulting array is 0 *and* the first element of the array is equal to the original string.  If both conditions are true, it indicates that the delimiter was not found.
