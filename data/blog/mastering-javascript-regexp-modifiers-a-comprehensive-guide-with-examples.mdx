...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'regex', 'regexp', 'regular expressions', 'modifiers', 'flags', 'case-insensitive', 'global', 'multiline', 'dotall', 'unicode', 'sticky', 'hasIndices']
draft: false
...
The `g` modifier finds *all* occurrences of the pattern in the string. Without it, `RegExp.exec()` and `String.match()` would only return the first match.
...
The `m` modifier changes how the `^` (beginning of string) and `$` (end of string) anchors work.  Without `m`, they refer to the beginning and end of the entire string. With `m`, they refer to the beginning and end of *each line* within the string.
...
By default, the dot (`.`) character in a regular expression matches any character *except* a newline (`\n`). The `s` modifier changes this behavior, allowing the dot to match *any* character, including newlines.
