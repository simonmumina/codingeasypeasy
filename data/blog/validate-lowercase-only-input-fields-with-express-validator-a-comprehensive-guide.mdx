...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['express-validator', 'express', 'node.js', 'validation', 'input validation', 'lowercase validation', 'middleware', 'backend']
draft: false
...
app.post('/validate-lowercase-regex', [
  body('username')
...
This method is suitable when you *want* to enforce lowercase but are okay with converting the input.  It first sanitizes the input to lowercase and then validates if the original input was *already* lowercase. This is useful for ensuring the stored data is lowercase while informing the user if they didn't originally enter it that way.

```javascript
// app.js (continued)

app.post('/validate-and-sanitize-lowercase', [
  body('username')
    .notEmpty()
    .withMessage('Username cannot be empty.')
    .custom((value, { req }) => {
...
*   `.toLowerCase()`: This is a *sanitizer*, not a validator.  It converts the input to lowercase before further processing.  Critically, it's applied *after* the custom validation.
...
*   The `custom` validator checks if the original value is different from its lowercase version. If it is, it sets the `req.isOriginalLowercase` flag to `false` (indicating that the input was converted) but *allows* the validation to pass.
...
**Testing**
*Example POST request (Valid with Sanitization):*
...
```
*Example POST request (Valid Original Lowercase):*
