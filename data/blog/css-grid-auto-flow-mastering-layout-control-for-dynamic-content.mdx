...
**Important Note:** The `dense` keyword can change the *visual* order of your elements.  While Item 2 is still the second child in the HTML, it might appear later in the grid layout due to the dense packing algorithm.  This can be problematic for accessibility if the visual order doesn't match the logical order of your content.
...
When `grid-auto-flow` creates new rows or columns to accommodate auto-placed items, these newly created tracks are part of the *implicit grid*. The size of these implicit tracks is controlled by `grid-auto-rows` and `grid-auto-columns`, as mentioned earlier. For example:
