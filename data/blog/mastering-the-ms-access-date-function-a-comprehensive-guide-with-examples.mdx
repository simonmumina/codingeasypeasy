...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['MS Access', 'Date Function', 'VBA', 'Database', 'SQL', 'Date Formatting', 'Access Tutorial', 'Access Date', 'Access DateTime']
draft: false
...
This query will return all orders from the `Orders` table where the `OrderDate` is equal to the current date. **Important note:** This query will only work correctly if `OrderDate` contains *only* the date component. If `OrderDate` also contains the time, you'll need to remove the time component.
