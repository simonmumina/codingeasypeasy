...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['css', 'css variables', 'var()', 'dynamic styling', 'reusable code', 'frontend development', 'web development', 'css custom properties']
draft: false
...
.button:hover {
  background-color: var(--primary-color, darkblue); /* Fallback to darkblue if --primary-color is not defined */
}
```
...
.card h2 {
  color: var(--card-title-color, black); /* Uses --card-title-color if defined within .card, otherwise defaults to black */
}
...
*   **`getComputedStyle(element).getPropertyValue("--variable-name")`:** Gets the *computed* value of a CSS variable. This takes into account all CSS rules that apply to the element, not just inline styles.
...
2.  **Dynamically load different CSS files:** You can use JavaScript to change the `href` attribute of a `<link>` tag that references your theme CSS file.
    ...
    if (isDarkModeEnabled()) {  // some function to detect dark mode preference
