...
*   **Only First Match:** Returns the index of the *first* match. For finding all matches, use the `matchAll()` method with a global regular expression (`/g` flag).
...
Even though the regex has a capture group (the parentheses around "W3Schools"), the `search()` method still only returns the *index* of the match, not the matched groups themselves.  For accessing matched groups, use the `match()` method (described later).
...
*   **`indexOf()`:**  This method searches for a *literal* string value (not a regular expression) and returns the index of the first occurrence.  It's faster than `search()` when searching for simple string literals.
...
*   **`match()`:**  This method retrieves the matches for a search that uses a regular expression.
    ...
    *   If the regular expression *does not* have the `g` (global) flag, `match()` returns an array containing the first match and any capturing groups.
    *   If the regular expression *does* have the `g` flag, `match()` returns an array containing all matches.
...
| Global Flag    | Not applicable; only finds the *first* match       | Not applicable                              | Influences behavior (all vs. first match)                         | Required (for retrieving all matches)                                       |
