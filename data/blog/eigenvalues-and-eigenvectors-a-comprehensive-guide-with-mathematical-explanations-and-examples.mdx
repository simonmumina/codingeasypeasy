...
This equation tells us that when the matrix A acts on the vector v, the resulting vector is simply a scaled version of v, where the scaling factor is λ. The eigenvector _v_ remains in the same direction (or opposite direction if λ is negative) after the transformation by _A_.
...
For a non-trivial solution (where _v_ is not the zero vector), the determinant of (A - λI) must be zero:
...
Once we have the eigenvalues, we can find the corresponding eigenvectors. For each eigenvalue, we substitute it back into the equation **(A - λ I) v = 0** and solve for the eigenvector _v_.
...
- **Eigenvectors are not unique:** If _v_ is an eigenvector of _A_, then any scalar multiple of _v_ (e.g., 2v, -v) is also an eigenvector. This is because the equation `A(cv) = λ(cv)` holds true for any scalar `c`. That's why we often normalize them to have a magnitude of 1.
