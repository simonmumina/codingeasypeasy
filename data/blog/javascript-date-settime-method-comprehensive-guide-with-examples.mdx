...
`Date.UTC()` returns the milliseconds since the epoch for a *specific UTC date and time*. `setTime()` sets the internal representation to this UTC time.  The `toUTCString()` method then displays the date in UTC format.  The `toLocaleString()` method will output the date and time adjusted to the user's local timezone.
