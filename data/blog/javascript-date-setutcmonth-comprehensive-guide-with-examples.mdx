...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['javascript', 'date', 'setUTCMonth', 'utc', 'date manipulation', 'programming', 'web development']
draft: false
...
*   **dayValue (Optional):** An integer representing the day of the month. If specified, it sets the day of the month *after* setting the month. If omitted, the day of the month remains unchanged.
...
*   **Immutability:**  The `setUTCMonth()` method modifies the original `Date` object *in place*.  If you need to preserve the original date, create a copy of the `Date` object before using `setUTCMonth()`.  You can do this using the `Date()` constructor with the original date as an argument:
