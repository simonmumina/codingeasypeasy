...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['linux', 'ram', 'memory', 'check ram', 'available memory', 'free command', 'vmstat', 'top', 'htop', 'memory usage', 'linux commands']
draft: false
...
*   **`free`:** The amount of RAM that is completely unused. **However, this value *doesn't* represent the total available memory. Linux aggressively uses free RAM for caching to improve performance.**
...
*   **`available`:**  **This is the most important value.** It represents the *estimated* amount of memory available for starting new applications without swapping.  It considers the `free` memory plus the `buff/cache` memory that can be reclaimed.
...
*   **`swpd`:** Amount of virtual memory used as swap space.  A high value here indicates that your system is relying heavily on swap, which is *much* slower than RAM.
