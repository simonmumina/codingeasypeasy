...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['express', 'node.js', 'architecture', 'best practices', 'server', 'app', 'modularization', 'testing', 'separation of concerns']
draft: false
...
The core principle behind separating `app.js` and `server.js` is *separation of concerns*.  This architectural pattern aims to divide an application into distinct sections, each addressing a specific concern.  In this context, we separate:

* **Application Logic (`app.js`):** This file focuses on defining the Express application, including middleware, routes, error handling, and any application-specific logic.  It's responsible for *what* the application does.
* **Server Instantiation (`server.js`):** This file focuses on creating and starting the server, listening on a specific port, and handling server-level configurations.  It's responsible for *how* the application is served.
...
Notice that `app.js` defines the Express application but *doesn't* start the server.  Instead, it exports the `app` instance using `module.exports`. This is a crucial step that allows `server.js` to import and utilize the application.
