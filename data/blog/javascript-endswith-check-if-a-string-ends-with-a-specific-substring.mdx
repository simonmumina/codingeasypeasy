...
*   **`searchString`**: The substring to search for at the end of the string.  This is a *required* parameter.
*   **`length`** (Optional):  This parameter specifies the *length* of the string to search, treating the string as if it were only this long. If `length` is omitted, it defaults to the length of the entire string. This allows you to check substrings within a string, rather than just the very end.

## Return Value

The `endsWith()` method returns:

*   `true`: If the string *does* end with the specified `searchString`.
*   `false`: If the string *does not* end with the specified `searchString`.
...
*   **Empty String as `searchString`**:  `string.endsWith("")` will *always* return `true`, as any string implicitly ends with an empty string.
