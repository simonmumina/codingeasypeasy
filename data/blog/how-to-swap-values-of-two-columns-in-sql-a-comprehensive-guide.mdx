...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['SQL', 'Database', 'Column Swap', 'Update Statement', 'Data Manipulation', 'T-SQL', 'MySQL', 'PostgreSQL', 'SQL Server', 'Data Engineering']
draft: false
...
The above approach seems straightforward, but it's **incorrect**.  The second assignment (`column2 = column1`) will use the *newly updated* value of `column1`, not the original value. Therefore, both `column1` and `column2` will end up with the same value (initially the value of `column2`).
...
While you can't directly *update* column values using a CTE, you *can* use them to *select* the swapped values, which can be useful for creating views or reporting:
