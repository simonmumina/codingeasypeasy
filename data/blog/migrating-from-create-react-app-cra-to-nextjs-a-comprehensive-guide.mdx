...
*   **`getServerSideProps`:**  Fetch data on every request.  This is useful for dynamic content that needs to be updated frequently.  This function runs *only* on the server.
...
*   **`getStaticProps`:** Fetch data at build time.  This is ideal for content that doesn't change often. This function also runs *only* on the server.
