...
date: '2024-10-27'
lastmod: '2024-10-28'
tags: ['react-native', 'touchablewithoutfeedback', 'touchable', 'ui-development', 'mobile-development', 'accessibility', 'react-native-components']
draft: false
...
## When *Not* to Use TouchableWithoutFeedback
...
*   **Accessibility Concerns:** Without visual feedback, it can be difficult for users with visual impairments or cognitive disabilities to understand if an element is interactive.  You *must* provide alternative feedback mechanisms (e.g., screen reader announcements) when using `TouchableWithoutFeedback`.
...
*   **`onPress`:** (Function) Called when the touch is released, but *not* if cancelled (like when a scroll view begins scrolling). This is the primary handler for tap events.
...
*   **Lack of Any Feedback:** Avoid creating touchable areas with *no* visual or auditory feedback.  Users need to know when they've interacted with an element.
