...
*   **`fs.access()`:**  The code now uses `fs.access()` to check if the file exists *before* attempting to download it. This prevents errors being thrown by `res.download()` if the file is missing. The `fs.constants.F_OK` flag checks for the file's existence.  Other flags can check for read or write permissions.
