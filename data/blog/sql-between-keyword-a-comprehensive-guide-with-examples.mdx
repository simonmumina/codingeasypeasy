...
**Important:** The `BETWEEN` operator is *inclusive*, meaning that it includes both the `value1` and `value2` in the result set.
...
The query selects all products from the `Products` table where the `Price` is between $50 and $200, *inclusive*.

## Using NOT BETWEEN

To select values *outside* a specific range, you can use the `NOT BETWEEN` operator:
...
This query will return all products *except* those with prices between $50 and $200:
