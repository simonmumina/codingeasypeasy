...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['bootstrap-vue', 'accessibility', 'screen-readers', 'vue.js', 'a11y', 'modals', 'web-development']
draft: false
...
*   **`triggeringElement`:** We store the currently focused element *before* the modal opens. This allows us to return focus to the correct element after the modal is closed.  The `document.activeElement` property gives us the currently focused element.
*   **`onShown()`:**  This method is called when the modal becomes visible.
    ...
    *   If *no* focusable elements are found, the modal container itself is focused. This is why we added  `this.$el.setAttribute('tabindex', '0');` in the `mounted()` lifecycle hook.  Making the modal container focusable ensures *something* inside the modal receives focus.
...
*   **`trapFocus(e)`:** This method handles keyboard navigation and the Escape key.
    ...
    *   If the user is tabbing forward (Shift + Tab not pressed) and focus is on the *last* focusable element, it moves focus back to the *first* focusable element.
    *   If the user is tabbing backward (Shift + Tab pressed) and focus is on the *first* focusable element, it moves focus to the *last* focusable element.
...
Here, we're using a BootstrapVue `b-button` to trigger the modal.  We capture the button element using `this.$refs.triggerButton.$el` in the `showModal` method and store it in the `triggeringElement` data property *before* opening the modal. This ensures `onHidden` knows which button to return focus to.  Make sure your `ref` names are appropriate for your button!
...
*   **ARIA Labels for Interactive Elements:** If an interactive element lacks clear visible text, provide an ARIA label to describe its purpose.  For example:
    ...
    <b-button aria-label="Close Dialog" @click="$bvModal.hide('myModal')">
      <i class="fas fa-times"></i> <!--  An icon-only close button -->
    </b-button>
    ```
