...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['nodejs', 'mongodb', 'express', 'signup form', 'user authentication', 'password hashing', 'web development', 'javascript']
draft: false
...
1.  **Create `models/user.js`:**
    ...
    *   We use the `pre('save', ...)` middleware to hash the password *before* saving the user to the database. This ensures that passwords are never stored in plain text.  We use `bcrypt.genSalt(10)` to generate a salt (a random string added to the password before hashing) and `bcrypt.hash()` to hash the password.
