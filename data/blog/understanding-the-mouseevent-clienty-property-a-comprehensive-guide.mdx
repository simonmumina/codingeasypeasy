...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['javascript', 'mouseevent', 'clienty', 'dom', 'event handling', 'web development', 'frontend']
draft: false
...
This is crucial when you need the position relative to the *entire document*, not just the viewport.
...
*   **`clientY`:**  Vertical position relative to the *viewport*.  Doesn't account for scrolling.
*   **`screenY`:** Vertical position relative to the *user's screen*.  This is typically rarely used in web development.
*   **`pageY`:** Vertical position relative to the *entire document*. This accounts for scrolling.  `pageY` may not be supported in older browsers, which is why we might manually calculate it using `clientY + window.pageYOffset` (as shown in example 3).
