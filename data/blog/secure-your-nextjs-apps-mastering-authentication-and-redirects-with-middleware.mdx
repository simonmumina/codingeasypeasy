...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['nextjs', 'middleware', 'authentication', 'redirects', 'security', 'javascript', 'web development', 'next-auth', 'edge functions']
draft: false
...
2.  **Define Your Middleware Function:**
    ...
    *   **`config.matcher`**: This is an essential part.  It allows you to specify which paths your middleware should run for. Without a `matcher`, the middleware will run for *every* request, which can impact performance.
...
export function middleware(request: NextRequest) {
  ...
  if (authToken && !protectedRoutes.includes(pathname) && !publicRoutes.includes(pathname) && pathname !== "/") {
...
5.  **Redirection Logic:**
    ...
    *   If a user is *not* authenticated and tries to access a protected route (e.g., `/profile`), they are redirected to the `/login` page.  The original path (`pathname`) is passed as a query parameter (`redirect`) so that after successful login, the user can be redirected back to the originally requested protected page.
...
export function middleware(request: NextRequest) {
  ...
  const preferredLanguage = request.headers.get('accept-language')?.split(',')[0]?.substring(0, 2) || defaultLanguage;
