...
*   **`document.getElementsByClassName(className)`:** Returns an HTMLCollection of all elements with the specified class name.  An HTMLCollection is *live* - meaning it updates automatically if the DOM changes.
...
*   **`document.querySelector(selector)`:** Returns the *first* element that matches a specified CSS selector. Uses standard CSS selector syntax.
...
*   **`document.querySelectorAll(selector)`:** Returns a NodeList of all elements that match a specified CSS selector. A NodeList is *static* - meaning it doesn't update automatically if the DOM changes.
