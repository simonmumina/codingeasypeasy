...
const mutations = {
  SET_ITEMS(state, { items, totalRows }) { // Expect server to return items and totalRows
    state.items = items;
...
*   **`fetchItems` action:**
    ...
    *   Expects the server to return the *paginated* `items` *and* the `totalRows` (the total number of items *before* pagination).
