...
date: '2024-10-27'
lastmod: '2024-10-28'
tags: ['git', 'version control', 'amend', 'commit', 'history', 'tutorial', 'best practices', 'command line']
draft: false
...
`git amend` is a powerful Git command that allows you to modify the *most recent* commit.  Think of it as a "redo" button for your last commit. It's not just about fixing typos in your commit message; you can also add or remove staged changes, essentially rewriting the history of that single commit.

**Important Note:**  Amending a commit creates a *new* commit with a different SHA-1 hash.  This means that if you've already pushed the original commit to a remote repository, amending it and pushing the changes will require a force push, which can be disruptive if others are collaborating on the same branch. Use with caution, especially when working on shared branches.
...
Another powerful feature of `git amend` is the ability to add changes that you forgot to stage *before* your last commit.
...
You can use `git add -p` (or `git add --patch`) *before* amending to selectively stage parts of changes, giving you even more control.  This is helpful when you've made multiple changes to a single file, but only want to include some of them in the amended commit.
...
You can also *remove* changes from the last commit using `git reset HEAD~`:
