...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['html5 canvas', 'canvas scale', 'canvas transformation', 'javascript graphics', 'canvas tutorial', 'canvas animation']
draft: false
...
**Important Note:** `scale()` affects *all* subsequent drawing operations. Think of it as transforming the canvas's coordinate system, not just the shapes you're drawing.
...
2.  We draw a blue rectangle at coordinates (50, 50) with a width of 100 and a height of 50. This is *before* scaling.
3.  We call `ctx.scale(2, 2)`, which scales the entire coordinate system.  Now, 1 unit in the canvas corresponds to 2 real pixels.
4.  We draw a red rectangle at the *same* coordinates (50, 50) with the same dimensions (100, 50). However, because the canvas is scaled, this rectangle will appear larger.  It will be twice as wide and twice as tall as the original blue rectangle.
...
It's crucial to understand that `scale()` doesn't just change the size of the shape. It transforms the entire coordinate system. This affects everything that's drawn *after* the `scale()` call.  Coordinates are relative to the *transformed* coordinate system.
...
The `scale()` method is often used in conjunction with `translate()` to position scaled objects correctly. Remember that `scale()` transforms the coordinate system *before* the object is drawn.
