...
date: '2024-01-05'
lastmod: '2024-01-05'
tags: ['express-validator', 'express', 'node.js', 'validation', 'float validation', 'input validation', 'backend development', 'javascript']
draft: false
...
Here, we validate the `amount` field using the French locale (`fr-FR`), where commas are used as decimal separators.  Crucially, we *replace* the comma with a period before using `parseFloat()` because `parseFloat()` in Javascript expects a period.  **Note:**  Always be careful about locale-specific parsing and ensure you handle the conversion to a standard format correctly before using the data.
...
`express-validator` also provides sanitization functions to clean up user input. Sanitization is distinct from validation.  While validation verifies that the input *conforms* to a set of rules, sanitization modifies the input to make it safe or consistent.

While `express-validator` doesn't offer a specific sanitizer dedicated *only* to ensuring a string *is* a float, you can use the `toFloat()` sanitizer in conjunction with the `isFloat()` validator.  The `toFloat()` sanitizer attempts to convert the input value to a float number.  If the input cannot be converted to a float, it will become `NaN`.  Combining this with `isFloat()` gives the desired effect of "float validation only":
...
In this example, if the `amount` input cannot be converted to a float, `toFloat()` will set it to `NaN`, and the subsequent `isFloat()` validator will then fail.  This combination effectively enforces that the input *must* be a valid float string.  The value in `req.body.amount` will now be the *float* value, not a string.
