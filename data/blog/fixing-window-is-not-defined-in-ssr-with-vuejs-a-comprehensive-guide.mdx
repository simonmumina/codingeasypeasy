...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['vuejs', 'ssr', 'server-side-rendering', 'javascript', 'window is not defined', 'nuxt', 'vue-server-renderer', 'vue', 'javascript error', 'hydration']
draft: false
...
export default {
  ...
  mounted() {  // Use mounted instead of created. mounted only runs client-side
    if (process.client) {
      this.windowWidth = window.innerWidth;
    }
  },
