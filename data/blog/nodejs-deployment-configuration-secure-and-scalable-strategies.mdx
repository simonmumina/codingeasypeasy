...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['nodejs', 'deployment', 'configuration', 'environment variables', 'secrets management', 'docker', 'ci/cd', 'dotenv', 'config']
draft: false
...
4.  **Use the `config` package in your code:**
    ...
    app.get('/', (req, res) => {
      res.send(`Hello! Connecting to database at ${databaseHost}:${config.get('database.port')}/${databaseName} in debug mode: ${debug}`);
    });
