...
*   If the user clicks "OK", the function returns `true`, allowing the reset operation to proceed. If the user clicks "Cancel", the function returns `false`, preventing the reset.  Returning `false` from an `onreset` event handler *cancels* the reset operation.
