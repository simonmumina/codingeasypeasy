...
1.  **Hooks can only be called *inside* React function components or custom Hooks.**  You cannot call them in regular JavaScript functions, class components, or outside the scope of a React component.

2.  **Hooks must be called at the *top level* of a React function component.**  You cannot call them inside loops, conditions (if/else statements), or nested functions.  The order of hook calls must remain consistent across renders.
...
2.  **Conditional Hook Calls:**  Hooks must be called in the same order on every render. Conditional logic that skips a hook call will lead to inconsistencies.
    ...
    **Solution:** Always call hooks unconditionally at the top level.
    ...
    function MyComponent({ showCount }) {
      ...
      return (
        <div>
          {showCount ? <div>Count: {count}</div> : <div>No Count</div>}
        </div>
      );
    }
    ```
...
5.  **Mixing Client and Server Components (Next.js, Remix):** When working with frameworks like Next.js or Remix, which support both server and client components, it's important to understand where hooks can be used.
    ...
    *   **Hooks *cannot* be used in server components.** Server components run on the server and do not have access to browser APIs or the client-side React context.
