...
*   `disconnect()`:  Closes the WebSocket connection using the `websocketService.disconnect()` method and updates the `connectionStatus`.  Crucially, it also *removes* the `message` event listener using `this.websocket.off('message', this.handleMessage)`.
...
*   `mutations`:  Define synchronous functions that modify the state.  `addMessage` adds a new message to the `messages` array, and `clearMessages` clears the array.  **Mutations are the *only* way to modify the state directly.**
...
// In your component's methods:
  async connect() {
    try {
      ...
      this.connectionStatus = 'Error connecting';
      console.error('Failed to connect:', error);
      this.permanentConnectionFailure = true; // Add this line
    }
  },
```
...
<template>
  <div class="hello">
    ...
    <p v-if="permanentConnectionFailure" style="color: red;">
      Connection to the server could not be established. Please check your internet connection and server status.
    </p>
