...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'html', 'dom', 'insertAdjacentElement', 'web development', 'dynamic html', 'dom manipulation']
draft: false
...
`insertAdjacentElement()` is a method available on HTML DOM elements that allows you to insert a new element relative to a specific element in the DOM tree. Unlike methods like `appendChild()` which only adds elements as the *last* child of a parent element, `insertAdjacentElement()` gives you fine-grained control over *where* the new element is inserted.
...
This code will insert the `newElement` *before* the `myElement` in the DOM:
...
This code will insert the `newElement` *after* the `myElement` in the DOM:
...
This code will insert the `newElement` as the *first* child of `myElement`:
...
This code will insert the `newElement` as the *last* child of `myElement`:
...
*   **`appendChild()`**: Adds a node as the *last* child of a parent. Simpler for basic cases, but less control.
*   **`insertBefore()`**: Inserts a node *before* a specified child of a parent. More verbose than `insertAdjacentElement()`.  Requires you to know the child element you're inserting before.
