...
**Important Note:** When using the global flag (`g`), you *must* use a `while` loop (or similar looping construct) to repeatedly call `exec()` to find all matches. If you only call `exec()` once, you will only find the first match.  Also, remember to create the RegExp object *outside* of the loop; otherwise, you'll reset the `lastIndex` on each iteration.
...
*   **Global Flag (`g`):** When the `g` flag is used:
    *   `match()` returns an array of all matched substrings directly.  It does *not* include information about captured groups or the index of each match.
...
*   **No Global Flag:** When the `g` flag is *not* used:
