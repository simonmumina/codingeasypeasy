...
3.  **CSS:**
    ...
    *   `.item.ng-enter-active` is the class that's added after a brief delay.  It's active *during* the transition. We set its `opacity` to 1.  The `transition` property on the `.item` class will then smoothly transition the opacity from 0 to 1 over 0.5 seconds.
...
angular.module('myApp', ['ngAnimate'])
  .controller('MyController', function($scope, $animate) {
    $scope.items = [];
    $scope.addItem = function() {
      var newItem = angular.element('<div class="item">' + ('Item ' + ($scope.items.length + 1)) + '</div>');
      angular.element(document.querySelector('[ng-repeat="item in items"]')).parent().append(newItem);
      $animate.enter(newItem, angular.element(document.querySelector('[ng-repeat="item in items"]')).parent());
...
1.  **JavaScript:**
    ...
    *   The `done` callback is called when the animation is complete. This is *critical* for AngularJS animations.  AngularJS uses this to track the state of animations.
...
<body>
  <div ng-controller="MyController">
    ...
    <div class="my-element" ng-class="{ 'visible': isVisible }">
