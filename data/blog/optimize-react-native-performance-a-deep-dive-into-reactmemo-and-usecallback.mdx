...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react-native', 'performance', 'optimization', 'react.memo', 'useCallback', 'memoization', 'hooks']
draft: false
...
When a component wrapped in `React.memo` receives new props, it performs a *shallow comparison* of the current props with the previous props. If the props are the same (according to shallow equality), the component will not re-render, and the cached result from the previous render will be used.
...
Even though `ExpensiveComponent` is wrapped in `React.memo`, it will re-render *every time* `ParentComponent` re-renders. Why? Because `setData({ value: data.value + 1 })` creates a *new* `data` object each time, and shallow equality will always see it as different.  Similarly, a new `onUpdate` function is being created on every render of `ParentComponent`.
