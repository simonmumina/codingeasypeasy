...
date: '2024-02-29'
lastmod: '2024-03-08'
tags: ['nextjs', 'server actions', 'mutations', 'react', 'data fetching', 'web development', 'form handling', 'optimization', 'best practices']
draft: false
...
*   `'use server';`:  This directive *inside* the `handleLike` function tells Next.js to execute the code within that function on the server.
...
*   **Client Component Required:**  Server Actions *must* be invoked from within a client component. You can't directly use them in server components.
...
*   `'use server';` is placed at the *top* of the `actions.js` file.  This marks *all* exports from that file as Server Actions.
