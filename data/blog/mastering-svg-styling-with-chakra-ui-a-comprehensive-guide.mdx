...
*   **Caution with `<style>`:**  While the above *looks* like it should work, using `<style>` tags *directly* inside a React component and relying on CSS classes alone can sometimes be problematic, especially with CSS-in-JS solutions like Chakra UI. Chakra UI encourages the use of the `sx` prop for styling. The `sx` prop will provide more reliable results.
...
function MyIcon({ color = "gray.500", size = "24px" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M12 2L2 7L12 12L22 7L12 2ZM2 17L12 22L22 17M2 12L12 17L22 12" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
}
