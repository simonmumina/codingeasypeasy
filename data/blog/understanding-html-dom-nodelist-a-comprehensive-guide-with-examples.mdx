...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['HTML DOM', 'NodeList', 'JavaScript', 'Web Development', 'DOM Manipulation', 'HTML', 'getElementsByTagName', 'querySelectorAll']
draft: false
...
*   **Live vs. Static:** This is perhaps the *most important* aspect of `NodeList` to understand. Some `NodeList`s are *live*, meaning they automatically update to reflect changes in the DOM. Others are *static*, meaning they represent a snapshot of the DOM at the time the `NodeList` was created and do not update.

    *   `getElementsByTagName()` and `getElementsByClassName()` generally return *live* `NodeList`s.
    *   `querySelectorAll()` generally returns a *static* `NodeList`.
    *   `element.childNodes` generally returns a *live* `NodeList`.
...
*   **Not a True Array:**  While you can access elements by index, a `NodeList` is *not* a true JavaScript array. This means you can't directly use array methods like `forEach`, `map`, `filter`, or `reduce` on it without converting it.
...
In this example, `paragraphs` is a *live* `NodeList`.  When we add a new paragraph to the container, the `length` property of the `NodeList` automatically updates.
...
Here, `items` is a *static* `NodeList`.  Adding a new paragraph with the class `item` *does not* change the length of the `NodeList` because it's a snapshot taken at the time `querySelectorAll` was called.
...
**d. Using `forEach` directly (available on *some* NodeLists, but not universally reliable before ES6):**

While *some* implementations of `NodeList` support `forEach` directly, it's best practice to avoid relying on it for maximum compatibility, especially with older browsers. It's always safer to convert the `NodeList` to an array.
...
The `childNodes` property returns a *live* `NodeList` of all the direct child nodes of an element, including text nodes and comment nodes.
