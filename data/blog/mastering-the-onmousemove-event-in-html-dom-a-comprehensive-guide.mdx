...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'html', 'dom', 'events', 'onmousemove', 'mouse events', 'event listeners', 'web development']
draft: false
...
The `onmousemove` event fires every time the mouse pointer moves while it's within the boundaries of an element.  This allows you to continuously track the mouse's position and trigger actions based on its movement.  It's important to remember that the event only fires while the mouse is *over* the element to which the event listener is attached.
...
*   **`event.clientX`:** The horizontal coordinate of the mouse pointer, relative to the *viewport* (the visible area of the browser window).
*   **`event.clientY`:** The vertical coordinate of the mouse pointer, relative to the *viewport*.
*   **`event.screenX`:** The horizontal coordinate of the mouse pointer, relative to the *screen*.
*   **`event.screenY`:** The vertical coordinate of the mouse pointer, relative to the *screen*.
*   **`event.offsetX`:** The horizontal coordinate of the mouse pointer, relative to the *target element*.
*   **`event.offsetY`:** The vertical coordinate of the mouse pointer, relative to the *target element*.
*   **`event.pageX`:** The horizontal coordinate of the mouse pointer, relative to the *entire document*.
*   **`event.pageY`:** The vertical coordinate of the mouse pointer, relative to the *entire document*.
