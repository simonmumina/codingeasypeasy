...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['HTML Canvas', 'Canvas API', 'JavaScript Graphics', 'HTML5', 'Web Development', 'Interactive Graphics', 'Game Development', 'Animation', 'Data Visualization']
draft: false
...
**Important:**  Always set the width and height attributes directly in the HTML tag. Using CSS to resize the canvas can distort the drawn content because it scales the *rendered* output, not the underlying drawing surface.
...
if (ctx) {
  ...
  console.error("Canvas context failed to initialize.  Check your browser's support for the Canvas API.");
}
```
...
*   **`img.onload`**:  A function that executes when the image has finished loading.  **Crucially important:** You should only draw the image to the canvas *after* it has loaded.
