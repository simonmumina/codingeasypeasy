...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['fastly', 'compute-at-edge', 'flask', 'cdn', 'stale-while-revalidate', 'performance', 'edge-computing', 'python']
draft: false
...
This configuration means that the first request to `/data` will be served from the origin server. Subsequent requests within the first 60 seconds will be served from the cache.  After 60 seconds, the cache will serve the *stale* content, but it will simultaneously make a background request to the origin server to update the cache with the latest data.  For up to 1 day after the initial cache, if the origin server isn't responding, the stale content will continue to be served.
...
3.  **Requests After 60 seconds:**  After 60 seconds, refresh the page again.  You should see the *old* data served almost instantly (from the stale cache), but the data will be updated in the background. Refresh again shortly after, and you should see the *new* data.
