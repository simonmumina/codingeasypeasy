...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'dom events', 'event handling', 'event listeners', 'html events', 'javascript tutorial', 'web development', 'event bubbling', 'event capturing', 'prevent default']
draft: false
...
1.  **Capturing Phase:** The event travels *down* the DOM tree, starting from the window and moving towards the target element.  Event listeners attached in the capturing phase are executed *before* listeners attached directly to the target element.  This phase is less commonly used.

2.  **Bubbling Phase:** The event travels *up* the DOM tree, starting from the target element and moving towards the window. Event listeners attached in the bubbling phase are executed *after* listeners attached directly to the target element. This is the default behavior.
...
*   **`event.stopImmediatePropagation()`:** This method prevents further listeners from being executed on the *same* element, in addition to stopping the event from bubbling/capturing.
...
In this example, clicking the button will trigger the button's click handler and the inner div's click handler. However, because `event.stopPropagation()` is called in the inner div's handler, the outer div's click handler will *not* be executed.
...
Now, when you click the button, the outer div's click handler (in the capturing phase) will be executed *before* the button's click handler.
