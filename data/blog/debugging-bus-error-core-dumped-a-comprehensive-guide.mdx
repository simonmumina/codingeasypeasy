...
date: '2024-01-01'
lastmod: '2024-01-05'
tags: ['bus error', 'core dumped', 'debugging', 'c++', 'segmentation fault', 'linux', 'gdb', 'valgrind', 'memory management']
draft: false
...
A bus error, in essence, is a signal sent by the operating system to a process indicating that the process attempted to access memory in a way that the CPU's bus architecture doesn't allow. Unlike a segmentation fault, which typically stems from attempting to access memory the process doesn't own, a bus error usually indicates an *alignment* issue or a problem with the *physical* interface to memory.
...
2.  **Incorrect Pointer Arithmetic:**  Mistakes in pointer arithmetic can lead to accessing memory outside the allocated region, sometimes resulting in misaligned addresses.
    ...
    **Example (C):**
    ...
    Here, we allocate space for 5 integers but then try to access the 7th integer (index 6), resulting in memory corruption or a bus error.  Note that out-of-bounds access can *also* lead to segmentation faults.
