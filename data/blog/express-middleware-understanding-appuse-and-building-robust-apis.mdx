...
In this example, when a request is made to `/error`, an error is thrown.  The `next(error)` call passes the error to the error-handling middleware.  The error handler logs the error stack trace and sends a 500 Internal Server Error response to the client. **Error-handling middleware *must* be defined after all other route handlers and middleware**.
