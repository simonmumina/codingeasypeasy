...
When you run `npm install <package-name>`, npm downloads the specified package and its dependencies and places them in your project's `node_modules` directory.  Crucially, to know *which* packages to install, npm reads the `package.json` file located in your project's root directory.
...
The `npm install --save <package-name>` command installs the specified package and, more importantly, adds it as a *dependency* to the `dependencies` section of your `package.json` file.  This signifies that the package is **essential** for your application to run in production.
...
The `npm install --save-dev <package-name>` command installs the specified package and adds it as a *development dependency* to the `devDependencies` section of your `package.json` file. This signifies that the package is **only needed during the development process** and is not required for the application to run in production.
