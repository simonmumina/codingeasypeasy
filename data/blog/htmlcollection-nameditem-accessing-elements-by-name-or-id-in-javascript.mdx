...
Before diving into `namedItem()`, let's quickly review what an `HTMLCollection` is. An `HTMLCollection` is an *array-like* object representing a collection of HTML elements. It's *live*, meaning it automatically updates as the underlying DOM changes. Common ways to get an `HTMLCollection` include:

*   `document.getElementsByTagName()`
*   `document.getElementsByClassName()`
*   `element.children`

Unlike a true JavaScript array, you *cannot* use array methods like `forEach()` or `map()` directly on an `HTMLCollection`. You need to convert it to an array first or iterate using a traditional `for` loop.
...
*   `namedItem()` searches for elements with *either* a matching `name` or a matching `id` attribute.
*   If multiple elements have the same `name` or `id`, `namedItem()` will return the *first* matching element in the collection.
*   The search order prioritizes elements with a matching `name` attribute before searching for elements with a matching `id` attribute *if the collection is a `NamedNodeMap`*.  In the context of `HTMLCollection` resulting from `getElementsByName` `namedItem` will function as you would expect it to.
