...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react-native', 'react-query', 'data-fetching', 'mobile-development', 'caching', 'state-management']
draft: false
...
const AppWrapper = () => {
  return (
    <SafeAreaProvider> {/* Optional, but useful for safe area handling */}
...
*   `useQuery('todos', fetchTodos)` is the core of our data fetching.
    *   `'todos'` is the *query key*.  This key is crucial for React Query to identify and manage the query. Use a consistent key for the same data.
...
*   **`onMutate`:** This function is called *before* the mutation function is executed.
    ...
    *   `queryClient.setQueryData('todos', ...)`:  We *optimistically* update the cache.  We filter out the todo that's being deleted, effectively removing it from the UI *immediately*.
...
*   **Server State vs. Client State:**  React Query is best for managing *server state* (data that comes from an API).  For managing local UI state (e.g., the state of a form), consider using React's built-in state management tools (`useState`, `useReducer`) or a dedicated client-side state management library like Zustand or Jotai.
...
const AppWrapper = () => {
    ...
    return (
        <SafeAreaProvider>
            <PersistQueryClientProvider client={queryClient} persistOptions={{ persistStorage: asyncStoragePersister }}>
                <App />
