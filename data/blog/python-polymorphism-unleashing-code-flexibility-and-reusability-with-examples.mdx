...
**Inheritance:** Inheritance allows you to create new classes (child classes) that inherit properties and methods from existing classes (parent classes). This establishes an "is-a" relationship between the classes. For example, a `Car` _is-a_ `Vehicle`.
