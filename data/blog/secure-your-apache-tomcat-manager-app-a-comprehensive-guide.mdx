...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['Tomcat', 'Security', 'Java', 'Web Security', 'Manager App', 'Web Application Security', 'Apache']
draft: false
...
While not directly configurable in `tomcat-users.xml` (which is usually based on the `MemoryRealm`), here's a conceptual example of how you *might* integrate a custom Realm that uses a stronger hashing algorithm (assuming you've written the `MySecureRealm` class):
...
*   **Firewall:** Consider using a firewall (e.g., `iptables` on Linux) to further restrict access to the Tomcat server.  This provides an additional layer of defense.  A firewall is *more* robust than relying solely on `RemoteAddrValve` as it operates at the network layer.
...
2.  **Update the `<Context>` in `server.xml`:**  If you have a `<Context>` element defined for the Manager application in `$CATALINA_HOME/conf/server.xml`, update the `path` attribute to match the new directory name.  If there *isn't* a `Context` defined, don't create one.
