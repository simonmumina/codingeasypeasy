...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['nextjs', 'server-side rendering', 'ssr', 'react', 'performance', 'seo', 'getserversideprops']
draft: false
...
Next.js simplifies SSR through the `getServerSideProps` function. This asynchronous function is executed on the server for each request, allowing you to fetch data and pass it as props to your page component.  It runs *only* on the server and never on the client, guaranteeing data security.
...
*   `getServerSideProps` can *only* be exported from a **page**.  It cannot be used within regular React components.
*   You cannot use `useEffect` or other client-side lifecycle methods inside `getServerSideProps` as it executes *solely* on the server.
