...
3.  **Create Stack Navigators for each screen:** Each screen in the drawer uses a Stack Navigator.  This allows you to have a header and push other screens onto the stack within that section of the drawer. This is important as otherwise you cannot navigate *within* each drawer item without replacing the whole drawer itself.  We are also adding a button to each header to open the drawer if the user doesn't swipe.
