...
*   If the tag is *not* found, a new meta tag is created and appended to the `head`. This is important because many SPAs might not include all the necessary Open Graph meta tags initially.
...
*   **Dynamic Viewport Settings:** While less common, you could theoretically adjust the viewport meta tag based on specific user preferences or device characteristics. *However, this is generally discouraged as it can lead to inconsistent user experiences.*
...
*   The cleanup function within the `useEffect` hook resets the title and description to default values when the component unmounts. This is important to prevent incorrect metadata from persisting on other pages.  *Note: A real implementation would likely use more sophisticated logic to determine the default description.*
