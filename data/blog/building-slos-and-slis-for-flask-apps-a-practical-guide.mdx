...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['flask', 'slo', 'sli', 'sre', 'monitoring', 'observability', 'python', 'web development', 'performance monitoring', 'service level objectives', 'service level indicators']
draft: false
...
*   **Service Level Objective (SLO):**  An SLO is a target level of performance for a service, expressed as a percentage.  It represents the level of service you *aim* to provide to your users.  For example, you might have an SLO stating that your API will respond to requests with a latency of less than 200ms 99.9% of the time.  SLOs are promises you make to your users (and yourself!) about the quality of your service.

*   **Service Level Indicator (SLI):** An SLI is a metric used to measure the performance of a service. It quantifies the *actual* performance against the SLO. For the SLO example above, the SLI would be the percentage of requests that are served with a latency of less than 200ms over a given period (e.g., a month).
...
In essence, you *define* an SLO, *measure* an SLI, and *promise* an SLA.
