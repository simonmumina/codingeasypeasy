...
export class MlService {
  ...
  async onModuleInit() {
    try {
      // Load the model
      this.model = await tf.loadGraphModel('file://' + path.join(process.cwd(), 'public', 'model', 'model.json'));
...
```typescript
import { Controller, Get, Post, Body, HttpException, HttpStatus, UsePipes, ValidationPipe, Logger } from '@nestjs/common';
...
export class MlController {
  ...
  async predict(@Body() predictDto: PredictDto): Promise<any> {
    try {
      ...
      this.logger.log(`Prediction requested with input: ${JSON.stringify(predictDto.inputData)}, Result: ${JSON.stringify(predictionData)}`);
