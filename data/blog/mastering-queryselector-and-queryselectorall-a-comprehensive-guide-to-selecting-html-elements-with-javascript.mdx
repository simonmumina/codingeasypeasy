...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'dom manipulation', 'queryselector', 'queryselectorall', 'html', 'web development', 'front end']
draft: false
...
*   **`querySelector(selector)`:**  Returns the *first* element within the document that matches the specified CSS selector. If no element matches, it returns `null`.

*   **`querySelectorAll(selector)`:** Returns a `NodeList` containing *all* elements within the document that match the specified CSS selector.  If no elements match, it returns an empty `NodeList`.  A `NodeList` is *not* an array, but it is array-like and can be iterated over using a `for` loop or the `forEach` method.
