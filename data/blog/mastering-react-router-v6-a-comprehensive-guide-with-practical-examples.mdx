...
*   **`<Route path="/dashboard" element={<Dashboard />}>`:**  This defines the parent route for the dashboard. The children routes are defined *inside* this tag.
...
*   **`<Route path="*" element={<NotFound />} />`:** This route uses the wildcard character (`*`) to match any URL that doesn't match any of the other routes. This ensures that the `NotFound` component is rendered for any invalid URL. **It must be the *last* route defined in the `<Routes>` component.**
