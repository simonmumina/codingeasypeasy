...
date: '2024-02-29'
lastmod: '2024-02-29'
tags: ['http cookies', 'cookies', 'nodejs', 'express', 'web development', 'session management', 'authentication', 'javascript']
draft: false
...
*   **`Expires` / `Max-Age`**:  Defines the lifetime of the cookie. `Expires` specifies a date and time when the cookie should expire.  `Max-Age` specifies the number of seconds after which the cookie should expire.  If neither is specified, the cookie is a *session cookie* and will be deleted when the browser is closed.
...
3.  **Configure `express-session` middleware:**
    *   `secret`:  A secret key used to sign the session ID cookie.  **Important:** Replace `'your-secret-key'` with a strong, randomly generated secret.  Do *not* hardcode this in your application; use environment variables.
