...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react native', 'graphql', 'apollo client', 'relay', 'mobile development', 'javascript', 'data fetching']
draft: false
...
import { View, Text, ActivityIndicator, StyleSheet, ScrollView, TextInput, Button } from 'react-native';
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.createPostContainer}>
        ...
        {mutationError && <Text style={styles.errorText}>Error creating post: {mutationError.message}</Text>}
      </View>
      {data.posts.map((post) => (
        <View key={post.id} style={styles.postContainer}>
          <Text style={styles.postTitle}>{post.title}</Text>
...
function fetchQuery(
  ...
  return fetch('http://localhost:4000/graphql', { // Replace with your GraphQL endpoint
    method: 'POST',
    headers: {
