...
date: '2024-01-26'
lastmod: '2024-01-27'
tags: ['node.js', 'socket.io', 'realtime', 'chat application', 'javascript', 'web development', 'tutorial', 'guide', 'coding']
draft: false
...
*   **`io.emit('chat message', msg)`:** This is crucial. It *broadcasts* the received message to *all* connected clients.  This is what makes the chat application real-time.
...
4.  **`public/script.js`:**
    ...
    *   **`socket.on('chat message', (msg) => { ... })`:** This listens for the `chat message` event *from the server*.  When a message is received, it creates a new list item and appends it to the `messages` list.  The `window.scrollTo` line keeps the chat scrolled to the bottom so you always see the latest messages.
...
4.  **Server Receives and Broadcasts:** The server's `socket.on('chat message', (msg) => { ... })` block receives the message.  Crucially, it then broadcasts the message to *all* connected clients using `io.emit('chat message', msg)`.
