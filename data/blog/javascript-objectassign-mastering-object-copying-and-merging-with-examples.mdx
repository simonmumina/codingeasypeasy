...
*   **`target`:** The target object, to which the source properties will be copied.  This object *will be mutated*.
...
This creates a new object `newObject` that contains all the properties from `obj1` and `obj2` without modifying the original objects. **Important:** This is a *shallow* copy, as we'll discuss next.

## Shallow Copy vs. Deep Copy

`Object.assign()` performs a **shallow copy**.  This means that it copies the *references* to object properties, not the values themselves.  If a property is a primitive (string, number, boolean, null, undefined, symbol), the value is copied.  However, if a property is an object or array, the *reference* to that object or array is copied.
...
Even though we modified `obj1.b.c`, `obj2.b.c` also changed because `obj1.b` and `obj2.b` are references to the *same* object in memory.
...
`Object.assign()` is *not* suitable for deep copying. You'll need to use other methods:
...
`Object.assign()` copies both string-keyed and symbol-keyed properties. It also only copies *enumerable* properties.
