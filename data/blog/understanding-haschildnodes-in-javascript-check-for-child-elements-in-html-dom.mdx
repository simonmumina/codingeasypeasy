...
While `hasChildNodes()` only checks the *existence* of child nodes, you can combine it with other properties to get more information.  Here's how you can count the child nodes:
...
**Important Note:** The `childNodes` property returns *all* child nodes, including text nodes (representing whitespace), comment nodes, and element nodes. To only count *element* nodes, you would need to filter the `childNodes` list.
