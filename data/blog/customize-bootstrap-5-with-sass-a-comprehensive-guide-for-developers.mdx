...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['bootstrap 5', 'sass', 'css', 'frontend', 'web development', 'customization', 'theming', 'responsive design', 'variables', 'mixins']
draft: false
...
1.  **Import Bootstrap's Source Files:** In your `custom.scss` file, import Bootstrap's source files.  Crucially, you **must** import the `functions` file *before* the `variables` file to ensure your variable overrides take effect.  Then, import the `mixins` file if you plan to use them, followed by the rest of the bootstrap imports.
...
5.  **Include CSS in Your HTML:**  Link the compiled CSS file in your HTML document **after** the Bootstrap CSS file (if you are including the default bootstrap css).  However, in this customization guide, we are compiling *all* of Bootstrap, so you should *only* include your custom css file.
...
*   **SASS Modules:**  Utilize the `@use` rule in newer versions of SASS for better organization and encapsulation of styles. `@use` allows you to import another stylesheet as a *module*, preventing variable name collisions and improving code clarity.
