...
*   **During Page Load:** When `document.writeln()` is called during the initial parsing of the HTML document (i.e., within a `<script>` tag that's encountered while the browser is still building the DOM), it inserts content into the document stream *at the point where the script is encountered*. This is the most common and generally the least problematic use case (although still not ideal).

*   **After the Page Has Loaded:**  This is where `document.writeln()` can exhibit unexpected and potentially destructive behavior. If you call `document.writeln()` *after* the page has finished loading (e.g., within an event handler triggered by a button click), it will implicitly call `document.open()`, which *clears the entire existing document*. The new content written by `document.writeln()` will then become the *only* content in the document.
