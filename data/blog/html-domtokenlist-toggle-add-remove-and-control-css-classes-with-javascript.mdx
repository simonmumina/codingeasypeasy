...
1.  **`toggle(token)`**:  This form toggles the given *token* (CSS class name). If the *token* is present in the `DOMTokenList`, it's removed. If it's not present, it's added. The method returns `true` if the *token* is added and `false` if it's removed.

2.  **`toggle(token, force)`**: This form adds or removes the *token* based on the value of the optional `force` argument.  If `force` is `true`, the *token* is always added. If `force` is `false`, the *token* is always removed.  This version is particularly useful when you need explicit control over adding or removing a class.  The method also returns `true` if the token is added and `false` if it is removed.
