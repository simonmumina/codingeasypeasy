...
*   **`intersect`**: The masks are intersected. Only areas covered by *both* masks are visible.
...
This example is similar to the previous one, but we're using `[-webkit-mask-composite:intersect] [mask-composite:intersect]`.  This means only the area where *both* the red div *and* the mask overlap will be visible.  The rest will be hidden.
...
Here, we have two divs side-by-side, each with a different mask and background color.  `mask-composite:exclude` ensures that only the areas covered by *either* the triangle or the square mask, but *not both*, are visible.  This creates an interesting overlapping effect.
