...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['linux', 'permissions', 'rwx', 'chmod', 'command line', 'file security', 'unix', 'symbolic permissions']
draft: false
...
*   **Understanding Default Permissions (umask):**  The `umask` setting determines the default permissions for newly created files and directories.  It acts as a mask to *remove* permissions. You can check your `umask` value with the `umask` command.  The `umask` command is subtracted from the default file and directory permissions. The default file permissions are 666 (-rw-rw-rw-) and the default directory permissions are 777 (drwxrwxrwx).
