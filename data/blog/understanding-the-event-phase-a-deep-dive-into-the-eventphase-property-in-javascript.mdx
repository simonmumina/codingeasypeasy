...
1.  **Capturing Phase:** The event travels down the DOM tree from the window to the target element. Event listeners attached in the capturing phase are triggered *before* the event reaches the target element itself.
...
}, false); // Third argument 'false' or omitted enables bubbling (default)
...
}, true); // Capture all clicks from document level down
...
myLink.addEventListener('click', (event) => {
  if (event.eventPhase === 2) { // Only prevent default at the target
...
While `eventPhase` informs you about the *current* phase of event propagation, `stopPropagation()` is a method on the `Event` object that *prevents* further propagation of the event. When called, `stopPropagation()` prevents the event from continuing to bubble up (or capture down) the DOM tree, depending on the phase.
...
*   **Use capturing sparingly:** Capturing is less common than bubbling and can sometimes make event handling more complex.  Use it only when you need to intercept events *before* they reach the target element.
...
*   **Understand `preventDefault()` vs. `stopPropagation()`:**  `preventDefault()` prevents the *default* action associated with the event, while `stopPropagation()` prevents the event from propagating further up or down the DOM tree.  Choose the appropriate method based on your specific needs.
