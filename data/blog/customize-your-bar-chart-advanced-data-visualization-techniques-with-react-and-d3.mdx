...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['data visualization', 'bar chart', 'react', 'd3.js', 'customization', 'javascript', 'charting library']
draft: false
...
const BarChart = ({ data, width = 600, height = 400, margin = { top: 20, right: 30, bottom: 40, left: 40 } }) => {
  const svgRef = useRef();
...
return (
  <div>
    ...
    <div id="tooltip" style={{ position: 'absolute', background: 'white', border: '1px solid gray', padding: '5px', visibility: 'hidden' }}></div>
  </div>
);
```
...
*   **Descriptive Titles and Labels:** Provide clear and concise titles and axis labels to explain the chart's purpose.  Use the `aria-label` attribute on the SVG element to provide a screen reader-friendly description.
    ...
    ```jsx
    <svg ref={svgRef} width={width} height={height} aria-label="Bar chart showing values for different categories"></svg>
    ```
