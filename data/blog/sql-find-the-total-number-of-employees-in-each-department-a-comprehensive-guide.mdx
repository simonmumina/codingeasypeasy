...
date: '2024-10-27'
lastmod: '2024-10-28'
tags: ['SQL', 'Database', 'Querying', 'Employee Management', 'Department Analysis', 'GROUP BY', 'COUNT', 'SQL Tutorial', 'Data Analysis']
draft: false
...
*   **`LEFT JOIN departments d ON d.department_id = e.department_id`**:  A `LEFT JOIN` (or `LEFT OUTER JOIN`) ensures that *all* rows from the "left" table (in this case, `departments`) are included in the result, even if there's no matching row in the "right" table (`employees`).  If there's no matching employee for a department, the `employee_id` will be `NULL`.
