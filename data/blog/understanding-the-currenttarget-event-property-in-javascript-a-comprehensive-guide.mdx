...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'event-handling', 'dom', 'currenttarget', 'event-propagation', 'event-listeners']
draft: false
...
*   **`target`:** The `target` property represents the element that *triggered* the event. This is the originating element where the event occurred.
...
*   **Event Bubbling:**  Events "bubble up" from the target element to its parent elements, triggering any event listeners attached along the way.  In the bubbling phase, `currentTarget` will change as the event propagates up the DOM tree, always referring to the element whose event listener is currently being executed. `target` will *not* change; it will always refer to the original element that triggered the event.
...
*   Even though the buttons are added *after* the event listener is attached to the container, the event listener still works correctly.  When a dynamically added button is clicked, the `event.target` will be the clicked button, and `event.currentTarget` will be the `container` div.
...
Here, the event listener is attached to the `body` element. When a button inside the body is clicked, `event.target` will be the button, and `event.currentTarget` will be the `body` element.  This demonstrates how `currentTarget` always refers to the element that *owns* the listener.
