...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['expo', 'react-native', 'pedometer', 'mobile development', 'health tracking', 'motion sensor']
draft: false
...
5.  **useEffect Dependency:** The `useEffect` now depends on `hasPermission`.  This ensures that `getSteps` is called again *after* the permission is requested and the `hasPermission` state updates.  This avoids trying to get steps before permission is granted.
