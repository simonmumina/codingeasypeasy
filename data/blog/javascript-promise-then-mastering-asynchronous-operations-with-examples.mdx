...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'promise', 'then', 'asynchronous', 'es6', 'promise-chaining', 'error-handling', 'asynchronous-javascript']
draft: false
...
2.  The first `.then()` receives the data from `fetchData`, logs it, and then transforms it to uppercase using `toUpperCase()`.  It then *returns* the uppercase string. This returned string becomes the resolved value of the Promise returned by the first `.then()`.
...
3.  The first `.then()` receives the data from `fetchData` and *returns* the result of calling `processData(data)`.  This is crucial.  Since `processData(data)` returns a Promise, the `.then()` method *waits* for that Promise to resolve before moving on.
4.  The second `.then()` receives the *resolved value* of the `processData` Promise, logs it, and finishes the chain.
...
**Important:** A single `.catch()` block will catch any rejection that occurs *anywhere* in the Promise chain above it.
