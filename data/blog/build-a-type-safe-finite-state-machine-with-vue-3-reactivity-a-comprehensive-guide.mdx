...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['vuejs', 'finite state machine', 'typescript', 'reactive programming', 'state management', 'vue 3']
draft: false
...
export function useFSM(config: FSMConfig) {
  ...
  const transition = (event: Event, payload?: any) => {
    ...
    if (typeof nextState === 'function') {
      ...
      console.warn(`Invalid transition: Event '${event}' not allowed in state '${currentState.value}'`);
    }
  };
...
const fetchData = async () => {
  ...
  try {
    ...
    fsm.data.value = data; // Store the data in the reactive data store
    fsm.transition('DATA_RECEIVED');
  } catch (error: any) { // Type the error as any for simplicity
    const errorPayload: ErrorPayload = {
      type: error.message.includes('NetworkError') ? 'NETWORK_ERROR' : error.message.includes('Timeout') ? 'TIMEOUT_ERROR' : 'UNKNOWN_ERROR',
