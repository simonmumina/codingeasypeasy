...
Web workers are background scripts that run independently of the main browser thread.  They are essential for performing computationally intensive tasks without blocking the user interface.  Importantly, web workers do *not* have access to the `window` object. Instead, they have their own global scope represented by the `self` object.
