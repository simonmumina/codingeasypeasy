...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['react-native', 'performance', 'bottleneck', 'debugging', 'profiling', 'javascript', 'mobile-development']
draft: false
...
function MyComponent() {
  ...
  return (
    <Button title="Process Data" onPress={handleClick} />
  );
}
```
...
2.  **Configure Reactotron:**  Add Reactotron to your React Native project:
    ...
    if (__DEV__) {
      Reactotron
        .configure({ name: 'YourAppName' }) // controls connection & communication settings
...
*   **Prop Changes:**  A component re-renders when its props change.  Even if the prop value is the same, if the *reference* to the prop changes, the component will re-render.
