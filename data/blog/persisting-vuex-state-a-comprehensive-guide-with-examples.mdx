...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['vuex', 'vue js', 'state management', 'persistence', 'localstorage', 'sessionstorage', 'vuex plugins']
draft: false
...
2.  **Mutations to Set and Persist:**  The `setUsername` and `setTheme` mutations now *also* save the updated values to `localStorage`.
...
const persistState = (store) => {
  ...
  if (localStorage.getItem('my-app-state')) {
    try {
      store.replaceState(
        JSON.parse(localStorage.getItem('my-app-state'))
      );
    } catch (e) {
