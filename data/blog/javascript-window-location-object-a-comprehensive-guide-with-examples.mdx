...
You can modify the URL without triggering a full page reload using the `history.pushState()` or `history.replaceState()` methods, which are often used in single-page applications (SPAs).  This is *not* directly part of the `window.location` object, but it's essential to mention it when discussing URL manipulation.
