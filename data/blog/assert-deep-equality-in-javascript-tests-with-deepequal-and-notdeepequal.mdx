...
*   `expect(obj1).to.not.deep.equal(obj3)`: Asserts that `obj1` is *not* deeply equal to `obj3`.  This uses `notDeepEqual` assertion which we will cover in the next section.
...
*   `expect(obj1).not.toEqual(obj3)`: Asserts that `obj1` is *not* deeply equal to `obj3`.
...
The `notDeepEqual` assertion (or its equivalent in various testing frameworks) verifies that two objects or arrays are *not* deeply equal.  This is the logical opposite of `deepEqual`.
...
*   **Confusing shallow and deep equality:**  Using `===` to compare objects will only check if they are the *same object in memory*, not if they have the same content.
