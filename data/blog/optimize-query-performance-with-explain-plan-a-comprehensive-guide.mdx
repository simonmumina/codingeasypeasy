...
*   **`possible_keys`:**  The indexes that the database *could* use for this table.
...
*   **PostgreSQL:** PostgreSQL's `EXPLAIN ANALYZE` actually runs the query and provides runtime statistics, which are more accurate than the estimated costs.  Use with caution on production databases as it *will* execute the query.
