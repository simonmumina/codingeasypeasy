...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['vuejs', 'tree-shaking', 'component libraries', 'webpack', 'rollup', 'performance optimization', 'code splitting']
draft: false
...
3.  **Side Effects Annotations (package.json):** Component library authors should accurately specify side effects in their `package.json` file using the `sideEffects` property. This property informs the bundler about files that have side effects, meaning they *should not* be removed even if they appear unused.  A common example is CSS files that inject styles into the DOM. A value of `false` in `package.json` indicates that the package has no side effects and can be fully tree-shaken. Specific file patterns can also be used.
