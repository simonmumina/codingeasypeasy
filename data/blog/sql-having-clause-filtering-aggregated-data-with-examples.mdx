...
*   **`WHERE` clause:** Filters individual rows *before* any grouping or aggregation occurs. It operates directly on the data in the table.
*   **`HAVING` clause:** Filters groups *after* the `GROUP BY` clause has been applied and aggregation functions (like `SUM`, `AVG`, `COUNT`, `MIN`, `MAX`) have been calculated.
...
*   `WHERE`: An *optional* clause that filters rows before they are grouped.
*   `GROUP BY`: Groups rows based on one or more columns.
*   `HAVING`: Filters the grouped results based on a condition.  The condition *must* involve an aggregate function or a grouping column.
*   `ORDER BY`: An *optional* clause to sort the results.
...
Now, let's combine `WHERE` and `HAVING`.  Suppose we want to find customers who placed orders *after* January 1, 2024, and have a total quantity greater than 10.
...
1.  `WHERE OrderDate > '2024-01-01'`: This filters the *individual rows* before grouping. Only orders placed after January 1, 2024, are considered.
2.  `GROUP BY CustomerID`: Groups the remaining rows by `CustomerID`.
3.  `HAVING SUM(Quantity) > 10`: Filters the *grouped results*.  Only customers who meet the date criteria *and* have a total quantity greater than 10 are included in the final result.
...
(Note: This is the same result as Example 1, but it's only coincidentally the same.  If Customer 102 had placed a large order before 2024-01-01, but orders totaling only 5 after that date, they would *not* appear in this result).
...
*   **Using `WHERE` instead of `HAVING`:**  A common mistake is trying to use the `WHERE` clause to filter based on aggregated values.  `WHERE` filters rows *before* aggregation; `HAVING` filters groups *after* aggregation.  If you need to filter based on the result of an aggregate function, use `HAVING`.
