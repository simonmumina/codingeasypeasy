...
date: "2024-10-26"
lastmod: "2024-10-26"
tags: ["react", "tailwind css", "nextjs", "sidebar", "collapsible menu", "responsive design", "ui design", "frontend development"]
draft: false
...
const SidebarItem = ({ label, href, icon, children }) => {
  ...
  return (
    <li className="mb-2">
      {href ? (
        <Link href={href} className="flex items-center p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-white">
          {icon && <span className="mr-2">{icon}</span>}
          <span>{label}</span>
        </Link>
      ) : (
        <button onClick={toggleDropdown} className="flex w-full items-center p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-white">
          {icon && <span className="mr-2">{icon}</span>}
          <span>{label}</span>
          {hasChildren && (
            <svg className={`w-4 h-4 ml-auto transition-transform ${isOpen ? 'rotate-90' : ''}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      ...
      {hasChildren && (
        <ul className={`ml-4 mt-1 overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-screen' : 'max-h-0'}`}>
...
const Sidebar = () => {
  ...
  return (
    <div className={`flex h-screen ${isCollapsed ? 'w-16' : 'w-64'} transition-width duration-300 ease-in-out bg-gray-50 dark:bg-gray-800 dark:border-gray-700`}>
      <aside className="flex flex-col flex-grow border-r border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between p-4">
          <span className={`text-2xl font-semibold dark:text-white ${isCollapsed ? 'hidden' : ''}`}>Your Brand</span>
          <button onClick={toggleSidebar} className="p-2 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500">
            {isCollapsed ? '➡️' : '⬅️'}
          </button>
        </div>
        ...
        <div className="p-4">
            <p className={`text-sm text-gray-500 dark:text-gray-400 ${isCollapsed ? 'hidden' : ''}`}>Some Footer Text</p>
        </div>
      </aside>
    </div>
  );
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <main className="flex-grow p-4">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-300">This is the main content area of your application.</p>
      </main>
    </div>
  );
};
...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <div className={`flex h-screen ${isCollapsed ? 'w-16' : 'w-64'}  md:w-64 transition-width duration-300 ease-in-out bg-gray-50 dark:bg-gray-800 dark:border-gray-700 fixed top-0 left-0 z-40 md:static ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <aside className="flex flex-col flex-grow border-r border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between p-4">
            <span className={`text-2xl font-semibold dark:text-white ${isCollapsed ? 'hidden' : ''}`}>Your Brand</span>
            <button onClick={toggleSidebar} className="p-2 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500">
      ...
      {/*  Optional Backdrop */}
      {isMobileMenuOpen && <div onClick={toggleMobileMenu} className="fixed top-0 left-0 w-full h-full bg-black opacity-50 z-30 md:hidden"></div>}
...
const Home = () => {
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
       <Sidebar />
      <main className="flex-grow p-4 md:ml-64"> {/* Added md:ml-64 */}
        <h1 className="text-2xl font-semibold dark:text-white">Welcome to Your Application</h1>
        <p className="mt-2 text-gray-600 dark:text-gray-300">This is the main content area of your application.</p>
      </main>
    </div>
