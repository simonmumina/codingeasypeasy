...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['node.js', 'file system', 'fs module', 'directory', 'file', 'path checking', 'is file', 'is directory', 'nodejs tutorial']
draft: false
...
While not directly determining *whether* it's a file or directory, `fs.existsSync()` is a synchronous function that checks if a path exists. You can use this in conjunction with `fs.statSync()` to avoid errors when the path doesn't exist.  However, avoid relying heavily on `fs.existsSync()` as it introduces a race condition (the path could be removed between the existence check and the subsequent `fs.statSync()` call).

**Code Example (Illustrative but *NOT* Recommended for Production):**
