...
The `FLOOR()` function in MySQL takes a single numeric argument and returns the largest integer that is less than or equal to the input argument. In simpler terms, it rounds a number *down* to the nearest whole number.
...
When dealing with negative numbers, the `FLOOR()` function returns the next *lower* integer.  Therefore, -5.75 is rounded down to -6 because -6 is less than -5.75.
...
*   **`FLOOR(X)`:** Rounds *down* to the nearest integer (towards negative infinity).
*   **`CEILING(X)` (or `CEIL(X)`):** Rounds *up* to the nearest integer (towards positive infinity).
