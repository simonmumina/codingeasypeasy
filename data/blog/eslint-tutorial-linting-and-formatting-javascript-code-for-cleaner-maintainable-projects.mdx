...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['eslint', 'javascript', 'linting', 'code quality', 'code formatting', 'prettier', 'style guide', 'coding standards', 'best practices']
draft: false
...
*   **`'semi': ['warn', 'always']`:** This rule will issue a warning if you don't use semicolons at the end of statements.  The second argument, `'always'`, specifies that semicolons are *required*. You could also set it to `'never'` to disallow semicolons.
...
*   **`'no-unused-vars': ['warn', { varsIgnorePattern: '^_', argsIgnorePattern: '^_' }]`:**  This rule warns about unused variables, *except* for those whose names start with an underscore (`_`).  This is a common convention for indicating that a variable is intentionally unused.
...
ESLint focuses on *linting*, identifying potential code quality issues and style violations.  Prettier, on the other hand, focuses on *formatting* code automatically to enforce a consistent style.  While ESLint has some formatting rules, Prettier is generally considered the better tool for formatting.
...
3.  **Configure ESLint:**
    ...
    **Important:** Place `'prettier'` as the *last* element in the `extends` array. This ensures that `eslint-config-prettier` disables all conflicting ESLint rules.
