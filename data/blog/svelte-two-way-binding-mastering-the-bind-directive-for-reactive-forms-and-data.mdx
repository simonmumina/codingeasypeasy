...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['svelte', 'two-way binding', 'bind directive', 'reactive forms', 'data binding', 'svelte tutorial', 'front-end development']
draft: false
...
**Why this is incorrect:**  While this code *appears* to work, Svelte might not detect the change to `person.name` because you're directly mutating the object. Svelte relies on re-assigning the entire object to trigger updates.
...
*   `person = { ...person, name: event.target.value };`: This creates a *new* `person` object with the updated `name` value. The spread syntax (`...person`) copies all the existing properties from the old `person` object to the new one. By re-assigning the `person` variable with a new object, you trigger Svelte's reactivity system, and the UI updates correctly.
