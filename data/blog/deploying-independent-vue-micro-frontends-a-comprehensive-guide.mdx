...
date: '2024-01-26'
lastmod: '2024-01-27'
tags: ['vue', 'micro-frontends', 'deployment', 'webpack', 'module federation', 'single-spa', 'qiankun', 'frontend architecture', 'scalable frontend', 'independent deployment']
draft: false
...
*   **`remotes`:**  Defines the location of remote modules to be consumed.  Crucially, this uses the name defined in the *other* app and points to its `remoteEntry.js`.
...
*   **`publicPath`:**  This *must* be set correctly in each webpack configuration. It determines where webpack looks for chunks and assets.  When deploying, this will often be the URL to the publicly accessible folder.
