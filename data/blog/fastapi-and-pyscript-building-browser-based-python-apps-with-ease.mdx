    ...
    <body>
        ...
        <py-script>
            ...
            async def get_items():
                ...
                output_div.element.innerText = "Fetching items..."
                try:
                    response = await pyfetch("http://127.0.0.1:8000/items/")
                    if response.status == 200:
                        items = await response.json()
                        output_html = "<ul>"
                        for item in items:
                            output_html += f"<li>{item['name']} - ${item['price']}</li>"
                        output_html += "</ul>"
                        output_div.element.innerHTML = output_html
                    else:
                        output_div.element.innerText = f"Error: {response.status} - {response.statusText}"
                except Exception as e:
                    output_div.element.innerText = f"An error occurred: {e}"

            def handle_get_items_click(event):
                asyncio.ensure_future(get_items())

            get_items_button = Element("get-items-button")
            get_items_button.element.onclick = handle_get_items_click
        </py-script>
    </body>
    </html>
    ```
    ...
    <body>
        ...
        <py-script>
            ...
            async def create_item(event):
                ...
                output_div = Element("output")
                output_div.element.innerText = "Creating item..."
                ...
                try:
                    ...
                    if response.status == 200:
                        output_div.element.innerText = "Item created successfully!"
                        await get_items() # Refresh the list after creating an item
                    else:
                        output_div.element.innerText = f"Error creating item: {response.status} - {response.statusText}"
                except Exception as e:
                    output_div.element.innerText = f"An error occurred: {e}"
            ...
            def handle_form_submit(event):
                asyncio.ensure_future(create_item(event))

            get_items_button = Element("get-items-button")
            get_items_button.element.onclick = handle_get_items_click

            item_form = Element("item-form")
            item_form.element.addEventListener("submit", handle_form_submit)
        </py-script>
    </body>
    </html>
    ```
