...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['canvas', 'javascript', 'html5', 'transform', '2d graphics', 'web development', 'tutorial', 'graphics programming']
draft: false
...
**Important Note:** The `transform()` method *multiplies* the current transformation matrix with the specified matrix. This means subsequent `transform()` calls accumulate transformations.
...
*   **`transform(a, b, c, d, e, f)`:**  *Multiplies* the current transformation matrix by the matrix described by the arguments. This means it *adds* the new transformation on top of the existing ones.
*   **`setTransform(a, b, c, d, e, f)`:** *Replaces* the current transformation matrix with the matrix described by the arguments. This resets any previous transformations.
...
The gold rectangle is translated by (50, 50).  The silver rectangle is translated by a *further* (30, 30), resulting in a total translation of (80, 80) from the original origin. The bronze rectangle is drawn at the original (50, 50) position because `setTransform()` reset the transformation matrix.
...
The navy rectangle is translated by (100, 100). After `restore()`, the canvas transformation matrix is reset to the state it was in *before* the `transform()` call, so the maroon rectangle is drawn at the original (50, 50) position.
