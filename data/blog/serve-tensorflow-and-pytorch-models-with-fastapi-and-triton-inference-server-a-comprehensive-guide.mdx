...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['machine learning', 'mlops', 'tensorflow', 'pytorch', 'fastapi', 'triton inference server', 'model serving', 'api', 'deployment', 'inference']
draft: false
...
*   `input`: Defines the input tensor(s). The input name `"input__0"` can be inferred when tracing model, or if it has a single input node, Triton assigns a default name of "input__0".
*   `output`: Defines the output tensor(s). The output name `"output__0"` can be inferred when tracing model, or if it has a single output node, Triton assigns a default name of "output__0".
