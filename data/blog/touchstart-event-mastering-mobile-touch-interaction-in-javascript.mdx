...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'html', 'dom', 'touch events', 'mobile development', 'touchstart', 'touchend', 'touchmove', 'touchcancel', 'event handling', 'responsive design']
draft: false
...
*   **`touches`**: A `TouchList` object representing all current touch points on the *screen*, regardless of the target element.
*   **`targetTouches`**: A `TouchList` object representing all touch points that are touching the *target element*.
*   **`changedTouches`**: A `TouchList` object representing touch points whose state *changed* during the event (in this case, the touch points that just started touching the element).
...
This code snippet retrieves the first touch point that initiated the `touchstart` event and logs its X and Y coordinates relative to the viewport.  Remember to use `event.changedTouches` here, as it will always contain the touch points that triggered the *specific* `touchstart` event.
