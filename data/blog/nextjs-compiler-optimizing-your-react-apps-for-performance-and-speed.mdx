...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['next-js', 'compiler', 'swc', 'rust', 'performance', 'optimization', 'react', 'web-development', 'frontend']
draft: false
...
While not directly related to the compiler *itself*, using the `<Image>` component from `next/image` is essential for image optimization, and the compiler works seamlessly with it.  `next/image` automatically optimizes images, serving them in the correct size and format for the user's device. This significantly improves page load times and user experience.
