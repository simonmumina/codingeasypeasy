...
Unicode characters, such as emojis or characters from non-Latin alphabets, can be represented using multiple code units in JavaScript.  This means that the `.length` property might not always accurately reflect the number of *perceived* characters in the string.
...
*   Each element in the array represents a single *perceived* character, even if it's composed of multiple code units.
