    ...
    <ul id="myList">
      {listItems.map(item => (
        <li
          ...
          style={{ cursor: 'pointer', padding: '5px', border: '1px solid #ccc', marginBottom: '5px', backgroundColor: item.selected ? '#eee' : 'white' }}
        >
          {item.text}
        </li>
      ))}
...
3.  On each list item's click event:
    *   If the Shift key is pressed *and* a `lastSelectedItem` exists, we calculate the range of items between the `lastSelectedItem` and the currently clicked item.
