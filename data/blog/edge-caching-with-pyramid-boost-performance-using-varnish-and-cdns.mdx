...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['pyramid', 'python', 'web development', 'edge caching', 'varnish', 'cdn', 'performance optimization']
draft: false
...
*   **Edge Cache TTL:**  Determines how long Cloudflare should cache your content on its edge servers.  This should align with the `s-maxage` or `max-age` values you set in your Pyramid application's `Cache-Control` headers.  **Important:** If your site has Varnish in front of it, the Edge Cache TTL in Cloudflare should be *longer* than the TTL configured in Varnish.  Otherwise Cloudflare will continually request new content from Varnish, defeating the purpose of Varnish.
