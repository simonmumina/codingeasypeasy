...
*   **`AnimatePresence`:**  This component, from Framer Motion which Chakra UI utilizes, wraps the conditional rendering of the `Box`. Crucially, it *must* wrap the element being animated.
...
*   **`key`:**  A unique `key` prop is *required* within the `motion.div` for `AnimatePresence` to properly track the component's presence in the DOM. Without a key, it won't be able to detect changes and apply animations correctly.  Use a dynamically generated or stable unique ID.
...
By default, `AnimatePresence` attempts to animate exiting and entering components concurrently.  This is often what you want, but sometimes you might want the exiting component to fully animate out *before* the entering component starts animating in.  This is useful for scenarios where you want to avoid visual overlapping or glitches.
