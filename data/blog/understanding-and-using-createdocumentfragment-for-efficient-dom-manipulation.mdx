...
`DocumentFragment` can also be used to efficiently move existing DOM elements.  When you append an existing DOM node to a `DocumentFragment`, it's *moved* from its current location in the DOM to the fragment.
