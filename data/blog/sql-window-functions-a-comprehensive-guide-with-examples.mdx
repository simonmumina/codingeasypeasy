...
*   **`LAG(column, offset, default_value)`:**  Accesses the value of the `column` from the row `offset` rows *before* the current row.  If there's no row at the specified offset, it returns `default_value`.
*   **`LEAD(column, offset, default_value)`:**  Accesses the value of the `column` from the row `offset` rows *after* the current row.  If there's no row at the specified offset, it returns `default_value`.
...
*   **`RANGE BETWEEN value PRECEDING AND value FOLLOWING`:** This frame is based on the *values* of the `ORDER BY` column rather than row numbers.
