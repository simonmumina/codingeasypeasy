...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react-native', 'react-navigation', 'deep-linking', 'mobile-development', 'url-schemes', 'universal-links', 'app-links']
draft: false
...
Deep linking refers to the ability to navigate users to a specific location or piece of content *within* an application from an external source.  Think of it as a way to bypass the app's main landing page and direct the user directly to the information they're looking for.
...
1.  **Configure the `assetlinks.json` file:**  Similar to the `apple-app-site-association` file, Android uses an `assetlinks.json` file to verify your app's association with your website.  This file must be hosted at `https://yourdomain.com/.well-known/assetlinks.json` and must be served with the `application/json` MIME type.
    ...
    [{
      ...
      "target": {
        ...
        ["YOUR_SHA256_CERTIFICATE_FINGERPRINT"] // Your app's SHA256 certificate fingerprint
      }
    }]
    ```
