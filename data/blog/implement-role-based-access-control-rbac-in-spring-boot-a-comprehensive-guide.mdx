...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['spring boot', 'rbac', 'role based access control', 'security', 'java', 'spring security', 'authorization', 'authentication']
draft: false
...
*   **`hasRole()` vs. `hasAuthority()`:**  `hasRole("ADMIN")` is shorthand for `hasAuthority("ROLE_ADMIN")`.  Spring Security automatically prefixes "ROLE_" to the role name.  You can use `hasAuthority()` directly if you want to use a different authority string.
