...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['node.js', 'syntax highlighting', 'code formatting', 'server-side highlighting', 'javascript', 'prismjs', 'highlight.js', 'code blocks']
draft: false
...
2.  **Create a Component for Rendering Code:**
    ...
    *   **`className` on `<code>` tag:** The `language-${language}` class on the `<code>` tag tells Prism.js which language to use when highlighting the code *client-side*, in case `highlightAllUnder` fails for some reason (e.g. disabled javascript).
