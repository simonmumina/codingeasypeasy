...
*   The parent component then uses the `item` data to customize the rendering of each list item. If the parent component *doesn't* provide a slot, the default content `{{ item.name }}` within the `<slot>` tag in the child component will be rendered.
