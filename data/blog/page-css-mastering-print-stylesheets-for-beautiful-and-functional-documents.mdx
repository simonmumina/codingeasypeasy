...
Unlike other CSS rules that target specific HTML elements, `@page` targets the *page itself*.  It operates in the realm of *paged media*, a category of CSS designed specifically for print.
...
One of the most powerful features of `@page` is the ability to create headers and footers using *margin boxes*.  These are pre-defined areas outside the main content area of the page that can be used to display information like page numbers, document titles, or company logos.
...
*   `page-break-before`: Specifies whether a page break should occur *before* an element.
*   `page-break-after`: Specifies whether a page break should occur *after* an element.
*   `page-break-inside`: Specifies whether a page break should be allowed *inside* an element.
...
For professional printing, you might need to include bleed and crop marks.  *Bleed* refers to extending the content slightly beyond the edge of the page to ensure that no white space appears when the page is trimmed.  *Crop marks* are small lines indicating where the page should be cut.
