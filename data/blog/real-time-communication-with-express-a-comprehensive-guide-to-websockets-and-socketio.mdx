...
2.  **Handle Custom Events:**
    ...
    This code listens for the `chat message` event from a connected client.  When it receives a message, it logs it to the console and then *broadcasts* the message to *all* connected clients using `io.emit('chat message', msg)`.  This ensures that all clients receive the message.  If you only want to send the message back to the client that sent it, you would use `socket.emit('chat message', msg)` instead of `io.emit`.
...
2.  **Add the following HTML:**
    ...
    <html>
      ...
      <body>
        ...
        <script src="/socket.io/socket.io.js"></script> <!-- Important: Socket.IO Client library -->
        <script>
...
*   `socket.to('room1').emit()`: Sends a message to all clients in the room "room1" *except* the sender.
