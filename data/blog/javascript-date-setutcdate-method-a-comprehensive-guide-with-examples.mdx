...
2.  We use `setUTCDate(15)` to set the day of the month to the 15th, according to UTC. Note that the `toString()` method displays the date in *local time*, and the `toUTCString()` displays it in UTC time.  The actual milliseconds since the epoch will be the same, though.
