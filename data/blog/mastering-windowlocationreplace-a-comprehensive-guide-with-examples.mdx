...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'window.location', 'replace()', 'url', 'navigation', 'web development', 'browser', 'history api']
draft: false
...
## When *Not* to Use `window.location.replace()`
...
*   **`window.location.assign(URL)`:** Loads a new document at the specified URL, *adding* the current page to the browser history.  The user can use the back button to return.
*   **`window.location.replace(URL)`:** Loads a new document at the specified URL, *replacing* the current page in the browser history.  The user cannot use the back button.
