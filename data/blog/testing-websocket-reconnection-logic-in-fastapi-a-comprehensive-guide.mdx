...
This code defines a simple WebSocket endpoint that echoes messages back to the client and broadcasts them to all connected clients. It also handles disconnections gracefully. Note, however, that it doesn't explicitly handle *reconnection*. The client will need to initiate the reconnection process.
...
Now, let's create a test that simulates a server restart during a WebSocket connection.  We'll connect a client, send a message, then abruptly kill the server process, simulating a crash.  The client *should* automatically reconnect and attempt to send another message.
...
5.  **Simulating Server Restart:**  This is the core of the test. We terminate the Uvicorn process (simulating a server crash).  We *then* need to restart the process for the client to be able to reconnect.  `app_process` is terminated and `run_app` is used to start it up again. We introduce `asyncio.sleep(2)` to ensure that the server is running again before we attempt reconnection and message sending.
6.  **Sending After Restart:**  After simulating the restart, we attempt to send another message.  The client's reconnection logic *should* kick in, re-establish the connection, and successfully send the message. The assertions verify that the reconnection was successful and that the second message was delivered.
...
*   This is a *simplified* example. Error handling (e.g., checking if `iptables` is installed) and more robust cleanup are crucial in a production environment.
