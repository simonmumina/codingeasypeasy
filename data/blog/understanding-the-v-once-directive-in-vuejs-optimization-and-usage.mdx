...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['vuejs', 'vue', 'v-once', 'directive', 'optimization', 'performance', 'frontend', 'javascript']
draft: false
...
The `v-once` directive in Vue.js tells Vue to render an element or component *only once*.  Subsequent re-renders of the parent component will cause that element/component, and all its children, to skip rendering. Vue effectively freezes it in place. This means that any data bindings or event listeners within the `v-once` section will be processed during the initial render but will *not* be updated or react to changes in the data after that.
...
The primary reason to use `v-once` is to **optimize performance**.  If you have parts of your UI that you *know* won't change after their initial render, using `v-once` can prevent unnecessary re-renders.  This is particularly useful for:
...
Consider a scenario where you fetch a list of categories from an API and display them. While the *list itself* might be static, individual items within the list may still need reactivity for events or dynamic styling. In this case, applying `v-once` directly to the entire list might not be the best approach. Let's demonstrate the *incorrect* way first:
...
**Why this is problematic:** If the `categories` array is later modified (e.g., adding a new category), the list will *not* update because `v-once` is applied to the entire `ul`. This defeats the purpose of Vue's reactivity.

**The Correct Approach (If individual items *don't* need reactivity):**  If *none* of the category names need to be dynamic, you could iterate over the data once and store the result in a variable:
...
This is generally *not* recommended though, because if any element *does* need reactivity, you would need to completely refactor this.

**The Recommended Approach (If individual items *do* need reactivity, but the initial list is static):**  You should only use `v-once` if the initial list of items are *completely* static:
...
In this improved version, only the `span` containing the category name uses `v-once`. The list itself remains reactive, meaning if `categories` is modified (e.g., by adding a new category), the list will update.  However, the *names* of existing categories will remain unchanged even if the underlying `category.name` changes.  This allows for optimized initial render of the category name, while still allowing for list updates.
...
*   **`v-once`:**  Prevents re-rendering of the element and its children *in the template*. It freezes the DOM after the initial render.
