...
This example demonstrates that `delete()` correctly removes key-value pairs even when the keys are objects, functions, numbers, or booleans. **Important:** When using objects or functions as keys, ensure that you are passing the *exact same* object or function instance to the `delete()` method.  A new object or function with the same properties or code will *not* match the existing key.
...
It's generally **not recommended** to modify a `Map` (or any data structure) while iterating over it. However, if you absolutely need to, be extremely careful.  Deleting elements can disrupt the iteration process. A safer approach is often to collect the keys to delete and then delete them *after* the iteration.
