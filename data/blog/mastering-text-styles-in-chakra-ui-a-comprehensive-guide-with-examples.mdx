...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['chakra-ui', 'text styles', 'styling', 'react', 'ui library', 'component library', 'theming', 'typography']
draft: false
...
*   `h1`, `h2`, `body`: These are just examples of semantic names.  You can name them whatever you want, as long as it makes sense in your context.  Think about *what* the style is used for (e.g., `pageTitle`, `sectionHeader`, `paragraph`) rather than *how* it looks (e.g., `boldLargeText`).
...
function MyComponent() {
  return (
    <Box>
      <Heading sx={{ textStyle: 'heading1' }}>
        This is a Level 1 Heading
      </Heading>
      <Text sx={{ textStyle: 'body1' }}>
        This is the main body text of my component. Notice how the text styles
        are applied using the sx prop.
      </Text>
      <Text sx={{ textStyle: 'body2' }}>
        This is secondary body text with a smaller font size.
      </Text>
...
The `useStyleConfig` hook is generally used to style entire components, not just text elements. However, you *could* use it to apply text styles within a component's styles. It's less common and less direct than using `sx`, so it's generally best to stick with `sx` for applying text styles.
...
*   The `useStyleConfig` hook is better suited for styling *entire components* (e.g., a custom button) rather than just individual text elements.
