...
- **`\B` (Non-Word Boundary):** Matches any position that is _not_ a word boundary.
...
- **`[^ ]` (Negated Character Set):** Defines a custom set of characters _not_ to match.
...
- **`(?: )` (Non-Capturing Group):** Groups parts of the regular expression together _without_ creating a capturing group. This can improve performance if you don't need to refer back to the matched group.
