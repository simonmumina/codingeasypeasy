...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'jquery', 'error handling', 'web development', 'front-end development', 'debugging', 'uncaught referenceerror', '$ is not defined']
draft: false
...
*   **Incorrect Inclusion Order:**  Your JavaScript code that uses jQuery is executed *before* the jQuery library is loaded.  The browser processes code in the order it's presented in the HTML.
...
*   `your-script.js`: This line includes *your* custom JavaScript file that uses jQuery.  **Important: This needs to be *after* the jQuery library include!**
...
This is arguably the *most common* cause of the "Uncaught ReferenceError: $ is not defined" error.  **Your custom JavaScript code that uses jQuery *must* be included *after* the jQuery library itself.**  The browser parses the HTML from top to bottom. If it encounters your jQuery code before it has loaded the jQuery library, the `$` symbol will be undefined.
