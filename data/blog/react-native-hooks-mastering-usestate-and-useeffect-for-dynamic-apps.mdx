...
*   **With Dependencies (`[someValue]`)**: The effect runs when the component mounts *and* whenever any of the values in the dependency array change.  This is useful for effects that need to respond to changes in props or state.
*   **No Dependency Array (`useEffect(() => { ... });`)**:  The effect runs *after every render*. This is generally not recommended, as it can lead to performance issues or infinite loops. Use with caution and only when absolutely necessary.
