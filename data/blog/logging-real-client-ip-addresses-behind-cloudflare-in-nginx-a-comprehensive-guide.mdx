...
*   `real_ip_recursive on`: This directive tells Nginx to recursively traverse the `X-Forwarded-For` header and use the *last* IP address that isn't in the trusted ranges (the Cloudflare IPs). This ensures you get the *original* client IP, even if there are multiple proxies in the chain.  If set to `off`, Nginx will use the first IP address in the `X-Forwarded-For` header.  Using `on` is almost always the correct choice when behind Cloudflare.
