...
date: '2024-01-26'
lastmod: '2024-10-27'
tags: ['react', 'nextjs', 'page transitions', 'client-side routing', 'animation', 'framer-motion', 'gsap', 'smooth navigation', 'user experience', 'javascript', 'web development']
draft: false
  ...
  useEffect(() => {
    ...
    const transitionPage = (newRoute) => {
      ...
      tl.to(containerRef.current, {
        ...
        .then(() => { // This is important: Wait for the animation to finish *before* routing!
...
2.  **`onComplete` in Timeline:** The `onComplete` callback within the GSAP timeline is essential.  It ensures that `router.push(newRoute)` is only called *after* the exit animation is completely finished. This avoids the "jumpy" transition that would occur if you were to route before the element fully faded out.

3.  **`router.events.on('routeChangeStart')`:**  This event is used to trigger the exit animation *before* Next.js starts loading the new page.
