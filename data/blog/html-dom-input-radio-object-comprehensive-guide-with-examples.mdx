...
*   **`document.getElementsByName()`**:  Radio buttons within a group *must* share the same `name` attribute. This method returns an `HTMLCollection` (an array-like object) of all elements with the specified name.  You can then access individual radio buttons using their index.
...
*   **`document.querySelector()` and `document.querySelectorAll()`**: These methods provide more flexible CSS selector-based access to elements.
    ...
    `querySelector()` returns the *first* matching element, while `querySelectorAll()` returns a `NodeList` containing *all* matching elements.
