...
While `MouseEvent.button` tells you the *specific* button that triggered the event, `MouseEvent.buttons` provides information about *all* mouse buttons that are currently pressed.  This is especially useful for scenarios where the user might be holding down multiple mouse buttons simultaneously.  The `MouseEvent.buttons` property returns a bitmask representing the state of the buttons.  You'll need to use bitwise operators to check which buttons are pressed.
