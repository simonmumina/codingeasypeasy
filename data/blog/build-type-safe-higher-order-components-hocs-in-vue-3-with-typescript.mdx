...
date: '2024-01-05'
lastmod: '2024-01-05'
tags: ['vue', 'vue3', 'typescript', 'hoc', 'higher-order component', 'type-safe', 'components', 'composition api']
draft: false
...
Now, TypeScript can infer the types of props that the `WrappedComponent` expects and ensure that you're not passing incorrect props to it. While this doesn't *completely* eliminate all possible type errors (especially if the original component has `any` types in its prop definitions), it provides a much stronger level of type safety than using `any` directly.
