...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['SQL', 'HAVING Clause', 'GROUP BY', 'Database', 'Querying', 'Filtering', 'Aggregate Functions']
draft: false
...
This query will result in an error because the `WHERE` clause is evaluated *before* the `GROUP BY` clause and the `AVG()` function.  The `average_salary` alias is not yet defined when the `WHERE` clause is executed.
...
You can use both `WHERE` and `HAVING` clauses in the same query. The `WHERE` clause filters the rows *before* grouping, while the `HAVING` clause filters the groups *after* grouping.
