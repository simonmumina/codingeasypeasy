...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['css', 'web fonts', 'typography', 'performance', 'font-face', 'google fonts', 'self-hosting', 'font optimization', 'font display', 'variable fonts']
draft: false
...
*   **`display=swap`:** (covered below).  This is *essential* for a good user experience.
...
2.  **Subset Fonts:**  Fonts often contain characters that you don't actually need on your website.  Subsetting involves creating a smaller version of the font that only includes the characters you're using.  Tools like [FontForge](http://fontforge.github.io/) or online subsetting services can help with this.  *Be careful with subsetting - it can make it difficult to add new content later.*
...
4.  **Use `font-display`:** The `font-display` property controls how the browser handles font loading. It's crucial for preventing the "Flash of Invisible Text" (FOIT) and improving the perceived performance of your website.  Here are the possible values:
    ...
    *   **`auto`:**  The browser's default font display strategy.  This often results in FOIT.  *Avoid using this.*
    *   **`block`:**  The browser initially renders invisible text until the font is loaded.  Then, it swaps the invisible text with the font.  This leads to FOIT and a poor user experience. *Avoid using this.*
    *   **`swap`:**  The browser immediately renders text using a fallback font.  Once the web font is loaded, it swaps the fallback font with the web font.  This provides the best perceived performance and avoids FOIT. *This is the recommended value.*
