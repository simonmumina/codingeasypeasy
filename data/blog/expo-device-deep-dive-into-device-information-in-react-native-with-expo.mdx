...
date: '2024-01-01'
lastmod: '2024-10-27'
tags: ['react-native', 'expo', 'device-information', 'expo-device', 'mobile-development', 'ios', 'android']
draft: false
...
*   **`supportedCpuArchitectures`**: `(Array<string>)`  Returns a list of supported CPU architecture ABIs for the current device. For example: `["arm64-v8a", "armeabi-v7a", "armeabi"]`.  *Android only*.
*   **`totalMemory`**: (number) The total memory of the device in bytes.  *Android and iOS Only*
*   **`isDevice`**: (boolean) Returns `true` if the app is running on a real device. *Web only*.
*   **`isHeadless`**: (boolean) Returns `true` if the app is running in a headless environment (e.g., server-side rendering).
*   **`isRootedExperimental`**: (boolean) Returns `true` if the device has root access (jailbroken on iOS, rooted on Android). *Experimental API*.
