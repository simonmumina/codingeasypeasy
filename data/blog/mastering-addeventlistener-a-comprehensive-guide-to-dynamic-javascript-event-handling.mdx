...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'addEventListener', 'event handling', 'DOM manipulation', 'web development', 'window object', 'event listeners']
draft: false
...
The `addEventListener()` method allows you to attach *event listeners* to HTML elements (including the `window` object).  An event listener is a function that executes when a specific event occurs on the element. Think of it as setting up an alarm system: you specify the event (e.g., a button click, a key press, the window loading), and the function (the alarm) triggers when that event happens.
...
*   **`options` (optional)**:  An object that specifies characteristics about the event listener.  Common options include:
    *   **`capture`**: A boolean indicating whether the event listener should be triggered during the *capture phase* (more on this later).  Defaults to `false`.
...
By default, `addEventListener()` registers event listeners in the *bubbling phase*. To register an event listener in the *capture phase*, set the `capture` option to `true`.
...
*   **`listener`**: The function that was originally attached as the event listener.  **This is crucial:** You must pass the *exact same function reference* that was used when adding the listener.  Anonymous functions cannot be removed, as there is no way to reference them.
...
**Important:**  You *cannot* remove an event listener that was attached using an anonymous function unless you use an `AbortSignal`.
