...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'html', 'dom', 'clientheight', 'web development', 'browser', 'size', 'dimensions']
draft: false
...
The `clientHeight` property returns the interior height of an element in pixels, including padding but excluding borders, scrollbars (if present and rendered), and margins.  Essentially, it represents the height of the element's *content area*, as it's visually rendered to the user.
...
*   **Borders Excluded:**  The width of the element's border is *not* included.
...
*   The `clientHeight` returns the height of the visible content area *including* the padding (10px top + 10px bottom = 20px) plus the specified height of the div (100px), resulting in 120px. The scrollbar itself does *not* affect the `clientHeight`.
...
*   **`scrollHeight` represents the *total* height of the content, even if it's larger than the visible area, whereas `clientHeight` represents only the visible height.** This is important when dealing with scrollable elements.
*   **`style.height` is a CSS property, while the others are DOM properties.**  `style.height` reflects the *declared* height, not necessarily the *actual* rendered height.  It also needs to be parsed to get the numerical value (e.g., stripping the "px" unit).
