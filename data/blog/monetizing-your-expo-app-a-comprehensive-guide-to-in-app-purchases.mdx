...
date: '2024-01-26'
lastmod: '2024-10-27'
tags: ['expo', 'react-native', 'in-app purchases', 'monetization', 'ios', 'android', 'mobile development', 'revenue', 'eas']
draft: false
            ...
            if (responseCode === IAP.IAPResponseCode.OK) {
              results.forEach(purchase => {
                if (purchase.purchaseState === IAP.PurchaseState.PURCHASED) {
                  ...
                  Alert.alert("Pending Purchase", `Your purchase of ${purchase.productId} is pending.`);
                  setIsLoading(false); // Stop loading even if pending
                } else {
                  console.warn(`Purchase ${purchase.productId} unexpected state: ${purchase.purchaseState}`);
  ...
  const consumePurchase = async (purchaseToken) => {
    try {
      ...
      if (result.responseCode === IAP.IAPResponseCode.OK) {
        Alert.alert("Purchase Consumed", "The product has been consumed and can be purchased again.");
      } else {
