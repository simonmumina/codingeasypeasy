...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['mantine', 'notifications', 'react', 'ui library', 'user interface', 'web development', 'accessibility']
draft: false
...
2.  **Configure `NotificationsProvider`:**
    ...
    *   `NotificationsProvider`:  This provider is *essential* for the notification system to function.  It creates a React Context that allows components to trigger notifications using the `showNotification` hook.
...
3.  **Example with Customization:**
    ...
    function CustomNotification() {
      ...
      return (
        <Button onClick={showCustomNotification}>Show Custom Notification</Button>
      );
    }
    ```
...
1.  **Updating a Notification:**
    ...
    To update a notification, you need its `id`.  You can then call `showNotification` again with the *same* `id` and the updated properties.
    ...
    *   The `showNotification` function *updates* the existing notification with the new properties (title, message, color, loading, autoClose, disallowClose) instead of creating a new one.
...
2.  **Closing a Notification:**
    ...
    function CloseNotification() {
      ...
      return (
        <Button onClick={showAndCloseNotification}>Show and Close Notification</Button>
      );
    }
    ```
...
2.  **Custom CSS Classes:**
    ...
    function StyledNotification() {
      ...
      return (
        <Button onClick={showStyledNotification}>Show Styled Notification</Button>
      );
    }
    ```
