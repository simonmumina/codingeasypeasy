...
date: '2024-10-27'
lastmod: '2024-10-28'
tags: ['nestjs', 'aws', 'deployment', 'serverless', 'ec2', 'docker', 'api gateway', 'lambda', 'cloudformation', 'ci/cd']
draft: false
...
2.  **Modify your `main.ts` or `main.js` file:**
    ...
    *   We create a `bootstrap` function that initializes the NestJS application only once (if `server` is null) and wraps it with `serverless()`.  Caching the server is *crucial* for performance, as it avoids re-initializing the NestJS app on every Lambda invocation.
