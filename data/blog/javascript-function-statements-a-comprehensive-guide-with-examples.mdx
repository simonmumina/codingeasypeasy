...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'functions', 'function statements', 'function declarations', 'javascript tutorial', 'programming', 'web development']
draft: false
...
One of the most important characteristics of Function Statements is that they are **hoisted**.  Hoisting means that the JavaScript interpreter effectively moves the declaration of the function to the top of the current scope (either the global scope or the function scope) *before* the code is executed.

This allows you to call a function *before* it's actually defined in the code.  This is a key difference between Function Statements and Function Expressions.
...
Even though the `add` function is defined *after* the `console.log` statement that calls it, the code works because the function declaration is hoisted to the top of the scope.
...
In this case, `multiply` is a Function Expression.  While the `multiply` variable is hoisted, the *function definition* is not.  Attempting to call `multiply` before its definition results in an error.  The variable is declared but not initialized until the line where the function is assigned to it.

## Function Parameters and Arguments

Functions can accept input values through parameters. When you *call* a function, you provide the actual values, which are called *arguments*.
