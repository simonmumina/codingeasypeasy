...
Make sure both `bootstrap-vue` *and* `bootstrap` are installed. BootstrapVue relies on Bootstrap's CSS and JavaScript.
...
*   **`import 'bootstrap/dist/css/bootstrap.css'`:**  This line is *crucial*.  It imports the core Bootstrap CSS, which BootstrapVue depends on.  Without this, the Bootstrap styles won't be applied.
...
*   **Forgetting to install `bootstrap`:** Many developers install only `bootstrap-vue` and wonder why the styles aren't working.  `bootstrap` is a *dependency* of `bootstrap-vue`.
...
CSS rules are applied in the order they appear in the stylesheet. If your custom CSS is loaded *before* Bootstrap CSS, it can override Bootstrap's styles, leading to unexpected results.  Similarly, if `bootstrap-vue.css` loads before `bootstrap.css`, it might not be able to properly extend and style the core Bootstrap elements.
...
*   **Mixing global and local registration:**  Don't try to use a globally registered component *and* import it locally. It can lead to confusion and unpredictable behavior.  Choose one method and stick with it.
...
<head>
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-vue@2.21.2/dist/bootstrap-vue.min.css" integrity="sha384-nS9e4D16b+a0j6bM7w5L6B/29QW4rF2nC8/g+LzUa4w0W+sLg8V6z6L12E5lX+n" crossorigin="anonymous">
</head>
<body>
    <div id="app">
...
*   **CSS Order:**  The Bootstrap CSS *must* be loaded before the BootstrapVue CSS.
*   **JavaScript Order:** Vue.js *must* be loaded before BootstrapVue's JavaScript.
