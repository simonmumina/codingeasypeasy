...
date: '2023-10-27'
lastmod: '2024-01-15'
tags: ['react', 'react hooks', 'usestate', 'useeffect', 'usecontext', 'javascript', 'frontend development', 'react tutorial']
draft: false
...
4.  **`increment` and `decrement` functions:**  These functions call `setCount` to update the `count` state when the corresponding buttons are clicked.  Note that `setCount` takes the *new* value of the state, not just a delta.
...
**Important Note:** When updating objects or arrays, always create a *new* object or array using the spread operator (`...`) or other techniques for immutability. Modifying the existing object/array directly can lead to unexpected behavior as React might not detect the change.
