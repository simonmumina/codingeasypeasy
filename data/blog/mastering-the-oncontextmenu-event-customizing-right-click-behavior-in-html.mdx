...
In this example, right-clicking the `div` will display an alert box. The `return false;` statement is crucial; it prevents the default browser context menu from appearing.  Without it, *both* the alert and the default menu would be displayed.
