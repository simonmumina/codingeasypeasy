...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['vue', 'javascript', 'performance', 'lazy loading', 'images', 'optimization', 'web development', 'front-end']
draft: false
      ...
      inserted: (el) => {
        function loadImage() {
          ...
          if (imageSrc) {
            ...
            el.onload = () => { // Optional: Handle the image loading completion
...
*   **Consider Using `loading="lazy"` attribute (Native Browser Lazy Loading):** Modern browsers support native lazy loading using the `loading="lazy"` attribute on `<img>` elements. While not fully supported across all browsers, it's worth considering as a progressive enhancement.  If you use a polyfill, ensure the polyfill does not conflict with the Intersection Observer or third party library approaches.
    ...
    Values for the `loading` attribute:
        * `lazy`:  Defers loading of the image until it reaches a calculated distance from the viewport.
        * `eager`: Loads the image immediately, regardless of its position on the page. (Default behavior)
        * `auto`: Let the browser decide whether or not to lazy load.
