...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['pyramid', 'python', 'web framework', 'permissions', 'acl', 'authorization', 'security', 'access control']
draft: false
...
Pyramid uses ACLs to manage authorization, the process of determining whether a user is allowed to perform a specific action. Authentication, on the other hand, is the process of verifying a user's identity.  While Pyramid provides mechanisms for both, this article focuses primarily on the *authorization* aspect handled via ACLs.
...
*   `Deny, Everyone, 'view'`: This rule is important! Even though authenticated users are granted permission, `Deny` rules take precedence.  Because everyone *includes* authenticated users, we must add `Allow, Authenticated, 'view'` first to permit access to authenticated users *before* the deny-all.  If this rule were omitted, *everyone* would be allowed to view.
