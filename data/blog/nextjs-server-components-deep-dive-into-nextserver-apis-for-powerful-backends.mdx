...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['next-js', 'server-components', 'next/server', 'api-routes', 'middleware', 'edge-functions', 'backend', 'react', 'performance']
draft: false
...
*   **`'use server';`:**  Marks the `createPost` function as a Server Action. This is crucial; without it, the function will not be executed on the server.  It *must* be at the very beginning of the function's scope.
