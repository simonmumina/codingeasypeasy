...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['javascript', 'for...in loop', 'object iteration', 'property enumeration', 'javascript fundamentals', 'javascript loops']
draft: false
...
The `for...in` statement iterates over all *enumerable* properties of an object, including inherited properties. For each property, the loop assigns the *property name* (as a string) to a specified variable.
...
The `for...in` loop only iterates over *enumerable* properties.  Enumerability refers to whether a property will appear when enumerated by loops like `for...in` and methods like `Object.keys()`.  By default, properties you add directly to an object are enumerable. However, properties inherited from the object's prototype are also enumerable.  This is a crucial distinction!
...
To avoid iterating over inherited properties and focus solely on properties directly defined on the object itself, use the `hasOwnProperty()` method.  This method returns `true` if the object has a property with the specified name as its *own* property (i.e., not inherited).
...
While you *can* use `for...in` with arrays, it's **not recommended**.
...
*   **`for...in`:** Iterates over the *property names* (keys) of an object (including inherited properties). It's designed for objects.
*   **`for...of`:** Iterates over the *values* of an iterable object (e.g., arrays, strings, Maps, Sets, etc.). It's designed for iterables.
