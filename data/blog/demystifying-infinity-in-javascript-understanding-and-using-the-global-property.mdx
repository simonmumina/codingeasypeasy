...
*   **`Number.MIN_VALUE`:** Represents the smallest positive number closest to zero that JavaScript can represent. Note that it is *not* the smallest representable number overall.  For that, consider `-Number.MAX_VALUE`.
...
*   **`isNaN()` Check:**  Remember that `isNaN(Infinity)` returns `false`. `Infinity` is still a *number*, just a very large one.  `isNaN()` is for detecting `NaN` (Not a Number).
