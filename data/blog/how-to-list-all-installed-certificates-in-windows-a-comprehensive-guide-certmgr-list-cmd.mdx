...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['windows', 'certificates', 'security', 'certmgr', 'command line', 'powershell', 'ssl', 'tls', 'digital certificates', 'certificate manager']
draft: false
...
**Important Note:**  The `certmgr.exe` command itself does *not* have a `/list` option.  The primary use is opening the GUI focused on a specific store.  To actually *list* the certificates in a usable format, you'll need PowerShell.
...
To list certificates from *all* stores, you'll need to iterate through the store locations and retrieve the certificates from each. Here's a PowerShell script that accomplishes this:
