...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['tailwind', 'css', 'overflow-wrap', 'word-break', 'text-overflow', 'responsive design', 'web development']
draft: false
...
*   **`overflow-wrap` (formerly `word-wrap`):** This property specifies whether the browser is allowed to break lines within words to prevent overflow when an otherwise unbreakable string is too long to fit in its containing box.
    ...
    *   `break-word`: Allows the browser to break lines *within* words if an otherwise unbreakable string overflows. This is the key to preventing long URLs or concatenated strings from breaking the layout.
...
*   `break-words`:  **Confusingly, this actually applies `overflow-wrap: break-word;` and *not* `word-break`.**
