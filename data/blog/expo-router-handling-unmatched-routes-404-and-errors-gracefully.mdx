...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['expo', 'react-native', 'expo-router', 'routing', '404', 'error handling', 'navigation']
draft: false
    ...
    if (this.state.hasError) {
      ...
      return (
        <View style={styles.container}>
          ...
          <Link href="/" asChild>
             <Button title="Go to Home" onPress={() => this.setState({hasError:false, error: null, errorInfo: null})}/>
          </Link>
        </View>
      );
    }
...
*   **`getDerivedStateFromError(error)`:**  This static method is called when an error is thrown in a descendant component.  It updates the state to indicate that an error has occurred.  Crucially, this method *must* be static.
