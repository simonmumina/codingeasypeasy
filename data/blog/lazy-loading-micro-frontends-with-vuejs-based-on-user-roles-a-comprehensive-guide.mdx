...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['micro-frontends', 'vuejs', 'lazy-loading', 'user-roles', 'performance', 'dynamic-imports', 'code-splitting']
draft: false
...
<template>
  ...
  <div v-else>Loading...</div>  <!-- Or a more sophisticated loading indicator -->
</template>
    ...
    canLoad: {
      ...
      handler(canLoad) {
        if (canLoad) {
          ...
          this.error = new Error("User doesn't have required roles to load component: " + this.microFrontendName)
  ...
  <div>
    <nav>
      <ul>
        ...
        <li v-if="$store.getters.isAuthenticated && $store.getters.userRoles.includes('admin')"><router-link to="/admin">Admin</router-link></li>
    ...
    <MicroFrontendLoader microFrontendName="UserProfile" :requiredRoles="['user', 'editor', 'admin']" />
...
<template>
  ...
  <div v-else>
    <div v-if="error">
      Error loading {{ microFrontendName }}: {{ error.message }}
    </div>
