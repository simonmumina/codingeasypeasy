...
date: '2023-10-27'
lastmod: '2024-01-20'
tags: ['javascript', 'type conversion', 'type coercion', 'data types', 'primitive types', 'object types', 'string conversion', 'number conversion', 'boolean conversion', 'type casting', 'toPrimitive', 'valueOf', 'toString']
draft: false
...
The loose equality operator (`==`) performs type conversion before comparing values.  This can lead to unexpected results.  The strict equality operator (`===`) does *not* perform type conversion; it compares values directly.
...
Internally, JavaScript uses an abstract operation called `ToPrimitive` to convert objects to primitive values. This operation takes an input value and an optional *preferred type* (either `Number` or `String`).

* If *preferred type* is `Number`:
    1. If `valueOf` returns a primitive, use it.
    2. Otherwise, if `toString` returns a primitive, use it.
    3. Otherwise, throw a `TypeError`.

* If *preferred type* is `String`:
    1. If `toString` returns a primitive, use it.
    2. Otherwise, if `valueOf` returns a primitive, use it.
    3. Otherwise, throw a `TypeError`.

* If no *preferred type* is specified, the behavior is implementation-dependent, but generally behaves as if `Number` is specified for `Date` objects and `String` for other objects.
