...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['CSS', 'attr()', 'Dynamic Styling', 'CSS Variables', 'Accessibility', 'HTML Attributes', 'CSS Functions', 'Web Development']
draft: false
...
While this example *uses* `target="_blank"` as a *selector*, let's focus on how `attr()` could *display* that information.  For example,  we might want to visually indicate external links even without bolding them:
...
```html
<div data-highlight-color="yellow" style="padding: 10px; border: 1px solid black;">This is a highlighted section.</div>
```
...
While you can't directly animate attribute values with CSS transitions or animations, you *can* use CSS variables to bridge the gap.  Change the attribute value via Javascript, then trigger a CSS transition on the CSS variable that's derived from the attribute using `attr()`.
