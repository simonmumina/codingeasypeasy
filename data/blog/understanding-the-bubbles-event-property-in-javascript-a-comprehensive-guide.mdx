...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'events', 'dom', 'bubbles', 'event-propagation', 'event-handling', 'web-development']
draft: false
...
*   **`false`:** The event will *not* bubble up the DOM tree. This is less common, but some events, like `focus` and `blur`, do not bubble by default.
...
}, { capture: true }); //  or  true (shorthand)
```

With the `capture` option set to `true`, the parent's event listener will be triggered *before* the child's event listener (even if the child's listener is triggered in the bubbling phase).
...
*   **`event.target`:** Refers to the element that *triggered* the event (the originating element).  It remains constant throughout the capturing and bubbling phases.
*   **`this`:** Refers to the element to which the event listener is *attached*.  Its value depends on how the event listener was bound. In standard event listeners, it refers to the element the listener is attached to.
