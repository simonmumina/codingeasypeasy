...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['javascript', 'event listener', 'addEventListener', 'DOM', 'HTML', 'events', 'event handling', 'web development']
draft: false
...
To optimize performance, you can use the `passive` option in `addEventListener()`.  Setting `passive: true` tells the browser that the event listener will *not* call `preventDefault()`.  This allows the browser to continue with the default action without waiting for the event listener, resulting in smoother scrolling.
