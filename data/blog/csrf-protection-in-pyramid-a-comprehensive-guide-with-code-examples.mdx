...
*   **Crucially,** always validate the token *before* performing any state-changing operations (e.g., updating the database).
...
*   **Validate Before Processing:** Always validate the CSRF token *before* performing any data modification operations.
