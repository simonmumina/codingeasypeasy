...
date: '2024-02-29'
lastmod: '2024-03-08'
tags: ['grep', 'linux', 'text extraction', 'command line', 'regular expressions', 'data analysis', 'bash', 'command line tools']
draft: false
...
The `-v` option inverts the match, printing only the lines that *do not* contain the specified pattern.
...
If `example.txt` contained the word `example123`, it *would not* be matched with `-w`.
...
*   `[^]` (caret in brackets): Matches any single character *not* within the brackets.
