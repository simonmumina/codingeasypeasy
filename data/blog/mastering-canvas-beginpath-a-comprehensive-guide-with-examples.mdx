...
*   **Creating Separate Shapes:**  The most common use case.  If you want to draw two rectangles that are not connected, you *must* use `beginPath()` between drawing them.
...
*   **Unnecessary `beginPath()` Calls:**  While it's better to err on the side of caution, avoid calling `beginPath()` unnecessarily *within* the same connected path. For example, when drawing a single complex polygon, only call `beginPath()` once at the beginning.
...
*   **Combining Fill and Stroke:** Remember that `fill()` and `stroke()` operate on the *current* path.  So, the order in which you call them, along with the styles you set *before* calling them, will determine the appearance of your shape.
