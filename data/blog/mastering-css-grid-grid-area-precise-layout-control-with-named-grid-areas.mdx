...
date: '2024-01-01'
lastmod: '2024-10-27'
tags: ['css', 'css grid', 'grid-area', 'layout', 'responsive design', 'web development', 'front-end']
draft: false
...
.item1 { grid-area: 1 / 1 / 2 / 3; background-color: lightblue; padding: 10px; } /* Spans row 1, columns 1 and 2 */
.item2 { grid-area: 2 / 1 / 3 / 2; background-color: lightgreen; padding: 10px; } /* Spans row 2, column 1 */
.item3 { grid-area: 2 / 2 / 3 / 4; background-color: lightcoral; padding: 10px; } /* Spans row 2, columns 2 and 3 */
```
...
*   The sidebar is pushed below the main content using `order`. This is crucial because simply changing the `grid-template-areas` re-orders the *visual* layout, but not necessarily the DOM order. Without `order`, the sidebar would still appear *before* the main content in the source code, which might be undesirable for accessibility.
