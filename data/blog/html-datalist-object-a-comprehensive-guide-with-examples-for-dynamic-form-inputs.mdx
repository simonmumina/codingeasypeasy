...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['html', 'dom', 'datalist', 'input', 'autocomplete', 'javascript', 'form', 'web development', 'frontend']
draft: false
...
When the user selects an option from the datalist or types a matching value, the selected or typed value is submitted with the form. If the user enters a value that is *not* present in the datalist, that value will still be submitted.  The `<datalist>` **does not** enforce validation by itself.  You can use JavaScript to validate the user's input against the datalist values.
...
You can style the `<input>` element associated with the datalist using CSS.  However, styling the `<datalist>` itself is more limited and browser-dependent. Styling the *dropdown* list that appears is very limited and often requires JavaScript to create a completely customized solution.
