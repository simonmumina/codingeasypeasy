...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'html', 'dom', 'events', 'onchange', 'web development', 'interactive elements', 'form validation', 'dynamic updates']
draft: false
...
The `onChange` event *doesn't* fire immediately after every keystroke. Instead, it typically fires when the element *loses focus* (e.g., when the user clicks outside the input field) *after* the value has been changed. For `<select>` elements, it fires immediately after a different option is selected.
...
This example uses the `oninput` event instead of `onchange`. `oninput` fires *every time* the value of the input element is updated, providing a more immediate user experience for dynamic updates like character counts. We're also setting the `maxlength` attribute to the textarea to limit input.

## `onChange` vs. `onInput`

It's important to understand the difference between the `onChange` and `oninput` events:

*   **`onChange`**: Fires when the element loses focus *after* its value has been changed.  Specifically appropriate for capturing final committed changes in forms.
*   **`oninput`**: Fires immediately *every time* the value of the element is changed, including every keystroke. Ideal for real-time updates, like character counters or immediate validation feedback.
