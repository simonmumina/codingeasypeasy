...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['bootstrap', 'modal', 'scrollbar', 'mobile', 'css', 'javascript', 'frontend', 'web-development']
draft: false
...
*   **`show.bs.modal` event:** This event is fired immediately when the show instance method is called.  Crucially, it fires *before* the modal is actually displayed.
...
**Why this approach is preferred:**  It dynamically disables body scrolling *only* when the modal is open, minimizing potential side effects on other page elements. It also avoids potential conflicts with other CSS rules that might affect the `<body>` element's `overflow` property.
