...
This mechanism allows you to define multiple views that respond to the *same* route but under different conditions.
...
*   **`custom_predicates`:** This is a *list* of callable functions which are the heart of extending the power of Pyramid's routing. We will cover this in detail later.
...
*   `@view_config(route_name='admin_page', renderer='templates/admin_page.pt', is_admin=True)`: In the `view_config`, we now use our custom predicate 'is_admin'. This view will *only* be called if the `is_admin` predicate returns `True`.  If the user isn't an admin, the second view configured for 'admin_page' which has no is_admin predicate will be matched and rendered.
