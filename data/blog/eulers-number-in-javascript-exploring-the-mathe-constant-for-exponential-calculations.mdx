...
- **Base of the Natural Logarithm:** The natural logarithm (ln) is the logarithm to the base _e_. In other words, ln(x) is the power to which _e_ must be raised to equal x.
...
The `Math.exp()` function returns _e_ raised to the power of a given number. It's the inverse of the natural logarithm function (`Math.log()`).
...
In this example, we calculate _e_<sup>2</sup>, which is approximately 7.389.
