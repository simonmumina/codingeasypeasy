...
date: '2024-01-26'
lastmod: '2024-01-27'
tags: ['CSS', 'backfaceVisibility', 'HTML', 'front-end', 'web development', '3D transforms', 'CSS properties']
draft: false
...
Imagine a playing card.  By default, you can see both sides. `backfaceVisibility` allows you to make it so that when you flip the card around, you *don't* see the back.
...
A common use case is creating card flipping animations. You want to see one side of the card, then the other, but you *don't* want to see both at the same time during the flip.
...
*  Note the crucial `transform: rotateY(180deg);` on the `.card-back`. This ensures that the back face is correctly oriented when it's *supposed* to be visible.
