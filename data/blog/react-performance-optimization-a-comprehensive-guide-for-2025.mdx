...
date: '2025-01-23'
lastmod: '2025-1-27'
tags: ['react', 'performance', 'optimization', 'javascript', 'web development', 'front-end', 'best practices']
draft: false
...
The `useMemo` hook memoizes the *result* of a function call.  It's useful for optimizing expensive calculations or creating stable references to objects or functions that are passed as props to child components.
...
function MyComponent({ a, b }) {
  ...
  return (
    <div>
      The expensive value is: {expensiveValue}
    </div>
  );
}
...
The `useCallback` hook memoizes a *function* itself. This is extremely useful when passing callbacks down to components optimized with `React.memo`. Without `useCallback`, a new function instance would be created on every render of the parent, causing the `memo`'d child to always re-render because it sees the prop as always changing.
...
const MyContextProvider = React.memo(({ children }) => {
  ...
  const contextValue = useMemo(() => ({
    value,
    setValue,
  }), [value, setValue]); // Only recreate if value or setValue changes
...
While not strictly *React* optimizations, SSR and SSG are architectural choices that can significantly impact performance, especially initial load time and SEO.
