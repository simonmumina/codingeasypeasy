...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['canvas', 'html5', 'javascript', 'rotate', 'transform', 'graphics', 'animation', 'canvas tutorial']
draft: false
...
The `rotate()` method in the HTML5 Canvas API is a transformation function that rotates the canvas context around the *current origin*.  The current origin is, by default, the top-left corner of the canvas (0, 0).  Rotation is performed in radians, not degrees. This means you'll often need to convert degrees to radians to achieve the desired effect.

In essence, `rotate()` changes the coordinate system.  Anything drawn *after* the `rotate()` call will be drawn as if the canvas had been physically rotated around the origin. It's important to remember that `rotate()` is a stateful function, meaning it affects subsequent drawing operations until you save and restore the canvas state.
...
7.  `ctx.fillRect(200, 50, 100, 50)`: This draws another rectangle at position (200, 50) with a width of 100 and a height of 50. This rectangle is *not* rotated because the canvas context has been restored to its original state.
...
2.  **Rotate:** `ctx.rotate(degreesToRadians(30))` rotates the canvas around the *new* origin (the center of the rectangle).
