...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['node.js', 'fs', 'fs.stats', 'isBlockDevice', 'file system', 'block device', 'file system operations', 'nodejs tutorial', 'nodejs fs module']
draft: false
...
*   **`fs.lstat(path, callback)`:**  Retrieves file system statistics for the given path, but does *not* follow symbolic links. It provides information about the link itself.
...
1.  We use `fs.lstat` to check the status of the *link itself*, not the target.  This will tell us if the `linkPath` is a symbolic link.
2.  If it's a symbolic link, we then use `fs.stat` to follow the link and retrieve the stats of the *target* of the symbolic link.
