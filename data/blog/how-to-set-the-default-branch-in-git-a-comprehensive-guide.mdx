...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['git', 'version control', 'default branch', 'github', 'gitlab', 'bitbucket', 'git config', 'git init']
draft: false
...
4.  **Click "Update".**
5.  **GitHub will prompt you to update any open pull requests targeting `master` to target `main` instead.**  It will typically provide a one-click option to do this. Review and accept these changes carefully.  This is *critical* to avoid merge conflicts and broken builds.
...
3.  **Remove the old default branch (optional, but recommended):**
    ...
    **Important:** This will only work if you have merged all your changes from `master` into `main`. If you have unmerged changes on `master`, you'll need to merge them into `main` first, or use `-D` to force the deletion (which will *lose* any unmerged changes).
...
*   **Communicate the change:**  Inform your team *before* making the change to the default branch.  This prevents confusion and ensures everyone updates their local repositories correctly.
*   **Update CI/CD pipelines:**  Make sure your continuous integration and continuous deployment pipelines are updated to use the new default branch. This is *absolutely critical* to avoid broken builds and deployments.
