...
When an element creates a new stacking context, all of its child elements participate in that context. The `z-index` values of these child elements are only meaningful *within that stacking context*.  This means that an element with a high `z-index` value within one stacking context cannot be placed on top of an element in a *different* stacking context, even if the latter element has a lower `z-index` value.
