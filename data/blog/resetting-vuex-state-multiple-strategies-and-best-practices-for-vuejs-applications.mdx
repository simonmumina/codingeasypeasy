...
<template>
  <div>
    <p v-if="$store.getters['user/isUserLoggedIn']">Welcome, {{ $store.getters['user/userName'] }}!</p>
    <button @click="logout">Logout</button>
  </div>
</template>
...
*   **Asynchronous operations:**  If you're performing asynchronous operations to fetch initial data, ensure that the persisted state is loaded *before* those operations are executed.  Otherwise, the persisted state might be overwritten.
