...
*   **`spring-boot-starter`:** While technically optional, including `spring-boot-starter` brings in commonly needed Spring Boot features.  Using `scope=provided` means that the application that *uses* your starter is responsible for providing the `spring-boot-starter` dependency itself.
