...
date: '2023-10-27'
lastmod: '2024-01-16'
tags: ['css', 'units', 'px', 'em', 'rem', 'percentage', 'viewport', 'responsive design', 'web development', 'front-end']
draft: false
...
`em` is a relative unit that is based on the font size of the *parent* element. If the parent element has a font size of 16px, then `1em` is equal to 16px.
...
In this example, the `.parent` element will have a font size of 32px (2em * 16px). The `.child` element, nested inside `.parent`, will have a font size of 16px (0.5em * 32px).  Notice how the `em` value is relative to the immediately containing parent.

## 3. Rems (rem) - Relative to the Root Element

`rem` stands for "root em" and is a relative unit that is based on the font size of the *root* element, which is usually the `<html>` element. This provides a more predictable and manageable scaling system compared to `em`.
...
`rem` is generally preferred over `em` for font sizing because it avoids the compounding issues and provides a more consistent scaling behavior across the website.  `em` can still be useful for sizing elements *within* a component where you want the size to be relative to the component's text size.

## 4. Percentages (%) - Relative to Parent

The percentage unit `%` is relative to the size of the *parent* element. The specific property that the percentage applies to influences what aspect of the parent is used.  For example, `width: 50%` will make the element half the width of its parent.  `font-size: 150%` will make the element's font size 1.5 times larger than its parent's.
...
*   **`em`:** Use for elements where the size should be relative to the parent element's font size (often for spacing *within* a component).
