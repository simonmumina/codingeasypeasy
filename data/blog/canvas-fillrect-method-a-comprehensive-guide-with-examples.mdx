...
- **`width` and `height` Dimensions:** These parameters define the size of the rectangle. `width` specifies the horizontal extent of the rectangle, and `height` specifies the vertical extent. Positive values draw the rectangle to the right (for width) and down (for height) from the starting point. Negative values _can_ be used, but they can lead to unexpected behavior and are generally avoided. Using negative values for `width` would, for instance, draw the rectangle to the left of the `x` coordinate.
