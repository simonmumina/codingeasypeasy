...
date: "2024-10-27"
lastmod: "2024-10-27"
tags: ["WSL2", "Windows Subsystem for Linux", "Network Bridge", "Networking", "Docker", "Development", "Linux", "Windows", "Virtualization", "Hyper-V"]
draft: false
...
*   Select **Use the following IP address:**
    *   **IP address:** Choose an IP address on your subnet that is *not* already in use. This is crucial! You can usually pick a high number IP like `192.168.1.250` if your network is `192.168.1.0/24`.  Important: This IP needs to be on the same subnet as your router.
...
*   **Important:** Replace the following with your actual values:
    ...
    *   `192.168.1.150/24`:  A static IP address on your subnet that is *not* already in use.  The IP address must be on the same subnet as the IP address configured on your Windows adapter.
