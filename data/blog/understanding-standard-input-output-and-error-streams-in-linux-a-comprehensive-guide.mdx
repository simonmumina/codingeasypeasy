...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['linux', 'standard input', 'standard output', 'standard error', 'stdin', 'stdout', 'stderr', 'redirection', 'pipes', 'bash', 'command line', 'linux tutorial', 'input/output', 'linux streams']
draft: false
...
*   **Standard Input (stdin):**  This is the stream from which a program receives its input.  By default, stdin is connected to the keyboard.  Think of it as the program *listening* for information you type.
*   **Standard Output (stdout):** This is the stream to which a program sends its normal output.  By default, stdout is connected to the terminal screen.  Think of it as the program *talking* to you, displaying results, or showing progress.
*   **Standard Error (stderr):** This is the stream to which a program sends error messages and diagnostic information.  By default, stderr is also connected to the terminal screen.  Think of it as the program *reporting* problems separately from its normal output.
...
While the default connection is to the terminal (keyboard and screen), the power of standard streams comes from the ability to *redirect* these streams to other locations, such as files or even other programs. This is the cornerstone of many powerful command-line techniques.

## Standard Input (stdin)

Standard input provides data *to* a program.  The most common source of stdin is the keyboard, allowing you to interact directly with a running program.  However, you can also redirect stdin from a file, effectively feeding the program the contents of that file as if it were typed manually.
...
Standard output is where a program *writes* its normal results.  By default, this output is displayed on the terminal screen.  Redirecting stdout allows you to save the program's output to a file, pass it to another program, or suppress it entirely.
...
The `>>` operator *appends* stdout to a file.  If the file exists, the new output is added to the end of the file.  If the file doesn't exist, it will be created.
...
In this case, *both* the normal output and the error messages are written to `output.txt`. The order is important: you must redirect `stdout` *before* redirecting `stderr` to `stdout`.
...
This command combines piping and redirection.  It pipes the output of `ls -l` to `grep`, and then redirects the *output* of `grep` (the lines containing "myfile.txt") to the file `results.txt`.
