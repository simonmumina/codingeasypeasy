...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['react-native', 'testing', 'jest', 'detox', 'unit testing', 'integration testing', 'end-to-end testing', 'mobile development']
draft: false
...
*   `queryByText`, `queryByTestId`: Similar to `getByText` and `getByTestId`, but return `null` if the element is not found, instead of throwing an error. Useful for asserting that an element *doesn't* exist.
...
describe('UserProfile Component', () => {
  ...
  it('should display an error message if the API call fails', async () => {
      ...
      expect(screen.getByTestId('error-message').props.children).toEqual(
          'Error: API Error'
      );
  });
});
