...
Therefore, the argument concludes, Achilles can never overtake the tortoise, because he must always first reach the point where the tortoise _was_, by which time the tortoise has moved on.
...
The key to resolving the paradox lies in understanding the concept of an infinite geometric series. The distances Achilles covers in each step form a decreasing geometric progression. While there are infinitely many steps, the _sum_ of the distances is finite.
...
This paradox states that motion is impossible because before an object can travel a certain distance, it must first travel half that distance. Before it can travel half that distance, it must travel half of _that_ distance, and so on, ad infinitum. Therefore, motion can never even begin.
...
Therefore, the sum of the infinite series of distances _does_ equal the total distance, resolving the paradox.
...
- **Calculus Perspective:** Calculus addresses this by defining velocity as the _limit_ of the change in position over an infinitesimally small interval of time (Δt approaching 0). The arrow isn't "frozen" at a single point; it's the _rate_ of change over a very small time period that defines its motion. The concept of an "instant" is not a static, indivisible unit but rather a point in a continuum.
