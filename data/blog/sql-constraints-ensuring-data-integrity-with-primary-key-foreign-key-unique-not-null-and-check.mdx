...
date: '2024-01-26'
lastmod: '2024-01-27'
tags: ['SQL', 'Database', 'Data Integrity', 'Constraints', 'PRIMARY KEY', 'FOREIGN KEY', 'UNIQUE', 'NOT NULL', 'CHECK']
draft: false
...
The `FOREIGN KEY` constraint establishes a relationship between two tables. It ensures that values in a column (or set of columns) in one table (the *child* table) must match values in the `PRIMARY KEY` column of another table (the *parent* table). This is crucial for maintaining referential integrity.
...
The `UNIQUE` constraint ensures that all values in a column (or set of columns) are distinct. Unlike the `PRIMARY KEY`, a table can have multiple `UNIQUE` constraints, and a `UNIQUE` column *can* allow `NULL` values (typically, only one `NULL` value is allowed per `UNIQUE` constraint).
