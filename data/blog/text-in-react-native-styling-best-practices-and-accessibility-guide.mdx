  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.bodyText} numberOfLines={2} ellipsizeMode="tail">
        This is some longer text that will be truncated if it exceeds two lines. Notice the '...' at the end. This is great for summaries and short descriptions. We can also use different ellipsize modes.
      </Text>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.highlightedText}>
        This is a <Text style={styles.bold}>bold</Text> word within the sentence. And another <Text style={styles.italic}>italic</Text> word.
      </Text>
    </View>
  );
};
...
5.  **Use the Font Family:**  You can now use the `fontFamily` style property to specify the font family in your `<Text>` components.
    ...
    **Important:**  The `fontFamily` value must match the *PostScript name* of the font, not necessarily the file name. You can find the PostScript name using font editing software.
