...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['web services', 'API', 'REST', 'SOAP', 'microservices', 'SOA', 'architecture', 'webservices', 'wsdl', 'json', 'xml']
draft: false
...
*   **Uniform Interface:**  A consistent way of interacting with the service, using standard HTTP methods, resource identifiers (URIs), and data formats.  The uniform interface includes:
    ...
    *   **Hypermedia as the Engine of Application State (HATEOAS):** The server provides links in its responses that allow the client to discover and navigate the API.  While often desirable, HATEOAS is not strictly *required* to be considered RESTful.
