...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['linux', 'diff', 'file comparison', 'command line', 'text processing', 'unix', 'terminal', 'patch', 'version control']
draft: false
...
*   **`3c3`**:  This indicates that line 3 in `file1.txt` has been *changed* to line 3 in `file2.txt`.  The `c` stands for "change."
...
*   **`5d4`**: This indicates that line 5 in `file1.txt` has been *deleted*. The `d` stands for "delete."  It specifies line 5 from the first file, and that effectively results in it being deleted from the second file by affecting line 4.
...
The `-u` option is particularly important because it generates a *unified diff*.  This format is widely used for creating *patch files*. A patch file contains a description of the differences between two files in a standardized format, allowing you to apply the changes to one file to transform it into the other.
...
*   `@@ -1,5 +1,4 @@`: This is a *hunk header*.  It describes the location of the changes.
