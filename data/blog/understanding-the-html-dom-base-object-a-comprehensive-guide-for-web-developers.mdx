...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['HTML', 'DOM', 'Base Object', 'Web Development', 'JavaScript', 'Document Object Model', 'Node', 'Element', 'API']
draft: false
...
*   **The common properties and methods available to *all* DOM nodes.** This saves you from having to learn different sets of functionalities for different element types.
...
*   **`NodeList`:**  A generic collection of nodes of any type (elements, text, comments, etc.).  Returned by methods like `childNodes` and `querySelectorAll`. `NodeList` can be *static* or *live*.  `querySelectorAll` returns a static `NodeList`.
*   **`HTMLCollection`:**  A collection of *element* nodes. Returned by methods like `children`, `getElementsByTagName`, and `getElementsByClassName`.  `HTMLCollection` is *always live*.
