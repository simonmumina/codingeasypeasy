...
date: '2024-01-01'
lastmod: '2024-01-05'
tags: ['javascript', 'html', 'dom', 'events', 'onfocusin', 'accessibility', 'focus', 'web development']
draft: false
...
*   **Target:** The element that *actually* received focus.
...
The `onfocus` event only fires when the element to which it is attached directly receives focus.  `onfocusin`, on the other hand, will fire even when a *child* of the element to which it is attached receives focus, due to its bubbling nature.
...
1.  **Focusin Listener:** The `focusin` listener on `modalContent` detects when focus goes *outside* the modal.  This happens when the modal opens and focus is initially outside of it. The listener then refocuses the `modalFirst` element.
