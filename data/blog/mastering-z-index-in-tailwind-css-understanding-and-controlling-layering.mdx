...
Notice the `-1` value added above. Negative `z-index` values are particularly useful when you want to place an element *behind* other elements in the stacking context.

```jsx
<div className="relative w-64 h-48 bg-gray-200">
  <div className="absolute inset-0 bg-red-500 z-10">Above</div>
  <div className="absolute inset-0 bg-blue-500 z-0">Middle</div>
  <div className="absolute inset-0 bg-green-500 z--1">Below</div>
</div>
```
...
Understanding stacking contexts is crucial for effectively managing `z-index`.  A stacking context is created by any element that meets *any* of the following conditions:
...
The `z-index` values are only compared *within the same stacking context*. This means an element with `z-index: 9999` inside a stacking context will *never* appear above an element with `z-index: 1` in a *different* stacking context.
