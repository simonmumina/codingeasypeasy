...
While not directly using a built-in opacity modifier, this demonstrates how to override a theme color with a custom rgba value.  Chakra UI doesn't *natively* have opacity modifiers directly appended to color names (like `blue.500.opacity-50`).  You'd typically use CSS to achieve this.  However, you can extend the theme to include custom opacity variants.
...
function HoverOpacityExample() {
  return (
    <Button bg="green.500" color="white" _hover={{ bg: 'green.500', opacity: 0.8 }} _active={{ bg: 'green.700', opacity: 0.9 }}>
      Hover Me
    </Button>
  );
}
