...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'nullish coalescing operator', 'javascript tutorial', 'es2020', 'null', 'undefined', 'coding', 'web development', 'js tips']
draft: false
...
Before the nullish coalescing operator, developers often used the logical OR operator (`||`) to provide default values.  While the `||` operator works, it has a significant drawback: it treats any *falsy* value (e.g., `0`, `''`, `false`, `NaN`, `null`, `undefined`) as equivalent.  This can lead to unexpected behavior when you *want* to use `0` or `''` as valid values.
...
Now, the output is `0`, which is the intended behavior. The `??` operator *only* checks for `null` or `undefined`, not for falsy values in general.  Since `quantity` is `0`, which is not `null` or `undefined`, the expression evaluates to `0`.
