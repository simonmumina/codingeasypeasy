...
date: '2024-01-05'
lastmod: '2024-01-06'
tags: ['javascript', 'XMLHttpRequest', 'PUT request', 'HTTP library', 'web development', 'AJAX', 'frontend']
draft: false
...
// http.js
const http = {
  put: function(url, data, contentType, callback) { // Added contentType parameter
    const xhr = new XMLHttpRequest();
    ...
    if (contentType === 'application/json') {
      ...
      dataToSend = Object.keys(data).map(key => encodeURIComponent(key) + '=' + encodeURIComponent(data[key])).join('&');
    } else {
...
http.put('https://example.com/posts/1', data, 'application/x-www-form-urlencoded', function(err, response) {
  if (err) {
    console.error('Error:', err);
  } else {
    console.log('Response:', response);
