...
date: '2024-01-26'
lastmod: '2024-01-27'
tags: ['R', 'Data Visualization', 'ggplot2', 'R Programming', 'Data Science', 'Statistical Graphics', 'Interactive Plots']
draft: false
...
The backbone of data visualization in R is the `ggplot2` package.  It's based on the *Grammar of Graphics*, which provides a structured framework for creating plots.
...
*   `ggplot(data = mtcars, aes(x = wt, y = mpg))`:  This initializes the plot, specifying the dataset (`mtcars`) and mapping the `wt` variable to the x-axis and the `mpg` variable to the y-axis.  The `aes()` function defines *aesthetics*, which control how data variables are mapped to visual properties.
