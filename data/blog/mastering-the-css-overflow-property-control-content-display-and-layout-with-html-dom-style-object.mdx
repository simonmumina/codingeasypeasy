...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['CSS', 'Overflow', 'HTML DOM', 'Styling', 'Layout', 'Web Development', 'CSS Properties', 'Content Management']
draft: false
...
*   **`scroll`:** The content is clipped, and scrollbars are displayed to allow users to view the hidden content.  The scrollbars are *always* displayed, even if the content doesn't overflow.

*   **`auto`:**  The content is clipped, and scrollbars are displayed *only* when the content overflows the element's box. This is often the most preferred option.
...
Scrollbars will only appear on the *first* `div` because its content exceeds the container's dimensions.  The second `div`'s content fits, so no scrollbars are displayed.
