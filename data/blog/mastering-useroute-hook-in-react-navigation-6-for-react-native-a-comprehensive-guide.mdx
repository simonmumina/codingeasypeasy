...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['react-native', 'react-navigation', 'useRoute', 'navigation', 'mobile-development', 'javascript', 'hooks', 'guide', 'tutorial']
draft: false
  ...
  return (
    <View>
      <Button
        title="Go to Profile"
        onPress={() =>
          navigation.navigate('Profile', { userId: 123, username: 'johndoe' })
        }
      />
    </View>
  );
...
2.  **`ProfileScreenRoute`**:  This uses the `RouteProp` generic type from `@react-navigation/native` to define the type of the `route` object for the `Profile` screen.  It takes two type parameters:
    *   `RootStackParamList`: This type is *crucial* and should be defined in your navigation configuration file. It's an object type where the keys are the route names and the values are the types of the parameters for each route. If a route has no parameters, use `undefined`.
