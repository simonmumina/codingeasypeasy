...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['javascript', 'decodeURIComponent', 'uri', 'encoding', 'decoding', 'url encoding', 'javascript tutorial', 'javascript examples']
draft: false
...
*   **`decodeURIComponent()`:** Decodes all characters that `encodeURIComponent()` encoded. This includes characters like `;`, `/`, `?`, `:`, `@`, `&`, `=`, `+`, `$`, `,`, and `#`. It's meant for decoding individual URI *components* (e.g., a query parameter value).

*   **`decodeURI()`:** Decodes only those characters that were encoded to represent characters with special meaning within a *complete* URI. It does *not* decode reserved characters like `;`, `/`, `?`, `:`, `@`, `&`, `=`, `+`, `$`, `,`, and `#` because these characters have specific meanings in the URI syntax. It's primarily used for decoding an entire URI.
