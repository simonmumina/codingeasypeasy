...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['git', 'diff tool', 'merging', 'configuration', 'development', 'version control', 'vscode', 'meld', 'p4merge', 'kdiff3']
draft: false
...
To configure the diff tool for a specific repository, navigate to the repository's directory in your terminal and run `git config` *without* the `--global` option. This will modify the `.git/config` file within the repository.
...
To configure VS Code for a *specific* repository, remove the `--global` flag from the above commands and run them from the repository's root directory.
