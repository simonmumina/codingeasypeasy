...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['spring boot', 'webflux', 'jpa', 'reactive programming', 'blocking code', 'concurrency', 'async', 'r2dbc', 'kotlin coroutines']
draft: false
...
*   Use `subscribeOn()` when you need to offload the *source* of the reactive stream (the initial blocking operation) to a different thread.  This is the most common use case for blocking JPA operations.
*   Use `publishOn()` when you need to switch threads *during* the processing of the reactive stream, perhaps to perform a different type of operation on a different thread pool.
...
While not strictly a way to *mix* reactive and blocking code, R2DBC offers a completely reactive solution for database access.  If you have the option and your database supports it, migrating to R2DBC can eliminate the need for blocking JPA operations altogether.
