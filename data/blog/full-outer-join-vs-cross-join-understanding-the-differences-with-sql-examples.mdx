...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['SQL', 'Database', 'Join', 'Full Outer Join', 'Cross Join', 'Relational Database', 'Data Analysis']
draft: false
...
Before diving into the specifics, let's briefly recap what SQL JOINs are. A JOIN clause is used to combine rows from two or more tables, based on a related column between them.  Different JOIN types define *how* these rows are combined, especially when there isn't a direct match.

## Understanding the Full Outer Join

A **Full Outer Join** (sometimes called a Full Join) returns all rows from both tables involved in the join. If there's a match in the joining column(s), the corresponding columns from both tables are combined into a single row.  If there's no match, the non-matching columns from the table that *doesn't* have a match will be filled with `NULL` values.
...
*   Each customer is paired with *every* order, regardless of whether the `CustomerID` matches.  This is typically *not* what you want when trying to relate customers to their orders.
