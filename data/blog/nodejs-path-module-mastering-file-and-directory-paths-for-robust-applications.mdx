...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['node.js', 'path module', 'file system', 'directory paths', 'path manipulation', 'javascript', 'cross-platform', 'os', '__dirname', '__filename']
draft: false
...
The `path.join()` method joins all given path segments together using the platform-specific separator (`/` on POSIX, `\` on Windows) as a delimiter, then normalizes the resulting path.  It is the *most commonly used* method for constructing file paths.
