...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['NoSQL', 'Firebase', 'Cassandra', 'Database', 'Cloud Firestore', 'Apache Cassandra', 'Data Modeling', 'Database Tutorial', 'Backend Development']
draft: false
...
*   **Collections:** Groups of documents. Collections are similar to tables in a relational database.  A document *must* belong to a collection.
...
Cassandra's data modeling is driven by **queries**. You need to understand how your data will be accessed *before* you design your data model.
...
*   **`CREATE TABLE posts_by_author`**:  This table is designed to answer the query: *Get all posts by a specific author, sorted by creation date (newest first)*.
...
*   **`CREATE TABLE posts_by_id`**: This table is designed to answer the query: *Get a specific post by its ID*.  The `post_id` is the primary key.
