...
**Important:** `getElementsByClassName` returns a _live_ HTMLCollection. This means that if you add or remove elements with the specified class after you get the collection, the collection will be updated automatically.
...
Like `getElementsByClassName`, `getElementsByTagName` also returns a _live_ HTMLCollection.

### 4. `document.querySelector()`

This method returns the _first_ element within the document that matches the specified CSS selector. It's a powerful and versatile method that allows you to use CSS selectors to target elements.
