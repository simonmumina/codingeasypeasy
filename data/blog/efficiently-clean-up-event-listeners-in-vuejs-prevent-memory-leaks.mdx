...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['vue.js', 'event listeners', 'memory leaks', 'cleanup', 'javascript', 'vue lifecycle', 'performance optimization', 'component lifecycle']
draft: false
...
2.  `this.handleClick = this.handleClick.bind(this);` is *crucial*. Without it, the `this` context inside `handleClick` might not be the Vue component instance, leading to errors when you try to access component data or methods.
3.  In the `beforeDestroy` lifecycle hook, we use `removeEventListener` to detach the listener before the component is destroyed.  We must pass the *exact same* listener function (including the bound `this`) as we used in `addEventListener`.
...
*   **Direct DOM Manipulation:** Avoid directly manipulating the DOM within Vue components unless absolutely necessary. Vue's virtual DOM provides better performance and maintainability. If you *must* use direct DOM manipulation, always clean up event listeners in `beforeDestroy` or `destroyed`.
...
*   **`v-show`:** Use when you want to toggle the visibility of an element without removing it from the DOM.  *Careful!*  If the element has event listeners that are no longer needed when hidden, you must manually clean them up in the `beforeDestroy` or `destroyed` hooks.
