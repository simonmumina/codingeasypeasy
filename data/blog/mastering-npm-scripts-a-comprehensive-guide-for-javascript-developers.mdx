...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['npm', 'npm scripts', 'javascript', 'build automation', 'task runner', 'front-end development', 'package.json', 'command line', 'developer tools', 'node.js']
draft: false
...
*   **`&&` (AND):** Executes the next command only if the previous command succeeds (exits with a code of 0).
    ...
    This script will first run ESLint.  If ESLint finds no errors, it will then run Webpack. If ESLint finds errors, Webpack will *not* run.
