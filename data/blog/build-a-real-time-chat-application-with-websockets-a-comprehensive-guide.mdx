...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['websockets', 'real-time chat', 'javascript', 'node.js', 'socket.io', 'web development', 'chat application tutorial']
draft: false
...
3.  **Create `server.js`:**
    ...
    *   We listen for `chat message` events. When a client sends a chat message, we log it to the console and then broadcast it to all connected clients using `io.emit('chat message', msg)`. `io.emit` sends the message to *all* connected clients. `socket.broadcast.emit` would send it to all clients *except* the one who sent it.
