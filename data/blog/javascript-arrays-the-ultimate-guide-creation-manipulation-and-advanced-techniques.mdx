...
date: '2023-10-27'
lastmod: '2024-01-15'
tags: ['javascript', 'arrays', 'data structures', 'programming', 'web development', 'array methods', 'array manipulation']
draft: false
...
In essence, a JavaScript array is an ordered collection of data. These data elements, also known as elements, can be of any JavaScript data type – numbers, strings, booleans, other arrays, objects, or even functions.  The key feature of an array is that each element is associated with an index, starting from 0 for the first element.
...
Creates a *new* array with the results of calling a provided function on every element in the calling array.  The original array remains unchanged.
...
Creates a *new* array with all elements that pass the test implemented by the provided function. The original array remains unchanged.
...
Reverses an array *in place*. The first array element becomes the last, and the last array element becomes the first.  It modifies the original array.
