...
date: '2023-10-27'
lastmod: '2024-10-26'
tags: ['nextjs', 'cms', 'content management system', 'headless cms', 'react', 'javascript', 'web development', 'vercel', 'netlify', 'contentful', 'strapi', 'sanity', 'prismic']
draft: false
...
const renderOptions = {
  renderNode: {
    [BLOCKS.EMBEDDED_ASSET]: (node, children) => {
      ...
      return (
        <img src={imageUrl} alt={title} title={description} />
      );
    },
  },
};
...
export default function BlogPost({ post, content }) {
  ...
  return (
    <div>
      ...
      <div>
        {content && documentToReactComponents(content.fields.body, renderOptions)}
      </div>
    </div>
  );
}
