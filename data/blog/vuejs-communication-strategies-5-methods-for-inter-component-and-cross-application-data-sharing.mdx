...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['vue', 'vuejs', 'communication', 'inter-component', 'cross-application', 'data-sharing', 'props', 'events', 'vuex', 'event-bus', 'microfrontends']
draft: false
...
*   **Mutations:**  Synchronous functions that modify the state.  You *must* commit mutations, you cannot directly modify the state.
...
*   **Not Reactive by Default:** Changes to the provided values in the provider component will *not* automatically update in the injected component. You need to use reactive data or reactive functions in the `provide`. For example, `provide: () => ({ myReactiveValue: ref(initialValue) })`.
...
window.dispatchEvent(new CustomEvent('my-app-event', {
  detail: {
