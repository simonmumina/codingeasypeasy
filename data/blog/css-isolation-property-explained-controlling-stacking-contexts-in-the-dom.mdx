...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['css', 'isolation', 'stacking context', 'html', 'dom', 'style', 'web development', 'css properties', 'z-index', 'performance']
draft: false
...
By applying `isolation: isolate` to `.modal-container`, you ensure that any elements within the modal, regardless of their own `z-index` values, will always render above the `.page-content` (assuming the `.modal-container` is displayed).  The overlay will always render beneath the `.modal-content` because of its lower z-index *within* the isolated modal stacking context.
