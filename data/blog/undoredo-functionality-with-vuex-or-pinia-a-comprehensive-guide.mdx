...
*   **`addItem` and `removeItem` mutations**: These mutations are responsible for adding and removing items from the `items` array, respectively.  Critically, *before* applying the change to `items`, these mutations *save the current state of* `items` to the `history` array. Note the slicing of the history array: If the user makes a new change after having undone some actions, the "future" history must be discarded.
...
*   **Selective History:** Instead of storing the entire state on every change, store only the minimal changes required to revert to a previous state. This is particularly useful for large or complex states. This is known as *differential* or *delta* history.  You'll need custom logic to apply these deltas during undo/redo.
