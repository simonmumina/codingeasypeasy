...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['vuejs', 'vue router', 'authentication', 'authorization', 'route protection', 'vue guards', 'security']
draft: false
...
4.  **`beforeEach` guard:**
    ...
    *   If the route requires authentication and the user *is* authenticated, the `next()` function is called to proceed to the route.
...
*   `beforeRouteEnter`: Called before the route that renders this component is confirmed.  Does *not* have access to `this` component instance because the component hasn't been created yet. Use `next(vm => { ... })` to access the component instance.
