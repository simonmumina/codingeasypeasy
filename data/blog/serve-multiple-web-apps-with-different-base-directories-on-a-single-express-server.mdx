...
date: "2024-10-27"
lastmod: "2024-10-27"
tags: ["express", "node.js", "web server", "static files", "multiple apps", "base directory", "middleware", "server configuration", "hosting", "deployment"]
draft: false
...
If you want each application to run on a different port on your local machine, you'd have to create separate Express apps and run them individually.  This approach isn't serving multiple "apps" from a single *Express server instance*, but it's a related configuration that's useful for development.
