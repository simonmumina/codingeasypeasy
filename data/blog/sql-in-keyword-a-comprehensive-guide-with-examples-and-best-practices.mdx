...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['SQL', 'IN Keyword', 'Database', 'SQL Queries', 'SQL Tutorial', 'SQL Best Practices', 'Data Filtering', 'Relational Databases']
draft: false
...
The `NOT IN` operator is the opposite of `IN`. It selects rows where the specified column's value is *not* in the list of values.

To find employees who are *not* in the 'Sales' or 'IT' departments:
...
Will *not* return the 'Null Value' row, even though the Department is `NULL` and therefore *not* equal to 'Sales' or 'IT'.  This is because the `IN` clause implicitly includes `NULL`, resulting in no rows returned. To handle `NULL` values correctly, you need to explicitly account for them in your `WHERE` clause:
