...
*   **Difficult Rollbacks:** Rolling back to a specific version becomes challenging without a consistent way to identify and retrieve the corresponding build artifacts.  Knowing *which* build ID was associated with *which* commit is crucial for effective rollbacks.
...
*   We check if the `NEXT_PUBLIC_BUILD_ID` environment variable is set. *Crucially, we prefix the variable with `NEXT_PUBLIC_`.*  This ensures that the build ID is available in the browser if needed (e.g., for debugging or displaying the current version).
