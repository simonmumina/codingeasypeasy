...
date: '2024-01-26'
lastmod: '2024-10-27'
tags: ['expo', 'authentication', 'oauth', 'openid connect', 'react native', 'mobile development', 'security', 'expo authentication', 'social login']
draft: false
...
*   **OAuth (Open Authorization):**  OAuth enables your application to access user resources hosted by another service (e.g., Google Photos, Facebook profile) without needing the user's credentials directly.  It grants *limited* access, allowing users to control what your app can access.

*   **OpenID Connect (OIDC):** Built on top of OAuth 2.0, OIDC adds an *identity layer*.  It allows your application to verify the identity of the user by retrieving basic profile information, like their name and email, through a standardized format called an ID Token. Think of it as OAuth + user identity.
...
export default function App() {
  ...
  const getUserInfo = async (accessToken) => {
    try {
      ...
      Alert.alert("Error fetching user info", "Please check your internet connection or try again later.");
    }
  };
...
*   **Configuration:**  Replace `YOUR_GOOGLE_CLIENT_ID` with your actual Google Client ID.  **Crucially, update the `REDIRECT_URI`**  with the correct URI.  For Expo Go, this usually is `exp://localhost:19006` or a similar `exp://` address. For standalone apps, it's a deep link like `yourapp://redirect`. *Double-check your `app.json` and Google Cloud Console settings to ensure they match!*
*   **`useAuthRequest` Hook:** This hook from `expo-auth-session` manages the authentication request.
    *   `clientId`: Your Google Client ID.
    *   `redirectUri`:  The URI Google redirects to after authentication.  **This is case-sensitive and *must* match the URI configured in your Google Cloud Console.**
