...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['epidemiology', 'disease modeling', 'SIR model', 'compartmental models', 'infectious diseases', 'public health', 'mathematical modeling', 'R programming', 'Python']
draft: false
...
*   **β (Beta):**  The *transmission rate*.  It represents the probability of infection given contact between a susceptible and an infected individual. Higher β means faster spread.
*   **γ (Gamma):**  The *recovery rate*. It represents the proportion of infected individuals who recover per unit time. γ is the inverse of the average infectious period (1/γ).
...
2.  **`parameters`:**  This vector sets the values for `beta` and `gamma`. *Crucially, these are example values!*  In a real-world scenario, you would estimate these parameters based on epidemiological data for the specific disease you are modeling.  A higher `beta` value will lead to faster spread, while a higher `gamma` leads to quicker recovery.
