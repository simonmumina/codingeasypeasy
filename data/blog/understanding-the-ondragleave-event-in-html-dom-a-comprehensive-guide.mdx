...
The `event.relatedTarget` provides you with the ability to understand _where_ the drag is leaving the target to go.  In this scenario, we check if the `relatedTarget` is null or if it's *not* a child element of our droppable area.
...
* We use `droppableArea.contains(event.relatedTarget)` to check if the element that's now under the mouse cursor is within the `droppableArea`. If it is, we *don't* reset the background color, because the dragged element hasn't actually left the `droppableArea`.
