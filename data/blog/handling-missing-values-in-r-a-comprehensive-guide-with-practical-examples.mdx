...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['R programming', 'Missing data', 'Data cleaning', 'Data analysis', 'NA values', 'Imputation', 'Data science', 'R tutorial']
draft: false
...
While definitively proving MCAR, MAR or MNAR is impossible, you can use statistical tests to check if the data *appears* to be MCAR.  Little's MCAR test is a common choice.
...
A small p-value (typically less than 0.05) suggests that the data is *not* MCAR, which might indicate MAR or MNAR.  However, a large p-value does *not* prove MCAR, only that there is not enough evidence to reject the null hypothesis.
