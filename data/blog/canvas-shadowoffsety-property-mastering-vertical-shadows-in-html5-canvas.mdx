...
The `shadowOffsetY` property of the Canvas 2D API specifies the vertical distance that the shadow will be offset from the shape. Think of it as determining how much the shadow is moved _down_ (positive value) or _up_ (negative value) from the object casting the shadow. The value is expressed in pixels.
