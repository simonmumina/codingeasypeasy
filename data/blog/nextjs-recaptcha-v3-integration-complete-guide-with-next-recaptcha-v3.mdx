...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['nextjs', 'recaptcha', 'recaptcha v3', 'security', 'form validation', 'spam protection', 'next-recaptcha-v3', 'google recaptcha']
draft: false
...
function ContactForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!executeRecaptcha) {
      ...
      setSubmissionResult({ success: false, message: 'reCAPTCHA not ready. Please try again later.' });
      return;
    }
    ...
    if (!token) {
      ...
      setIsSubmitting(false);
      setSubmissionResult({ success: false, message: 'Failed to generate reCAPTCHA token. Please try again later.' });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmissionResult({ success: false, message: data.message || 'An error occurred. Please try again later.' });
      }
...
*   `executeRecaptcha('contact_form')`:  This function triggers the reCAPTCHA v3 analysis and returns a token.  The argument `'contact_form'` is an *action name*. Using action names allows you to differentiate between different reCAPTCHA calls on your site and analyze the reCAPTCHA scores separately.  Choose descriptive action names like `login`, `signup`, `comment`, etc.
