...
This is the recommended approach for production environments to reduce your CSS bundle size.  In `resources/sass/app.scss`, import only the modules you need.  This approach *requires* you to import `functions`, `variables`, and `mixins` before any other components.
...
To further customize Bootstrap's styling, you can override its default variables in your `resources/sass/app.scss` (for Laravel Mix) or `resources/css/app.css` (for Vite, by either importing and then overriding the variables *before* importing Bootstrap, or by defining custom CSS rules in a separate block *after* importing Bootstrap).
...
This requires installing `sass` using `npm install -D sass`. Then, import `_variables.scss` *before* importing Bootstrap in `resources/css/app.scss`.  The `_variables.scss` file will contain your customized variables. You will also need to modify vite.config.js to handle .scss compilation.
