...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'event handling', 'dom', 'web development', 'target property', 'event delegation']
draft: false
...
*   **`event.target`:** The element that *triggered* the event.  It's the element where the event originated.
*   **`event.currentTarget`:** The element where the event listener is *attached*.  It's the element that's actually handling the event.
...
6.  The outer `div`'s click event listener executes.  Crucially, inside *this* listener:

    *   `event.target` still points to the *button* (the element that triggered the event).
    *   `event.currentTarget` points to the *div* (the element where the listener is attached).
