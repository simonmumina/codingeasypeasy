...
A `NodeList` is an ordered collection of nodes extracted from an HTML document.  It's *not* an array, although it shares some similarities.  `NodeList` objects are often returned by methods like:
...
*   **Static `NodeList`**: The content of the `NodeList` is fixed at the time it is created. Any changes made to the DOM *after* the `NodeList` is created will *not* be reflected in the `NodeList`. `document.querySelectorAll()` returns a static `NodeList`.
