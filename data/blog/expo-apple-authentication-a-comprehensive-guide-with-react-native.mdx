...
date: '2023-10-27'
lastmod: '2024-10-26'
tags: ['expo', 'react-native', 'apple-authentication', 'sign-in-with-apple', 'authentication', 'ios', 'mobile-development']
draft: false
...
3.  **Create a Services ID (optional, but recommended for backend verification):**
    ...
    *   Provide a description and an identifier (e.g., `com.yourcompany.yourapp.auth`). This should be *different* from your app's Bundle ID.
...
export default function App() {
  ...
  const onSignIn = async () => {
    try {
      ...
      const appleId = Platform.OS === 'ios' || Platform.OS === 'macos' ?
      {
      ...
      if (appleAuthRequestResponse && appleAuthRequestResponse.identityToken) {
        ...
        Alert.alert('Sign In Successful!', `User ID: ${appleAuthRequestResponse.user}, Full Name: ${appleAuthRequestResponse.fullName?.givenName} ${appleAuthRequestResponse.fullName?.familyName}, Email: ${appleAuthRequestResponse.email}`);
      } else {
      ...
      const credentialState = await AppleAuthentication.getCredentialStateForUserAsync(appleAuthRequestResponse.user);
...
*   **`APPLE_CLIENT_ID`:**  This **must** be your **Service ID** (created in the Apple Developer portal) if you created one.  If not, use your **App ID**.  This ensures the token is intended for *your* application.
