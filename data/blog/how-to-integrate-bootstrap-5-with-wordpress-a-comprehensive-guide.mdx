...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['wordpress', 'bootstrap 5', 'integration', 'theme development', 'cms', 'front-end', 'responsive design']
draft: false
...
As mentioned above, using a child theme is *essential* for preserving your customizations.  It's not really a method of integrating Bootstrap on its own, but rather a *requirement* when using the `functions.php` method.  See the steps in Method 1 for creating and activating a child theme.
...
*   **Override Styles in `style.css`:**  The easiest way to customize Bootstrap is to override its default styles in your child theme's `style.css` file.  Make sure to load your child theme's `style.css` *after* the Bootstrap CSS (as shown in the `functions.php` example).
...
*   **Using Bootstrap's Theme Variables (Intermediate):** You can override Bootstrap's default variables to change the look and feel of the entire framework. This is done by defining new values for the variables *before* importing Bootstrap's CSS. You can create a separate CSS file for your variable overrides and enqueue it *before* the Bootstrap CSS file.
    ...
    Then, modify your `functions.php` file to enqueue this file *before* Bootstrap's CSS:
...
*   **Bootstrap styles not applying:**
    *   **Check enqueue order:** Ensure your child theme's `style.css` is loaded *after* the Bootstrap CSS.
...
*   **JavaScript errors:**
    *   **jQuery dependency:** Verify that jQuery is loaded *before* Bootstrap's JavaScript. WordPress usually loads jQuery by default, but some plugins might interfere.
...
*   **Use a Child Theme:**  This is *absolutely essential*.
