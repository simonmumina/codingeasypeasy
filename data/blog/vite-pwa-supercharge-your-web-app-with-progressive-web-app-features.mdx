...
date: '2024-01-12'
lastmod: '2024-10-27'
tags: ['vite', 'pwa', 'progressive web app', 'vite plugin', 'offline support', 'caching', 'service worker', 'web development', 'web app', 'manifest.json']
draft: false
...
*   **`devOptions: { enabled: true }`:** This enables PWA features during development, making it easier to test your PWA locally.  Without this, the service worker might not register in development mode.  *Important: Remove or set to `false` for production builds.*
*   **`manifest`:**  This object defines the `manifest.json` file, which provides information about your PWA to the browser and operating system.  Crucially, you *must* include the `icons` array, and these image files *must* exist in your `public` directory. Without these, the PWA won't be installable.  The `purpose: 'any maskable'` ensures your icon adapts well to various Android devices. `theme_color` is used to style the operating system around the app, like the status bar in Android.
...
*   **HTTPS:** PWAs *must* be served over HTTPS in production. Use a local HTTPS server for testing if needed (e.g., using `mkcert`).
...
<template>
  <div>
    <button v-if="needsRefresh" @click="updateServiceWorker">
      Update Available!
    </button>
    <!-- Your app content -->
  </div>
