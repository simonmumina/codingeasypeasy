...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['kubernetes', 'fastapi', 'health checks', 'liveness probe', 'readiness probe', 'application deployment', 'docker', 'containerization', 'reliability', 'availability']
draft: false
...
1.  **Liveness Probe:** The liveness probe determines whether a container is running. If the liveness probe fails, Kubernetes will restart the container.  Think of it as a check for whether the *application is alive*. It's a drastic measure, indicating the application has entered an unrecoverable state.

2.  **Readiness Probe:** The readiness probe determines whether a container is ready to accept traffic. If the readiness probe fails, Kubernetes will stop routing traffic to the pod. Think of it as a check for whether the *application is ready to serve requests*. This is useful for scenarios like initializing databases, loading configurations, or completing startup tasks.
...
*   **Database Migrations:**  Ensure your database is migrated *before* the readiness probe starts checking the database connection.  Otherwise, your application may be marked as unhealthy during the migration process.
