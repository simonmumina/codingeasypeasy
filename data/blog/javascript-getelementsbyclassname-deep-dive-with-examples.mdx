...
`getElementsByClassName()` is a method available on the `document` object (and also on individual `HTMLElement` objects) that returns a *live* HTMLCollection of all elements in the document that have all of the specified class names.

**Key characteristics:**

*   **Returns an HTMLCollection:** This is a *live* collection, meaning that if elements are added to or removed from the DOM with the specified class name(s), the HTMLCollection will automatically update to reflect these changes.
...
In this example, we first get the element with the ID "container". Then, we use `getElementsByClassName()` on the container element to find all elements with the class "item" *only* within the container.  Therefore, "Item 3" will not be affected.
...
This example highlights the *live* nature of the HTMLCollection. When the button is clicked and a new `div` with the class "dynamic" is added to the DOM, the `dynamicElements` HTMLCollection automatically updates its length. This is a critical difference between `getElementsByClassName()` and methods like `querySelectorAll()` which return a static NodeList.
