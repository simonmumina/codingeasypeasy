...
date: '2024-10-26'
lastmod: '2024-10-27'
tags: ['javascript', 'mouse events', 'event handling', 'web development', 'front-end development', 'MouseEvent', 'buttons property']
draft: false
...
The `MouseEvent.buttons` property is an integer representing a bit field, where each bit corresponds to a specific mouse button. It indicates which mouse button(s) are pressed or held down *during* the triggering of the `MouseEvent`.  This is crucial for distinguishing between different button combinations and enabling actions based on these combinations.

Think of it as a binary code where each position represents a specific button. If the button is pressed, the corresponding bit is set to 1; otherwise, it's 0. Because this is a *bit field*, multiple buttons can be represented simultaneously.
...
*   **`MouseEvent.button`**:  Returns a single number representing the button that *triggered* the event. It's mainly used for distinguishing between left (0), middle (1), and right (2) clicks on *single button* events. It doesn't provide information about other buttons that might be pressed at the same time.  The `button` property is also deprecated in newer standards and using `buttons` is recommended.

*   **`MouseEvent.buttons`**: Returns an integer representing a *bit field* indicating all mouse buttons currently pressed *during* the event.  This is significantly more powerful as it allows you to detect combinations of buttons being pressed simultaneously.
