...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['nestjs', 'event-driven architecture', 'event emitter', 'eventemitter2', 'microservices', 'nestjs best practices']
draft: false
...
export class UserCreatedListener {
  ...
  async handleUserCreatedEvent(event: UserCreatedEvent) {
    try {
      ...
      this.logger.error(`Error sending welcome email to ${event.email}: ${error.message}`, error.stack);
...
export class UserCreatedListenerDefault implements OnModuleInit {
  ...
  handleUserCreatedEvent(event: UserCreatedEvent) {
    console.log(`[Default Priority] Sending welcome email to ${event.email} for user ${event.userId}`);
  }
}
```
