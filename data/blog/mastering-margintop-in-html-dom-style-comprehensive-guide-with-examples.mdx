...
The `marginTop` property is part of the HTML DOM Style object, which provides programmatic access to an element's inline styles. Specifically, `marginTop` allows you to get or set the top margin of an element.  The margin is the space *outside* of the element's border.
...
*   **`%`**: Specifies a margin in percent of the width of the *containing block*.  Examples: "10%", "50%".
...
Here, the `marginTop` is set to 10%.  This means the margin will be 10% of the width of the *containing block* which is the outer `div` (500px in this case). So the `marginTop` will be 50px.
...
*   **Incorrectly Assuming `auto` Centers Vertically:** The `auto` value for `marginTop` does *not* generally center elements vertically.
