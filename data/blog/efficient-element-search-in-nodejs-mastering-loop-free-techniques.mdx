...
date: '2024-10-26'
lastmod: '2024-10-26'
tags: ['node.js', 'javascript', 'search', 'algorithm', 'performance', 'optimization', 'array methods', 'loop-free', 'element search', 'functional programming']
draft: false
...
The `find()` method returns the *first* element in an array that satisfies a provided testing function. If no elements satisfy the condition, it returns `undefined`.
...
The `findIndex()` method returns the *index* of the first element in an array that satisfies a provided testing function. If no elements satisfy the condition, it returns `-1`.
...
The `filter()` method creates a *new* array with all elements that pass the test implemented by the provided function.  It returns an empty array if no elements match.
...
//Find key by value
const key = _.findKey(object, function(o) { return o == 2; });
