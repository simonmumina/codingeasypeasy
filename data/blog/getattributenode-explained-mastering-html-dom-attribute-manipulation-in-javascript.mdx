...
date: '2024-02-29'
lastmod: '2024-10-27'
tags: ['javascript', 'html dom', 'getAttributeNode', 'attributes', 'web development', 'dom manipulation']
draft: false
...
Here, we retrieve the `data-count` attribute, which is a custom data attribute.  Each time the button is clicked, we increment the `data-count` attribute's value directly through the `Attr` node. This demonstrates the power of `getAttributeNode()` to not only retrieve but also *modify* attribute values.
...
| ----------------- | ------------------------------------------------------ | ------------------------------------------------------- |
| Return Value      | An `Attr` node representing the attribute.            | The *value* of the attribute as a string.             |
