...
date: '2024-10-27'
lastmod: '2024-10-27'
tags: ['react', 'react hooks', 'eslint', 'eslint plugin', 'javascript', 'code quality', 'bug prevention', 'react best practices']
draft: false
...
In this bad example, the `useEffect` hook depends on both `count` and `name`, but only `count` is included in the dependency array.  This means that the effect will only re-run when `count` changes, *not* when `name` changes.  As a result, the document title will not update when the user types in the input field, leading to a stale closure.
...
In rare cases, you might encounter a situation where the `exhaustive-deps` rule flags a dependency that you intentionally want to omit.  You can suppress the warning using the `// eslint-disable-next-line react-hooks/exhaustive-deps` comment, but *only do this if you fully understand the consequences and are confident that it won't lead to bugs.*
