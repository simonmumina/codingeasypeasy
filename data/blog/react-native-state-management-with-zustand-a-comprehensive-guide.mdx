...
**Important:** Zustand handles memoization automatically when you select specific parts of the state using the selector function in `useStore`.  This ensures that the component only re-renders when the selected part of the state changes.  In the `TodoList` component, if the `todos` array changes but the number of completed todos remains the same, the `completedTodosCount` component *will not* re-render unnecessarily.
