...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['react', 'memoization', 'performance', 'useMemo', 'useCallback', 'React.memo', 'optimization']
draft: false
...
The `useMemo` hook is used to memoize the *result* of a function.  It only re-executes the function when one of its dependencies changes.
...
In this example, the `factorial` function is memoized using `useMemo`.  It will only be re-calculated when the `number` state changes.  If you type in the "Enter your name" input, the component will re-render, but the `factorial` calculation will *not* be re-executed because `number` hasn't changed.  You can verify this by observing the console output.
...
The `useCallback` hook is used to memoize a *function* itself. It returns a memoized version of the callback function that only changes if one of the dependencies has changed.  This is particularly useful when passing callbacks as props to child components that use `React.memo`.
...
Because `ChildComponent` is wrapped with `React.memo`, it will *only* re-render if its props change. Since the `onClick` prop (which is the memoized `handleClick` function) remains the same, the `ChildComponent` avoids unnecessary re-renders, leading to performance improvements.
