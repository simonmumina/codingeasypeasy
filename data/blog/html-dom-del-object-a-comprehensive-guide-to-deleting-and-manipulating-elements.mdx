...
date: '2024-01-26'
lastmod: '2024-01-26'
tags: ['html', 'dom', 'javascript', 'web development', 'delete element', 'remove element', 'dom manipulation', 'del object']
draft: false
...
## Actually *Deleting* Elements:  `removeChild()` and Related Methods

As emphasized, the `del` element itself doesn't delete elements.  To actually remove elements from the DOM, you need to use methods like `removeChild()`. The `del` element is used for *marking* content as deleted semantically, not actually removing it from the page.
...
You might use `<del>` in conjunction with eventual removal for scenarios like content moderation or version control.  You might *initially* mark content as deleted using `<del>`, and then later, based on user actions or server-side decisions, permanently remove the content from the DOM.
...
<p id="myParagraph">This is some text.  <span id="removableText">This is the text to be potentially deleted.</span>  More text.</p>
...
*   **`querySelector` Limitation:** The `permanentlyDelete()` function uses `document.querySelector("del")`.  This will only remove the *first* `<del>` element on the page.  For multiple `<del>` elements, you would need to use `document.querySelectorAll("del")` and iterate through the resulting NodeList, removing each element individually.
